...
- **9068**: Enter #$470607P. Q. R. S. T. U. V. W. X. Cancel. Confirm.
- **9506**: Use the device? [Yes./No.]
- **9507**: You may teleport once to any entrance lobby in the three nations or Jeuno.
- **9508**: Teleport to which lobby? [Stay put./San d'Oria./Bastok./Windurst./Jeuno.]
...
# Dead code (unreachable instructions):
     ...
     0x083A [0x1D] PRINT_EVENT_MESSAGE(message_id=9051*)
    → "I'm sorry. Things are a little confusing now with all these entries. Would you mind coming back a little later? Thank you."
     ...
     0x084B [0x1D] PRINT_EVENT_MESSAGE(message_id=9037*)
    → "I am so, so sorry, but it seems that you do not possess enough gil to enter your chocobo in the next race."
     ...
     0x085D [0x1D] PRINT_EVENT_MESSAGE(message_id=9036*)
    → "I am so sorry, but it seems you do not possess enough chocobucks to participate in the next race."
     ...
     0x086F [0x1D] PRINT_EVENT_MESSAGE(message_id=9052*)
...
# Dead code (unreachable instructions):
     ...
     0x09C8 [0x24] CREATE_DIALOG(message_id=9068*, default_option=0*, option_flags=0*)
...
```
  ...
  1: 0x0AD7 [0x24] CREATE_DIALOG(message_id=9506*, default_option=1*, option_flags=0*)
...
```
  ...
  1: 0x0B1A [0x24] CREATE_DIALOG(message_id=9506*, default_option=1*, option_flags=0*)
    → "Use the device? [Yes./No.]"
...
```
  ...
  1: 0x0B5D [0x24] CREATE_DIALOG(message_id=9506*, default_option=1*, option_flags=0*)
    → "Use the device? [Yes./No.]"
...
```
  ...
  1: 0x0BA0 [0x24] CREATE_DIALOG(message_id=9506*, default_option=1*, option_flags=0*)
    → "Use the device? [Yes./No.]"
...
```
  ...
  1: 0x0BE3 [0x24] CREATE_DIALOG(message_id=9506*, default_option=1*, option_flags=0*)
    → "Use the device? [Yes./No.]"
...
```
  ...
  1: 0x0C26 [0x24] CREATE_DIALOG(message_id=9506*, default_option=1*, option_flags=0*)
    → "Use the device? [Yes./No.]"
...
```
  ...
  1: 0x0C69 [0x24] CREATE_DIALOG(message_id=9506*, default_option=1*, option_flags=0*)
    → "Use the device? [Yes./No.]"
...
```
  ...
  1: 0x0CAC [0x24] CREATE_DIALOG(message_id=9506*, default_option=1*, option_flags=0*)
    → "Use the device? [Yes./No.]"
...
```
  ...
  1: 0x0CEF [0x24] CREATE_DIALOG(message_id=9506*, default_option=1*, option_flags=0*)
    → "Use the device? [Yes./No.]"
...
```
  ...
  1: 0x0D32 [0x24] CREATE_DIALOG(message_id=9506*, default_option=1*, option_flags=0*)
    → "Use the device? [Yes./No.]"
...
```
  0: 0x0D9C [0x45] LOAD_SCHEDULED_TASK: Load scheduler "fdi1" with entities [LocalPlayer, LocalPlayer], work=[200*, 0*]
  1: 0x0DAD [0x62] LOAD_EVENT_SCHEDULER: Load scheduler "main" with entities [LocalPlayer, LocalPlayer], work=[2*, 0*]
