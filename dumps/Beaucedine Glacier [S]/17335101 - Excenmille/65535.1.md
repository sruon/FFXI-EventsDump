# Event 65535

## Metadata

| Field           | Value    |
|-----------------|----------|
| Event Index     | 4/19     |
| Offset          | 0x0003   |
| Data Size       | 44 bytes |
| Calculated Size | 44 bytes |
| Instructions    | 5        |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0000:          5B 00 80 F8 FF  FF 7F F8 FF FF 7F 73 68     [..........sh
0010: 61 31 53 F8 FF FF 7F F8  FF FF 7F 73 68 61 31 59  a1S........sha1Y
0020: 01 F8 FF FF 7F 01 80 4B  F8 FF FF 7F 02 80 00     .......K....... 
```

## Opcodes

```
  0: 0x0003 [0x5B] LOAD_EXT_SCHEDULER(work_offset=1301*, entity1=EventEntity, entity2=EventEntity, action="sha1")
  1: 0x0012 [0x53] WAIT_SCHEDULER_TASK(entity1=EventEntity, entity2=EventEntity, action_id="sha1")
  2: 0x001F [0x59] UPDATE_ENTITY_DATA: Set EventEntity turn speed = 50*
  3: 0x0027 [0x4B] UPDATE_ENTITY_YAW(entity=EventEntity, yaw=8.24°*)
  4: 0x002E [0x00] END_REQSTACK()
```
