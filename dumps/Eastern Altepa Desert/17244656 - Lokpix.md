...
- **11084**: Who da heck are ya? Me? I's Lokpix. I goes all 'round da Altepa Desert, gatherin' information and scoutin' da land.
- **11085**: I's sure I knows more 'bouts dese dunes dan anybody in da hist'ry of Vana'diel. Fer example, ya know 'bouts da trick door in da Quicksand Caves?
- **11086**: Do ya know? [Doesn't everybody?/Not at all.]
- **11087**: Of course ya don't. Anyways, Lets me enlighten ya. Its origin is one of dem long-ago-and-far-away type of stories, but to cut out da borin' part, it relies on a special gadgetywhatsit dat's weight-sensitive. One Galka's enough to get dat baby to move, but any odda race would need more people to activates it.
- **11088**: Boy, has I gots some good news fer ya. Dis is somethin' only us Goblins know. It involves openin' dat door wit'out da help of odda adventurers, y'see.
- **11089**: You wanna knows how to do dat, don'tcha? How coulds ya not?
- **11090**: Well, do ya? [Without a doubt./I couldn't care less.]
- **11091**: Yer loss if ya don't wanna learn dis super-duper Gobby trick. You'll regrets it. I can already tells.
- **11092**: Well den, lets me impart my knowledges to ya, so dey say. Also, you knows da phrase, "Nothin' in life is free"? Well, dat goes double fer Goblin knowledge.
- **11093**: Bring me $0 and $1 if ya wants da secret. I ain't picky, neither, so if ya got no $1, then $2 will do ya right.
- **11094**: And if fer some reason ya ain't got one of dem either, I ain't averse to collectin' some $3. Yeah, twelve of 'em should do da trick.
- **11095**: And play by da numbers, kid. I ain't givin' out no discounts. Why? ...Because! Dat's why.
- **11096**: I see ya broughts 'em. Now stand back whiles I works my magic.
- **11097**: Ta-da! You's now da proud owner of yer very own $3!
- **11098**: Smackin' dis rock o' wonders causes it to get heavier fer da briefest o' moments. Don't bother aksin' how I can makes somethin' so astonishin'. Dat's a Goblin trade secret.
- **11099**: Dese fer me, eh? So nice doin' busin... Ya tryin' to pull a fast one on me!? Dese ain't what I aksed fer! Ya wants dis super-special secret or not? Den listen close now...
- **11100**: You been usin' dat $3 to enter dem Quicksand Caves all by yerself? Ya ain't gots to thank me. Helpin' friends in need is whats I do...fer a price. Enjoy lootin' and pillagin' dem caverns, [Mr./Ms.] satisfied customer.
...
```
  ...
  5: 0x0024 [0x1D] PRINT_EVENT_MESSAGE(message_id=11084*)
  ...
  7: 0x0028 [0x1D] PRINT_EVENT_MESSAGE(message_id=11085*)
  ...
  9: 0x002C [0x24] CREATE_DIALOG(message_id=11086*, default_option=1*, option_flags=0*)
 ...
 15: 0x004A [0x1D] PRINT_EVENT_MESSAGE(message_id=11087*)
...
```
  ...
  7: 0x00A4 [0x1D] PRINT_EVENT_MESSAGE(message_id=11096*)
 ...
 34: 0x0191 [0x1D] PRINT_EVENT_MESSAGE(message_id=11097*)
    → "Ta-da! You's now da proud owner of yer very own $3!"
 35: 0x0194 [0x23] WAIT_FOR_DIALOG_INTERACTION
 36: 0x0195 [0x1D] PRINT_EVENT_MESSAGE(message_id=11098*)
...
```
  ...
  5: 0x01EB [0x1D] PRINT_EVENT_MESSAGE(message_id=11099*)
...
```
  ...
  6: 0x021C [0x1D] PRINT_EVENT_MESSAGE(message_id=11100*)
...
# Dead code (unreachable instructions):
     0x0222 [0x1D] PRINT_EVENT_MESSAGE(message_id=11088*)
    → "Boy, has I gots some good news fer ya. Dis is somethin' only us Goblins know. It involves openin' dat door wit'out da help of odda adventurers, y'see."
     ...
     0x0226 [0x1D] PRINT_EVENT_MESSAGE(message_id=11089*)
    → "You wanna knows how to do dat, don'tcha? How coulds ya not?"
     0x0229 [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x022A [0x24] CREATE_DIALOG(message_id=11090*, default_option=1*, option_flags=0*)
    → "Well, do ya? [Without a doubt./I couldn't care less.]"
     ...
     0x0256 [0x1D] PRINT_EVENT_MESSAGE(message_id=11092*)
    → "Well den, lets me impart my knowledges to ya, so dey say. Also, you knows da phrase, "Nothin' in life is free"? Well, dat goes double fer Goblin knowledge."
     ...
     0x02DD [0x1D] PRINT_EVENT_MESSAGE(message_id=11091*)
    → "Yer loss if ya don't wanna learn dis super-duper Gobby trick. You'll regrets it. I can already tells."
     ...
     0x02FE [0x1D] PRINT_EVENT_MESSAGE(message_id=11093*)
    → "Bring me $0 and $1 if ya wants da secret. I ain't picky, neither, so if ya got no $1, then $2 will do ya right."
     ...
     0x0302 [0x1D] PRINT_EVENT_MESSAGE(message_id=11094*)
    → "And if fer some reason ya ain't got one of dem either, I ain't averse to collectin' some $3. Yeah, twelve of 'em should do da trick."
     ...
     0x0306 [0x1D] PRINT_EVENT_MESSAGE(message_id=11095*)
