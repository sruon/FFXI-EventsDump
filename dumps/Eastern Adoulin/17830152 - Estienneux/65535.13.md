# Event 65535

## Metadata

| Field           | Value    |
|-----------------|----------|
| Event Index     | 15/18    |
| Offset          | 0x017F   |
| Data Size       | 33 bytes |
| Calculated Size | 33 bytes |
| Instructions    | 3        |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0170:                                               5F                 _
0180: 06 0D 80 F8 FF FF 7F F8  FF FF 7F 74 6C 63 31 0B  ...........tlc1.
0190: 80 5F 07 F8 FF FF 7F F8  FF FF 7F 74 6C 63 31 00  ._.........tlc1.
```

## Opcodes

```
  0: 0x017F [0x5F] MULTI_HANDLER_COMPLEX(mode=0x06 - Load ext scheduler with flag (OpCode 0x5B mode 1, flag 1), ref=References[13], entity1=EventEntity, entity2=EventEntity, string="tlc1", extra=References[11])
  1: 0x0191 [0x5F] MULTI_HANDLER_COMPLEX(mode=0x07 - Wait scheduler task (OpCode 0x53))
  2: 0x019F [0x00] END_REQSTACK()
```
