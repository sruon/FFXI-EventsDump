# Event 65535

## Metadata

| Field           | Value    |
|-----------------|----------|
| Event Index     | 9/9      |
| Offset          | 0x0062   |
| Data Size       | 32 bytes |
| Calculated Size | 32 bytes |
| Instructions    | 4        |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0060:       5B 00 80 F8 FF FF  7F F8 FF FF 7F 64 61 69    [..........dai
0070: 33 1C 01 80 53 F8 FF FF  7F F8 FF FF 7F 64 61 69  3...S........dai
0080: 33 00                                             3.              
```

## Opcodes

```
  0: 0x0062 [0x5B] LOAD_EXT_SCHEDULER(work_offset=24*, entity1=EventEntity, entity2=EventEntity, action="dai3")
  1: 0x0071 [0x1C] WAIT(15* ticks)
  2: 0x0074 [0x53] WAIT_SCHEDULER_TASK(entity1=EventEntity, entity2=EventEntity, action_id="dai3")
  3: 0x0081 [0x00] END_REQSTACK()
```
