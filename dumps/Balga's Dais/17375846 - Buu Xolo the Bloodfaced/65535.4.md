# Event 65535

## Metadata

| Field           | Value    |
|-----------------|----------|
| Event Index     | 7/7      |
| Offset          | 0x01E9   |
| Data Size       | 73 bytes |
| Calculated Size | 73 bytes |
| Instructions    | 7        |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
01E0:                             45 00 80 F8 FF FF 7F           E......
01F0: F8 FF FF 7F 73 30 37 34  01 80 1C 03 80 52 00 80  ....s074.....R..
0200: F8 FF FF 7F F8 FF FF 7F  73 30 37 34 45 03 80 F0  ........s074E...
0210: FF FF 7F F0 FF FF 7F 6F  76 6C 31 01 80 45 00 80  .......ovl1..E..
0220: F0 FF FF 7F F0 FF FF 7F  73 30 37 35 01 80 1C 0F  ........s075....
0230: 80 00                                             ..              
```

## Opcodes

```
  0: 0x01E9 [0x45] LOAD_SCHEDULED_TASK(work_offset1=211*, entity1=EventEntity, entity2=EventEntity, scheduler_id="s074", work_offset2=0*)
  1: 0x01FA [0x1C] WAIT(200* ticks)
  2: 0x01FD [0x52] END_LOAD_SCHEDULER: End scheduler "s074" with entities [EventEntity, EventEntity], work=211*
  3: 0x020C [0x45] LOAD_SCHEDULED_TASK(work_offset1=200*, entity1=LocalPlayer, entity2=LocalPlayer, scheduler_id="ovl1", work_offset2=0*)
  4: 0x021D [0x45] LOAD_SCHEDULED_TASK(work_offset1=211*, entity1=LocalPlayer, entity2=LocalPlayer, scheduler_id="s075", work_offset2=0*)
  5: 0x022E [0x1C] WAIT(100* ticks)
  6: 0x0231 [0x00] END_REQSTACK()
```
