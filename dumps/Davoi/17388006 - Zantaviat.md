...
- **7423**: I am Zantaviat, a scout for San d'Oria. I was sent to watch the Orc camps in Davoi. Be careful, it's a veritable nest of beastmen in there.
- **7424**: A message? Finally! I am Zantaviat of the Temple Knights. I've been monitoring Orc activity here for several weeks. They grow stronger in numbers by the day.
- **7426**: The Temple Knights have been observing Davoi through a number of methods. We've collected our findings in a document: $6.
- **7427**: However...one of our men infiltrated the area last night, and ran into an Orc hunting party on his way out. He dropped the last page of his report before he could escape.
- **7428**: The Orcs caught wind of our presence, so we could not retrieve the lost page so easily.
- **7429**: According to our man, the page lies somewhere near the platform on the small pond up ahead. Go retrieve it for us.
- **7430**: With the Orcs on alert, any direct attack would only incite their full wrath. Take care not to be discovered!
- **7434**: You found the page! Well done. Now the $3 is complete. Take this to San d'Oria swiftly. Give it to the lord knight at the gatehouse.
- **7437**: Ah, it's you, <Player>. I was waiting for you. Let us dispense with the formalities; we want you to infiltrate Davoi.
- **7438**: Davoi is the central stronghold of the Orcs. We have delved as deep as we dare, but there is still much we do not know.
- **7439**: Come to think of it, an adventurer came along the other day. He told me to be strong out here by myself, and then he was off towards Davoi all alone. Got courage, that one.
- **7440**: But that is neither here nor there, eh?
- **7441**: Let me explain your mission. There are three Orc settlements in Davoi: East Block, South Block, and North Block.
- **7442**: We know that they've left coded memos concerning all three blocks. They should be somewhere in East Block.
- **7443**: East Block is not far from this entrance. I want you to find three of those coded memos.
- **7444**: Be aware that this mission will surely require combat. You way wish to gather a party to aid you.
- **7448**: That's all three blocks covered. I will notify the homeland of your contribution. All you need to do is return to San d'Oria and report in at the gatehouse.
- **7504**: A message? Finally! I am Zantaviat of the Temple Knights.
- **7505**: I've been monitoring Orc activity here for several weeks. They grow stronger in numbers by the day.
- **7506**: In our recent investigations, we have observed that Orcs are working towards a single purpose. Prince Pieuje was right all along...
- **7507**: I had prepared $6 and given it to one of our officers to deliver...
- **7508**: ...but that officer dropped one of the pages while fleeing from an Orc ambush last night.
- **7509**: Now the Orcs have doubled their forces in that area, and we can not easily retrieve the lost page.
- **7510**: According to the officer, he says he dropped the page near a crucifixion mound by that pond you see before us.
- **7511**: With the Orcs forces doubled like this, attacking them head-on would be futile. Sneaking in and retrieving the page unnoticed is the most logical method of carrying out this mission.
- **7512**: You found the page! Well done. Now the $3 is complete. Take this to San d'Oria swiftly and deliver it to Prince Pieuje.
...
```
  0: 0x0001 [0x1D] PRINT_EVENT_MESSAGE(message_id=7424*)
  ...
  3: 0x000A [0x1D] PRINT_EVENT_MESSAGE(message_id=7426*)
  ...
  5: 0x000E [0x1D] PRINT_EVENT_MESSAGE(message_id=7427*)
  ...
  7: 0x0012 [0x1D] PRINT_EVENT_MESSAGE(message_id=7428*)
  ...
  9: 0x0016 [0x1D] PRINT_EVENT_MESSAGE(message_id=7429*)
 ...
 11: 0x001A [0x1D] PRINT_EVENT_MESSAGE(message_id=7430*)
...
```
  ...
  1: 0x0025 [0x1D] PRINT_EVENT_MESSAGE(message_id=7434*)
...
```
  0: 0x002B [0x1D] PRINT_EVENT_MESSAGE(message_id=7423*)
...
```
  0: 0x0031 [0x1D] PRINT_EVENT_MESSAGE(message_id=7437*)
  ...
  2: 0x0035 [0x1D] PRINT_EVENT_MESSAGE(message_id=7438*)
  ...
  4: 0x0039 [0x1D] PRINT_EVENT_MESSAGE(message_id=7439*)
  ...
  6: 0x003D [0x1D] PRINT_EVENT_MESSAGE(message_id=7440*)
    → "But that is neither here nor there, eh?"
  7: 0x0040 [0x23] WAIT_FOR_DIALOG_INTERACTION
  8: 0x0041 [0x1D] PRINT_EVENT_MESSAGE(message_id=7441*)
 ...
 10: 0x0045 [0x1D] PRINT_EVENT_MESSAGE(message_id=7442*)
 ...
 12: 0x0049 [0x1D] PRINT_EVENT_MESSAGE(message_id=7443*)
 ...
 14: 0x004D [0x1D] PRINT_EVENT_MESSAGE(message_id=7444*)
...
```
  0: 0x0053 [0x1D] PRINT_EVENT_MESSAGE(message_id=7448*)
...
```
  0: 0x0095 [0x1D] PRINT_EVENT_MESSAGE(message_id=7504*)
...
```
  0: 0x009A [0x1D] PRINT_EVENT_MESSAGE(message_id=7505*)
...
```
  0: 0x009F [0x1D] PRINT_EVENT_MESSAGE(message_id=7506*)
...
```
  0: 0x00A4 [0x1D] PRINT_EVENT_MESSAGE(message_id=7507*)
...
```
  0: 0x00A9 [0x1D] PRINT_EVENT_MESSAGE(message_id=7508*)
...
```
  0: 0x00AE [0x1D] PRINT_EVENT_MESSAGE(message_id=7509*)
...
```
  0: 0x00B3 [0x1D] PRINT_EVENT_MESSAGE(message_id=7510*)
...
```
  0: 0x00B8 [0x1D] PRINT_EVENT_MESSAGE(message_id=7511*)
...
```
  0: 0x00BD [0x1D] PRINT_EVENT_MESSAGE(message_id=7512*)
