# Event 65535

## Metadata

| Field           | Value    |
|-----------------|----------|
| Event Index     | 11/11    |
| Offset          | 0x010B   |
| Data Size       | 31 bytes |
| Calculated Size | 31 bytes |
| Instructions    | 4        |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0100:                                   5B 14 80 F8 FF             [....
0110: FF 7F F8 FF FF 7F 66 61  6C 31 53 F8 FF FF 7F F8  ......fal1S.....
0120: FF FF 7F 66 61 6C 31 22  01 00                    ...fal1"..      
```

## Opcodes

```
  0: 0x010B [0x5B] LOAD_EXT_SCHEDULER(work_offset=14*, entity1=EventEntity, entity2=EventEntity, action="fal1")
  1: 0x011A [0x53] WAIT_SCHEDULER_TASK(entity1=EventEntity, entity2=EventEntity, action_id="fal1")
  2: 0x0127 [0x22] ENTITY_HIDE_FLAG(enabled=0x01)
  3: 0x0129 [0x00] END_REQSTACK()
```
