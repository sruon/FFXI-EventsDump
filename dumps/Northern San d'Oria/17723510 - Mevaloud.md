...
| [661](#event-661)        | 0x008A       |     33 |             12 |
| [662](#event-662)        | 0x00AB       |     33 |             12 |
...
- **7272**: Nearby is a large square with a fountain: the Parade Grounds. Each country has a consulate there.
- **7273**: Where are the consulates? Really, you adventurers should learn to open your main menu and select "Map" before asking directions!
- **7274**: Nearby is a large square with a fountain: the Parade Grounds. Each country has a consulate there.
- **7275**: You want to find the consulates? Really, you should go to Southern San d'Oria and buy a map from the lady in front the Westgate!
...
0090: 70 29 08 76 70 0E 01 01  1D 03 80 23 1D 04 80 23  p).vp......#...#
00A0: 29 08 76 70 0E 01 02 20  00 21 00                 ).vp... .!.     
```
...
```
  ...
  3: 0x0091 [0x29] REQ_SET_WAIT(priority=0x08, entity_id=Mevaloud (ID: 17723510/0x010E7076), tag_num=0x01)
  4: 0x0098 [0x1D] PRINT_EVENT_MESSAGE(message_id=7272*)
    → "Nearby is a large square with a fountain: the Parade Grounds. Each country has a consulate there."
  5: 0x009B [0x23] WAIT_FOR_DIALOG_INTERACTION
  6: 0x009C [0x1D] PRINT_EVENT_MESSAGE(message_id=7273*)
    → "Where are the consulates? Really, you adventurers should learn to open your main menu and select "Map" before asking directions!"
  7: 0x009F [0x23] WAIT_FOR_DIALOG_INTERACTION
  8: 0x00A0 [0x29] REQ_SET_WAIT(priority=0x08, entity_id=Mevaloud (ID: 17723510/0x010E7076), tag_num=0x02)
...
```
  ...
  4: 0x00B9 [0x1D] PRINT_EVENT_MESSAGE(message_id=7274*)
    → "Nearby is a large square with a fountain: the Parade Grounds. Each country has a consulate there."
  5: 0x00BC [0x23] WAIT_FOR_DIALOG_INTERACTION
  6: 0x00BD [0x1D] PRINT_EVENT_MESSAGE(message_id=7275*)
    → "You want to find the consulates? Really, you should go to Southern San d'Oria and buy a map from the lady in front the Westgate!"
