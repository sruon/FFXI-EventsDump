...
- **7702**: Oh my Gob! You're one of those adventurerurers, aren't ya? You got nothin' on me, [man./lady.] I was just mindin' my own businesesess, lookin' for junk an' stuff.
- **7703**: Hey, watch out. You don't wanna fight me. I'll turn you into meat jerky! Wait, how about you and me be friends instead? Sound good?
- **7704**: I travel all over the world and hear lots of differererent stories. I even hear your stories. I can tell you one, if you want.
- **7705**: Now, listen close...
- **7706**: Oh... Well, if that's the case, I'm outta here.
- **7712**: Would you like to hear a story? [No, thank you./Let's hear one./Let's hear one(2).]
- **7713**: Which story would you like to be told? [On second thought, maybe later./Leafallia(pt.1)./Leafallia(pt.2)./Rosulatia's Promise./The Lightsland(pt.1)./The Lightsland(pt.2)./The Light of Dawn Comes.../Cries from the Deep./Heroes, Unite!/Yggdrasil Beckons./The Key to the Turris./Blood for Blood./The Light Within./Vagary(pt.1)./Vagary(pt.2)./Vagary(pt.3)./Vagary(pt.4)./Vagary(pt.5)./Vagary(pt.6)./Vagary(pt.7)./Vagary(pt.8)./Vagary(pt.9)./Vagary(pt.10)./Vagary(pt.11)./The Silent Forest(pt.1)./The Silent Forest(pt.2)./The Silent Forest(pt.3)./Winds of Eternity(pt.1)./Winds of Eternity(pt.2)./Phantasmic Heroes(pt.1)./Phantasmic Heroes(pt.2)./Phantasmic Heroes(pt.3).]
- **7714**: Which story would you like to be told? [On second thought, maybe later./The Prime Weapons(pt.1)./The Prime Weapons(pt.2)./The Prime Weapons(pt.3).]
- **7715**: 
- **7716**: 
...
```
 ...
 15: 0x005A [0x1D] PRINT_EVENT_MESSAGE(message_id=7702*)
 ...
 17: 0x005E [0x1D] PRINT_EVENT_MESSAGE(message_id=7703*)
 ...
 19: 0x0062 [0x1D] PRINT_EVENT_MESSAGE(message_id=7704*)
 ...
 29: 0x0092 [0x24] CREATE_DIALOG(message_id=7712*, default_option=0*, option_flags=ExtData[1]->WorkLocal[0])
    → "Would you like to hear a story? [No, thank you./Let's hear one./Let's hear one(2).]"
 30: 0x0099 [0x25] WAIT_DIALOG_SELECT()
 31: 0x009A [0x02] IF !(Work_Zone[0] == 1*) GOTO 0x00DE

SUBROUTINE_00A2:
 32: 0x00A2 [0x24] CREATE_DIALOG(message_id=7713*, default_option=0*, option_flags=Work_Zone[2])
 ...
 37: 0x00C8 [0x1D] PRINT_EVENT_MESSAGE(message_id=7706*)
...
SUBROUTINE_00E6:
 43: 0x00E6 [0x24] CREATE_DIALOG(message_id=7714*, default_option=0*, option_flags=Work_Zone[3])
    → "Which story would you like to be told? [On second thought, maybe later./The Prime Weapons(pt.1)./The Prime Weapons(pt.2)./The Prime Weapons(pt.3).]"
 ...
 48: 0x010C [0x1D] PRINT_EVENT_MESSAGE(message_id=7706*)
    → "Oh... Well, if that's the case, I'm outta here."
...
SUBROUTINE_012A:
 54: 0x012A [0x24] CREATE_DIALOG(message_id=7715*, default_option=0*, option_flags=Work_Zone[4])
    → ""
 ...
 59: 0x0150 [0x1D] PRINT_EVENT_MESSAGE(message_id=7706*)
    → "Oh... Well, if that's the case, I'm outta here."
...
SUBROUTINE_016E:
 65: 0x016E [0x24] CREATE_DIALOG(message_id=7716*, default_option=0*, option_flags=Work_Zone[5])
    → ""
 ...
 70: 0x0194 [0x1D] PRINT_EVENT_MESSAGE(message_id=7706*)
    → "Oh... Well, if that's the case, I'm outta here."
...
SUBROUTINE_01A7:
 ...
 77: 0x01C0 [0x1D] PRINT_EVENT_MESSAGE(message_id=7706*)
    → "Oh... Well, if that's the case, I'm outta here."
...
SUBROUTINE_01C6:
 81: 0x01C6 [0x1D] PRINT_EVENT_MESSAGE(message_id=7705*)
    → "Now, listen close..."
