...
- **7665**: @
- **7960**: You see a diary.
- **7961**: Read diary? [Read it./Leave it be.]
- **7962**: Read which page? [On second thought, I'll pass./Page 1./Page 2./Page 3.]
- **7963**: Went to the grotto at Yughott yesterday. Aided an adventurer there who claimed to be on the Kingdom's duty.
- **7964**: Why could it be that the fiends are increasing so evidently in number?
- **7965**: I have visited many places, attempting to uncover the beastmen's plans, but never have I encountered such a creature.
- **7966**: I saw him there at the entrance to the cave. Thankfully, he did not note my presence.
- **7967**: He was the same as he has always been--both the good parts, and the bad.
- **7968**: That reminds me, I saw the waterfall again. I remember when I was a squire in training, going to fetch $6, so long ago.
- **7969**: A traveler told me the other day that a certain kind of moss can help alleviate severe coughs.
- **7970**: Perhaps I shall gather some on my next patrol, to give to that child and his mother. I do not know how much I will be able to carry, however.
- **7971**: If only there was someone who could deliver it in my stead...
- **9675**: Read which page? [On second thought, I'll pass./Page 1./Page 2./Page 3./Page 4.]
- **9676**: I have come across the most frightening information concerning the dark schemes of the beastmen. It appears they have succeeded in awakening their divinity.
- **9677**: The "Kindred" that inhabit the Northlands are said to have been created by this deity. Could the Twilight God have returned...?
- **9678**: I must leave for the north immediately, and determine the truth of these rumors firsthand. Lord Excenmille's army has begun operations to completely obliterate the fiendish forces that lie beyond the Northlands.
- **9679**: If I can but reach their camp, I should be able to further research this god of the beastmen. Fortunately, I was able to send word to the count, and an advance party will be waiting to greet me.
- **9680**: However, I must complete the journey across the Uleguerand Range alone. Without a map of the area, I foresee much backtracking and other such delays I cannot afford.
- **9681**: Though I am reluctant to do so, I will meet with my nephew, Antreneau. As a Royal Knight, he will have access to the charts I require.
...
```
  0: 0x0001 [0x48] [System] [7960*]:
    → "You see a diary."
  1: 0x0004 [0x23] WAIT_FOR_DIALOG_INTERACTION
  2: 0x0005 [0x24] CREATE_DIALOG(message_id=7961*, default_option=0*, option_flags=0*)
  ...
  8: 0x0036 [0x48] [System] [7665*]:
    → "@"
  9: 0x0039 [0x23] WAIT_FOR_DIALOG_INTERACTION
 10: 0x003A [0x48] [System] [7963*]:
...
```
  0: 0x0088 [0x48] [System] [7960*]:
    → "You see a diary."
  1: 0x008B [0x23] WAIT_FOR_DIALOG_INTERACTION
  2: 0x008C [0x24] CREATE_DIALOG(message_id=7961*, default_option=0*, option_flags=0*)
    → "Read diary? [Read it./Leave it be.]"
  3: 0x0093 [0x25] WAIT_DIALOG_SELECT()
  4: 0x0094 [0x02] IF !(Work_Zone[0] == 0*) GOTO 0x0190
  5: 0x009C [0x24] CREATE_DIALOG(message_id=7962*, default_option=0*, option_flags=8*)
...
```
  0: 0x01A2 [0x48] [System] [7960*]:
    → "You see a diary."
  1: 0x01A5 [0x23] WAIT_FOR_DIALOG_INTERACTION
  2: 0x01A6 [0x24] CREATE_DIALOG(message_id=7961*, default_option=0*, option_flags=0*)
    → "Read diary? [Read it./Leave it be.]"
  3: 0x01AD [0x25] WAIT_DIALOG_SELECT()
  4: 0x01AE [0x02] IF !(Work_Zone[0] == 0*) GOTO 0x0313
  5: 0x01B6 [0x24] CREATE_DIALOG(message_id=7962*, default_option=0*, option_flags=0*)
    → "Read which page? [On second thought, I'll pass./Page 1./Page 2./Page 3.]"
...
```
  0: 0x0325 [0x48] [System] [7960*]:
    → "You see a diary."
  1: 0x0328 [0x23] WAIT_FOR_DIALOG_INTERACTION
  2: 0x0329 [0x24] CREATE_DIALOG(message_id=7961*, default_option=0*, option_flags=0*)
    → "Read diary? [Read it./Leave it be.]"
  3: 0x0330 [0x25] WAIT_DIALOG_SELECT()
  4: 0x0331 [0x02] IF !(Work_Zone[0] == 0*) GOTO 0x0517
  5: 0x0339 [0x24] CREATE_DIALOG(message_id=9675*, default_option=0*, option_flags=0*)
...
```
  0: 0x0529 [0x48] [System] [7960*]:
    → "You see a diary."
  1: 0x052C [0x23] WAIT_FOR_DIALOG_INTERACTION
  2: 0x052D [0x24] CREATE_DIALOG(message_id=7961*, default_option=0*, option_flags=0*)
    → "Read diary? [Read it./Leave it be.]"
  3: 0x0534 [0x25] WAIT_DIALOG_SELECT()
  4: 0x0535 [0x02] IF !(Work_Zone[0] == 0*) GOTO 0x0713
  5: 0x053D [0x24] CREATE_DIALOG(message_id=9675*, default_option=0*, option_flags=0*)
    → "Read which page? [On second thought, I'll pass./Page 1./Page 2./Page 3./Page 4.]"
