...
```
  ...
  3: 0x0021 [0x1D] PRINT_EVENT_MESSAGE(message_id=3662*)
    → "There ain't enuff War Warlocks for us to fight a war right now. Afta' all, it's only been twenty -dd years since the last one...\u007F1\u0000\u0007"
  4: 0x0024 [0x23] WAIT_FOR_DIALOG_INTERACTION
  5: 0x0025 [0x1D] PRINT_EVENT_MESSAGE(message_id=3663*)
    → "There ain't been enuff time to train skilled people, so we'd be in a right proper fix if a war started all o' a sudden, wouldn't we now?\u007F1\u0000\u0007"
