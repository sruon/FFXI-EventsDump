# Event 65535

## Metadata

| Field           | Value    |
|-----------------|----------|
| Event Index     | 14/14    |
| Offset          | 0x006C   |
| Data Size       | 15 bytes |
| Calculated Size | 15 bytes |
| Instructions    | 5        |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0060:                                      32 07 80 1F              2...
0070: 00 0E 80 0F 80 0C 80 1F  01 6F 00                 .........o.     
```

## Opcodes

```
  0: 0x006C [0x32] ExtData[1]->MainSpeed = 11* * 0.1
  1: 0x006F [0x1F] MOVE_ENTITY: EventEntity moves to X=-527.476*, Z=647.097*, Y=0.050*
  2: 0x0077 [0x1F] MOVE_ENTITY: Update entity position (mode=1)
  3: 0x0079 [0x6F] WAIT_FRAME_DELAY: Yield until WaitTime reaches zero
  4: 0x007A [0x00] END_REQSTACK()
```
