# 17203827 - Marshal

## Common Data

| Field            | Value                   |
|------------------|-------------------------|
| Zone             | Jugner Forest (ID: 104) |
| Block Size       | 6060 bytes              |
| Total Events     | 2                       |
| References Count | 279                     |

## List of Events

| Event ID              | Entrypoint   |   Size |   Instructions |
|-----------------------|--------------|--------|----------------|
| [65535](#event-65535) | 0x0000       |      1 |              1 |
| [26](#event-26)       | 0x0001       |   4919 |           1171 |

## DAT References (imed_data)

|   Index | Hex Value   |   Dec Value |
|---------|-------------|-------------|
|       0 | 0x40000000  |  1073741824 |
|       1 | 0x0000      |           0 |
|       2 | 0x0001      |           1 |
|       3 | 0x0006      |           6 |
|       4 | 0x0002      |           2 |
|       5 | 0x0003      |           3 |
|       6 | 0x0005      |           5 |
|       7 | 0x1D5E      |        7518 |
|       8 | 0x07D0      |        2000 |
|       9 | 0x1D5F      |        7519 |
|      10 | 0x1D60      |        7520 |
|      11 | 0x000A      |          10 |
|      12 | 0x1D61      |        7521 |
|      13 | 0x1D62      |        7522 |
|      14 | 0x000F      |          15 |
|      15 | 0x0004      |           4 |
|      16 | 0x0007      |           7 |
|      17 | 0x0008      |           8 |
|      18 | 0x0009      |           9 |
|      19 | 0x000B      |          11 |
|      20 | 0x000C      |          12 |
|      21 | 0x000D      |          13 |
|      22 | 0x000E      |          14 |
|      23 | 0x0010      |          16 |
|      24 | 0x1D63      |        7523 |
|      25 | 0x001E      |          30 |
|      26 | 0x0011      |          17 |
|      27 | 0x106E      |        4206 |
|      28 | 0xFFFFFFFD  |  4294967293 |
|      29 | 0x1D65      |        7525 |
|      30 | 0x1D64      |        7524 |
|      31 | 0x1D6B      |        7531 |
|      32 | 0x1D69      |        7529 |
|      33 | 0x1D66      |        7526 |
|      34 | 0x1D67      |        7527 |
|      35 | 0x1D68      |        7528 |
|      36 | 0x1D75      |        7541 |
|      37 | 0x1D76      |        7542 |
|      38 | 0x1D77      |        7543 |
|      39 | 0x1D78      |        7544 |
|      40 | 0x1D79      |        7545 |
|      41 | 0x1D7A      |        7546 |
|      42 | 0xFFFFFFFB  |  4294967291 |
|      43 | 0x1DA3      |        7587 |
|      44 | 0x1DA4      |        7588 |
|      45 | 0xFFFFFFFF  |  4294967295 |
|      46 | 0x1DA6      |        7590 |
|      47 | 0xFFFFFFFE  |  4294967294 |
|      48 | 0x1DA7      |        7591 |
|      49 | 0x1DA1      |        7585 |
|      50 | 0x1D7E      |        7550 |
|      51 | 0x1D80      |        7552 |
|      52 | 0x1D81      |        7553 |
|      53 | 0x1D83      |        7555 |
|      54 | 0x1D84      |        7556 |
|      55 | 0x1D7F      |        7551 |
|      56 | 0x1D82      |        7554 |
|      57 | 0x1D85      |        7557 |
|      58 | 0x1DA0      |        7584 |
|      59 | 0x1D74      |        7540 |
|      60 | 0x97E00     |      622080 |
|      61 | 0x02AA      |         682 |
|      62 | 0x02AB      |         683 |
|      63 | 0x02AC      |         684 |
|      64 | 0x02AD      |         685 |
|      65 | 0x1D86      |        7558 |
|      66 | 0x1D87      |        7559 |
|      67 | 0xFFFFFFF9  |  4294967289 |
|      68 | 0x1D89      |        7561 |
|      69 | 0x1D7C      |        7548 |
|      70 | 0x1D7D      |        7549 |
|      71 | 0x1DA5      |        7589 |
|      72 | 0x1D9F      |        7583 |
|      73 | 0x1D6C      |        7532 |
|      74 | 0x1D6D      |        7533 |
|      75 | 0x1D6E      |        7534 |
|      76 | 0x1D6F      |        7535 |
|      77 | 0x1010      |        4112 |
|      78 | 0x027B      |         635 |
|      79 | 0x1014      |        4116 |
|      80 | 0x027C      |         636 |
|      81 | 0x002D      |          45 |
|      82 | 0x1018      |        4120 |
|      83 | 0x027D      |         637 |
|      84 | 0x005A      |          90 |
|      85 | 0x1020      |        4128 |
|      86 | 0x027E      |         638 |
|      87 | 0x0014      |          20 |
|      88 | 0x1024      |        4132 |
|      89 | 0x027F      |         639 |
|      90 | 0x003C      |          60 |
|      91 | 0x1028      |        4136 |
|      92 | 0x0280      |         640 |
|      93 | 0x0078      |         120 |
|      94 | 0x1031      |        4145 |
|      95 | 0x0281      |         641 |
|      96 | 0x00C8      |         200 |
|      97 | 0x1032      |        4146 |
|      98 | 0x0282      |         642 |
|      99 | 0x02BC      |         700 |
|     100 | 0x1033      |        4147 |
|     101 | 0x0283      |         643 |
|     102 | 0x012C      |         300 |
|     103 | 0x1068      |        4200 |
|     104 | 0x0284      |         644 |
|     105 | 0x147E      |        5246 |
|     106 | 0x0285      |         645 |
|     107 | 0x147F      |        5247 |
|     108 | 0x0286      |         646 |
|     109 | 0x0032      |          50 |
|     110 | 0x1480      |        5248 |
|     111 | 0x0287      |         647 |
|     112 | 0x1481      |        5249 |
|     113 | 0x0288      |         648 |
|     114 | 0x1482      |        5250 |
|     115 | 0x0289      |         649 |
|     116 | 0x1483      |        5251 |
|     117 | 0x028A      |         650 |
|     118 | 0x1484      |        5252 |
|     119 | 0x028B      |         651 |
|     120 | 0x106C      |        4204 |
|     121 | 0x02BE      |         702 |
|     122 | 0x0012      |          18 |
|     123 | 0x103B      |        4155 |
|     124 | 0x02D9      |         729 |
|     125 | 0x0064      |         100 |
|     126 | 0x0013      |          19 |
|     127 | 0x1035      |        4149 |
|     128 | 0x02DA      |         730 |
|     129 | 0x14C8      |        5320 |
|     130 | 0x02DB      |         731 |
|     131 | 0x004B      |          75 |
|     132 | 0x0015      |          21 |
|     133 | 0x14CA      |        5322 |
|     134 | 0x02F5      |         757 |
|     135 | 0x0016      |          22 |
|     136 | 0x02BF      |         703 |
|     137 | 0x1DAB      |        7595 |
|     138 | 0x1DAC      |        7596 |
|     139 | 0x1DAD      |        7597 |
|     140 | 0x1DAE      |        7598 |
|     141 | 0x1DAF      |        7599 |
|     142 | 0x1DB0      |        7600 |
|     143 | 0x1DB1      |        7601 |
|     144 | 0x1DB2      |        7602 |
|     145 | 0x1DB3      |        7603 |
|     146 | 0x1DB4      |        7604 |
|     147 | 0x1DB5      |        7605 |
|     148 | 0x1DB6      |        7606 |
|     149 | 0x1DB7      |        7607 |
|     150 | 0x1DB8      |        7608 |
|     151 | 0x1DB9      |        7609 |
|     152 | 0x1DBA      |        7610 |
|     153 | 0x1DBB      |        7611 |
|     154 | 0x1DBC      |        7612 |
|     155 | 0x1DBD      |        7613 |
|     156 | 0x1DBE      |        7614 |
|     157 | 0x1DBF      |        7615 |
|     158 | 0x1DC0      |        7616 |
|     159 | 0x1DC1      |        7617 |
|     160 | 0x1DC2      |        7618 |
|     161 | 0x1DC3      |        7619 |
|     162 | 0x1DC4      |        7620 |
|     163 | 0x1DC5      |        7621 |
|     164 | 0x1DC6      |        7622 |
|     165 | 0x1DC7      |        7623 |
|     166 | 0x1DC8      |        7624 |
|     167 | 0x1DC9      |        7625 |
|     168 | 0x1DCA      |        7626 |
|     169 | 0x1DCB      |        7627 |
|     170 | 0x1DCC      |        7628 |
|     171 | 0x1DCD      |        7629 |
|     172 | 0x1DCE      |        7630 |
|     173 | 0x1DCF      |        7631 |
|     174 | 0x1DD4      |        7636 |
|     175 | 0x1DD5      |        7637 |
|     176 | 0x1DD6      |        7638 |
|     177 | 0x0A20      |        2592 |
|     178 | 0x1DD7      |        7639 |
|     179 | 0x1DD8      |        7640 |
|     180 | 0x1DD9      |        7641 |
|     181 | 0x1DDA      |        7642 |
|     182 | 0x1DDB      |        7643 |
|     183 | 0x1DDC      |        7644 |
|     184 | 0x1DD0      |        7632 |
|     185 | 0x1DD1      |        7633 |
|     186 | 0x1DD2      |        7634 |
|     187 | 0x1DD3      |        7635 |
|     188 | 0x1DDD      |        7645 |
|     189 | 0x1DDE      |        7646 |
|     190 | 0x1DDF      |        7647 |
|     191 | 0x1DE0      |        7648 |
|     192 | 0x1DE1      |        7649 |
|     193 | 0x1DE2      |        7650 |
|     194 | 0x1DE3      |        7651 |
|     195 | 0x1DE4      |        7652 |
|     196 | 0x1DE5      |        7653 |
|     197 | 0x1DE6      |        7654 |
|     198 | 0x1DE7      |        7655 |
|     199 | 0x1DE8      |        7656 |
|     200 | 0x1DE9      |        7657 |
|     201 | 0x1DEA      |        7658 |
|     202 | 0x1DEB      |        7659 |
|     203 | 0x02AE      |         686 |
|     204 | 0x1DEC      |        7660 |
|     205 | 0x02AF      |         687 |
|     206 | 0x1DED      |        7661 |
|     207 | 0x02B0      |         688 |
|     208 | 0x1DEE      |        7662 |
|     209 | 0x02B1      |         689 |
|     210 | 0x1DEF      |        7663 |
|     211 | 0x02B2      |         690 |
|     212 | 0x1DF0      |        7664 |
|     213 | 0x02B3      |         691 |
|     214 | 0x1DF1      |        7665 |
|     215 | 0x02B4      |         692 |
|     216 | 0x1DF2      |        7666 |
|     217 | 0x02B5      |         693 |
|     218 | 0x1DF3      |        7667 |
|     219 | 0x02B6      |         694 |
|     220 | 0x1DF4      |        7668 |
|     221 | 0x02B7      |         695 |
|     222 | 0x1DF5      |        7669 |
|     223 | 0x02B8      |         696 |
|     224 | 0x1DF6      |        7670 |
|     225 | 0x02B9      |         697 |
|     226 | 0x1DF7      |        7671 |
|     227 | 0x02BA      |         698 |
|     228 | 0x1DF8      |        7672 |
|     229 | 0x02BB      |         699 |
|     230 | 0x1DF9      |        7673 |
|     231 | 0x1DFA      |        7674 |
|     232 | 0x0309      |         777 |
|     233 | 0x1DFB      |        7675 |
|     234 | 0x030A      |         778 |
|     235 | 0x1DFC      |        7676 |
|     236 | 0x030B      |         779 |
|     237 | 0x1DFD      |        7677 |
|     238 | 0x0393      |         915 |
|     239 | 0x1DFE      |        7678 |
|     240 | 0x0017      |          23 |
|     241 | 0x0394      |         916 |
|     242 | 0x1DFF      |        7679 |
|     243 | 0x0018      |          24 |
|     244 | 0x0737      |        1847 |
|     245 | 0x1E00      |        7680 |
|     246 | 0x0019      |          25 |
|     247 | 0x0738      |        1848 |
|     248 | 0x1E01      |        7681 |
|     249 | 0x025E      |         606 |
|     250 | 0x3600      |       13824 |
|     251 | 0x1D96      |        7574 |
|     252 | 0x1D97      |        7575 |
|     253 | 0x1D98      |        7576 |
|     254 | 0x00C9      |         201 |
|     255 | 0x1D70      |        7536 |
|     256 | 0x1D71      |        7537 |
|     257 | 0x1D73      |        7539 |
|     258 | 0x1DA8      |        7592 |
|     259 | 0x1D9A      |        7578 |
|     260 | 0xFFFFFFFC  |  4294967292 |
|     261 | 0xFFFFFFFA  |  4294967290 |
|     262 | 0x1D8A      |        7562 |
|     263 | 0x1D8E      |        7566 |
|     264 | 0x1D8F      |        7567 |
|     265 | 0x1D90      |        7568 |
|     266 | 0x1D8C      |        7564 |
|     267 | 0x1D8D      |        7565 |
|     268 | 0x001B      |          27 |
|     269 | 0x1D91      |        7569 |
|     270 | 0x1D92      |        7570 |
|     271 | 0x1D93      |        7571 |
|     272 | 0x1D95      |        7573 |
|     273 | 0x1D94      |        7572 |
|     274 | 0x001A      |          26 |
|     275 | 0x1DA2      |        7586 |
|     276 | 0x001D      |          29 |
|     277 | 0x001C      |          28 |
|     278 | 0x001F      |          31 |

## String References

- **7518**: I have been dispatched by the IRNC to handle all affairs dealing with Ballista Points and Ballista Point Ranking. My records show that you scored $0 Ballista [Point/Points] in your previous match, bringing your cumulative total to $1.
- **7519**: You have obtained the maximum number of Ballista Points ($0). Please note that you cannot earn any more until you have used the ones you possess.
- **7520**: What would you like to do? [Exchange points for items./View the previous match MVPs./Sign up for B.P. Ranking./View the B.P. Ranking./Learn more about Ballista Points./Learn more about ranking./Nothing at the moment.]
- **7521**: You currently possess $0 Ballista [Point/Points] and $1 temporary item [voucher/vouchers]. Remember, you can carry a maximum of $2 [voucher/vouchers] at a time, but no more than one of each type.
- **7522**: You currently possess $0 Ballista [Point/Points] and $1 temporary item [voucher/vouchers]. Remember, you can carry a maximum of $2 [voucher/vouchers] at a time, but no more than one of each type.
- **7523**: Exchange points for... [Previous page./6 ($1 [pt./pts.])/$6 ($3 [pt./pts.])/$6 ($5 [pt./pts.])/$6 ($7 [pt./pts.])/$6 ($9 [pt./pts.])/$6 ($11 [pt./pts.])/$6 ($13 [pt./pts.])/$6 ($15 [pt./pts.])/$6 ($17 [pt./pts.])/$6 ($19 [pt./pts.])/$6 ($21 [pt./pts.])/$6 ($23 [pt./pts.])/$6 ($25 [pt./pts.])/$6 ($27 [pt./pts.])/$6 ($29 [pt./pts.])/Next page./Nothing.]
- **7524**: I see that you are wearing $6. Ballista point exchange authorization has been granted.
- **7525**: I am sorry. You have not earned the right to exchange your points for that item.
- **7526**: The following is the item that will be automatically presented to you at the start of the Ballista match in exchange for the $3:
- **7527**: Obtain this item for $1 [point/points]? [Yes./Return./No.]
- **7528**: You do not have enough Ballista Points to purchase that item.
- **7529**: You already possess $6. Remember, you cannot hold more than one of each type of temporary item voucher.
- **7531**: Adventurers are only allowed to possess a maximum of $3 temporary item [voucher/vouchers]. I am sorry, but I cannot proceed with the transaction.
- **7532**: Understood. Let me read to you the official Ballista rulebook chapter on Ballista Points.
- **7533**: Is there anything else I can do for you?
- **7534**: Understood. Let me read to you the official Ballista rulebook chapter on Ballista Point Ranking.
- **7535**: Is there anything else I can do for you?
- **7539**: Well done, <Player>! You are right on track to become the next "Balli$25star"!
- **7540**: Ballista Point Ranking registration [/for the Jugner Forest match/for the Pashhow Marshlands match/for the Meriphataud Mountains match] has concluded. Please do not forget to register your Ballista Points immediately after each match to avoid any confusion.
- **7541**: You did not obtain any Ballista Points in the previous match; thus, I cannot register you.
- **7542**: Please participate in a future match to qualify once again for Ballista Point Ranking. The next exercise is scheduled to take place [soon/in the Jugner Forest/in the Pashhow Marshlands/in the Meriphataud Mountains].
- **7543**: Your current job does not coincide with the data we have recorded for the previous match. You must return to your residence and change jobs before I can register you for Ballista Point Ranking.
- **7544**: According to the [/Jugner Forest/Pashhow Marshlands/Meriphataud Mountains] match records for , you earned $1 Ballista [Point/Points].
- **7545**: Shall I register your statistics? (This may take a few moments.)
- **7546**: Register? [Yes./No.]
- **7548**: It may take a few moments for me to find the ranking list.
- **7549**: Do you still wish to see it? [Yes./No.]
- **7550**: Your points have been registered. Congratulations! You currently rank in the top ten!
- **7551**: Your points have been registered. Congratulations! You are currently ranked as the top [/warrior/monk/white mage/black mage/red mage/thief/paladin/dark knight/beastmaster/bard/ranger/samurai/ninja/dragoon/summoner/blue mage/corsair/puppetmaster]! (Total [/warriors/monks/white mages/black mages/red mages/thieves/paladins/dark knights/beastmasters/bards/rangers/samurais/ninjas/dragoons/summoners/blue mages/corsairs/puppetmasters/dancers/scholars/geomancers/rune fencers] registered: $7)
- **7552**: We ask that you check the rankings on the following date: Vana'diel: Earth:
- **7553**: If, at that time, you still remain in the top ten, you will be rewarded with a special prize.
- **7554**: If, at this time, you still remain in first place, you will be recognized as the match MVP.
- **7555**: However, all records of the match will be destroyed on (Vana'diel) or (Earth).
- **7556**: If you do not speak with a Marshal by that date, you will lose the opportunity to claim that prize.
- **7557**: If you do not speak with a Marshal by that date, you forfeit all benefits that accompany MVP status.
- **7558**: You currently possess $6. Ballista regulations allow you to wear that chevron until (Vana'diel), or (Earth).
- **7559**: Our records show that you have previously been presented the following rewards: Gold chevron: $1 [time/times] Mythril chevron: $2 [time/times] Silver chevron: $3 [time/times] Bronze chevron: $4 [time/times] Job-specific chevron: $5 [time/times]
- **7561**: Please note that because the point registration period for the previous match has not yet ended, your ranking may change. We ask that you check back later for the final results.
- **7562**: Your performance in the last match earned you a place in the overall ranking. However, because you did not check with a Herald before the records were destroyed, you have lost the opportunity to claim your prize.
- **7564**: Congratulations! Ballista Point Ranking registration for the previous match has come to a close, and your final overall rank is $0! You shall be presented with $6 and a new title.
- **7565**: Good show! Ballista Point Ranking registration for the previous match has come to a close, and your final rank among participating [/warriors/monks/white mages/black mages/red mages/thieves/paladins/dark knights/beastmasters/bards/rangers/samurais/ninjas/dragoons/summoners/blue mage/corsairs/puppetmasters/dancers/scholars/geomancers/rune fencers] ($3 total) is first! You shall be presented with $6 and a new title.
- **7566**: Well done! Ballista Point Ranking registration for the previous match has come to a close, and your final overall rank is $0! In addition, your final rank among participating [/warriors/monks/white mages/black mages/red mages/thieves/paladins/dark knights/beastmasters/bards/rangers/samurais/ninjas/dragoons/summoners/blue mage/corsairs/puppetmasters/dancers/scholars/geomancers/rune fencers] ($3 total) is first! You shall be presented with $6 or $6, as well as a new title.
- **7567**: While our records will clearly indicate that you were rewarded both chevrons, Ballista regulations only allow participants to wear one at a time. Therefore, you must choose which one you wish to receive at this time.
- **7568**: Please select one... [$3./$3.]
- **7569**: You are currently wearing $6. If you wish to accept the chevron you earned this match, you must return the one you currently possess.
- **7570**: Accept the $3? [Yes./No.]
- **7571**: Are you sure you wish to exchange your $3 for $6?
- **7572**: Are you sure you do not wish to accept the $3? Remember that even if you do not accept the chevron, a record of your award will be kept.
- **7573**: Well? [I'm sure./No.]
- **7574**: As long as you are in possession of the $3, you will bear the aforementioned title while participating in Ballista.
- **7575**: Upon the chance another contestant "checks" you during a match, not only will he be able to see your title, but also the total number of chevrons you have accumulated in your battles.
- **7576**: Finally, the official rules of Ballista state that after a set period of time, you are required to return any chevrons received. When that time has come, a Herald will collect your chevrons at the time of entry into a new match.
- **7578**: You have already confirmed your data for the [/Jugner Forest/Pashhow Marshlands/Meriphataud Mountains] match. We look forward to your participation in future matches!
- **7583**: Your data did not make the final ranking list.
- **7584**: I have finished registering your data. Your final Ballista Point Ranking for the previous match is: $7.
- **7585**: I registered your point total; however, it seems you did not make the final list. But do not fret. You will have many chances to redeem yourself in future matches. Good luck, <Player>!
- **7586**: Your final Ballista Point Ranking for the previous match is: $0. (Rank among [/warriors/monks/white mages/black mages/red mages/thieves/paladins/dark knights/beastmasters/bards/rangers/samurais/ninjas/dragoons/summoners/blue mages/corsairs/puppetmasters/dancers/scholars/geomancers/rune fencers]: $3) However, a little more practice, and the champion's throne is yours for the taking!
- **7587**: I am sorry, but we are no longer accepting Ballista Point Ranking registrations for the match in which you previously participated.
- **7588**: For future reference, please note that the registration period is exactly four hours (Vana'diel time) from the close of each Ballista match.
- **7591**: You have already registered your Ballista Points for the previous match. Your current rank is $0.
- **7592**: $26$3V$3X$3e$3 :$0@$26$13$P13o%^$20:$1 $26$)$7<Player>J:$2@$26$P13o%^$3W$3 $3u:$3 $26$P13/$3W$3 $3u$P13o%^$20:$4@$26$3W$3 $3u$P15J$)$7<Player>J:$5 $26$P13o%^BP:$6@$26TIME:$7
- **7595**: What do you wish to know about? [Basic rules./About the Petra./About Sprint./About Scout./About Gate Breach./About area-specific rules./About returning to battle./About special rules./Nothing at the moment.]
- **7596**: Ballista is an exercise once used by the San d'Orian armies to train the Royal Knights in the art of the siege. The word "ballista" means "to throw a stone," and the game has been a part of Vana'dielian military history for centuries.
- **7597**: The rules are simple. Ballista is played by two teams over a number of hours (Vana'diel time). The team with the most points at the end of the match is the victor.
- **7598**: Players from both teams must search for Petras that a Herald has buried in various locations across the battlefield. The players must then take these stones and throw them into targets known as Rooks.
- **7599**: These Rooks can appear in various locations around the battlefield, and both teams are allowed to throw their Petras into them.
- **7600**: Finally, a player's strength is adjusted depending on his or her level. It is felt that this ultimately allows for a more balanced battle.
- **7601**: Petras can be buried anywhere. Once a player believes he or she has found one, the Quarry (/quarry) command should be used to dig up the area at his or her feet.
- **7602**: Sometimes a player may find items other than Petras, such as potions. These temporary items can only be used during the current match (or Best of Three series).
- **7603**: Players can carry more than one Petra, but they cannot throw the stones away once they have obtained them. However, players will lose their Petras if they: (1) Successfully throw them into a rook. (2) Leave the area. (3) Are knocked out.
- **7604**: But don't forget, a player will be unable to use the Sprint command while in possession of a Petra. And, players can only use the Scout command if they carry one of the stones.
- **7605**: As one can see, careful planning and impeccable timing are the keys to victory.
- **7606**: All participants of Ballista will have the ability to use the Sprint command. This will temporarily increase one's movement speed, creating a better chance to find and obtain Petras.
- **7607**: However, remember that once a player has obtained a Petra, he or she will be unable to use the Sprint command. In its place, though, one may use the Scout command.
- **7608**: The locations of the Rooks are constantly changing. This makes it quite difficult to find the targets by just randomly walking about the area.
- **7609**: Communication with one's team members is, of course, one way to locate Rooks. However, another way is by using the Scout command. This will inform a player of the approximate location of the nearest Rook.
- **7610**: Finally, Scout can only be used when a player is carrying a Petra. Because Sprint cannot be used at this time, players must be weary of approaching enemies.
- **7611**: So, a Petra has been obtained and a Rook located. Now all that's left is tossing the stone in, right? Wrong.
- **7612**: There is one aspect of Ballista that has yet to be mentioned...and this is where the real battle begins.
- **7613**: To throw a Petra into a Rook, a player must first wait until the Rook's gate is open. This takes impeccable timing, as the gate is only open for a short while.
- **7614**: But not just anyone can score points this way. A player must first take down a member of the opposing team and achieve Gate Breach status. Only then will his or her shots count.
- **7615**: This extreme method of combat is what made the ancient game of Ballista famous, and transformed its participants into heroes.
- **7616**: Running about looking for stones is no different from a child's game of hide-and-seek. Knowing when to strike is what separates the men from the boys.
- **7617**: The following match types are area-specific: All or Nothing (one match) * Normal Items Allowed: Jugner Forest All or Nothing (one match) * Normal Items Restricted: Pashhow Marshlands Best of Three (three matches) * Normal Items Allowed: Meriphataud Mountains
- **7618**: Best of Three series rules are as follows: Winning a match will earn a nation one "match point." The nation with the most match points at the end of the series will be declared the victor. Winning the third and final match and producing the high scorer will result in two match points awarded. This allows both sides the chance for "come-from-behind" victories.
- **7619**: Between each of the matches, a short interval will be implemented. During this interval, Invisible and Sneak will be cast on all participants. We suggest this time be used to discuss strategies with the other members of your nation's forces.
- **7620**: Players will be forbidden from using any items they bring to the battlefield. Only temporary items that are quarried may be used. A total of fifteen temporary items (though only one of each type) can be carried.
- **7621**: When this rule is in effect, all players will be presented with five complimentary temporary items at the beginning of the match.
- **7622**: For more information on temporary items, refer to the "special rules."
- **7623**: After being KO'd, if a player chooses to be revived at the spot he was defeated, 100% of all HP and 50% of all MP will be restored. In addition, the player will maintain 50% of the TP he had earned.
- **7624**: If a player chooses to return to their force's camp, HP and MP will be fully restored, but all TP will be lost.
- **7625**: Please note that many of the rules previously considered "special" have been fully implemented. Also, several new special rules have been added.
- **7626**: Learn about which special rules? [Temporary items./Ballista Points./Scoring restrictions./Gate Breach radius expansion./Resistance to Sleep./Open entries./Minimum level restrictions./Re-entry restrictions./Participant & Rook numbers./Experience Points./Return.]
- **7627**: Previously, participants in Ballista were required to use an item the moment they dug it up using the /quarry command. However, players can now stock these items as temporary items for use at any time during the match.
- **7628**: Players can hold up to ten temporary items, but only one of each type. If a player finds an item that they already possess, they must use it immediately or throw it away.
- **7629**: If players who possess ten temporary items find an item that they have not already obtained, they will also have the option to exchange it with one of their current items.
- **7630**: Also, players will not, at any time, be allowed to trade temporary items. However, there are items that can be used on other Ballista participants. Please note that players may not be able to use these special items as soon as they are quarried.
- **7631**: Finally, the Heralds will collect temporary items when the match (the third in a Best of Three series) has concluded.
- **7632**: Ballista Points are rewarded to each competitor based on his or her performance in a single Ballista match. The maximum number of points that can be accumulated per match is $0.
- **7633**: While it is taken into consideration, scoring Petras is not the only way to earn Ballista Points. Assisting allies and defeating members of the enemy forces can also earn a high score.
- **7634**: Ballista Points can be traded for special "vouchers," which may be exchanged for free temporary items at the start of new matches.
- **7635**: However, remember that depending on the level restriction of the match, certain vouchers will not be redeemed at the beginning of a match. Speak with a Marshal directly after a match to register your points.
- **7636**: Players will only be allowed to score up to five Petras at a time. Also, a player's Gate Breach status will be automatically disengaged if a Petra is not scored within a certain amount of time. The amount of time is increased for those members of the team currently behind in points.
- **7637**: All allies within a certain radius will achieve Gate Breach status when a member of the enemy forces is defeated.
- **7638**: Players' resistance to sleep will be increased.
- **7639**: During the entry period, from $0:, all adventurers will be allowed to join the forces with the smallest number of participants, regardless of allegiance. Open entries will be accepted until both sides have reached maximum capacity.
- **7640**: In order to ensure balanced matches, the following level limits are enforced: Level 30 matches: 1\`30
- **7641**: Level 40 matches: 31\`40 Level 50 matches: 41\`50 Level 60 matches: 51\`60 Unrestricted matches: 61\`
- **7642**: Please note that any participant who cancels his or her registration will not be allowed to re-apply for the same match.
- **7643**: When the total number of participants in a match is less than 40, the location of the two camps will be adjusted. Also in this case, the number of Rooks will be reduced to two.
- **7644**: Any adventurer whose main level is over 30 will receive EXP for participating in a Ballista match. However, no EXP will be rewarded if the adventurer fails to actively take part in the match.
- **7645**: Read which chapter on B.P. Ranking? [Basic Information./Registering Points./Rewards./Chevron & Title List./None.]
- **7646**: Ballista Point Ranking is based upon all Ballista Points registered from the past ten matches in each of the three participating battle areas.
- **7647**: Anyone who wishes to take part in the ranking needs to register [his/her] points within four hours (Vana'diel time) from the end of the previous match.
- **7648**: After the registration period ends, players can compare their record with those of the previous nine matches.
- **7649**: If, at any time, a player ranks in the top ten (overall) or first for a specific job, [he/she] will have the opportunity to receive a special chevron as well as a new title.
- **7650**: There are four types of chevrons and titles awarded to top-ten players (gold/mythril/silver/bronze), as well as twenty-two different types awarded for outstanding performances by players of each of the twenty-two jobs.
- **7651**: Wearing a chevron during a Ballista match will automatically change a player's current title. If a player wishes to possess this title after a match as well, [he/she] must contact a certain bard in San d'Oria.
- **7652**: Also, while wearing a chevron, players will be able to trade Ballista Points for temporary items not available to all players.
- **7653**: However, the rules of Ballista state that players may only possess a certain chevron for no longer than one half Vana'diel year. When that time comes, they are required to return the chevron before being allowed to participate in any further matches.
- **7654**: Finally, checking someone during a Ballista match will display the total number of chevrons that person has accumulated to date.
- **7655**: View which chevron/title? [1st (overall)./2nd (overall)./3rd (overall)./4th-10th (overall)./1st (warrior)./1st (monk)./1st (white mage)./1st (black mage)./1st (red mage)./1st (thief)./1st (paladin)./1st (dark knight)./1st (beastmaster)./1st (bard)./1st (ranger)./1st (samurai)./1st (ninja)./1st (dragoon)./1st (summoner)./1st (blue mage)./1st (corsair)./1st (puppetmaster)./1st (dancer)./1st (scholar)./1st (geomancer)./1st (rune fencer)./Return.]
- **7656**: Key Item: $3 Title: Gold Balli$25$25$25$25star
- **7657**: Key Item: $3 Title: Mythril Balli$25$25$25star
- **7658**: Key Item: $3 Title: Silver Balli$25$25star
- **7659**: Key Item: $3 Title: Bronze Balli$25star
- **7660**: Key Item: $3 Title: Searing$26Star
- **7661**: Key Item: $3 Title: Striking$26Star
- **7662**: Key Item: $3 Title: Soothing$26Star
- **7663**: Key Item: $3 Title: Sable$26Star
- **7664**: Key Item: $3 Title: Scarlet$26Star
- **7665**: Key Item: $3 Title: Sonic$26Star
- **7666**: Key Item: $3 Title: Saintly$26Star
- **7667**: Key Item: $3 Title: Shadowy$26Star
- **7668**: Key Item: $3 Title: Savage$26Star
- **7669**: Key Item: $3 Title: Singing$26Star
- **7670**: Key Item: $3 Title: Sniping$26Star
- **7671**: Key Item: $3 Title: Slicing$26Star
- **7672**: Key Item: $3 Title: Sneaking$26Star
- **7673**: Key Item: $3 Title: Spearing$26Star
- **7674**: Key Item: $3 Title: Summoning$26Star
- **7675**: Key Item: $3 Title: Sapphire$26Star
- **7676**: Key Item: $3 Title: Surging$26Star
- **7677**: Key Item: $3 Title: Swaying$26Star
- **7678**: Key Item: $3 Title: Sprightly$26Star
- **7679**: Key Item: $3 Title: Sagacious$26Star
- **7680**: Key Item: $3 Title: Spiritual$26Star
- **7681**: Key Item: $3 Title: Stippling$26Star

## Events

### Event 65535

#### Metadata

| Field        | Value   |
|--------------|---------|
| Entrypoint   | 0x0000  |
| Data Size    | 1 bytes |
| Instructions | 1       |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0000: 00                                                .               
```

#### Opcodes

```
  0: 0x0000 [0x00] END_REQSTACK()
```

### Event 26

#### Metadata

| Field        | Value      |
|--------------|------------|
| Entrypoint   | 0x0001     |
| Data Size    | 4919 bytes |
| Instructions | 559        |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0000:    03 01 10 00 80 20 01  1E F0 FF FF 7F 6F 70 03   ..... ......op.
0010: 04 00 02 10 03 05 00 03  10 03 08 00 04 10 3E 05  ..............>.
0020: 10 01 80 28 00 05 1E 00  3E 05 10 02 80 32 00 05  ...(....>....2..
0030: 20 00 03 12 00 06 10 03  13 00 07 10 03 1D 00 08   ...............
0040: 10 03 09 00 09 10 06 00  00 3E 1D 00 03 80 57 00  .........>....W.
0050: 3C 00 00 01 80 02 80 06  01 00 3E 1D 00 04 80 64  <.........>....d
0060: 00 05 01 00 3E 1D 00 05  80 6E 00 05 01 00 02 01  ....>....n......
0070: 00 01 80 00 7D 00 3C 00  00 02 80 02 80 3E 1D 00  ....}.<......>..
0080: 01 80 8E 00 3C 00 00 02  80 02 80 01 A3 00 3C 00  ....<.........<.
0090: 00 04 80 02 80 3C 00 00  05 80 02 80 3C 00 00 06  .....<......<...
00A0: 80 02 80 03 02 10 13 00  03 03 10 04 00 1D 07 80  ................
00B0: 23 02 04 00 08 80 04 C2  00 03 02 10 08 80 1D 09  #...............
00C0: 80 23 24 0A 80 01 80 00  00 25 02 00 10 01 80 00  .#$......%......
00D0: 9C 04 03 02 10 04 00 03  03 10 08 00 03 04 10 0B  ................
00E0: 80 02 08 00 01 80 02 F0  00 1D 0C 80 23 01 F4 00  ............#...
00F0: 1D 0D 80 23 06 06 00 06  07 00 06 00 00 03 03 00  ...#............
0100: 02 80 02 03 00 0E 80 05  65 02 03 22 00 06 00 1A  ........e.."....
0110: 2D 07 02 03 00 02 80 80  27 01 03 02 10 25 00 03  -.......'....%..
0120: 03 10 24 00 01 4D 02 02  03 00 04 80 80 3C 01 03  ..$..M.......<..
0130: 04 10 25 00 03 05 10 24  00 01 4D 02 02 03 00 05  ..%....$..M.....
0140: 80 80 51 01 03 06 10 25  00 03 07 10 24 00 01 4D  ..Q....%....$..M
0150: 02 02 03 00 0F 80 80 66  01 03 08 10 25 00 03 09  .......f....%...
0160: 10 24 00 01 4D 02 02 03  00 06 80 80 7B 01 03 00  .$..M.......{...
0170: 17 25 00 03 01 17 24 00  01 4D 02 02 03 00 03 80  .%....$..M......
0180: 80 90 01 03 02 17 25 00  03 03 17 24 00 01 4D 02  ......%....$..M.
0190: 02 03 00 10 80 80 A5 01  03 04 17 25 00 03 05 17  ...........%....
01A0: 24 00 01 4D 02 02 03 00  11 80 80 BA 01 03 06 17  $..M............
01B0: 25 00 03 07 17 24 00 01  4D 02 02 03 00 12 80 80  %....$..M.......
01C0: CF 01 03 08 17 25 00 03  09 17 24 00 01 4D 02 02  .....%....$..M..
01D0: 03 00 0B 80 80 E4 01 03  0A 17 25 00 03 0B 17 24  ..........%....$
01E0: 00 01 4D 02 02 03 00 13  80 80 F9 01 03 0C 17 25  ..M............%
01F0: 00 03 0D 17 24 00 01 4D  02 02 03 00 14 80 80 0E  ....$..M........
0200: 02 03 0E 17 25 00 03 0F  17 24 00 01 4D 02 02 03  ....%....$..M...
0210: 00 15 80 80 23 02 03 10  17 25 00 03 11 17 24 00  ....#....%....$.
0220: 01 4D 02 02 03 00 16 80  80 38 02 03 12 17 25 00  .M.......8....%.
0230: 03 13 17 24 00 01 4D 02  02 03 00 0E 80 80 4D 02  ...$..M.......M.
0240: 03 14 17 25 00 03 15 17  24 00 01 4D 02 02 23 00  ...%....$..M..#.
0250: 01 80 00 5C 02 3C 00 00  03 00 02 80 0B 03 00 0B  ...\.<..........
0260: 06 00 01 02 01 03 22 00  06 00 08 22 00 17 80 1A  ......"...."....
0270: 2D 07 02 23 00 01 80 00  81 02 3C 00 00 01 80 02  -..#......<.....
0280: 80 03 22 00 06 00 1A 2D  07 02 23 00 01 80 00 98  .."....-..#.....
0290: 02 3C 00 00 17 80 02 80  24 18 80 07 00 00 00 25  .<......$......%
02A0: 02 00 10 01 80 00 B6 02  06 07 00 08 06 00 19 80  ................
02B0: 01 FA 00 01 99 04 02 00  10 17 80 00 C7 02 06 07  ................
02C0: 00 01 FA 00 01 99 04 02  00 10 1A 80 00 D2 02 01  ................
02D0: 99 04 02 00 10 02 80 80  E2 02 03 07 00 02 80 01  ................
02E0: C2 03 02 00 10 04 80 80  F2 02 03 07 00 04 80 01  ................
02F0: C2 03 02 00 10 05 80 80  02 03 03 07 00 05 80 01  ................
0300: C2 03 02 00 10 0F 80 80  12 03 03 07 00 0F 80 01  ................
0310: C2 03 02 00 10 06 80 80  22 03 03 07 00 06 80 01  ........".......
0320: C2 03 02 00 10 03 80 80  32 03 03 07 00 03 80 01  ........2.......
0330: C2 03 02 00 10 10 80 80  42 03 03 07 00 10 80 01  ........B.......
0340: C2 03 02 00 10 11 80 80  52 03 03 07 00 11 80 01  ........R.......
0350: C2 03 02 00 10 12 80 80  62 03 03 07 00 12 80 01  ........b.......
0360: C2 03 02 00 10 0B 80 80  72 03 03 07 00 0B 80 01  ........r.......
0370: C2 03 02 00 10 13 80 80  82 03 03 07 00 13 80 01  ................
0380: C2 03 02 00 10 14 80 80  92 03 03 07 00 14 80 01  ................
0390: C2 03 02 00 10 15 80 80  A2 03 03 07 00 15 80 01  ................
03A0: C2 03 02 00 10 16 80 80  B2 03 03 07 00 16 80 01  ................
03B0: C2 03 02 00 10 0E 80 80  C2 03 03 07 00 0E 80 01  ................
03C0: C2 03 03 22 00 06 00 08  22 00 17 80 07 22 00 00  ..."...."...."..
03D0: 10 1A 2D 07 93 23 00 02  23 00 1B 80 00 08 04 03  ..-..#..#.......
03E0: 01 10 1C 80 43 00 43 01  03 01 10 00 80 02 09 10  ....C.C.........
03F0: 01 80 00 04 04 1D 1D 80  23 08 06 00 0E 80 01 FA  ........#.......
0400: 00 01 08 04 1D 1E 80 23  03 02 10 25 00 03 03 10  .......#...%....
0410: 24 00 03 04 10 23 00 02  08 00 0B 80 04 2B 04 03  $....#.......+..
0420: 05 10 0B 80 1D 1F 80 23  01 99 04 3E 05 00 22 00  .......#...>..".
0430: 41 04 1D 20 80 23 08 06  00 0E 80 01 FA 00 01 99  A.. .#..........
0440: 04 02 04 00 24 00 04 8A  04 1D 21 80 23 93 01 80  ....$.....!.#...
0450: 24 22 80 02 80 01 80 25  02 00 10 01 80 00 69 04  $".....%......i.
0460: 42 03 01 10 22 00 01 87  04 02 00 10 02 80 00 7C  B..."..........|
0470: 04 08 06 00 0E 80 01 FA  00 01 87 04 02 00 10 04  ................
0480: 80 00 87 04 01 87 04 01  99 04 1D 23 80 23 93 01  ...........#.#..
0490: 80 08 06 00 0E 80 01 FA  00 01 2B 07 02 00 10 02  ..........+.....
04A0: 80 00 AA 04 1A 5D 0E 01  2B 07 02 00 10 04 80 00  .....]..+.......
04B0: 14 06 3E 1D 00 02 80 00  06 1A 5D 0E 02 13 00 01  ..>.......].....
04C0: 80 00 D7 04 1A 12 0E 03  04 10 1C 00 1D 24 80 23  .............$.#
04D0: 1D 25 80 23 01 FD 05 02  1E 00 02 80 00 E6 04 1D  .%.#............
04E0: 26 80 23 01 FD 05 03 02  10 12 00 03 03 10 13 00  &.#.............
04F0: 1A 12 0E 03 04 10 1C 00  1D 27 80 23 1D 28 80 23  .........'.#.(.#
0500: 24 29 80 02 80 01 80 25  02 00 10 01 80 00 FD 05  $).....%........
0510: 1A 12 0E 03 01 10 2A 80  43 00 43 01 03 01 10 00  ......*.C.C.....
0520: 80 02 02 10 01 80 00 34  05 1D 2B 80 23 1D 2C 80  .......4..+.#.,.
0530: 23 01 FA 05 42 B3 03 1C  00 B3 05 1A 00 16 00 15  #...B...........
0540: 00 17 00 19 00 18 00 1B  00 14 00 1A EA 0E 02 1A  ................
0550: 00 2D 80 80 5D 05 1D 2E  80 23 01 FA 05 02 1A 00  .-..]....#......
0560: 2F 80 80 71 05 03 02 10  15 00 1D 30 80 23 01 FA  /..q.......0.#..
0570: 05 02 1A 00 1C 80 80 84  05 1D 2B 80 23 1D 2C 80  ..........+.#.,.
0580: 23 01 FA 05 02 15 00 01  80 80 93 05 1D 31 80 23  #............1.#
0590: 01 FA 05 03 01 10 2F 80  43 00 43 01 03 01 10 00  ....../.C.C.....
05A0: 80 03 01 00 14 00 1A 18  0E 02 15 00 0B 80 05 C8  ................
05B0: 05 1D 32 80 23 1D 33 80  23 1D 34 80 23 1D 35 80  ..2.#.3.#.4.#.5.
05C0: 23 1D 36 80 23 01 FA 05  02 18 00 02 80 00 F1 05  #.6.#...........
05D0: 03 08 10 17 00 03 09 10  19 00 1D 37 80 23 1D 33  ...........7.#.3
05E0: 80 23 1D 38 80 23 1D 35  80 23 1D 39 80 23 01 FA  .#.8.#.5.#.9.#..
05F0: 05 03 09 10 15 00 1D 3A  80 23 01 FD 05 01 0C 06  .......:.#......
0600: 1A 12 0E 03 04 10 1C 00  1D 3B 80 23 03 01 10 00  .........;.#....
0610: 80 01 2B 07 02 00 10 05  80 00 F9 06 1A 5D 0E 03  ..+..........]..
0620: 01 10 1C 80 43 00 43 01  03 01 10 00 80 03 1F 00  ....C.C.........
0630: 02 10 03 21 00 09 10 07  08 10 3C 80 03 00 17 3D  ...!......<....=
0640: 80 03 01 17 3E 80 03 02  17 3F 80 03 03 17 40 80  ....>....?....@.
0650: 02 21 00 01 80 01 5C 06  1D 41 80 23 1D 42 80 23  .!....\..A.#.B.#
0660: 1A 12 0E 03 01 10 43 80  43 00 43 01 03 01 10 00  ......C.C.C.....
0670: 80 02 02 10 02 80 00 91  06 1D 44 80 23 03 01 00  ..........D.#...
0680: 03 10 1A 18 0E 1D 33 80  23 03 01 10 00 80 01 F6  ......3.#.......
0690: 06 03 04 10 1C 00 1D 45  80 23 24 46 80 02 80 01  .......E.#$F....
06A0: 80 25 02 00 10 01 80 00  F1 06 42 B3 00 1C 00 B3  .%........B.....
06B0: 01 1A 00 16 00 15 00 17  00 19 00 18 00 1A EA 0E  ................
06C0: 02 1A 00 2D 80 00 D4 06  1D 47 80 23 03 01 10 00  ...-.....G.#....
06D0: 80 01 EE 06 1A 4B 0E 02  15 00 01 80 00 EB 06 1D  .....K..........
06E0: 48 80 23 03 01 10 00 80  01 EE 06 1A 1F 0F 01 F6  H.#.............
06F0: 06 03 01 10 00 80 01 2B  07 02 00 10 0F 80 00 12  .......+........
0700: 07 1D 49 80 23 1A 77 0B  1D 4A 80 23 01 C2 00 01  ..I.#.w..J.#....
0710: 2B 07 02 00 10 06 80 00  2B 07 1D 4B 80 23 1A 8D  +.......+..K.#..
0720: 0B 1D 4C 80 23 01 C2 00  01 2B 07 21 00 02 22 00  ..L.#....+.!..".
0730: 01 80 80 47 07 03 23 00  4D 80 03 25 00 4E 80 03  ...G..#.M..%.N..
0740: 24 00 0E 80 01 92 09 02  22 00 02 80 80 61 07 03  $......."....a..
0750: 23 00 4F 80 03 25 00 50  80 03 24 00 51 80 01 92  #.O..%.P..$.Q...
0760: 09 02 22 00 04 80 80 7B  07 03 23 00 52 80 03 25  .."....{..#.R..%
0770: 00 53 80 03 24 00 54 80  01 92 09 02 22 00 05 80  .S..$.T....."...
0780: 80 95 07 03 23 00 55 80  03 25 00 56 80 03 24 00  ....#.U..%.V..$.
0790: 57 80 01 92 09 02 22 00  0F 80 80 AF 07 03 23 00  W.....".......#.
07A0: 58 80 03 25 00 59 80 03  24 00 5A 80 01 92 09 02  X..%.Y..$.Z.....
07B0: 22 00 06 80 80 C9 07 03  23 00 5B 80 03 25 00 5C  ".......#.[..%.\
07C0: 80 03 24 00 5D 80 01 92  09 02 22 00 03 80 80 E3  ..$.].....".....
07D0: 07 03 23 00 5E 80 03 25  00 5F 80 03 24 00 60 80  ..#.^..%._..$.`.
07E0: 01 92 09 02 22 00 10 80  80 FD 07 03 23 00 61 80  ....".......#.a.
07F0: 03 25 00 62 80 03 24 00  63 80 01 92 09 02 22 00  .%.b..$.c.....".
0800: 11 80 80 17 08 03 23 00  64 80 03 25 00 65 80 03  ......#.d..%.e..
0810: 24 00 66 80 01 92 09 02  22 00 12 80 80 31 08 03  $.f....."....1..
0820: 23 00 67 80 03 25 00 68  80 03 24 00 66 80 01 92  #.g..%.h..$.f...
0830: 09 02 22 00 0B 80 80 4B  08 03 23 00 69 80 03 25  .."....K..#.i..%
0840: 00 6A 80 03 24 00 19 80  01 92 09 02 22 00 13 80  .j..$......."...
0850: 80 65 08 03 23 00 6B 80  03 25 00 6C 80 03 24 00  .e..#.k..%.l..$.
0860: 6D 80 01 92 09 02 22 00  14 80 80 7F 08 03 23 00  m.....".......#.
0870: 6E 80 03 25 00 6F 80 03  24 00 6D 80 01 92 09 02  n..%.o..$.m.....
0880: 22 00 15 80 80 99 08 03  23 00 70 80 03 25 00 71  ".......#.p..%.q
0890: 80 03 24 00 6D 80 01 92  09 02 22 00 16 80 80 B3  ..$.m.....".....
08A0: 08 03 23 00 72 80 03 25  00 73 80 03 24 00 6D 80  ..#.r..%.s..$.m.
08B0: 01 92 09 02 22 00 0E 80  80 CD 08 03 23 00 74 80  ....".......#.t.
08C0: 03 25 00 75 80 03 24 00  6D 80 01 92 09 02 22 00  .%.u..$.m.....".
08D0: 17 80 80 E7 08 03 23 00  76 80 03 25 00 77 80 03  ......#.v..%.w..
08E0: 24 00 6D 80 01 92 09 02  22 00 1A 80 80 01 09 03  $.m.....".......
08F0: 23 00 78 80 03 25 00 79  80 03 24 00 19 80 01 92  #.x..%.y..$.....
0900: 09 02 22 00 7A 80 80 1B  09 03 23 00 7B 80 03 25  ..".z.....#.{..%
0910: 00 7C 80 03 24 00 7D 80  01 92 09 02 22 00 7E 80  .|..$.}.....".~.
0920: 80 35 09 03 23 00 7F 80  03 25 00 80 80 03 24 00  .5..#....%....$.
0930: 60 80 01 92 09 02 22 00  57 80 80 4F 09 03 23 00  `.....".W..O..#.
0940: 81 80 03 25 00 82 80 03  24 00 83 80 01 92 09 02  ...%....$.......
0950: 22 00 84 80 80 69 09 03  23 00 85 80 03 25 00 86  "....i..#....%..
0960: 80 03 24 00 7D 80 01 92  09 02 22 00 87 80 80 83  ..$.}.....".....
0970: 09 03 23 00 1B 80 03 25  00 88 80 03 24 00 7D 80  ..#....%....$.}.
0980: 01 92 09 03 23 00 01 80  03 24 00 01 80 03 25 00  ....#....$....%.
0990: 01 80 1B 24 89 80 00 10  01 80 25 02 00 10 01 80  ...$......%.....
09A0: 00 BD 09 48 8A 80 23 48  8B 80 23 48 8C 80 23 48  ...H..#H..#H..#H
09B0: 8D 80 23 48 8E 80 23 01  93 09 01 76 0B 02 00 10  ..#H..#....v....
09C0: 02 80 00 DF 09 48 8F 80  23 48 90 80 23 48 91 80  .....H..#H..#H..
09D0: 23 48 92 80 23 48 93 80  23 01 93 09 01 76 0B 02  #H..#H..#....v..
09E0: 00 10 04 80 00 F5 09 48  94 80 23 48 95 80 23 01  .......H..#H..#.
09F0: 93 09 01 76 0B 02 00 10  05 80 00 0F 0A 48 96 80  ...v.........H..
0A00: 23 48 97 80 23 48 98 80  23 01 93 09 01 76 0B 02  #H..#H..#....v..
0A10: 00 10 0F 80 00 35 0A 48  99 80 23 48 9A 80 23 48  .....5.H..#H..#H
0A20: 9B 80 23 48 9C 80 23 48  9D 80 23 48 9E 80 23 01  ..#H..#H..#H..#.
0A30: 93 09 01 76 0B 02 00 10  06 80 00 5B 0A 48 9F 80  ...v.......[.H..
0A40: 23 48 A0 80 23 48 A1 80  23 48 A2 80 23 48 A3 80  #H..#H..#H..#H..
0A50: 23 48 A4 80 23 01 93 09  01 76 0B 02 00 10 03 80  #H..#....v......
0A60: 00 71 0A 48 A5 80 23 48  A6 80 23 01 93 09 01 76  .q.H..#H..#....v
0A70: 0B 02 00 10 10 80 00 76  0B 06 00 10 48 A7 80 23  .......v....H..#
0A80: 24 A8 80 00 10 01 80 25  02 00 10 01 80 00 AA 0A  $......%........
0A90: 48 A9 80 23 48 AA 80 23  48 AB 80 23 48 AC 80 23  H..#H..#H..#H..#
0AA0: 48 AD 80 23 01 80 0A 01  73 0B 02 00 10 02 80 00  H..#....s.......
0AB0: BB 0A 1A 77 0B 01 80 0A  01 73 0B 02 00 10 04 80  ...w.....s......
0AC0: 00 CD 0A 48 AE 80 23 01  80 0A 01 73 0B 02 00 10  ...H..#....s....
0AD0: 05 80 00 DF 0A 48 AF 80  23 01 80 0A 01 73 0B 02  .....H..#....s..
0AE0: 00 10 0F 80 00 F1 0A 48  B0 80 23 01 80 0A 01 73  .......H..#....s
0AF0: 0B 02 00 10 06 80 00 14  0B AA B1 80 01 00 01 00  ................
0B00: 01 00 01 00 02 10 03 10  01 00 48 B2 80 23 01 80  ..........H..#..
0B10: 0A 01 73 0B 02 00 10 03  80 00 2A 0B 48 B3 80 23  ..s.......*.H..#
0B20: 48 B4 80 23 01 80 0A 01  73 0B 02 00 10 10 80 00  H..#....s.......
0B30: 3C 0B 48 B5 80 23 01 80  0A 01 73 0B 02 00 10 11  <.H..#....s.....
0B40: 80 00 4E 0B 48 B6 80 23  01 80 0A 01 73 0B 02 00  ..N.H..#....s...
0B50: 10 12 80 00 60 0B 48 B7  80 23 01 80 0A 01 73 0B  ....`.H..#....s.
0B60: 02 00 10 0B 80 00 73 0B  03 00 10 10 80 01 93 09  ......s.........
0B70: 01 73 0B 01 76 0B 1B 03  02 10 08 80 48 B8 80 23  .s..v.......H..#
0B80: 48 B9 80 23 48 BA 80 23  48 BB 80 23 1B 24 BC 80  H..#H..#H..#.$..
0B90: 27 00 01 80 25 03 27 00  00 10 02 00 10 01 80 00  '...%.'.........
0BA0: AC 0B 48 BD 80 23 01 8D  0B 01 11 0E 02 00 10 02  ..H..#..........
0BB0: 80 00 C2 0B 48 BE 80 23  48 BF 80 23 01 8D 0B 01  ....H..#H..#....
0BC0: 11 0E 02 00 10 04 80 00  E8 0B 48 C0 80 23 48 C1  ..........H..#H.
0BD0: 80 23 48 C2 80 23 48 C3  80 23 48 C4 80 23 48 C5  .#H..#H..#H..#H.
0BE0: 80 23 01 8D 0B 01 11 0E  02 00 10 05 80 00 11 0E  .#..............
0BF0: 06 28 00 24 C6 80 28 00  01 80 25 03 28 00 00 10  .(.$..(...%.(...
0C00: 02 00 10 01 80 00 14 0C  03 02 10 3D 80 48 C7 80  ...........=.H..
0C10: 23 01 0B 0E 02 00 10 02  80 00 28 0C 03 02 10 3E  #.........(....>
0C20: 80 48 C8 80 23 01 0B 0E  02 00 10 04 80 00 3C 0C  .H..#.........<.
0C30: 03 02 10 3F 80 48 C9 80  23 01 0B 0E 02 00 10 05  ...?.H..#.......
0C40: 80 00 50 0C 03 02 10 40  80 48 CA 80 23 01 0B 0E  ..P....@.H..#...
0C50: 02 00 10 0F 80 00 64 0C  03 02 10 CB 80 48 CC 80  ......d......H..
0C60: 23 01 0B 0E 02 00 10 06  80 00 78 0C 03 02 10 CD  #.........x.....
0C70: 80 48 CE 80 23 01 0B 0E  02 00 10 03 80 00 8C 0C  .H..#...........
0C80: 03 02 10 CF 80 48 D0 80  23 01 0B 0E 02 00 10 10  .....H..#.......
0C90: 80 00 A0 0C 03 02 10 D1  80 48 D2 80 23 01 0B 0E  .........H..#...
0CA0: 02 00 10 11 80 00 B4 0C  03 02 10 D3 80 48 D4 80  .............H..
0CB0: 23 01 0B 0E 02 00 10 12  80 00 C8 0C 03 02 10 D5  #...............
0CC0: 80 48 D6 80 23 01 0B 0E  02 00 10 0B 80 00 DC 0C  .H..#...........
0CD0: 03 02 10 D7 80 48 D8 80  23 01 0B 0E 02 00 10 13  .....H..#.......
0CE0: 80 00 F0 0C 03 02 10 D9  80 48 DA 80 23 01 0B 0E  .........H..#...
0CF0: 02 00 10 14 80 00 04 0D  03 02 10 DB 80 48 DC 80  .............H..
0D00: 23 01 0B 0E 02 00 10 15  80 00 18 0D 03 02 10 DD  #...............
0D10: 80 48 DE 80 23 01 0B 0E  02 00 10 16 80 00 2C 0D  .H..#.........,.
0D20: 03 02 10 DF 80 48 E0 80  23 01 0B 0E 02 00 10 0E  .....H..#.......
0D30: 80 00 40 0D 03 02 10 E1  80 48 E2 80 23 01 0B 0E  ..@......H..#...
0D40: 02 00 10 17 80 00 54 0D  03 02 10 E3 80 48 E4 80  ......T......H..
0D50: 23 01 0B 0E 02 00 10 1A  80 00 68 0D 03 02 10 E5  #.........h.....
0D60: 80 48 E6 80 23 01 0B 0E  02 00 10 7A 80 00 7C 0D  .H..#......z..|.
0D70: 03 02 10 63 80 48 E7 80  23 01 0B 0E 02 00 10 7E  ...c.H..#......~
0D80: 80 00 90 0D 03 02 10 E8  80 48 E9 80 23 01 0B 0E  .........H..#...
0D90: 02 00 10 57 80 00 A4 0D  03 02 10 EA 80 48 EB 80  ...W.........H..
0DA0: 23 01 0B 0E 02 00 10 84  80 00 B8 0D 03 02 10 EC  #...............
0DB0: 80 48 ED 80 23 01 0B 0E  02 00 10 87 80 00 CC 0D  .H..#...........
0DC0: 03 02 10 EE 80 48 EF 80  23 01 0B 0E 02 00 10 F0  .....H..#.......
0DD0: 80 00 E0 0D 03 02 10 F1  80 48 F2 80 23 01 0B 0E  .........H..#...
0DE0: 02 00 10 F3 80 00 F4 0D  03 02 10 F4 80 48 F5 80  .............H..
0DF0: 23 01 0B 0E 02 00 10 F6  80 00 08 0E 03 02 10 F7  #...............
0E00: 80 48 F8 80 23 01 0B 0E  01 8D 0B 01 F3 0B 01 11  .H..#...........
0E10: 0E 1B 03 1C 00 02 80 1B  03 02 10 01 00 07 02 10  ................
0E20: F9 80 03 03 10 01 00 07  03 10 FA 80 1B 1D FB 80  ................
0E30: 23 1D FC 80 23 1D FD 80  23 45 FE 80 F0 FF FF 7F  #...#...#E......
0E40: F0 FF FF 7F 71 73 74 63  01 80 1B B3 02 B3 09 01  ....qstc........
0E50: 00 02 01 00 02 80 00 5C  0E 01 4D 0E 1B 03 01 10  .......\..M.....
0E60: 2D 80 43 00 43 01 03 01  10 00 80 03 0A 00 02 10  -.C.C...........
0E70: 03 0B 00 03 10 03 0C 00  04 10 03 0D 00 05 10 03  ................
0E80: 0E 00 06 10 03 0F 00 07  10 03 10 00 08 10 03 11  ................
0E90: 00 09 10 3E 1D 00 04 80  9E 0E 1D FF 80 23 3E 1D  ...>.........#>.
0EA0: 00 05 80 A9 0E 1D 00 81  23 06 01 00 02 09 00 0A  ........#.......
0EB0: 00 00 B7 0E 05 01 00 02  09 00 0B 00 00 C2 0E 05  ................
0EC0: 01 00 02 09 00 0C 00 00  CD 0E 05 01 00 02 09 00  ................
0ED0: 0D 00 00 D8 0E 05 01 00  02 01 00 02 80 00 E4 0E  ................
0EE0: 1D 01 81 23 03 01 10 00  80 1B 02 20 00 02 80 00  ...#....... ....
0EF0: 1E 0F 03 02 10 1A 00 03  03 10 16 00 03 04 10 15  ................
0F00: 00 03 05 10 17 00 03 06  10 19 00 03 07 10 18 00  ................
0F10: 03 08 10 1B 00 03 09 10  14 00 48 02 81 23 1B 06  ..........H..#..
0F20: 01 00 02 1C 00 02 80 80  37 0F 3E 1F 00 01 80 34  ........7.>....4
0F30: 0F 05 01 00 01 61 0F 02  1C 00 04 80 80 4C 0F 3E  .....a.......L.>
0F40: 1F 00 02 80 49 0F 05 01  00 01 61 0F 02 1C 00 05  ....I.....a.....
0F50: 80 80 61 0F 3E 1F 00 04  80 5E 0F 05 01 00 01 61  ..a.>....^.....a
0F60: 0F 02 01 00 02 80 00 75  0F 03 02 10 1C 00 1D 03  .......u........
0F70: 81 23 01 37 13 42 03 01  10 04 81 43 00 43 01 B3  .#.7.B.....C.C..
0F80: 07 1C 00 B3 05 1A 00 16  00 15 00 17 00 19 00 18  ................
0F90: 00 1B 00 14 00 1A EA 0E  02 1A 00 2D 80 00 AC 0F  ...........-....
0FA0: 1D 47 80 23 03 01 10 00  80 01 37 13 02 15 00 01  .G.#......7.....
0FB0: 80 00 BB 0F 1D 48 80 23  01 37 13 03 01 10 05 81  .....H.#.7......
0FC0: 43 00 43 01 02 02 10 2D  80 80 E8 0F 1D 44 80 23  C.C....-.....D.#
0FD0: 03 01 00 14 00 1A 18 0E  1D 33 80 23 1D 35 80 23  .........3.#.5.#
0FE0: 03 01 10 00 80 01 37 13  02 02 10 2F 80 80 FC 0F  ......7..../....
0FF0: 1D 06 81 23 03 01 10 00  80 01 37 13 02 02 10 02  ...#......7.....
1000: 80 80 32 13 06 01 10 06  01 00 06 02 10 06 03 10  ..2.............
1010: 06 04 10 06 05 10 06 06  10 02 15 00 0B 80 05 62  ...............b
1020: 10 3C 01 00 01 80 02 80  03 02 10 15 00 02 15 00  .<..............
1030: 02 80 80 3D 10 03 03 10  3D 80 01 62 10 02 15 00  ...=....=..b....
1040: 04 80 80 4D 10 03 03 10  3E 80 01 62 10 02 15 00  ...M....>..b....
1050: 05 80 80 5D 10 03 03 10  3F 80 01 62 10 03 03 10  ...]....?..b....
1060: 40 80 02 18 00 02 80 00  DB 11 3C 01 00 02 80 02  @.........<.....
1070: 80 03 04 10 17 00 03 05  10 19 00 02 17 00 02 80  ................
1080: 80 8B 10 03 06 10 CB 80  01 DB 11 02 17 00 04 80  ................
1090: 80 9B 10 03 06 10 CD 80  01 DB 11 02 17 00 05 80  ................
10A0: 80 AB 10 03 06 10 CF 80  01 DB 11 02 17 00 0F 80  ................
10B0: 80 BB 10 03 06 10 D1 80  01 DB 11 02 17 00 06 80  ................
10C0: 80 CB 10 03 06 10 D3 80  01 DB 11 02 17 00 03 80  ................
10D0: 80 DB 10 03 06 10 D5 80  01 DB 11 02 17 00 10 80  ................
10E0: 80 EB 10 03 06 10 D7 80  01 DB 11 02 17 00 11 80  ................
10F0: 80 FB 10 03 06 10 D9 80  01 DB 11 02 17 00 12 80  ................
1100: 80 0B 11 03 06 10 DB 80  01 DB 11 02 17 00 0B 80  ................
1110: 80 1B 11 03 06 10 DD 80  01 DB 11 02 17 00 13 80  ................
1120: 80 2B 11 03 06 10 DF 80  01 DB 11 02 17 00 14 80  .+..............
1130: 80 3B 11 03 06 10 E1 80  01 DB 11 02 17 00 15 80  .;..............
1140: 80 4B 11 03 06 10 E3 80  01 DB 11 02 17 00 16 80  .K..............
1150: 80 5B 11 03 06 10 E5 80  01 DB 11 02 17 00 0E 80  .[..............
1160: 80 6B 11 03 06 10 63 80  01 DB 11 02 17 00 17 80  .k....c.........
1170: 80 7B 11 03 06 10 E8 80  01 DB 11 02 17 00 1A 80  .{..............
1180: 80 8B 11 03 06 10 EA 80  01 DB 11 02 17 00 7A 80  ..............z.
1190: 80 9B 11 03 06 10 EC 80  01 DB 11 02 17 00 7E 80  ..............~.
11A0: 80 AB 11 03 06 10 EE 80  01 DB 11 02 17 00 57 80  ..............W.
11B0: 80 BB 11 03 06 10 F1 80  01 DB 11 02 17 00 84 80  ................
11C0: 80 CB 11 03 06 10 F4 80  01 DB 11 02 17 00 87 80  ................
11D0: 80 DB 11 03 06 10 F7 80  01 DB 11 06 01 10 02 01  ................
11E0: 00 01 80 01 DA 12 02 01  00 05 80 80 1A 12 1D 07  ................
11F0: 81 23 1D 08 81 23 24 09  81 01 80 01 80 25 02 00  .#...#$......%..
1200: 10 01 80 00 09 12 01 17  12 02 00 10 02 80 00 17  ................
1210: 12 01 35 12 01 17 12 01  44 12 02 01 00 02 80 80  ..5.....D.......
1220: 29 12 1D 0A 81 23 01 44  12 02 01 00 04 80 80 44  )....#.D.......D
1230: 12 1D 0B 81 23 03 03 10  06 10 3C 01 10 0C 81 02  ....#.....<.....
1240: 80 01 44 12 02 21 00 01  80 01 B9 12 03 02 10 21  ..D..!.........!
1250: 00 1D 0D 81 23 24 0E 81  01 80 01 80 25 02 00 10  ....#$......%...
1260: 01 80 00 82 12 1D 0F 81  23 24 10 81 01 80 01 80  ........#$......
1270: 25 02 00 10 02 80 00 7F  12 01 4C 12 01 7F 12 01  %.........L.....
1280: B9 12 02 00 10 02 80 00  B9 12 1D 11 81 23 24 10  .............#$.
1290: 81 01 80 01 80 25 02 00  10 01 80 00 A8 12 3C 01  .....%........<.
12A0: 10 12 81 02 80 01 B6 12  02 00 10 02 80 00 B6 12  ................
12B0: 01 4C 12 01 B6 12 01 B9  12 3E 01 10 12 81 C3 12  .L.......>......
12C0: 01 C6 12 1A 2D 0E 45 FE  80 F0 FF FF 7F F0 FF FF  ....-.E.........
12D0: 7F 71 73 74 63 01 80 01  F2 12 03 02 10 15 00 03  .qstc...........
12E0: 03 10 18 00 03 04 10 17  00 03 05 10 19 00 1D 13  ................
12F0: 81 23 02 1C 00 02 80 80  04 13 3C 01 10 19 80 02  .#........<.....
1300: 80 01 28 13 02 1C 00 04  80 80 16 13 3C 01 10 14  ..(.........<...
1310: 81 02 80 01 28 13 02 1C  00 05 80 80 28 13 3C 01  ....(.......(.<.
1320: 10 15 81 02 80 01 28 13  3C 01 10 16 81 02 80 01  ......(.<.......
1330: 37 13 03 01 10 00 80 1B                           7.......        
```

#### Opcodes

```
  0: 0x0001 [0x03] Work_Zone[1] = 1073741824*
  1: 0x0006 [0x20] SET_CLI_EVENT_UC_FLAG: Lock player control
  2: 0x0008 [0x1E] EventEntity looks at LocalPlayer and starts talking
  3: 0x000D [0x6F] WAIT_FRAME_DELAY: Yield until WaitTime reaches zero
  4: 0x000E [0x70] WAIT_ENTITY_RENDER_FLAG: Wait while EventEntity->Render.Flags3 bit 2 is set (cancel turn if not)
  5: 0x000F [0x03] ExtData[1]->WorkLocal[4] = Work_Zone[2]
  6: 0x0014 [0x03] ExtData[1]->WorkLocal[5] = Work_Zone[3]
  7: 0x0019 [0x03] ExtData[1]->WorkLocal[8] = Work_Zone[4]
  8: 0x001E [0x3E] IF !(Work_Zone[5] bit 0*) GOTO 0x0028
  9: 0x0025 [0x05] ExtData[1]->WorkLocal[30] = 1
 10: 0x0028 [0x3E] IF !(Work_Zone[5] bit 1*) GOTO 0x0032
 11: 0x002F [0x05] ExtData[1]->WorkLocal[32] = 1
 12: 0x0032 [0x03] ExtData[1]->WorkLocal[18] = Work_Zone[6]
 13: 0x0037 [0x03] ExtData[1]->WorkLocal[19] = Work_Zone[7]
 14: 0x003C [0x03] ExtData[1]->WorkLocal[29] = Work_Zone[8]
 15: 0x0041 [0x03] ExtData[1]->WorkLocal[9] = Work_Zone[9]
 16: 0x0046 [0x06] ExtData[1]->WorkLocal[0] = 0
 17: 0x0049 [0x3E] IF !(ExtData[1]->WorkLocal[29] bit 6*) GOTO 0x0057
 18: 0x0050 [0x3C] SET_BIT_FLAG_CONDITIONAL(target_work_offset=ExtData[1]->WorkLocal[0], bit_index_work_offset=0*, condition_work_offset=1*)
 19: 0x0057 [0x06] ExtData[1]->WorkLocal[1] = 0
 20: 0x005A [0x3E] IF !(ExtData[1]->WorkLocal[29] bit 2*) GOTO 0x0064
 21: 0x0061 [0x05] ExtData[1]->WorkLocal[1] = 1
 22: 0x0064 [0x3E] IF !(ExtData[1]->WorkLocal[29] bit 3*) GOTO 0x006E
 23: 0x006B [0x05] ExtData[1]->WorkLocal[1] = 1
 24: 0x006E [0x02] IF !(ExtData[1]->WorkLocal[1] == 0*) GOTO 0x007D
 25: 0x0076 [0x3C] SET_BIT_FLAG_CONDITIONAL(target_work_offset=ExtData[1]->WorkLocal[0], bit_index_work_offset=1*, condition_work_offset=1*)
 26: 0x007D [0x3E] IF !(ExtData[1]->WorkLocal[29] bit 0*) GOTO 0x008E
 27: 0x0084 [0x3C] SET_BIT_FLAG_CONDITIONAL(target_work_offset=ExtData[1]->WorkLocal[0], bit_index_work_offset=1*, condition_work_offset=1*)
 28: 0x008B [0x01] GOTO 0x00A3
 29: 0x008E [0x3C] SET_BIT_FLAG_CONDITIONAL(target_work_offset=ExtData[1]->WorkLocal[0], bit_index_work_offset=2*, condition_work_offset=1*)
 30: 0x0095 [0x3C] SET_BIT_FLAG_CONDITIONAL(target_work_offset=ExtData[1]->WorkLocal[0], bit_index_work_offset=3*, condition_work_offset=1*)
 31: 0x009C [0x3C] SET_BIT_FLAG_CONDITIONAL(target_work_offset=ExtData[1]->WorkLocal[0], bit_index_work_offset=5*, condition_work_offset=1*)

SUBROUTINE_00A3:
 32: 0x00A3 [0x03] Work_Zone[2] = ExtData[1]->WorkLocal[19]
 33: 0x00A8 [0x03] Work_Zone[3] = ExtData[1]->WorkLocal[4]
 34: 0x00AD [0x1D] PRINT_EVENT_MESSAGE(message_id=7518*)
    → "I have been dispatched by the IRNC to handle all affairs dealing with Ballista Points and Ballista Point Ranking. My records show that you scored $0 Ballista [Point/Points] in your previous match, bringing your cumulative total to $1."
 35: 0x00B0 [0x23] WAIT_FOR_DIALOG_INTERACTION
 36: 0x00B1 [0x02] IF !(ExtData[1]->WorkLocal[4] < 2000*) GOTO 0x00C2
 37: 0x00B9 [0x03] Work_Zone[2] = 2000*
 38: 0x00BE [0x1D] PRINT_EVENT_MESSAGE(message_id=7519*)
    → "You have obtained the maximum number of Ballista Points ($0). Please note that you cannot earn any more until you have used the ones you possess."
 39: 0x00C1 [0x23] WAIT_FOR_DIALOG_INTERACTION

SUBROUTINE_00C2:
 40: 0x00C2 [0x24] CREATE_DIALOG(message_id=7520*, default_option=0*, option_flags=ExtData[1]->WorkLocal[0])
    → "What would you like to do? [Exchange points for items./View the previous match MVPs./Sign up for B.P. Ranking./View the B.P. Ranking./Learn more about Ballista Points./Learn more about ranking./Nothing at the moment.]"
 41: 0x00C9 [0x25] WAIT_DIALOG_SELECT()
 42: 0x00CA [0x02] IF !(Work_Zone[0] == 0*) GOTO 0x049C
 43: 0x00D2 [0x03] Work_Zone[2] = ExtData[1]->WorkLocal[4]
 44: 0x00D7 [0x03] Work_Zone[3] = ExtData[1]->WorkLocal[8]
 45: 0x00DC [0x03] Work_Zone[4] = 10*
 46: 0x00E1 [0x02] IF !(ExtData[1]->WorkLocal[8] <= 0*) GOTO 0x00F0
 47: 0x00E9 [0x1D] PRINT_EVENT_MESSAGE(message_id=7521*)
    → "You currently possess $0 Ballista [Point/Points] and $1 temporary item [voucher/vouchers]. Remember, you can carry a maximum of $2 [voucher/vouchers] at a time, but no more than one of each type."
 48: 0x00EC [0x23] WAIT_FOR_DIALOG_INTERACTION
 49: 0x00ED [0x01] GOTO 0x00F4
 50: 0x00F0 [0x1D] PRINT_EVENT_MESSAGE(message_id=7522*)
    → "You currently possess $0 Ballista [Point/Points] and $1 temporary item [voucher/vouchers]. Remember, you can carry a maximum of $2 [voucher/vouchers] at a time, but no more than one of each type."
 51: 0x00F3 [0x23] WAIT_FOR_DIALOG_INTERACTION

SUBROUTINE_00F4:
 52: 0x00F4 [0x06] ExtData[1]->WorkLocal[6] = 0
 53: 0x00F7 [0x06] ExtData[1]->WorkLocal[7] = 0

SUBROUTINE_00FA:
 54: 0x00FA [0x06] ExtData[1]->WorkLocal[0] = 0
 55: 0x00FD [0x03] ExtData[1]->WorkLocal[3] = 1*

SUBROUTINE_0102:
 56: 0x0102 [0x02] IF !(ExtData[1]->WorkLocal[3] > 15*) GOTO 0x0265
 57: 0x010A [0x03] ExtData[1]->WorkLocal[34] = ExtData[1]->WorkLocal[6]
 58: 0x010F [0x1A] CALL_SUBROUTINE(address=0x072D)
 59: 0x0112 [0x02] IF !(ExtData[1]->WorkLocal[3] == 1*) GOTO 0x0127
 60: 0x011A [0x03] Work_Zone[2] = ExtData[1]->WorkLocal[37]
 61: 0x011F [0x03] Work_Zone[3] = ExtData[1]->WorkLocal[36]
 62: 0x0124 [0x01] GOTO 0x024D
 63: 0x0127 [0x02] IF !(ExtData[1]->WorkLocal[3] == 2*) GOTO 0x013C
 64: 0x012F [0x03] Work_Zone[4] = ExtData[1]->WorkLocal[37]
 65: 0x0134 [0x03] Work_Zone[5] = ExtData[1]->WorkLocal[36]
 66: 0x0139 [0x01] GOTO 0x024D
 67: 0x013C [0x02] IF !(ExtData[1]->WorkLocal[3] == 3*) GOTO 0x0151
 68: 0x0144 [0x03] Work_Zone[6] = ExtData[1]->WorkLocal[37]
 69: 0x0149 [0x03] Work_Zone[7] = ExtData[1]->WorkLocal[36]
 70: 0x014E [0x01] GOTO 0x024D
 71: 0x0151 [0x02] IF !(ExtData[1]->WorkLocal[3] == 4*) GOTO 0x0166
 72: 0x0159 [0x03] Work_Zone[8] = ExtData[1]->WorkLocal[37]
 73: 0x015E [0x03] Work_Zone[9] = ExtData[1]->WorkLocal[36]
 74: 0x0163 [0x01] GOTO 0x024D
 75: 0x0166 [0x02] IF !(ExtData[1]->WorkLocal[3] == 5*) GOTO 0x017B
 76: 0x016E [0x03] Work_Zone_1700[0] = ExtData[1]->WorkLocal[37]
 77: 0x0173 [0x03] Work_Zone_1700[1] = ExtData[1]->WorkLocal[36]
 78: 0x0178 [0x01] GOTO 0x024D
 79: 0x017B [0x02] IF !(ExtData[1]->WorkLocal[3] == 6*) GOTO 0x0190
 80: 0x0183 [0x03] Work_Zone_1700[2] = ExtData[1]->WorkLocal[37]
 81: 0x0188 [0x03] Work_Zone_1700[3] = ExtData[1]->WorkLocal[36]
 82: 0x018D [0x01] GOTO 0x024D
 83: 0x0190 [0x02] IF !(ExtData[1]->WorkLocal[3] == 7*) GOTO 0x01A5
 84: 0x0198 [0x03] Work_Zone_1700[4] = ExtData[1]->WorkLocal[37]
 85: 0x019D [0x03] Work_Zone_1700[5] = ExtData[1]->WorkLocal[36]
 86: 0x01A2 [0x01] GOTO 0x024D
 87: 0x01A5 [0x02] IF !(ExtData[1]->WorkLocal[3] == 8*) GOTO 0x01BA
 88: 0x01AD [0x03] Work_Zone_1700[6] = ExtData[1]->WorkLocal[37]
 89: 0x01B2 [0x03] Work_Zone_1700[7] = ExtData[1]->WorkLocal[36]
 90: 0x01B7 [0x01] GOTO 0x024D
 91: 0x01BA [0x02] IF !(ExtData[1]->WorkLocal[3] == 9*) GOTO 0x01CF
 92: 0x01C2 [0x03] Work_Zone_1700[8] = ExtData[1]->WorkLocal[37]
 93: 0x01C7 [0x03] Work_Zone_1700[9] = ExtData[1]->WorkLocal[36]
 94: 0x01CC [0x01] GOTO 0x024D
 95: 0x01CF [0x02] IF !(ExtData[1]->WorkLocal[3] == 10*) GOTO 0x01E4
 96: 0x01D7 [0x03] Work_Zone_1700[10] = ExtData[1]->WorkLocal[37]
 97: 0x01DC [0x03] Work_Zone_1700[11] = ExtData[1]->WorkLocal[36]
 98: 0x01E1 [0x01] GOTO 0x024D
 99: 0x01E4 [0x02] IF !(ExtData[1]->WorkLocal[3] == 11*) GOTO 0x01F9
100: 0x01EC [0x03] Work_Zone_1700[12] = ExtData[1]->WorkLocal[37]
101: 0x01F1 [0x03] Work_Zone_1700[13] = ExtData[1]->WorkLocal[36]
102: 0x01F6 [0x01] GOTO 0x024D
103: 0x01F9 [0x02] IF !(ExtData[1]->WorkLocal[3] == 12*) GOTO 0x020E
104: 0x0201 [0x03] Work_Zone_1700[14] = ExtData[1]->WorkLocal[37]
105: 0x0206 [0x03] Work_Zone_1700[15] = ExtData[1]->WorkLocal[36]
106: 0x020B [0x01] GOTO 0x024D
107: 0x020E [0x02] IF !(ExtData[1]->WorkLocal[3] == 13*) GOTO 0x0223
108: 0x0216 [0x03] Work_Zone_1700[16] = ExtData[1]->WorkLocal[37]
109: 0x021B [0x03] Work_Zone_1700[17] = ExtData[1]->WorkLocal[36]
110: 0x0220 [0x01] GOTO 0x024D
111: 0x0223 [0x02] IF !(ExtData[1]->WorkLocal[3] == 14*) GOTO 0x0238
112: 0x022B [0x03] Work_Zone_1700[18] = ExtData[1]->WorkLocal[37]
113: 0x0230 [0x03] Work_Zone_1700[19] = ExtData[1]->WorkLocal[36]
114: 0x0235 [0x01] GOTO 0x024D
115: 0x0238 [0x02] IF !(ExtData[1]->WorkLocal[3] == 15*) GOTO 0x024D
116: 0x0240 [0x03] Work_Zone_1700[20] = ExtData[1]->WorkLocal[37]
117: 0x0245 [0x03] Work_Zone_1700[21] = ExtData[1]->WorkLocal[36]
118: 0x024A [0x01] GOTO 0x024D

SUBROUTINE_024D:
119: 0x024D [0x02] IF !(ExtData[1]->WorkLocal[35] == 0*) GOTO 0x025C
120: 0x0255 [0x3C] SET_BIT_FLAG_CONDITIONAL(target_work_offset=ExtData[1]->WorkLocal[0], bit_index_work_offset=ExtData[1]->WorkLocal[3], condition_work_offset=1*)
121: 0x025C [0x0B] ExtData[1]->WorkLocal[3]++
122: 0x025F [0x0B] ExtData[1]->WorkLocal[6]++
123: 0x0262 [0x01] GOTO 0x0102
124: 0x0265 [0x03] ExtData[1]->WorkLocal[34] = ExtData[1]->WorkLocal[6]
125: 0x026A [0x08] ExtData[1]->WorkLocal[34] -= 16*
126: 0x026F [0x1A] CALL_SUBROUTINE(address=0x072D)
127: 0x0272 [0x02] IF !(ExtData[1]->WorkLocal[35] == 0*) GOTO 0x0281
128: 0x027A [0x3C] SET_BIT_FLAG_CONDITIONAL(target_work_offset=ExtData[1]->WorkLocal[0], bit_index_work_offset=0*, condition_work_offset=1*)
129: 0x0281 [0x03] ExtData[1]->WorkLocal[34] = ExtData[1]->WorkLocal[6]
130: 0x0286 [0x1A] CALL_SUBROUTINE(address=0x072D)
131: 0x0289 [0x02] IF !(ExtData[1]->WorkLocal[35] == 0*) GOTO 0x0298
132: 0x0291 [0x3C] SET_BIT_FLAG_CONDITIONAL(target_work_offset=ExtData[1]->WorkLocal[0], bit_index_work_offset=16*, condition_work_offset=1*)
133: 0x0298 [0x24] CREATE_DIALOG(message_id=7523*, default_option=ExtData[1]->WorkLocal[7], option_flags=ExtData[1]->WorkLocal[0])
    → "Exchange points for... [Previous page./6 ($1 [pt./pts.])/$6 ($3 [pt./pts.])/$6 ($5 [pt./pts.])/$6 ($7 [pt./pts.])/$6 ($9 [pt./pts.])/$6 ($11 [pt./pts.])/$6 ($13 [pt./pts.])/$6 ($15 [pt./pts.])/$6 ($17 [pt./pts.])/$6 ($19 [pt./pts.])/$6 ($21 [pt./pts.])/$6 ($23 [pt./pts.])/$6 ($25 [pt./pts.])/$6 ($27 [pt./pts.])/$6 ($29 [pt./pts.])/Next page./Nothing.]"
134: 0x029F [0x25] WAIT_DIALOG_SELECT()
135: 0x02A0 [0x02] IF !(Work_Zone[0] == 0*) GOTO 0x02B6
136: 0x02A8 [0x06] ExtData[1]->WorkLocal[7] = 0
137: 0x02AB [0x08] ExtData[1]->WorkLocal[6] -= 30*
138: 0x02B0 [0x01] GOTO 0x00FA

SUBROUTINE_03C2:
139: 0x03C2 [0x03] ExtData[1]->WorkLocal[34] = ExtData[1]->WorkLocal[6]
140: 0x03C7 [0x08] ExtData[1]->WorkLocal[34] -= 16*
141: 0x03CC [0x07] ExtData[1]->WorkLocal[34] += Work_Zone[0]
142: 0x03D1 [0x1A] CALL_SUBROUTINE(address=0x072D)
143: 0x03D4 [0x93] DISPLAY_ITEM_INFO(item_id=ExtData[1]->WorkLocal[35])
144: 0x03D7 [0x02] IF !(ExtData[1]->WorkLocal[35] == 4206*) GOTO 0x0408
145: 0x03DF [0x03] Work_Zone[1] = 4294967293*
146: 0x03E4 [0x43] SEND_EVENT_UPDATE: Send pending tag to server (packet 0x005B)
147: 0x03E6 [0x43] SEND_EVENT_UPDATE: Check pending flag (skip if not pending)
148: 0x03E8 [0x03] Work_Zone[1] = 1073741824*
149: 0x03ED [0x02] IF !(Work_Zone[9] == 0*) GOTO 0x0404
150: 0x03F5 [0x1D] PRINT_EVENT_MESSAGE(message_id=7525*)
    → "I am sorry. You have not earned the right to exchange your points for that item."
151: 0x03F8 [0x23] WAIT_FOR_DIALOG_INTERACTION
152: 0x03F9 [0x08] ExtData[1]->WorkLocal[6] -= 15*
153: 0x03FE [0x01] GOTO 0x00FA

SUBROUTINE_0408:
154: 0x0408 [0x03] Work_Zone[2] = ExtData[1]->WorkLocal[37]
155: 0x040D [0x03] Work_Zone[3] = ExtData[1]->WorkLocal[36]
156: 0x0412 [0x03] Work_Zone[4] = ExtData[1]->WorkLocal[35]
157: 0x0417 [0x02] IF !(ExtData[1]->WorkLocal[8] < 10*) GOTO 0x042B
158: 0x041F [0x03] Work_Zone[5] = 10*
159: 0x0424 [0x1D] PRINT_EVENT_MESSAGE(message_id=7531*)
    → "Adventurers are only allowed to possess a maximum of $3 temporary item [voucher/vouchers]. I am sorry, but I cannot proceed with the transaction."
160: 0x0427 [0x23] WAIT_FOR_DIALOG_INTERACTION
161: 0x0428 [0x01] GOTO 0x0499
162: 0x042B [0x3E] IF !(ExtData[1]->WorkLocal[5] bit ExtData[1]->WorkLocal[34]) GOTO 0x0441
163: 0x0432 [0x1D] PRINT_EVENT_MESSAGE(message_id=7529*)
    → "You already possess $6. Remember, you cannot hold more than one of each type of temporary item voucher."
164: 0x0435 [0x23] WAIT_FOR_DIALOG_INTERACTION
165: 0x0436 [0x08] ExtData[1]->WorkLocal[6] -= 15*
166: 0x043B [0x01] GOTO 0x00FA

SUBROUTINE_0487:
167: 0x0487 [0x01] GOTO 0x0499
168: 0x048A [0x1D] PRINT_EVENT_MESSAGE(message_id=7528*)
    → "You do not have enough Ballista Points to purchase that item."
169: 0x048D [0x23] WAIT_FOR_DIALOG_INTERACTION
170: 0x048E [0x93] DISPLAY_ITEM_INFO(item_id=0*)
171: 0x0491 [0x08] ExtData[1]->WorkLocal[6] -= 15*
172: 0x0496 [0x01] GOTO 0x00FA

SUBROUTINE_0499:
173: 0x0499 [0x01] GOTO 0x072B
174: 0x049C [0x02] IF !(Work_Zone[0] == 1*) GOTO 0x04AA
175: 0x04A4 [0x1A] CALL_SUBROUTINE(address=0x0E5D)
176: 0x04A7 [0x01] GOTO 0x072B
177: 0x04AA [0x02] IF !(Work_Zone[0] == 2*) GOTO 0x0614
178: 0x04B2 [0x3E] IF !(ExtData[1]->WorkLocal[29] bit 1*) GOTO 0x0600
179: 0x04B9 [0x1A] CALL_SUBROUTINE(address=0x0E5D)
180: 0x04BC [0x02] IF !(ExtData[1]->WorkLocal[19] == 0*) GOTO 0x04D7
181: 0x04C4 [0x1A] CALL_SUBROUTINE(address=0x0E12)
182: 0x04C7 [0x03] Work_Zone[4] = ExtData[1]->WorkLocal[28]
183: 0x04CC [0x1D] PRINT_EVENT_MESSAGE(message_id=7541*)
    → "You did not obtain any Ballista Points in the previous match; thus, I cannot register you."
184: 0x04CF [0x23] WAIT_FOR_DIALOG_INTERACTION
185: 0x04D0 [0x1D] PRINT_EVENT_MESSAGE(message_id=7542*)
    → "Please participate in a future match to qualify once again for Ballista Point Ranking. The next exercise is scheduled to take place [soon/in the Jugner Forest/in the Pashhow Marshlands/in the Meriphataud Mountains]."
186: 0x04D3 [0x23] WAIT_FOR_DIALOG_INTERACTION
187: 0x04D4 [0x01] GOTO 0x05FD
188: 0x04D7 [0x02] IF !(ExtData[1]->WorkLocal[30] == 1*) GOTO 0x04E6
189: 0x04DF [0x1D] PRINT_EVENT_MESSAGE(message_id=7543*)
    → "Your current job does not coincide with the data we have recorded for the previous match. You must return to your residence and change jobs before I can register you for Ballista Point Ranking."
190: 0x04E2 [0x23] WAIT_FOR_DIALOG_INTERACTION
191: 0x04E3 [0x01] GOTO 0x05FD
192: 0x04E6 [0x03] Work_Zone[2] = ExtData[1]->WorkLocal[18]
193: 0x04EB [0x03] Work_Zone[3] = ExtData[1]->WorkLocal[19]
194: 0x04F0 [0x1A] CALL_SUBROUTINE(address=0x0E12)
195: 0x04F3 [0x03] Work_Zone[4] = ExtData[1]->WorkLocal[28]
196: 0x04F8 [0x1D] PRINT_EVENT_MESSAGE(message_id=7544*)
    → "According to the [/Jugner Forest/Pashhow Marshlands/Meriphataud Mountains] match records for , you earned $1 Ballista [Point/Points]."
197: 0x04FB [0x23] WAIT_FOR_DIALOG_INTERACTION
198: 0x04FC [0x1D] PRINT_EVENT_MESSAGE(message_id=7545*)
    → "Shall I register your statistics? (This may take a few moments.)"
199: 0x04FF [0x23] WAIT_FOR_DIALOG_INTERACTION
200: 0x0500 [0x24] CREATE_DIALOG(message_id=7546*, default_option=1*, option_flags=0*)
    → "Register? [Yes./No.]"
201: 0x0507 [0x25] WAIT_DIALOG_SELECT()
202: 0x0508 [0x02] IF !(Work_Zone[0] == 0*) GOTO 0x05FD
203: 0x0510 [0x1A] CALL_SUBROUTINE(address=0x0E12)
204: 0x0513 [0x03] Work_Zone[1] = 4294967291*
205: 0x0518 [0x43] SEND_EVENT_UPDATE: Send pending tag to server (packet 0x005B)
206: 0x051A [0x43] SEND_EVENT_UPDATE: Check pending flag (skip if not pending)
207: 0x051C [0x03] Work_Zone[1] = 1073741824*
208: 0x0521 [0x02] IF !(Work_Zone[2] == 0*) GOTO 0x0534
209: 0x0529 [0x1D] PRINT_EVENT_MESSAGE(message_id=7587*)
    → "I am sorry, but we are no longer accepting Ballista Point Ranking registrations for the match in which you previously participated."
210: 0x052C [0x23] WAIT_FOR_DIALOG_INTERACTION
211: 0x052D [0x1D] PRINT_EVENT_MESSAGE(message_id=7588*)
    → "For future reference, please note that the registration period is exactly four hours (Vana'diel time) from the close of each Ballista match."
212: 0x0530 [0x23] WAIT_FOR_DIALOG_INTERACTION
213: 0x0531 [0x01] GOTO 0x05FA
214: 0x0534 [0x42] SET_CLI_EVENT_CANCEL_DATA()
215: 0x0535 [0xB3] RANKINGS_BOARD_HANDLER(case_type=0x03)
216: 0x0537 [0x1C] WAIT(0xB300 ticks)
217: 0x053A [0x05] ExtData[1]->WorkLocal[26] = 1
218: 0x053D [0x16] 0x1500 = sin(Work_Zone_1700[0]) * 0x1900
219: 0x0544 [0x00] END_REQSTACK()

SUBROUTINE_05FA:
220: 0x05FA [0x01] GOTO 0x05FD

SUBROUTINE_05FD:
221: 0x05FD [0x01] GOTO 0x060C
222: 0x0600 [0x1A] CALL_SUBROUTINE(address=0x0E12)
223: 0x0603 [0x03] Work_Zone[4] = ExtData[1]->WorkLocal[28]
224: 0x0608 [0x1D] PRINT_EVENT_MESSAGE(message_id=7540*)
    → "Ballista Point Ranking registration [/for the Jugner Forest match/for the Pashhow Marshlands match/for the Meriphataud Mountains match] has concluded. Please do not forget to register your Ballista Points immediately after each match to avoid any confusion."
225: 0x060B [0x23] WAIT_FOR_DIALOG_INTERACTION

SUBROUTINE_060C:
226: 0x060C [0x03] Work_Zone[1] = 1073741824*
227: 0x0611 [0x01] GOTO 0x072B
228: 0x0614 [0x02] IF !(Work_Zone[0] == 3*) GOTO 0x06F9
229: 0x061C [0x1A] CALL_SUBROUTINE(address=0x0E5D)
230: 0x061F [0x03] Work_Zone[1] = 4294967293*
231: 0x0624 [0x43] SEND_EVENT_UPDATE: Send pending tag to server (packet 0x005B)
232: 0x0626 [0x43] SEND_EVENT_UPDATE: Check pending flag (skip if not pending)
233: 0x0628 [0x03] Work_Zone[1] = 1073741824*
234: 0x062D [0x03] ExtData[1]->WorkLocal[31] = Work_Zone[2]
235: 0x0632 [0x03] ExtData[1]->WorkLocal[33] = Work_Zone[9]
236: 0x0637 [0x07] Work_Zone[8] += 622080*
237: 0x063C [0x03] Work_Zone_1700[0] = 682*
238: 0x0641 [0x03] Work_Zone_1700[1] = 683*
239: 0x0646 [0x03] Work_Zone_1700[2] = 684*
240: 0x064B [0x03] Work_Zone_1700[3] = 685*
241: 0x0650 [0x02] IF !(ExtData[1]->WorkLocal[33] == 0*) GOTO 0x065C
242: 0x0658 [0x1D] PRINT_EVENT_MESSAGE(message_id=7558*)
    → "You currently possess $6. Ballista regulations allow you to wear that chevron until (Vana'diel), or (Earth)."
243: 0x065B [0x23] WAIT_FOR_DIALOG_INTERACTION
244: 0x065C [0x1D] PRINT_EVENT_MESSAGE(message_id=7559*)
    → "Our records show that you have previously been presented the following rewards: Gold chevron: $1 [time/times] Mythril chevron: $2 [time/times] Silver chevron: $3 [time/times] Bronze chevron: $4 [time/times] Job-specific chevron: $5 [time/times]"
245: 0x065F [0x23] WAIT_FOR_DIALOG_INTERACTION
246: 0x0660 [0x1A] CALL_SUBROUTINE(address=0x0E12)
247: 0x0663 [0x03] Work_Zone[1] = 4294967289*
248: 0x0668 [0x43] SEND_EVENT_UPDATE: Send pending tag to server (packet 0x005B)
249: 0x066A [0x43] SEND_EVENT_UPDATE: Check pending flag (skip if not pending)
250: 0x066C [0x03] Work_Zone[1] = 1073741824*
251: 0x0671 [0x02] IF !(Work_Zone[2] == 1*) GOTO 0x0691
252: 0x0679 [0x1D] PRINT_EVENT_MESSAGE(message_id=7561*)
    → "Please note that because the point registration period for the previous match has not yet ended, your ranking may change. We ask that you check back later for the final results."
253: 0x067C [0x23] WAIT_FOR_DIALOG_INTERACTION
254: 0x067D [0x03] ExtData[1]->WorkLocal[1] = Work_Zone[3]
255: 0x0682 [0x1A] CALL_SUBROUTINE(address=0x0E18)
256: 0x0685 [0x1D] PRINT_EVENT_MESSAGE(message_id=7552*)
    → "We ask that you check the rankings on the following date: Vana'diel: Earth:"
257: 0x0688 [0x23] WAIT_FOR_DIALOG_INTERACTION
258: 0x0689 [0x03] Work_Zone[1] = 1073741824*
259: 0x068E [0x01] GOTO 0x06F6
260: 0x0691 [0x03] Work_Zone[4] = ExtData[1]->WorkLocal[28]
261: 0x0696 [0x1D] PRINT_EVENT_MESSAGE(message_id=7548*)
    → "It may take a few moments for me to find the ranking list."
262: 0x0699 [0x23] WAIT_FOR_DIALOG_INTERACTION
263: 0x069A [0x24] CREATE_DIALOG(message_id=7549*, default_option=1*, option_flags=0*)
    → "Do you still wish to see it? [Yes./No.]"
264: 0x06A1 [0x25] WAIT_DIALOG_SELECT()
265: 0x06A2 [0x02] IF !(Work_Zone[0] == 0*) GOTO 0x06F1
266: 0x06AA [0x42] SET_CLI_EVENT_CANCEL_DATA()
267: 0x06AB [0xB3] RANKINGS_BOARD_HANDLER(case_type=0x00)
268: 0x06AD [0x1C] WAIT(0xB300 ticks)
269: 0x06B0 [0x01] GOTO 0x001A

SUBROUTINE_06F6:
270: 0x06F6 [0x01] GOTO 0x072B
271: 0x06F9 [0x02] IF !(Work_Zone[0] == 4*) GOTO 0x0712
272: 0x0701 [0x1D] PRINT_EVENT_MESSAGE(message_id=7532*)
    → "Understood. Let me read to you the official Ballista rulebook chapter on Ballista Points."
273: 0x0704 [0x23] WAIT_FOR_DIALOG_INTERACTION
274: 0x0705 [0x1A] CALL_SUBROUTINE(address=0x0B77)
275: 0x0708 [0x1D] PRINT_EVENT_MESSAGE(message_id=7533*)
    → "Is there anything else I can do for you?"
276: 0x070B [0x23] WAIT_FOR_DIALOG_INTERACTION
277: 0x070C [0x01] GOTO 0x00C2

SUBROUTINE_072B:
278: 0x072B [0x21] END_EVENT
279: 0x072C [0x00] END_REQSTACK()

SUBROUTINE_072D:
280: 0x072D [0x02] IF !(ExtData[1]->WorkLocal[34] == 0*) GOTO 0x0747
281: 0x0735 [0x03] ExtData[1]->WorkLocal[35] = 4112*
282: 0x073A [0x03] ExtData[1]->WorkLocal[37] = 635*
283: 0x073F [0x03] ExtData[1]->WorkLocal[36] = 15*
284: 0x0744 [0x01] GOTO 0x0992
285: 0x0747 [0x02] IF !(ExtData[1]->WorkLocal[34] == 1*) GOTO 0x0761
286: 0x074F [0x03] ExtData[1]->WorkLocal[35] = 4116*
287: 0x0754 [0x03] ExtData[1]->WorkLocal[37] = 636*
288: 0x0759 [0x03] ExtData[1]->WorkLocal[36] = 45*
289: 0x075E [0x01] GOTO 0x0992
290: 0x0761 [0x02] IF !(ExtData[1]->WorkLocal[34] == 2*) GOTO 0x077B
291: 0x0769 [0x03] ExtData[1]->WorkLocal[35] = 4120*
292: 0x076E [0x03] ExtData[1]->WorkLocal[37] = 637*
293: 0x0773 [0x03] ExtData[1]->WorkLocal[36] = 90*
294: 0x0778 [0x01] GOTO 0x0992
295: 0x077B [0x02] IF !(ExtData[1]->WorkLocal[34] == 3*) GOTO 0x0795
296: 0x0783 [0x03] ExtData[1]->WorkLocal[35] = 4128*
297: 0x0788 [0x03] ExtData[1]->WorkLocal[37] = 638*
298: 0x078D [0x03] ExtData[1]->WorkLocal[36] = 20*
299: 0x0792 [0x01] GOTO 0x0992
300: 0x0795 [0x02] IF !(ExtData[1]->WorkLocal[34] == 4*) GOTO 0x07AF
301: 0x079D [0x03] ExtData[1]->WorkLocal[35] = 4132*
302: 0x07A2 [0x03] ExtData[1]->WorkLocal[37] = 639*
303: 0x07A7 [0x03] ExtData[1]->WorkLocal[36] = 60*
304: 0x07AC [0x01] GOTO 0x0992
305: 0x07AF [0x02] IF !(ExtData[1]->WorkLocal[34] == 5*) GOTO 0x07C9
306: 0x07B7 [0x03] ExtData[1]->WorkLocal[35] = 4136*
307: 0x07BC [0x03] ExtData[1]->WorkLocal[37] = 640*
308: 0x07C1 [0x03] ExtData[1]->WorkLocal[36] = 120*
309: 0x07C6 [0x01] GOTO 0x0992
310: 0x07C9 [0x02] IF !(ExtData[1]->WorkLocal[34] == 6*) GOTO 0x07E3
311: 0x07D1 [0x03] ExtData[1]->WorkLocal[35] = 4145*
312: 0x07D6 [0x03] ExtData[1]->WorkLocal[37] = 641*
313: 0x07DB [0x03] ExtData[1]->WorkLocal[36] = 200*
314: 0x07E0 [0x01] GOTO 0x0992
315: 0x07E3 [0x02] IF !(ExtData[1]->WorkLocal[34] == 7*) GOTO 0x07FD
316: 0x07EB [0x03] ExtData[1]->WorkLocal[35] = 4146*
317: 0x07F0 [0x03] ExtData[1]->WorkLocal[37] = 642*
318: 0x07F5 [0x03] ExtData[1]->WorkLocal[36] = 700*
319: 0x07FA [0x01] GOTO 0x0992
320: 0x07FD [0x02] IF !(ExtData[1]->WorkLocal[34] == 8*) GOTO 0x0817
321: 0x0805 [0x03] ExtData[1]->WorkLocal[35] = 4147*
322: 0x080A [0x03] ExtData[1]->WorkLocal[37] = 643*
323: 0x080F [0x03] ExtData[1]->WorkLocal[36] = 300*
324: 0x0814 [0x01] GOTO 0x0992
325: 0x0817 [0x02] IF !(ExtData[1]->WorkLocal[34] == 9*) GOTO 0x0831
326: 0x081F [0x03] ExtData[1]->WorkLocal[35] = 4200*
327: 0x0824 [0x03] ExtData[1]->WorkLocal[37] = 644*
328: 0x0829 [0x03] ExtData[1]->WorkLocal[36] = 300*
329: 0x082E [0x01] GOTO 0x0992
330: 0x0831 [0x02] IF !(ExtData[1]->WorkLocal[34] == 10*) GOTO 0x084B
331: 0x0839 [0x03] ExtData[1]->WorkLocal[35] = 5246*
332: 0x083E [0x03] ExtData[1]->WorkLocal[37] = 645*
333: 0x0843 [0x03] ExtData[1]->WorkLocal[36] = 30*
334: 0x0848 [0x01] GOTO 0x0992
335: 0x084B [0x02] IF !(ExtData[1]->WorkLocal[34] == 11*) GOTO 0x0865
336: 0x0853 [0x03] ExtData[1]->WorkLocal[35] = 5247*
337: 0x0858 [0x03] ExtData[1]->WorkLocal[37] = 646*
338: 0x085D [0x03] ExtData[1]->WorkLocal[36] = 50*
339: 0x0862 [0x01] GOTO 0x0992
340: 0x0865 [0x02] IF !(ExtData[1]->WorkLocal[34] == 12*) GOTO 0x087F
341: 0x086D [0x03] ExtData[1]->WorkLocal[35] = 5248*
342: 0x0872 [0x03] ExtData[1]->WorkLocal[37] = 647*
343: 0x0877 [0x03] ExtData[1]->WorkLocal[36] = 50*
344: 0x087C [0x01] GOTO 0x0992
345: 0x087F [0x02] IF !(ExtData[1]->WorkLocal[34] == 13*) GOTO 0x0899
346: 0x0887 [0x03] ExtData[1]->WorkLocal[35] = 5249*
347: 0x088C [0x03] ExtData[1]->WorkLocal[37] = 648*
348: 0x0891 [0x03] ExtData[1]->WorkLocal[36] = 50*
349: 0x0896 [0x01] GOTO 0x0992
350: 0x0899 [0x02] IF !(ExtData[1]->WorkLocal[34] == 14*) GOTO 0x08B3
351: 0x08A1 [0x03] ExtData[1]->WorkLocal[35] = 5250*
352: 0x08A6 [0x03] ExtData[1]->WorkLocal[37] = 649*
353: 0x08AB [0x03] ExtData[1]->WorkLocal[36] = 50*
354: 0x08B0 [0x01] GOTO 0x0992
355: 0x08B3 [0x02] IF !(ExtData[1]->WorkLocal[34] == 15*) GOTO 0x08CD
356: 0x08BB [0x03] ExtData[1]->WorkLocal[35] = 5251*
357: 0x08C0 [0x03] ExtData[1]->WorkLocal[37] = 650*
358: 0x08C5 [0x03] ExtData[1]->WorkLocal[36] = 50*
359: 0x08CA [0x01] GOTO 0x0992
360: 0x08CD [0x02] IF !(ExtData[1]->WorkLocal[34] == 16*) GOTO 0x08E7
361: 0x08D5 [0x03] ExtData[1]->WorkLocal[35] = 5252*
362: 0x08DA [0x03] ExtData[1]->WorkLocal[37] = 651*
363: 0x08DF [0x03] ExtData[1]->WorkLocal[36] = 50*
364: 0x08E4 [0x01] GOTO 0x0992
365: 0x08E7 [0x02] IF !(ExtData[1]->WorkLocal[34] == 17*) GOTO 0x0901
366: 0x08EF [0x03] ExtData[1]->WorkLocal[35] = 4204*
367: 0x08F4 [0x03] ExtData[1]->WorkLocal[37] = 702*
368: 0x08F9 [0x03] ExtData[1]->WorkLocal[36] = 30*
369: 0x08FE [0x01] GOTO 0x0992
370: 0x0901 [0x02] IF !(ExtData[1]->WorkLocal[34] == 18*) GOTO 0x091B
371: 0x0909 [0x03] ExtData[1]->WorkLocal[35] = 4155*
372: 0x090E [0x03] ExtData[1]->WorkLocal[37] = 729*
373: 0x0913 [0x03] ExtData[1]->WorkLocal[36] = 100*
374: 0x0918 [0x01] GOTO 0x0992
375: 0x091B [0x02] IF !(ExtData[1]->WorkLocal[34] == 19*) GOTO 0x0935
376: 0x0923 [0x03] ExtData[1]->WorkLocal[35] = 4149*
377: 0x0928 [0x03] ExtData[1]->WorkLocal[37] = 730*
378: 0x092D [0x03] ExtData[1]->WorkLocal[36] = 200*
379: 0x0932 [0x01] GOTO 0x0992
380: 0x0935 [0x02] IF !(ExtData[1]->WorkLocal[34] == 20*) GOTO 0x094F
381: 0x093D [0x03] ExtData[1]->WorkLocal[35] = 5320*
382: 0x0942 [0x03] ExtData[1]->WorkLocal[37] = 731*
383: 0x0947 [0x03] ExtData[1]->WorkLocal[36] = 75*
384: 0x094C [0x01] GOTO 0x0992
385: 0x094F [0x02] IF !(ExtData[1]->WorkLocal[34] == 21*) GOTO 0x0969
386: 0x0957 [0x03] ExtData[1]->WorkLocal[35] = 5322*
387: 0x095C [0x03] ExtData[1]->WorkLocal[37] = 757*
388: 0x0961 [0x03] ExtData[1]->WorkLocal[36] = 100*
389: 0x0966 [0x01] GOTO 0x0992
390: 0x0969 [0x02] IF !(ExtData[1]->WorkLocal[34] == 22*) GOTO 0x0983
391: 0x0971 [0x03] ExtData[1]->WorkLocal[35] = 4206*
392: 0x0976 [0x03] ExtData[1]->WorkLocal[37] = 703*
393: 0x097B [0x03] ExtData[1]->WorkLocal[36] = 100*
394: 0x0980 [0x01] GOTO 0x0992
395: 0x0983 [0x03] ExtData[1]->WorkLocal[35] = 0*
396: 0x0988 [0x03] ExtData[1]->WorkLocal[36] = 0*
397: 0x098D [0x03] ExtData[1]->WorkLocal[37] = 0*

SUBROUTINE_0992:
398: 0x0992 [0x1B] RETURN

SUBROUTINE_0B77:
399: 0x0B77 [0x03] Work_Zone[2] = 2000*
400: 0x0B7C [0x48] [System] [7632*]:
    → "Ballista Points are rewarded to each competitor based on his or her performance in a single Ballista match. The maximum number of points that can be accumulated per match is $0."
401: 0x0B7F [0x23] WAIT_FOR_DIALOG_INTERACTION
402: 0x0B80 [0x48] [System] [7633*]:
    → "While it is taken into consideration, scoring Petras is not the only way to earn Ballista Points. Assisting allies and defeating members of the enemy forces can also earn a high score."
403: 0x0B83 [0x23] WAIT_FOR_DIALOG_INTERACTION
404: 0x0B84 [0x48] [System] [7634*]:
    → "Ballista Points can be traded for special "vouchers," which may be exchanged for free temporary items at the start of new matches."
405: 0x0B87 [0x23] WAIT_FOR_DIALOG_INTERACTION
406: 0x0B88 [0x48] [System] [7635*]:
    → "However, remember that depending on the level restriction of the match, certain vouchers will not be redeemed at the beginning of a match. Speak with a Marshal directly after a match to register your points."
407: 0x0B8B [0x23] WAIT_FOR_DIALOG_INTERACTION
408: 0x0B8C [0x1B] RETURN

SUBROUTINE_0B8D:
409: 0x0B8D [0x24] CREATE_DIALOG(message_id=7645*, default_option=ExtData[1]->WorkLocal[39], option_flags=0*)
    → "Read which chapter on B.P. Ranking? [Basic Information./Registering Points./Rewards./Chevron & Title List./None.]"
410: 0x0B94 [0x25] WAIT_DIALOG_SELECT()
411: 0x0B95 [0x03] ExtData[1]->WorkLocal[39] = Work_Zone[0]
412: 0x0B9A [0x02] IF !(Work_Zone[0] == 0*) GOTO 0x0BAC
413: 0x0BA2 [0x48] [System] [7646*]:
    → "Ballista Point Ranking is based upon all Ballista Points registered from the past ten matches in each of the three participating battle areas."
414: 0x0BA5 [0x23] WAIT_FOR_DIALOG_INTERACTION
415: 0x0BA6 [0x01] GOTO 0x0B8D

SUBROUTINE_0BF3:
416: 0x0BF3 [0x24] CREATE_DIALOG(message_id=7655*, default_option=ExtData[1]->WorkLocal[40], option_flags=0*)
    → "View which chevron/title? [1st (overall)./2nd (overall)./3rd (overall)./4th-10th (overall)./1st (warrior)./1st (monk)./1st (white mage)./1st (black mage)./1st (red mage)./1st (thief)./1st (paladin)./1st (dark knight)./1st (beastmaster)./1st (bard)./1st (ranger)./1st (samurai)./1st (ninja)./1st (dragoon)./1st (summoner)./1st (blue mage)./1st (corsair)./1st (puppetmaster)./1st (dancer)./1st (scholar)./1st (geomancer)./1st (rune fencer)./Return.]"
417: 0x0BFA [0x25] WAIT_DIALOG_SELECT()
418: 0x0BFB [0x03] ExtData[1]->WorkLocal[40] = Work_Zone[0]
419: 0x0C00 [0x02] IF !(Work_Zone[0] == 0*) GOTO 0x0C14
420: 0x0C08 [0x03] Work_Zone[2] = 682*
421: 0x0C0D [0x48] [System] [7656*]:
    → "Key Item: $3 Title: Gold Balli$25$25$25$25star"
422: 0x0C10 [0x23] WAIT_FOR_DIALOG_INTERACTION
423: 0x0C11 [0x01] GOTO 0x0E0B
424: 0x0C14 [0x02] IF !(Work_Zone[0] == 1*) GOTO 0x0C28
425: 0x0C1C [0x03] Work_Zone[2] = 683*
426: 0x0C21 [0x48] [System] [7657*]:
    → "Key Item: $3 Title: Mythril Balli$25$25$25star"
427: 0x0C24 [0x23] WAIT_FOR_DIALOG_INTERACTION
428: 0x0C25 [0x01] GOTO 0x0E0B
429: 0x0C28 [0x02] IF !(Work_Zone[0] == 2*) GOTO 0x0C3C
430: 0x0C30 [0x03] Work_Zone[2] = 684*
431: 0x0C35 [0x48] [System] [7658*]:
    → "Key Item: $3 Title: Silver Balli$25$25star"
432: 0x0C38 [0x23] WAIT_FOR_DIALOG_INTERACTION
433: 0x0C39 [0x01] GOTO 0x0E0B
434: 0x0C3C [0x02] IF !(Work_Zone[0] == 3*) GOTO 0x0C50
435: 0x0C44 [0x03] Work_Zone[2] = 685*
436: 0x0C49 [0x48] [System] [7659*]:
    → "Key Item: $3 Title: Bronze Balli$25star"
437: 0x0C4C [0x23] WAIT_FOR_DIALOG_INTERACTION
438: 0x0C4D [0x01] GOTO 0x0E0B
439: 0x0C50 [0x02] IF !(Work_Zone[0] == 4*) GOTO 0x0C64
440: 0x0C58 [0x03] Work_Zone[2] = 686*
441: 0x0C5D [0x48] [System] [7660*]:
    → "Key Item: $3 Title: Searing$26Star"
442: 0x0C60 [0x23] WAIT_FOR_DIALOG_INTERACTION
443: 0x0C61 [0x01] GOTO 0x0E0B
444: 0x0C64 [0x02] IF !(Work_Zone[0] == 5*) GOTO 0x0C78
445: 0x0C6C [0x03] Work_Zone[2] = 687*
446: 0x0C71 [0x48] [System] [7661*]:
    → "Key Item: $3 Title: Striking$26Star"
447: 0x0C74 [0x23] WAIT_FOR_DIALOG_INTERACTION
448: 0x0C75 [0x01] GOTO 0x0E0B
449: 0x0C78 [0x02] IF !(Work_Zone[0] == 6*) GOTO 0x0C8C
450: 0x0C80 [0x03] Work_Zone[2] = 688*
451: 0x0C85 [0x48] [System] [7662*]:
    → "Key Item: $3 Title: Soothing$26Star"
452: 0x0C88 [0x23] WAIT_FOR_DIALOG_INTERACTION
453: 0x0C89 [0x01] GOTO 0x0E0B
454: 0x0C8C [0x02] IF !(Work_Zone[0] == 7*) GOTO 0x0CA0
455: 0x0C94 [0x03] Work_Zone[2] = 689*
456: 0x0C99 [0x48] [System] [7663*]:
    → "Key Item: $3 Title: Sable$26Star"
457: 0x0C9C [0x23] WAIT_FOR_DIALOG_INTERACTION
458: 0x0C9D [0x01] GOTO 0x0E0B
459: 0x0CA0 [0x02] IF !(Work_Zone[0] == 8*) GOTO 0x0CB4
460: 0x0CA8 [0x03] Work_Zone[2] = 690*
461: 0x0CAD [0x48] [System] [7664*]:
    → "Key Item: $3 Title: Scarlet$26Star"
462: 0x0CB0 [0x23] WAIT_FOR_DIALOG_INTERACTION
463: 0x0CB1 [0x01] GOTO 0x0E0B
464: 0x0CB4 [0x02] IF !(Work_Zone[0] == 9*) GOTO 0x0CC8
465: 0x0CBC [0x03] Work_Zone[2] = 691*
466: 0x0CC1 [0x48] [System] [7665*]:
    → "Key Item: $3 Title: Sonic$26Star"
467: 0x0CC4 [0x23] WAIT_FOR_DIALOG_INTERACTION
468: 0x0CC5 [0x01] GOTO 0x0E0B
469: 0x0CC8 [0x02] IF !(Work_Zone[0] == 10*) GOTO 0x0CDC
470: 0x0CD0 [0x03] Work_Zone[2] = 692*
471: 0x0CD5 [0x48] [System] [7666*]:
    → "Key Item: $3 Title: Saintly$26Star"
472: 0x0CD8 [0x23] WAIT_FOR_DIALOG_INTERACTION
473: 0x0CD9 [0x01] GOTO 0x0E0B
474: 0x0CDC [0x02] IF !(Work_Zone[0] == 11*) GOTO 0x0CF0
475: 0x0CE4 [0x03] Work_Zone[2] = 693*
476: 0x0CE9 [0x48] [System] [7667*]:
    → "Key Item: $3 Title: Shadowy$26Star"
477: 0x0CEC [0x23] WAIT_FOR_DIALOG_INTERACTION
478: 0x0CED [0x01] GOTO 0x0E0B
479: 0x0CF0 [0x02] IF !(Work_Zone[0] == 12*) GOTO 0x0D04
480: 0x0CF8 [0x03] Work_Zone[2] = 694*
481: 0x0CFD [0x48] [System] [7668*]:
    → "Key Item: $3 Title: Savage$26Star"
482: 0x0D00 [0x23] WAIT_FOR_DIALOG_INTERACTION
483: 0x0D01 [0x01] GOTO 0x0E0B
484: 0x0D04 [0x02] IF !(Work_Zone[0] == 13*) GOTO 0x0D18
485: 0x0D0C [0x03] Work_Zone[2] = 695*
486: 0x0D11 [0x48] [System] [7669*]:
    → "Key Item: $3 Title: Singing$26Star"
487: 0x0D14 [0x23] WAIT_FOR_DIALOG_INTERACTION
488: 0x0D15 [0x01] GOTO 0x0E0B
489: 0x0D18 [0x02] IF !(Work_Zone[0] == 14*) GOTO 0x0D2C
490: 0x0D20 [0x03] Work_Zone[2] = 696*
491: 0x0D25 [0x48] [System] [7670*]:
    → "Key Item: $3 Title: Sniping$26Star"
492: 0x0D28 [0x23] WAIT_FOR_DIALOG_INTERACTION
493: 0x0D29 [0x01] GOTO 0x0E0B
494: 0x0D2C [0x02] IF !(Work_Zone[0] == 15*) GOTO 0x0D40
495: 0x0D34 [0x03] Work_Zone[2] = 697*
496: 0x0D39 [0x48] [System] [7671*]:
    → "Key Item: $3 Title: Slicing$26Star"
497: 0x0D3C [0x23] WAIT_FOR_DIALOG_INTERACTION
498: 0x0D3D [0x01] GOTO 0x0E0B
499: 0x0D40 [0x02] IF !(Work_Zone[0] == 16*) GOTO 0x0D54
500: 0x0D48 [0x03] Work_Zone[2] = 698*
501: 0x0D4D [0x48] [System] [7672*]:
    → "Key Item: $3 Title: Sneaking$26Star"
502: 0x0D50 [0x23] WAIT_FOR_DIALOG_INTERACTION
503: 0x0D51 [0x01] GOTO 0x0E0B
504: 0x0D54 [0x02] IF !(Work_Zone[0] == 17*) GOTO 0x0D68
505: 0x0D5C [0x03] Work_Zone[2] = 699*
506: 0x0D61 [0x48] [System] [7673*]:
    → "Key Item: $3 Title: Spearing$26Star"
507: 0x0D64 [0x23] WAIT_FOR_DIALOG_INTERACTION
508: 0x0D65 [0x01] GOTO 0x0E0B
509: 0x0D68 [0x02] IF !(Work_Zone[0] == 18*) GOTO 0x0D7C
510: 0x0D70 [0x03] Work_Zone[2] = 700*
511: 0x0D75 [0x48] [System] [7674*]:
    → "Key Item: $3 Title: Summoning$26Star"
512: 0x0D78 [0x23] WAIT_FOR_DIALOG_INTERACTION
513: 0x0D79 [0x01] GOTO 0x0E0B
514: 0x0D7C [0x02] IF !(Work_Zone[0] == 19*) GOTO 0x0D90
515: 0x0D84 [0x03] Work_Zone[2] = 777*
516: 0x0D89 [0x48] [System] [7675*]:
    → "Key Item: $3 Title: Sapphire$26Star"
517: 0x0D8C [0x23] WAIT_FOR_DIALOG_INTERACTION
518: 0x0D8D [0x01] GOTO 0x0E0B
519: 0x0D90 [0x02] IF !(Work_Zone[0] == 20*) GOTO 0x0DA4
520: 0x0D98 [0x03] Work_Zone[2] = 778*
521: 0x0D9D [0x48] [System] [7676*]:
    → "Key Item: $3 Title: Surging$26Star"
522: 0x0DA0 [0x23] WAIT_FOR_DIALOG_INTERACTION
523: 0x0DA1 [0x01] GOTO 0x0E0B
524: 0x0DA4 [0x02] IF !(Work_Zone[0] == 21*) GOTO 0x0DB8
525: 0x0DAC [0x03] Work_Zone[2] = 779*
526: 0x0DB1 [0x48] [System] [7677*]:
    → "Key Item: $3 Title: Swaying$26Star"
527: 0x0DB4 [0x23] WAIT_FOR_DIALOG_INTERACTION
528: 0x0DB5 [0x01] GOTO 0x0E0B
529: 0x0DB8 [0x02] IF !(Work_Zone[0] == 22*) GOTO 0x0DCC
530: 0x0DC0 [0x03] Work_Zone[2] = 915*
531: 0x0DC5 [0x48] [System] [7678*]:
    → "Key Item: $3 Title: Sprightly$26Star"
532: 0x0DC8 [0x23] WAIT_FOR_DIALOG_INTERACTION
533: 0x0DC9 [0x01] GOTO 0x0E0B
534: 0x0DCC [0x02] IF !(Work_Zone[0] == 23*) GOTO 0x0DE0
535: 0x0DD4 [0x03] Work_Zone[2] = 916*
536: 0x0DD9 [0x48] [System] [7679*]:
    → "Key Item: $3 Title: Sagacious$26Star"
537: 0x0DDC [0x23] WAIT_FOR_DIALOG_INTERACTION
538: 0x0DDD [0x01] GOTO 0x0E0B
539: 0x0DE0 [0x02] IF !(Work_Zone[0] == 24*) GOTO 0x0DF4
540: 0x0DE8 [0x03] Work_Zone[2] = 1847*
541: 0x0DED [0x48] [System] [7680*]:
    → "Key Item: $3 Title: Spiritual$26Star"
542: 0x0DF0 [0x23] WAIT_FOR_DIALOG_INTERACTION
543: 0x0DF1 [0x01] GOTO 0x0E0B
544: 0x0DF4 [0x02] IF !(Work_Zone[0] == 25*) GOTO 0x0E08
545: 0x0DFC [0x03] Work_Zone[2] = 1848*
546: 0x0E01 [0x48] [System] [7681*]:
    → "Key Item: $3 Title: Stippling$26Star"
547: 0x0E04 [0x23] WAIT_FOR_DIALOG_INTERACTION
548: 0x0E05 [0x01] GOTO 0x0E0B
549: 0x0E08 [0x01] GOTO 0x0B8D

SUBROUTINE_0E0B:
550: 0x0E0B [0x01] GOTO 0x0BF3

SUBROUTINE_0E11:
551: 0x0E11 [0x1B] RETURN

SUBROUTINE_0E12:
552: 0x0E12 [0x03] ExtData[1]->WorkLocal[28] = 1*
553: 0x0E17 [0x1B] RETURN

SUBROUTINE_0E18:
554: 0x0E18 [0x03] Work_Zone[2] = ExtData[1]->WorkLocal[1]
555: 0x0E1D [0x07] Work_Zone[2] += 606*
556: 0x0E22 [0x03] Work_Zone[3] = ExtData[1]->WorkLocal[1]
557: 0x0E27 [0x07] Work_Zone[3] += 13824*
558: 0x0E2C [0x1B] RETURN
```

#### Data or dead code:

```
# Dead code (unreachable instructions):
     0x02B3 [0x01] GOTO 0x0499
     0x02C4 [0x01] GOTO 0x0499
# Dead code (unreachable instructions):
     0x0401 [0x01] GOTO 0x0408
# Dead code (unreachable instructions):
     0x043E [0x01] GOTO 0x0499
     0x0479 [0x01] GOTO 0x0487
# Dead code (unreachable instructions):
     0x0545 [0x18] 0x1B00 = atan2(0x1400, 0x1A00)
     0x054C [0xEA] UNKNOWN_0xEA
     0x054D [0x0E] 0x1A02 |= 0x2D00
     0x0552 [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 486890880/0x1D055D80))
     0x0557 [0x2E] SET_CLI_EVENT_CANCEL_FLAGS()
     0x0558 [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 100270371/0x05FA0123))
     0x055D [0x02] IF !(ExtData[1]->WorkLocal[26] == 4294967294*) GOTO 0x0571
     0x0565 [0x03] Work_Zone[2] = ExtData[1]->WorkLocal[21]
     0x056A [0x1D] PRINT_EVENT_MESSAGE(message_id=7591*)
    → "You have already registered your Ballista Points for the previous match. Your current rank is $0."
     0x056D [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x056E [0x01] GOTO 0x05FA
     0x0571 [0x02] IF !(ExtData[1]->WorkLocal[26] == 4294967293*) GOTO 0x0584
     0x0579 [0x1D] PRINT_EVENT_MESSAGE(message_id=7587*)
    → "I am sorry, but we are no longer accepting Ballista Point Ranking registrations for the match in which you previously participated."
     0x057C [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x057D [0x1D] PRINT_EVENT_MESSAGE(message_id=7588*)
    → "For future reference, please note that the registration period is exactly four hours (Vana'diel time) from the close of each Ballista match."
     0x0580 [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x0581 [0x01] GOTO 0x05FA
     0x0584 [0x02] IF !(ExtData[1]->WorkLocal[21] == 0*) GOTO 0x0593
     0x058C [0x1D] PRINT_EVENT_MESSAGE(message_id=7585*)
    → "I registered your point total; however, it seems you did not make the final list. But do not fret. You will have many chances to redeem yourself in future matches. Good luck, <Player>!"
     0x058F [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x0590 [0x01] GOTO 0x05FA
     0x0593 [0x03] Work_Zone[1] = 4294967294*
     0x0598 [0x43] SEND_EVENT_UPDATE: Send pending tag to server (packet 0x005B)
     0x059A [0x43] SEND_EVENT_UPDATE: Check pending flag (skip if not pending)
     0x059C [0x03] Work_Zone[1] = 1073741824*
     0x05A1 [0x03] ExtData[1]->WorkLocal[1] = ExtData[1]->WorkLocal[20]
     0x05A6 [0x1A] CALL_SUBROUTINE(address=0x0E18)
     0x05A9 [0x02] IF !(ExtData[1]->WorkLocal[21] > 10*) GOTO 0x05C8
     0x05B1 [0x1D] PRINT_EVENT_MESSAGE(message_id=7550*)
    → "Your points have been registered. Congratulations! You currently rank in the top ten!"
     0x05B4 [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x05B5 [0x1D] PRINT_EVENT_MESSAGE(message_id=7552*)
    → "We ask that you check the rankings on the following date: Vana'diel: Earth:"
     0x05B8 [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x05B9 [0x1D] PRINT_EVENT_MESSAGE(message_id=7553*)
    → "If, at that time, you still remain in the top ten, you will be rewarded with a special prize."
     0x05BC [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x05BD [0x1D] PRINT_EVENT_MESSAGE(message_id=7555*)
    → "However, all records of the match will be destroyed on (Vana'diel) or (Earth)."
     0x05C0 [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x05C1 [0x1D] PRINT_EVENT_MESSAGE(message_id=7556*)
    → "If you do not speak with a Marshal by that date, you will lose the opportunity to claim that prize."
     0x05C4 [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x05C5 [0x01] GOTO 0x05FA
     0x05C8 [0x02] IF !(ExtData[1]->WorkLocal[24] == 1*) GOTO 0x05F1
     0x05D0 [0x03] Work_Zone[8] = ExtData[1]->WorkLocal[23]
     0x05D5 [0x03] Work_Zone[9] = ExtData[1]->WorkLocal[25]
     0x05DA [0x1D] PRINT_EVENT_MESSAGE(message_id=7551*)
    → "Your points have been registered. Congratulations! You are currently ranked as the top [/warrior/monk/white mage/black mage/red mage/thief/paladin/dark knight/beastmaster/bard/ranger/samurai/ninja/dragoon/summoner/blue mage/corsair/puppetmaster]! (Total [/warriors/monks/white mages/black mages/red mages/thieves/paladins/dark knights/beastmasters/bards/rangers/samurais/ninjas/dragoons/summoners/blue mages/corsairs/puppetmasters/dancers/scholars/geomancers/rune fencers] registered: $7)"
     0x05DD [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x05DE [0x1D] PRINT_EVENT_MESSAGE(message_id=7552*)
    → "We ask that you check the rankings on the following date: Vana'diel: Earth:"
     0x05E1 [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x05E2 [0x1D] PRINT_EVENT_MESSAGE(message_id=7554*)
    → "If, at this time, you still remain in first place, you will be recognized as the match MVP."
     0x05E5 [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x05E6 [0x1D] PRINT_EVENT_MESSAGE(message_id=7555*)
    → "However, all records of the match will be destroyed on (Vana'diel) or (Earth)."
     0x05E9 [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x05EA [0x1D] PRINT_EVENT_MESSAGE(message_id=7557*)
    → "If you do not speak with a Marshal by that date, you forfeit all benefits that accompany MVP status."
     0x05ED [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x05EE [0x01] GOTO 0x05FA
     0x05F1 [0x03] Work_Zone[9] = ExtData[1]->WorkLocal[21]
     0x05F6 [0x1D] PRINT_EVENT_MESSAGE(message_id=7584*)
    → "I have finished registering your data. Your final Ballista Point Ranking for the previous match is: $7."
     0x05F9 [0x23] WAIT_FOR_DIALOG_INTERACTION
# Dead code (unreachable instructions):
     0x06B3 [0x16] 0x1500 = sin(Work_Zone_1700[0]) * 0x1900
     0x06BA [0x00] END_REQSTACK()
     0x06BB [0x18] 0x1A00 = atan2(0x0EEA, 0x1A02)
     0x06C2 [0x00] END_REQSTACK()
     0x06C3 [0x2D] CREATE_ZONE_SCHEDULER_TASK: Create scheduler 0x00100103 with entities [Unknown NPC (ID: 114557056/0x06D40080), Unknown NPC (ID: 595609373/0x2380471D)]
     0x06D0 [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 436661761/0x1A06EE01))
     0x06D5 [0x4B] UPDATE_ENTITY_YAW(entity=Unknown NPC (ID: 1376782/0x0015020E), yaw=0.0°*)
     0x06DC [0x00] END_REQSTACK()
     0x06DD [0xEB] UNKNOWN_0xEB
     0x06DE [0x06] 0x481D = 0
     0x06E1 [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 268501795/0x10010323))
     0x06E6 [0x00] END_REQSTACK()
     0x06E7 [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 436661761/0x1A06EE01))
     0x06EC [0x1F] MOVE_ENTITY: Update entity position (mode=15)
     0x06EE [0x01] GOTO 0x06F6
# Dead code (unreachable instructions):
     0x070F [0x01] GOTO 0x072B
     0x0728 [0x01] GOTO 0x072B
# Dead code (unreachable instructions):
     0x0993 [0x24] CREATE_DIALOG(message_id=7595*, default_option=Work_Zone[0], option_flags=0*)
    → "What do you wish to know about? [Basic rules./About the Petra./About Sprint./About Scout./About Gate Breach./About area-specific rules./About returning to battle./About special rules./Nothing at the moment.]"
     0x099A [0x25] WAIT_DIALOG_SELECT()
     0x099B [0x02] IF !(Work_Zone[0] == 0*) GOTO 0x09BD
     0x09A3 [0x48] [System] [7596*]:
    → "Ballista is an exercise once used by the San d'Orian armies to train the Royal Knights in the art of the siege. The word "ballista" means "to throw a stone," and the game has been a part of Vana'dielian military history for centuries."
     0x09A6 [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x09A7 [0x48] [System] [7597*]:
    → "The rules are simple. Ballista is played by two teams over a number of hours (Vana'diel time). The team with the most points at the end of the match is the victor."
     0x09AA [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x09AB [0x48] [System] [7598*]:
    → "Players from both teams must search for Petras that a Herald has buried in various locations across the battlefield. The players must then take these stones and throw them into targets known as Rooks."
     0x09AE [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x09AF [0x48] [System] [7599*]:
    → "These Rooks can appear in various locations around the battlefield, and both teams are allowed to throw their Petras into them."
     0x09B2 [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x09B3 [0x48] [System] [7600*]:
    → "Finally, a player's strength is adjusted depending on his or her level. It is felt that this ultimately allows for a more balanced battle."
     0x09B6 [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x09B7 [0x01] GOTO 0x0993
     0x09BA [0x01] GOTO 0x0B76
     0x09BD [0x02] IF !(Work_Zone[0] == 1*) GOTO 0x09DF
     0x09C5 [0x48] [System] [7601*]:
    → "Petras can be buried anywhere. Once a player believes he or she has found one, the Quarry (/quarry) command should be used to dig up the area at his or her feet."
     0x09C8 [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x09C9 [0x48] [System] [7602*]:
    → "Sometimes a player may find items other than Petras, such as potions. These temporary items can only be used during the current match (or Best of Three series)."
     0x09CC [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x09CD [0x48] [System] [7603*]:
    → "Players can carry more than one Petra, but they cannot throw the stones away once they have obtained them. However, players will lose their Petras if they: (1) Successfully throw them into a rook. (2) Leave the area. (3) Are knocked out."
     0x09D0 [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x09D1 [0x48] [System] [7604*]:
    → "But don't forget, a player will be unable to use the Sprint command while in possession of a Petra. And, players can only use the Scout command if they carry one of the stones."
     0x09D4 [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x09D5 [0x48] [System] [7605*]:
    → "As one can see, careful planning and impeccable timing are the keys to victory."
     0x09D8 [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x09D9 [0x01] GOTO 0x0993
     0x09DC [0x01] GOTO 0x0B76
     0x09DF [0x02] IF !(Work_Zone[0] == 2*) GOTO 0x09F5
     0x09E7 [0x48] [System] [7606*]:
    → "All participants of Ballista will have the ability to use the Sprint command. This will temporarily increase one's movement speed, creating a better chance to find and obtain Petras."
     0x09EA [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x09EB [0x48] [System] [7607*]:
    → "However, remember that once a player has obtained a Petra, he or she will be unable to use the Sprint command. In its place, though, one may use the Scout command."
     0x09EE [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x09EF [0x01] GOTO 0x0993
     0x09F2 [0x01] GOTO 0x0B76
     0x09F5 [0x02] IF !(Work_Zone[0] == 3*) GOTO 0x0A0F
     0x09FD [0x48] [System] [7608*]:
    → "The locations of the Rooks are constantly changing. This makes it quite difficult to find the targets by just randomly walking about the area."
     0x0A00 [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x0A01 [0x48] [System] [7609*]:
    → "Communication with one's team members is, of course, one way to locate Rooks. However, another way is by using the Scout command. This will inform a player of the approximate location of the nearest Rook."
     0x0A04 [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x0A05 [0x48] [System] [7610*]:
    → "Finally, Scout can only be used when a player is carrying a Petra. Because Sprint cannot be used at this time, players must be weary of approaching enemies."
     0x0A08 [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x0A09 [0x01] GOTO 0x0993
     0x0A0C [0x01] GOTO 0x0B76
     0x0A0F [0x02] IF !(Work_Zone[0] == 4*) GOTO 0x0A35
     0x0A17 [0x48] [System] [7611*]:
    → "So, a Petra has been obtained and a Rook located. Now all that's left is tossing the stone in, right? Wrong."
     0x0A1A [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x0A1B [0x48] [System] [7612*]:
    → "There is one aspect of Ballista that has yet to be mentioned...and this is where the real battle begins."
     0x0A1E [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x0A1F [0x48] [System] [7613*]:
    → "To throw a Petra into a Rook, a player must first wait until the Rook's gate is open. This takes impeccable timing, as the gate is only open for a short while."
     0x0A22 [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x0A23 [0x48] [System] [7614*]:
    → "But not just anyone can score points this way. A player must first take down a member of the opposing team and achieve Gate Breach status. Only then will his or her shots count."
     0x0A26 [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x0A27 [0x48] [System] [7615*]:
    → "This extreme method of combat is what made the ancient game of Ballista famous, and transformed its participants into heroes."
     0x0A2A [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x0A2B [0x48] [System] [7616*]:
    → "Running about looking for stones is no different from a child's game of hide-and-seek. Knowing when to strike is what separates the men from the boys."
     0x0A2E [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x0A2F [0x01] GOTO 0x0993
     0x0A32 [0x01] GOTO 0x0B76
     0x0A35 [0x02] IF !(Work_Zone[0] == 5*) GOTO 0x0A5B
     0x0A3D [0x48] [System] [7617*]:
    → "The following match types are area-specific: All or Nothing (one match) * Normal Items Allowed: Jugner Forest All or Nothing (one match) * Normal Items Restricted: Pashhow Marshlands Best of Three (three matches) * Normal Items Allowed: Meriphataud Mountains"
     0x0A40 [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x0A41 [0x48] [System] [7618*]:
    → "Best of Three series rules are as follows: Winning a match will earn a nation one "match point." The nation with the most match points at the end of the series will be declared the victor. Winning the third and final match and producing the high scorer will result in two match points awarded. This allows both sides the chance for "come-from-behind" victories."
     0x0A44 [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x0A45 [0x48] [System] [7619*]:
    → "Between each of the matches, a short interval will be implemented. During this interval, Invisible and Sneak will be cast on all participants. We suggest this time be used to discuss strategies with the other members of your nation's forces."
     0x0A48 [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x0A49 [0x48] [System] [7620*]:
    → "Players will be forbidden from using any items they bring to the battlefield. Only temporary items that are quarried may be used. A total of fifteen temporary items (though only one of each type) can be carried."
     0x0A4C [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x0A4D [0x48] [System] [7621*]:
    → "When this rule is in effect, all players will be presented with five complimentary temporary items at the beginning of the match."
     0x0A50 [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x0A51 [0x48] [System] [7622*]:
    → "For more information on temporary items, refer to the "special rules.""
     0x0A54 [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x0A55 [0x01] GOTO 0x0993
     0x0A58 [0x01] GOTO 0x0B76
     0x0A5B [0x02] IF !(Work_Zone[0] == 6*) GOTO 0x0A71
     0x0A63 [0x48] [System] [7623*]:
    → "After being KO'd, if a player chooses to be revived at the spot he was defeated, 100% of all HP and 50% of all MP will be restored. In addition, the player will maintain 50% of the TP he had earned."
     0x0A66 [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x0A67 [0x48] [System] [7624*]:
    → "If a player chooses to return to their force's camp, HP and MP will be fully restored, but all TP will be lost."
     0x0A6A [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x0A6B [0x01] GOTO 0x0993
     0x0A6E [0x01] GOTO 0x0B76
     0x0A71 [0x02] IF !(Work_Zone[0] == 7*) GOTO 0x0B76
     0x0A79 [0x06] Work_Zone[0] = 0
     0x0A7C [0x48] [System] [7625*]:
    → "Please note that many of the rules previously considered "special" have been fully implemented. Also, several new special rules have been added."
     0x0A7F [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x0A80 [0x24] CREATE_DIALOG(message_id=7626*, default_option=Work_Zone[0], option_flags=0*)
    → "Learn about which special rules? [Temporary items./Ballista Points./Scoring restrictions./Gate Breach radius expansion./Resistance to Sleep./Open entries./Minimum level restrictions./Re-entry restrictions./Participant & Rook numbers./Experience Points./Return.]"
     0x0A87 [0x25] WAIT_DIALOG_SELECT()
     0x0A88 [0x02] IF !(Work_Zone[0] == 0*) GOTO 0x0AAA
     0x0A90 [0x48] [System] [7627*]:
    → "Previously, participants in Ballista were required to use an item the moment they dug it up using the /quarry command. However, players can now stock these items as temporary items for use at any time during the match."
     0x0A93 [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x0A94 [0x48] [System] [7628*]:
    → "Players can hold up to ten temporary items, but only one of each type. If a player finds an item that they already possess, they must use it immediately or throw it away."
     0x0A97 [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x0A98 [0x48] [System] [7629*]:
    → "If players who possess ten temporary items find an item that they have not already obtained, they will also have the option to exchange it with one of their current items."
     0x0A9B [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x0A9C [0x48] [System] [7630*]:
    → "Also, players will not, at any time, be allowed to trade temporary items. However, there are items that can be used on other Ballista participants. Please note that players may not be able to use these special items as soon as they are quarried."
     0x0A9F [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x0AA0 [0x48] [System] [7631*]:
    → "Finally, the Heralds will collect temporary items when the match (the third in a Best of Three series) has concluded."
     0x0AA3 [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x0AA4 [0x01] GOTO 0x0A80
     0x0AA7 [0x01] GOTO 0x0B73
     0x0AAA [0x02] IF !(Work_Zone[0] == 1*) GOTO 0x0ABB
     0x0AB2 [0x1A] CALL_SUBROUTINE(address=0x0B77)
     0x0AB5 [0x01] GOTO 0x0A80
     0x0AB8 [0x01] GOTO 0x0B73
     0x0ABB [0x02] IF !(Work_Zone[0] == 2*) GOTO 0x0ACD
     0x0AC3 [0x48] [System] [7636*]:
    → "Players will only be allowed to score up to five Petras at a time. Also, a player's Gate Breach status will be automatically disengaged if a Petra is not scored within a certain amount of time. The amount of time is increased for those members of the team currently behind in points."
     0x0AC6 [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x0AC7 [0x01] GOTO 0x0A80
     0x0ACA [0x01] GOTO 0x0B73
     0x0ACD [0x02] IF !(Work_Zone[0] == 3*) GOTO 0x0ADF
     0x0AD5 [0x48] [System] [7637*]:
    → "All allies within a certain radius will achieve Gate Breach status when a member of the enemy forces is defeated."
     0x0AD8 [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x0AD9 [0x01] GOTO 0x0A80
     0x0ADC [0x01] GOTO 0x0B73
     0x0ADF [0x02] IF !(Work_Zone[0] == 4*) GOTO 0x0AF1
     0x0AE7 [0x48] [System] [7638*]:
    → "Players' resistance to sleep will be increased."
     0x0AEA [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x0AEB [0x01] GOTO 0x0A80
     0x0AEE [0x01] GOTO 0x0B73
     0x0AF1 [0x02] IF !(Work_Zone[0] == 5*) GOTO 0x0B14
     0x0AF9 [0xAA] VANA_DIEL_TIMESTAMP_CONVERTER(timestamp=2592*, year=ExtData[1]->WorkLocal[1], month=ExtData[1]->WorkLocal[1], day=ExtData[1]->WorkLocal[1], weekday=ExtData[1]->WorkLocal[1], hour=Work_Zone[2], minute=Work_Zone[3], moon=ExtData[1]->WorkLocal[1])
     0x0B0A [0x48] [System] [7639*]:
    → "During the entry period, from $0:, all adventurers will be allowed to join the forces with the smallest number of participants, regardless of allegiance. Open entries will be accepted until both sides have reached maximum capacity."
     0x0B0D [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x0B0E [0x01] GOTO 0x0A80
     0x0B11 [0x01] GOTO 0x0B73
     0x0B14 [0x02] IF !(Work_Zone[0] == 6*) GOTO 0x0B2A
     0x0B1C [0x48] [System] [7640*]:
    → "In order to ensure balanced matches, the following level limits are enforced: Level 30 matches: 1`30"
     0x0B1F [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x0B20 [0x48] [System] [7641*]:
    → "Level 40 matches: 31`40 Level 50 matches: 41`50 Level 60 matches: 51`60 Unrestricted matches: 61`"
     0x0B23 [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x0B24 [0x01] GOTO 0x0A80
     0x0B27 [0x01] GOTO 0x0B73
     0x0B2A [0x02] IF !(Work_Zone[0] == 7*) GOTO 0x0B3C
     0x0B32 [0x48] [System] [7642*]:
    → "Please note that any participant who cancels his or her registration will not be allowed to re-apply for the same match."
     0x0B35 [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x0B36 [0x01] GOTO 0x0A80
     0x0B39 [0x01] GOTO 0x0B73
     0x0B3C [0x02] IF !(Work_Zone[0] == 8*) GOTO 0x0B4E
     0x0B44 [0x48] [System] [7643*]:
    → "When the total number of participants in a match is less than 40, the location of the two camps will be adjusted. Also in this case, the number of Rooks will be reduced to two."
     0x0B47 [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x0B48 [0x01] GOTO 0x0A80
     0x0B4B [0x01] GOTO 0x0B73
     0x0B4E [0x02] IF !(Work_Zone[0] == 9*) GOTO 0x0B60
     0x0B56 [0x48] [System] [7644*]:
    → "Any adventurer whose main level is over 30 will receive EXP for participating in a Ballista match. However, no EXP will be rewarded if the adventurer fails to actively take part in the match."
     0x0B59 [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x0B5A [0x01] GOTO 0x0A80
     0x0B5D [0x01] GOTO 0x0B73
     0x0B60 [0x02] IF !(Work_Zone[0] == 10*) GOTO 0x0B73
     0x0B68 [0x03] Work_Zone[0] = 7*
     0x0B6D [0x01] GOTO 0x0993
     0x0B70 [0x01] GOTO 0x0B73
     0x0B73 [0x01] GOTO 0x0B76
     0x0B76 [0x1B] RETURN
# Dead code (unreachable instructions):
     0x0BA9 [0x01] GOTO 0x0E11
     0x0BBF [0x01] GOTO 0x0E11
     0x0BE5 [0x01] GOTO 0x0E11
# Dead code (unreachable instructions):
     0x0E0E [0x01] GOTO 0x0E11
# Dead code (unreachable instructions):
     0x0E2D [0x1D] PRINT_EVENT_MESSAGE(message_id=7574*)
    → "As long as you are in possession of the $3, you will bear the aforementioned title while participating in Ballista."
     0x0E30 [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x0E31 [0x1D] PRINT_EVENT_MESSAGE(message_id=7575*)
    → "Upon the chance another contestant "checks" you during a match, not only will he be able to see your title, but also the total number of chevrons you have accumulated in your battles."
     0x0E34 [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x0E35 [0x1D] PRINT_EVENT_MESSAGE(message_id=7576*)
    → "Finally, the official rules of Ballista state that after a set period of time, you are required to return any chevrons received. When that time has come, a Herald will collect your chevrons at the time of entry into a new match."
     0x0E38 [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x0E39 [0x45] LOAD_SCHEDULED_TASK: Load scheduler "qstc" with entities [LocalPlayer, LocalPlayer], work=[201*, 0*]
     0x0E4A [0x1B] RETURN
     0x0E4B [0xB3] RANKINGS_BOARD_HANDLER(case_type=0x02)
     0x0E4D [0xB3] RANKINGS_BOARD_HANDLER(case_type=0x09)
     0x0E4F [0x01] GOTO 0x0200
     0x0E52 [0x01] GOTO 0x0200
     0x0E55 [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 17718272/0x010E5C00))
     0x0E5A [0x4D] EventEntity->StatusEvent = 9 // Close door
     0x0E5B [0x0E] 0x031B |= Work_Zone[1]
     0x0EEA [0x02] IF !(ExtData[1]->WorkLocal[32] == 1*) GOTO 0x0F1E
     0x0EF2 [0x03] Work_Zone[2] = ExtData[1]->WorkLocal[26]
     0x0EF7 [0x03] Work_Zone[3] = ExtData[1]->WorkLocal[22]
     0x0EFC [0x03] Work_Zone[4] = ExtData[1]->WorkLocal[21]
     0x0F01 [0x03] Work_Zone[5] = ExtData[1]->WorkLocal[23]
     0x0F06 [0x03] Work_Zone[6] = ExtData[1]->WorkLocal[25]
     0x0F0B [0x03] Work_Zone[7] = ExtData[1]->WorkLocal[24]
     0x0F10 [0x03] Work_Zone[8] = ExtData[1]->WorkLocal[27]
     0x0F15 [0x03] Work_Zone[9] = ExtData[1]->WorkLocal[20]
     0x0F1A [0x48] [System] [7592*]:
    → "$26$3V$3X$3e$3 :$0@$26$13$P13o%^$20:$1 $26$)$7<Player>J:$2@$26$P13o%^$3W$3 $3u:$3 $26$P13/$3W$3 $3u$P13o%^$20:$4@$26$3W$3 $3u$P15J$)$7<Player>J:$5 $26$P13o%^BP:$6@$26TIME:$7"
     0x0F1D [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x0F1E [0x1B] RETURN
     0x0F1F [0x06] ExtData[1]->WorkLocal[1] = 0
     0x0F22 [0x02] IF !(ExtData[1]->WorkLocal[28] == 1*) GOTO 0x0F37
     0x0F2A [0x3E] IF !(ExtData[1]->WorkLocal[31] bit 0*) GOTO 0x0F34
     0x0F31 [0x05] ExtData[1]->WorkLocal[1] = 1
     0x0F34 [0x01] GOTO 0x0F61
     0x0F37 [0x02] IF !(ExtData[1]->WorkLocal[28] == 2*) GOTO 0x0F4C
     0x0F3F [0x3E] IF !(ExtData[1]->WorkLocal[31] bit 1*) GOTO 0x0F49
     0x0F46 [0x05] ExtData[1]->WorkLocal[1] = 1
     0x0F49 [0x01] GOTO 0x0F61
     0x0F4C [0x02] IF !(ExtData[1]->WorkLocal[28] == 3*) GOTO 0x0F61
     0x0F54 [0x3E] IF !(ExtData[1]->WorkLocal[31] bit 2*) GOTO 0x0F5E
     0x0F5B [0x05] ExtData[1]->WorkLocal[1] = 1
     0x0F5E [0x01] GOTO 0x0F61
     0x0F61 [0x02] IF !(ExtData[1]->WorkLocal[1] == 1*) GOTO 0x0F75
     0x0F69 [0x03] Work_Zone[2] = ExtData[1]->WorkLocal[28]
     0x0F6E [0x1D] PRINT_EVENT_MESSAGE(message_id=7578*)
    → "You have already confirmed your data for the [/Jugner Forest/Pashhow Marshlands/Meriphataud Mountains] match. We look forward to your participation in future matches!"
     0x0F71 [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x0F72 [0x01] GOTO 0x1337
     0x0F75 [0x42] SET_CLI_EVENT_CANCEL_DATA()
     0x0F76 [0x03] Work_Zone[1] = 4294967292*
     0x0F7B [0x43] SEND_EVENT_UPDATE: Send pending tag to server (packet 0x005B)
     0x0F7D [0x43] SEND_EVENT_UPDATE: Check pending flag (skip if not pending)
     0x0F7F [0xB3] RANKINGS_BOARD_HANDLER(case_type=0x07)
     0x0F81 [0x1C] WAIT(0xB300 ticks)
     0x0F84 [0x05] ExtData[1]->WorkLocal[26] = 1
     0x0F87 [0x16] 0x1500 = sin(Work_Zone_1700[0]) * 0x1900
     0x0F8E [0x00] END_REQSTACK()
     0x0F8F [0x18] 0x1B00 = atan2(0x1400, 0x1A00)
     0x0F96 [0xEA] UNKNOWN_0xEA
     0x0F97 [0x0E] 0x1A02 |= 0x2D00
     0x0F9C [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 487566336/0x1D0FAC00))
     0x0FA1 [0x47] WAIT_PLAYER_POS_UPDATE
     0x0FA3 [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x0FA4 [0x03] Work_Zone[1] = 1073741824*
     0x0FA9 [0x01] GOTO 0x1337
     0x0FAC [0x02] IF !(ExtData[1]->WorkLocal[21] == 0*) GOTO 0x0FBB
     0x0FB4 [0x1D] PRINT_EVENT_MESSAGE(message_id=7583*)
    → "Your data did not make the final ranking list."
     0x0FB7 [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x0FB8 [0x01] GOTO 0x1337
     0x0FBB [0x03] Work_Zone[1] = 4294967290*
     0x0FC0 [0x43] SEND_EVENT_UPDATE: Send pending tag to server (packet 0x005B)
     0x0FC2 [0x43] SEND_EVENT_UPDATE: Check pending flag (skip if not pending)
     0x0FC4 [0x02] IF !(Work_Zone[2] == 4294967295*) GOTO 0x0FE8
     0x0FCC [0x1D] PRINT_EVENT_MESSAGE(message_id=7561*)
    → "Please note that because the point registration period for the previous match has not yet ended, your ranking may change. We ask that you check back later for the final results."
     0x0FCF [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x0FD0 [0x03] ExtData[1]->WorkLocal[1] = ExtData[1]->WorkLocal[20]
     0x0FD5 [0x1A] CALL_SUBROUTINE(address=0x0E18)
     0x0FD8 [0x1D] PRINT_EVENT_MESSAGE(message_id=7552*)
    → "We ask that you check the rankings on the following date: Vana'diel: Earth:"
     0x0FDB [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x0FDC [0x1D] PRINT_EVENT_MESSAGE(message_id=7555*)
    → "However, all records of the match will be destroyed on (Vana'diel) or (Earth)."
     0x0FDF [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x0FE0 [0x03] Work_Zone[1] = 1073741824*
     0x0FE5 [0x01] GOTO 0x1337
     0x0FE8 [0x02] IF !(Work_Zone[2] == 4294967294*) GOTO 0x0FFC
     0x0FF0 [0x1D] PRINT_EVENT_MESSAGE(message_id=7562*)
    → "Your performance in the last match earned you a place in the overall ranking. However, because you did not check with a Herald before the records were destroyed, you have lost the opportunity to claim your prize."
     0x0FF3 [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x0FF4 [0x03] Work_Zone[1] = 1073741824*
     0x0FF9 [0x01] GOTO 0x1337
     0x0FFC [0x02] IF !(Work_Zone[2] == 1*) GOTO 0x1332
     0x1004 [0x06] Work_Zone[1] = 0
     0x1007 [0x06] ExtData[1]->WorkLocal[1] = 0
     0x100A [0x06] Work_Zone[2] = 0
     0x100D [0x06] Work_Zone[3] = 0
     0x1010 [0x06] Work_Zone[4] = 0
     0x1013 [0x06] Work_Zone[5] = 0
     0x1016 [0x06] Work_Zone[6] = 0
     0x1019 [0x02] IF !(ExtData[1]->WorkLocal[21] > 10*) GOTO 0x1062
     0x1021 [0x3C] SET_BIT_FLAG_CONDITIONAL(target_work_offset=ExtData[1]->WorkLocal[1], bit_index_work_offset=0*, condition_work_offset=1*)
     0x1028 [0x03] Work_Zone[2] = ExtData[1]->WorkLocal[21]
     0x102D [0x02] IF !(ExtData[1]->WorkLocal[21] == 1*) GOTO 0x103D
     0x1035 [0x03] Work_Zone[3] = 682*
     0x103A [0x01] GOTO 0x1062
     0x103D [0x02] IF !(ExtData[1]->WorkLocal[21] == 2*) GOTO 0x104D
     0x1045 [0x03] Work_Zone[3] = 683*
     0x104A [0x01] GOTO 0x1062
     0x104D [0x02] IF !(ExtData[1]->WorkLocal[21] == 3*) GOTO 0x105D
     0x1055 [0x03] Work_Zone[3] = 684*
     0x105A [0x01] GOTO 0x1062
     0x105D [0x03] Work_Zone[3] = 685*
     0x1062 [0x02] IF !(ExtData[1]->WorkLocal[24] == 1*) GOTO 0x11DB
     0x106A [0x3C] SET_BIT_FLAG_CONDITIONAL(target_work_offset=ExtData[1]->WorkLocal[1], bit_index_work_offset=1*, condition_work_offset=1*)
     0x1071 [0x03] Work_Zone[4] = ExtData[1]->WorkLocal[23]
     0x1076 [0x03] Work_Zone[5] = ExtData[1]->WorkLocal[25]
     0x107B [0x02] IF !(ExtData[1]->WorkLocal[23] == 1*) GOTO 0x108B
     0x1083 [0x03] Work_Zone[6] = 686*
     0x1088 [0x01] GOTO 0x11DB
     0x108B [0x02] IF !(ExtData[1]->WorkLocal[23] == 2*) GOTO 0x109B
     0x1093 [0x03] Work_Zone[6] = 687*
     0x1098 [0x01] GOTO 0x11DB
     0x109B [0x02] IF !(ExtData[1]->WorkLocal[23] == 3*) GOTO 0x10AB
     0x10A3 [0x03] Work_Zone[6] = 688*
     0x10A8 [0x01] GOTO 0x11DB
     0x10AB [0x02] IF !(ExtData[1]->WorkLocal[23] == 4*) GOTO 0x10BB
     0x10B3 [0x03] Work_Zone[6] = 689*
     0x10B8 [0x01] GOTO 0x11DB
     0x10BB [0x02] IF !(ExtData[1]->WorkLocal[23] == 5*) GOTO 0x10CB
     0x10C3 [0x03] Work_Zone[6] = 690*
     0x10C8 [0x01] GOTO 0x11DB
     0x10CB [0x02] IF !(ExtData[1]->WorkLocal[23] == 6*) GOTO 0x10DB
     0x10D3 [0x03] Work_Zone[6] = 691*
     0x10D8 [0x01] GOTO 0x11DB
     0x10DB [0x02] IF !(ExtData[1]->WorkLocal[23] == 7*) GOTO 0x10EB
     0x10E3 [0x03] Work_Zone[6] = 692*
     0x10E8 [0x01] GOTO 0x11DB
     0x10EB [0x02] IF !(ExtData[1]->WorkLocal[23] == 8*) GOTO 0x10FB
     0x10F3 [0x03] Work_Zone[6] = 693*
     0x10F8 [0x01] GOTO 0x11DB
     0x10FB [0x02] IF !(ExtData[1]->WorkLocal[23] == 9*) GOTO 0x110B
     0x1103 [0x03] Work_Zone[6] = 694*
     0x1108 [0x01] GOTO 0x11DB
     0x110B [0x02] IF !(ExtData[1]->WorkLocal[23] == 10*) GOTO 0x111B
     0x1113 [0x03] Work_Zone[6] = 695*
     0x1118 [0x01] GOTO 0x11DB
     0x111B [0x02] IF !(ExtData[1]->WorkLocal[23] == 11*) GOTO 0x112B
     0x1123 [0x03] Work_Zone[6] = 696*
     0x1128 [0x01] GOTO 0x11DB
     0x112B [0x02] IF !(ExtData[1]->WorkLocal[23] == 12*) GOTO 0x113B
     0x1133 [0x03] Work_Zone[6] = 697*
     0x1138 [0x01] GOTO 0x11DB
     0x113B [0x02] IF !(ExtData[1]->WorkLocal[23] == 13*) GOTO 0x114B
     0x1143 [0x03] Work_Zone[6] = 698*
     0x1148 [0x01] GOTO 0x11DB
     0x114B [0x02] IF !(ExtData[1]->WorkLocal[23] == 14*) GOTO 0x115B
     0x1153 [0x03] Work_Zone[6] = 699*
     0x1158 [0x01] GOTO 0x11DB
     0x115B [0x02] IF !(ExtData[1]->WorkLocal[23] == 15*) GOTO 0x116B
     0x1163 [0x03] Work_Zone[6] = 700*
     0x1168 [0x01] GOTO 0x11DB
     0x116B [0x02] IF !(ExtData[1]->WorkLocal[23] == 16*) GOTO 0x117B
     0x1173 [0x03] Work_Zone[6] = 777*
     0x1178 [0x01] GOTO 0x11DB
     0x117B [0x02] IF !(ExtData[1]->WorkLocal[23] == 17*) GOTO 0x118B
     0x1183 [0x03] Work_Zone[6] = 778*
     0x1188 [0x01] GOTO 0x11DB
     0x118B [0x02] IF !(ExtData[1]->WorkLocal[23] == 18*) GOTO 0x119B
     0x1193 [0x03] Work_Zone[6] = 779*
     0x1198 [0x01] GOTO 0x11DB
     0x119B [0x02] IF !(ExtData[1]->WorkLocal[23] == 19*) GOTO 0x11AB
     0x11A3 [0x03] Work_Zone[6] = 915*
     0x11A8 [0x01] GOTO 0x11DB
     0x11AB [0x02] IF !(ExtData[1]->WorkLocal[23] == 20*) GOTO 0x11BB
     0x11B3 [0x03] Work_Zone[6] = 916*
     0x11B8 [0x01] GOTO 0x11DB
     0x11BB [0x02] IF !(ExtData[1]->WorkLocal[23] == 21*) GOTO 0x11CB
     0x11C3 [0x03] Work_Zone[6] = 1847*
     0x11C8 [0x01] GOTO 0x11DB
     0x11CB [0x02] IF !(ExtData[1]->WorkLocal[23] == 22*) GOTO 0x11DB
     0x11D3 [0x03] Work_Zone[6] = 1848*
     0x11D8 [0x01] GOTO 0x11DB
     0x11DB [0x06] Work_Zone[1] = 0
     0x11DE [0x02] IF !(ExtData[1]->WorkLocal[1] == 0*) GOTO 0x12DA
     0x11E6 [0x02] IF !(ExtData[1]->WorkLocal[1] == 3*) GOTO 0x121A
     0x11EE [0x1D] PRINT_EVENT_MESSAGE(message_id=7566*)
    → "Well done! Ballista Point Ranking registration for the previous match has come to a close, and your final overall rank is $0! In addition, your final rank among participating [/warriors/monks/white mages/black mages/red mages/thieves/paladins/dark knights/beastmasters/bards/rangers/samurais/ninjas/dragoons/summoners/blue mage/corsairs/puppetmasters/dancers/scholars/geomancers/rune fencers] ($3 total) is first! You shall be presented with $6 or $6, as well as a new title."
     0x11F1 [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x11F2 [0x1D] PRINT_EVENT_MESSAGE(message_id=7567*)
    → "While our records will clearly indicate that you were rewarded both chevrons, Ballista regulations only allow participants to wear one at a time. Therefore, you must choose which one you wish to receive at this time."
     0x11F5 [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x11F6 [0x24] CREATE_DIALOG(message_id=7568*, default_option=0*, option_flags=0*)
    → "Please select one... [$3./$3.]"
     0x11FD [0x25] WAIT_DIALOG_SELECT()
     0x11FE [0x02] IF !(Work_Zone[0] == 0*) GOTO 0x1209
     0x1206 [0x01] GOTO 0x1217
     0x1209 [0x02] IF !(Work_Zone[0] == 1*) GOTO 0x1217
     0x1211 [0x01] GOTO 0x1235
     0x1214 [0x01] GOTO 0x1217
     0x1217 [0x01] GOTO 0x1244
     0x121A [0x02] IF !(ExtData[1]->WorkLocal[1] == 1*) GOTO 0x1229
     0x1222 [0x1D] PRINT_EVENT_MESSAGE(message_id=7564*)
    → "Congratulations! Ballista Point Ranking registration for the previous match has come to a close, and your final overall rank is $0! You shall be presented with $6 and a new title."
     0x1225 [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x1226 [0x01] GOTO 0x1244
     0x1229 [0x02] IF !(ExtData[1]->WorkLocal[1] == 2*) GOTO 0x1244
     0x1231 [0x1D] PRINT_EVENT_MESSAGE(message_id=7565*)
    → "Good show! Ballista Point Ranking registration for the previous match has come to a close, and your final rank among participating [/warriors/monks/white mages/black mages/red mages/thieves/paladins/dark knights/beastmasters/bards/rangers/samurais/ninjas/dragoons/summoners/blue mage/corsairs/puppetmasters/dancers/scholars/geomancers/rune fencers] ($3 total) is first! You shall be presented with $6 and a new title."
     0x1234 [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x1235 [0x03] Work_Zone[3] = Work_Zone[6]
     0x123A [0x3C] SET_BIT_FLAG_CONDITIONAL(target_work_offset=Work_Zone[1], bit_index_work_offset=27*, condition_work_offset=1*)
     0x1241 [0x01] GOTO 0x1244
     0x1244 [0x02] IF !(ExtData[1]->WorkLocal[33] == 0*) GOTO 0x12B9
     0x124C [0x03] Work_Zone[2] = ExtData[1]->WorkLocal[33]
     0x1251 [0x1D] PRINT_EVENT_MESSAGE(message_id=7569*)
    → "You are currently wearing $6. If you wish to accept the chevron you earned this match, you must return the one you currently possess."
     0x1254 [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x1255 [0x24] CREATE_DIALOG(message_id=7570*, default_option=0*, option_flags=0*)
    → "Accept the $3? [Yes./No.]"
     0x125C [0x25] WAIT_DIALOG_SELECT()
     0x125D [0x02] IF !(Work_Zone[0] == 0*) GOTO 0x1282
     0x1265 [0x1D] PRINT_EVENT_MESSAGE(message_id=7571*)
    → "Are you sure you wish to exchange your $3 for $6?"
     0x1268 [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x1269 [0x24] CREATE_DIALOG(message_id=7573*, default_option=0*, option_flags=0*)
    → "Well? [I'm sure./No.]"
     0x1270 [0x25] WAIT_DIALOG_SELECT()
     0x1271 [0x02] IF !(Work_Zone[0] == 1*) GOTO 0x127F
     0x1279 [0x01] GOTO 0x124C
     0x127C [0x01] GOTO 0x127F
     0x127F [0x01] GOTO 0x12B9
     0x1282 [0x02] IF !(Work_Zone[0] == 1*) GOTO 0x12B9
     0x128A [0x1D] PRINT_EVENT_MESSAGE(message_id=7572*)
    → "Are you sure you do not wish to accept the $3? Remember that even if you do not accept the chevron, a record of your award will be kept."
     0x128D [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x128E [0x24] CREATE_DIALOG(message_id=7573*, default_option=0*, option_flags=0*)
    → "Well? [I'm sure./No.]"
     0x1295 [0x25] WAIT_DIALOG_SELECT()
     0x1296 [0x02] IF !(Work_Zone[0] == 0*) GOTO 0x12A8
     0x129E [0x3C] SET_BIT_FLAG_CONDITIONAL(target_work_offset=Work_Zone[1], bit_index_work_offset=26*, condition_work_offset=1*)
     0x12A5 [0x01] GOTO 0x12B6
     0x12A8 [0x02] IF !(Work_Zone[0] == 1*) GOTO 0x12B6
     0x12B0 [0x01] GOTO 0x124C
     0x12B3 [0x01] GOTO 0x12B6
     0x12B6 [0x01] GOTO 0x12B9
     0x12B9 [0x3E] IF !(Work_Zone[1] bit 26*) GOTO 0x12C3
     0x12C0 [0x01] GOTO 0x12C6
     0x12C3 [0x1A] CALL_SUBROUTINE(address=0x0E2D)
     0x12C6 [0x45] LOAD_SCHEDULED_TASK: Load scheduler "qstc" with entities [LocalPlayer, LocalPlayer], work=[201*, 0*]
     0x12D7 [0x01] GOTO 0x12F2
     0x12DA [0x03] Work_Zone[2] = ExtData[1]->WorkLocal[21]
     0x12DF [0x03] Work_Zone[3] = ExtData[1]->WorkLocal[24]
     0x12E4 [0x03] Work_Zone[4] = ExtData[1]->WorkLocal[23]
     0x12E9 [0x03] Work_Zone[5] = ExtData[1]->WorkLocal[25]
     0x12EE [0x1D] PRINT_EVENT_MESSAGE(message_id=7586*)
    → "Your final Ballista Point Ranking for the previous match is: $0. (Rank among [/warriors/monks/white mages/black mages/red mages/thieves/paladins/dark knights/beastmasters/bards/rangers/samurais/ninjas/dragoons/summoners/blue mages/corsairs/puppetmasters/dancers/scholars/geomancers/rune fencers]: $3) However, a little more practice, and the champion's throne is yours for the taking!"
     0x12F1 [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x12F2 [0x02] IF !(ExtData[1]->WorkLocal[28] == 1*) GOTO 0x1304
     0x12FA [0x3C] SET_BIT_FLAG_CONDITIONAL(target_work_offset=Work_Zone[1], bit_index_work_offset=30*, condition_work_offset=1*)
     0x1301 [0x01] GOTO 0x1328
     0x1304 [0x02] IF !(ExtData[1]->WorkLocal[28] == 2*) GOTO 0x1316
     0x130C [0x3C] SET_BIT_FLAG_CONDITIONAL(target_work_offset=Work_Zone[1], bit_index_work_offset=29*, condition_work_offset=1*)
     0x1313 [0x01] GOTO 0x1328
     0x1316 [0x02] IF !(ExtData[1]->WorkLocal[28] == 3*) GOTO 0x1328
     0x131E [0x3C] SET_BIT_FLAG_CONDITIONAL(target_work_offset=Work_Zone[1], bit_index_work_offset=28*, condition_work_offset=1*)
     0x1325 [0x01] GOTO 0x1328
     0x1328 [0x3C] SET_BIT_FLAG_CONDITIONAL(target_work_offset=Work_Zone[1], bit_index_work_offset=31*, condition_work_offset=1*)
     0x132F [0x01] GOTO 0x1337
     0x1332 [0x03] Work_Zone[1] = 1073741824*
     0x1337 [0x1B] RETURN
```
