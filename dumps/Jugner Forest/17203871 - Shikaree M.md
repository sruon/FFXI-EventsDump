...
| [13](#event-13)            | 0x004E       |      1 |              1 |
| [65535.6](#event-655356)   | 0x004F       |     14 |              4 |
| [65535.7](#event-655357)   | 0x005D       |     14 |              4 |
| [65535.8](#event-655358)   | 0x006B       |     14 |              4 |
| [65535.9](#event-655359)   | 0x0079       |     14 |              4 |
| [65535.10](#event-6553510) | 0x0087       |      5 |              3 |
| [65535.11](#event-6553511) | 0x008C       |      5 |              3 |
| [65535.12](#event-6553512) | 0x0091       |      5 |              3 |
| [65535.13](#event-6553513) | 0x0096       |      5 |              3 |
| [65535.14](#event-6553514) | 0x009B       |     12 |              4 |
| [65535.15](#event-6553515) | 0x00A7       |      5 |              3 |
| [65535.16](#event-6553516) | 0x00AC       |      5 |              3 |
| [65535.17](#event-6553517) | 0x00B1       |     12 |              4 |
| [65535.18](#event-6553518) | 0x00BD       |      5 |              3 |
| [65535.19](#event-6553519) | 0x00C2       |      5 |              3 |
| [65535.20](#event-6553520) | 0x00C7       |     19 |              5 |
| [65535.21](#event-6553521) | 0x00DA       |     21 |              7 |
| [65535.22](#event-6553522) | 0x00EF       |      5 |              3 |
| [65535.23](#event-6553523) | 0x00F4       |      5 |              3 |
| [65535.24](#event-6553524) | 0x00F9       |      5 |              3 |
| [65535.25](#event-6553525) | 0x00FE       |      5 |              3 |
...
- **7969**: Perhaps it means she wishes to exchange your earring for herrrs?
- **7971**: Semih Lafihna, Sibyl Guard of Heavens Tower in Windurst. That is your title, correct?
- **7973**: I am a Mithran Tracker. Have you not heard of us? I have trrraveled from the Mithran homeland that lies across the seas.
- **7974**: I learned that a sinner was taking rrrefuge with a tribe that lived in Windurst.
- **7976**: Twenty years ago, in the time of the Great War, the tribal chieftainness of a certain region perpetrated a grrrave offense.
- **7977**: That chieftainness escaped to this land, but was hunted down by Perih Vashai.
- **7978**: We received word that the daughter of the chieftainness perrrished along with her mother. The Mithran Tracker who traveled here twenty years ago confirmed the report.
- **7979**: However, before the light of life left her, she confessed to her own sin: "The daughter still lives."
- **7980**: And so I have come. I will seek out the daughter of the chieftainness and make her atone for the sins of her mother.
- **7983**: Although all the other orphans of the war were sent to their rrrespective homelands, only you few were taken in by a different people. Was this done to evade the Mithran Trackers, perrrhaps?
- **7984**: And out of those orphans who went to live in Heavens Tower, you were the only one to have no rrrecord of her parents...
- **7985**: A sin is a sin, even if you have no memory of the fact. The child must pay for the sins of the mother when the mother can no longer make amends.
- **7986**: Trying to switch the earring will do you no good. That is not how I will know you as the child of the sinner.
- **7987**: Let me tell you a secret. A sinner carries the scent of sin. It is a scent that only these king's tear moths can detect. A scent that will not fade and that you cannot escape. A scent that will stay with you forrrever.
- **7989**: ...!? You don't carry the scent...?
- **7990**: ...... I must apologize. I seem to have been mistaken.
- **7992**: That is no longer my concern. Farewell, Semih Lafihna.
...
```
  ...
  1: 0x0052 [0x1F] MOVE_ENTITY: EventEntity moves to X=-9.189*, Z=316.644*, Y=0.257*
...
```
  ...
  1: 0x006E [0x1F] MOVE_ENTITY: EventEntity moves to X=-9.189*, Z=316.644*, Y=0.257*
...
```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0070:                             32 03 80 1F 00 0A 80           2......
0080: 0B 80 0C 80 1F 01 00                              .......         
```
...
```
  ...
  1: 0x007C [0x1F] MOVE_ENTITY: EventEntity moves to X=-3.359*, Z=302.813*, Y=0.858*
...
```
  0: 0x0087 [0x1D] PRINT_EVENT_MESSAGE(message_id=7969*)
...
```
  0: 0x008C [0x1D] PRINT_EVENT_MESSAGE(message_id=7971*)
...
```
  0: 0x0091 [0x1D] PRINT_EVENT_MESSAGE(message_id=7973*)
...
```
  0: 0x0096 [0x1D] PRINT_EVENT_MESSAGE(message_id=7974*)
...
```
  ...
  1: 0x00A2 [0x1D] PRINT_EVENT_MESSAGE(message_id=7976*)
...
```
  0: 0x00A7 [0x1D] PRINT_EVENT_MESSAGE(message_id=7977*)
...
```
  0: 0x00AC [0x1D] PRINT_EVENT_MESSAGE(message_id=7978*)
...
```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
00B0:    1D 14 80 23 29 08 9F  82 06 01 02 00            ...#).......   
```

#### Opcodes

```
  0: 0x00B1 [0x1D] PRINT_EVENT_MESSAGE(message_id=7979*)
...
```
  0: 0x00BD [0x1D] PRINT_EVENT_MESSAGE(message_id=7980*)
...
```
  0: 0x00C2 [0x1D] PRINT_EVENT_MESSAGE(message_id=7983*)
...
```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
00C0:                      29  08 9F 82 06 01 01 1D 17         )........
00D0: 80 23 29 08 9F 82 06 01  02 00                    .#).......      
```
...
```
  ...
  1: 0x00CE [0x1D] PRINT_EVENT_MESSAGE(message_id=7984*)
...
```
  0: 0x00DA [0x1D] PRINT_EVENT_MESSAGE(message_id=7985*)
  ...
  4: 0x00EA [0x1D] PRINT_EVENT_MESSAGE(message_id=7986*)
...
```
  0: 0x00EF [0x1D] PRINT_EVENT_MESSAGE(message_id=7987*)
...
```
  0: 0x00F4 [0x1D] PRINT_EVENT_MESSAGE(message_id=7989*)
...
```
  0: 0x00F9 [0x1D] PRINT_EVENT_MESSAGE(message_id=7990*)
...
```
  0: 0x00FE [0x1D] PRINT_EVENT_MESSAGE(message_id=7992*)
