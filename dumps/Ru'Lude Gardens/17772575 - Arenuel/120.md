# Event 120

## Metadata

| Field           | Value    |
|-----------------|----------|
| Event Index     | 3/6      |
| Offset          | 0x000B   |
| Data Size       | 56 bytes |
| Calculated Size | 56 bytes |
| Instructions    | 10       |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0000:                                   1E F0 FF FF 7F             .....
0010: 6F 70 5B 04 80 F8 FF FF  7F F8 FF FF 7F 74 6C 6B  op[..........tlk
0020: 30 1D 05 80 23 5B 04 80  F8 FF FF 7F F8 FF FF 7F  0...#[..........
0030: 74 6C 6B 31 53 F8 FF FF  7F F8 FF FF 7F 74 6C 6B  tlk1S........tlk
0040: 31 21 00                                          1!.             
```

## Opcodes

```
  0: 0x000B [0x1E] EventEntity looks at LocalPlayer and starts talking
  1: 0x0010 [0x6F] WAIT_FRAME_DELAY: Yield until WaitTime reaches zero
  2: 0x0011 [0x70] WAIT_ENTITY_RENDER_FLAG: Wait while EventEntity->Render.Flags3 bit 2 is set (cancel turn if not)
  3: 0x0012 [0x5B] LOAD_EXT_SCHEDULER(work_offset=10*, entity1=EventEntity, entity2=EventEntity, action="tlk0")
  4: 0x0021 [0x1D] PRINT_EVENT_MESSAGE(message_id=10247*)
    → "These works are part of Lord Eald'narche's collection. His Lordship is versed in history as well as art.\u007F1\u0000\u0007"
  5: 0x0024 [0x23] WAIT_FOR_DIALOG_INTERACTION
  6: 0x0025 [0x5B] LOAD_EXT_SCHEDULER(work_offset=10*, entity1=EventEntity, entity2=EventEntity, action="tlk1")
  7: 0x0034 [0x53] WAIT_SCHEDULER_TASK(entity1=EventEntity, entity2=EventEntity, action_id="tlk1")
  8: 0x0041 [0x21] END_EVENT
  9: 0x0042 [0x00] END_REQSTACK()
```
