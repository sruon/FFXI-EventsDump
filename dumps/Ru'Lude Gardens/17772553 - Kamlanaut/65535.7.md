# Event 65535

## Metadata

| Field           | Value    |
|-----------------|----------|
| Event Index     | 11/26    |
| Offset          | 0x00C6   |
| Data Size       | 34 bytes |
| Calculated Size | 34 bytes |
| Instructions    | 4        |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
00C0:                   5B 06  80 F8 FF FF 7F F8 FF FF        [.........
00D0: 7F 73 69 74 30 53 F8 FF  FF 7F F8 FF FF 7F 73 69  .sit0S........si
00E0: 74 30 5E 64 66 74 30 00                           t0^dft0.        
```

## Opcodes

```
  0: 0x00C6 [0x5B] LOAD_EXT_SCHEDULER(work_offset=105*, entity1=EventEntity, entity2=EventEntity, action="sit0")
  1: 0x00D5 [0x53] WAIT_SCHEDULER_TASK(entity1=EventEntity, entity2=EventEntity, action_id="sit0")
  2: 0x00E2 [0x5E] EventEntity goes idle (kills current action) (animation: "dft0")
  3: 0x00E7 [0x00] END_REQSTACK()
```
