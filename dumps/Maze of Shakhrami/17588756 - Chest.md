# 17588756 - Chest

## Common Data

| Field            | Value                       |
|------------------|-----------------------------|
| Zone             | Maze of Shakhrami (ID: 198) |
| Block Size       | 132 bytes                   |
| Total Events     | 3                           |
| References Count | 4                           |

## List of Events

| Event ID              | Entrypoint   |   Size |   Instructions |
|-----------------------|--------------|--------|----------------|
| [65535](#event-65535) | 0x0000       |      1 |              1 |
| [50](#event-50)       | 0x0001       |     41 |             11 |
| [51](#event-51)       | 0x002A       |     45 |             13 |

## DAT References (imed_data)

|   Index | Hex Value   |   Dec Value |
|---------|-------------|-------------|
|       0 | 0x1BAE      |        7086 |
|       1 | 0x0001      |           1 |
|       2 | 0x0000      |           0 |
|       3 | 0x0002      |           2 |

## String References

- **7086**: Take this coin? [Yes./No.]

## Events

### Event 65535

#### Metadata

| Field        | Value   |
|--------------|---------|
| Entrypoint   | 0x0000  |
| Data Size    | 1 bytes |
| Instructions | 1       |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0000: 00                                                .               
```

#### Opcodes

```
  0: 0x0000 [0x00] END_REQSTACK()
```

### Event 50

#### Metadata

| Field        | Value    |
|--------------|----------|
| Entrypoint   | 0x0001   |
| Data Size    | 41 bytes |
| Instructions | 11       |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0000:    24 00 80 01 80 02 80  25 02 00 10 02 80 00 1D   $......%.......
0010: 00 43 00 43 01 03 01 10  01 80 01 28 00 02 00 10  .C.C.......(....
0020: 01 80 00 28 00 01 28 00  21 00                    ...(..(.!.      
```

#### Opcodes

```
  0: 0x0001 [0x24] CREATE_DIALOG(message_id=7086*, default_option=1*, option_flags=0*)
    → "Take this coin? [Yes./No.]"
  1: 0x0008 [0x25] WAIT_DIALOG_SELECT()
  2: 0x0009 [0x02] IF !(Work_Zone[0] == 0*) GOTO 0x001D
  3: 0x0011 [0x43] SEND_EVENT_UPDATE: Send pending tag to server (packet 0x005B)
  4: 0x0013 [0x43] SEND_EVENT_UPDATE: Check pending flag (skip if not pending)
  5: 0x0015 [0x03] Work_Zone[1] = 1*
  6: 0x001A [0x01] GOTO 0x0028
  7: 0x001D [0x02] IF !(Work_Zone[0] == 1*) GOTO 0x0028
  8: 0x0025 [0x01] GOTO 0x0028

SUBROUTINE_0028:
  9: 0x0028 [0x21] END_EVENT
 10: 0x0029 [0x00] END_REQSTACK()
```

### Event 51

#### Metadata

| Field        | Value    |
|--------------|----------|
| Entrypoint   | 0x002A   |
| Data Size    | 45 bytes |
| Instructions | 13       |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0020:                                24 00 80 01 80 02            $.....
0030: 80 25 02 00 10 02 80 00  46 00 43 00 43 01 03 01  .%......F.C.C...
0040: 10 03 80 01 55 00 02 00  10 01 80 00 55 00 43 00  ....U.......U.C.
0050: 43 01 01 55 00 21 00                              C..U.!.         
```

#### Opcodes

```
  0: 0x002A [0x24] CREATE_DIALOG(message_id=7086*, default_option=1*, option_flags=0*)
    → "Take this coin? [Yes./No.]"
  1: 0x0031 [0x25] WAIT_DIALOG_SELECT()
  2: 0x0032 [0x02] IF !(Work_Zone[0] == 0*) GOTO 0x0046
  3: 0x003A [0x43] SEND_EVENT_UPDATE: Send pending tag to server (packet 0x005B)
  4: 0x003C [0x43] SEND_EVENT_UPDATE: Check pending flag (skip if not pending)
  5: 0x003E [0x03] Work_Zone[1] = 2*
  6: 0x0043 [0x01] GOTO 0x0055
  7: 0x0046 [0x02] IF !(Work_Zone[0] == 1*) GOTO 0x0055
  8: 0x004E [0x43] SEND_EVENT_UPDATE: Send pending tag to server (packet 0x005B)
  9: 0x0050 [0x43] SEND_EVENT_UPDATE: Check pending flag (skip if not pending)
 10: 0x0052 [0x01] GOTO 0x0055

SUBROUTINE_0055:
 11: 0x0055 [0x21] END_EVENT
 12: 0x0056 [0x00] END_REQSTACK()
```
