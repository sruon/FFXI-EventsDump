# Event 496

## Metadata

| Field           | Value    |
|-----------------|----------|
| Event Index     | 29/35    |
| Offset          | 0x024E   |
| Data Size       | 33 bytes |
| Calculated Size | 33 bytes |
| Instructions    | 12       |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0240:                                            1E F0                ..
0250: FF FF 7F 6F 70 29 08 34  E0 0E 01 01 1D 15 80 23  ...op).4.......#
0260: 1D 16 80 23 29 08 34 E0  0E 01 02 20 00 21 00     ...#).4.... .!. 
```

## Opcodes

```
  0: 0x024E [0x1E] EventEntity looks at LocalPlayer and starts talking
  1: 0x0253 [0x6F] WAIT_FRAME_DELAY: Yield until WaitTime reaches zero
  2: 0x0254 [0x70] WAIT_ENTITY_RENDER_FLAG: Wait while EventEntity->Render.Flags3 bit 2 is set (cancel turn if not)
  3: 0x0255 [0x29] REQ_SET_WAIT(priority=0x08, entity_id=Pechiru-Mashiru (ID: 17752116/0x010EE034), tag_num=0x01)
  4: 0x025C [0x1D] PRINT_EVENT_MESSAGE(message_id=8500*)
    → "To wake him up right away,\u0007to Paku-Nakku you must say...\u007F1\u0000\u0007"
  5: 0x025F [0x23] WAIT_FOR_DIALOG_INTERACTION
  6: 0x0260 [0x1D] PRINT_EVENT_MESSAGE(message_id=8501*)
    → "\u00072Wake up, fool! Rise now, I bid it!\u0007The automaton butler's the one who did it!\u00073\u0007You got that?\u007F1\u0000\u0007"
  7: 0x0263 [0x23] WAIT_FOR_DIALOG_INTERACTION
  8: 0x0264 [0x29] REQ_SET_WAIT(priority=0x08, entity_id=Pechiru-Mashiru (ID: 17752116/0x010EE034), tag_num=0x02)
  9: 0x026B [0x20] SET_CLI_EVENT_UC_FLAG: Unlock player control
 10: 0x026D [0x21] END_EVENT
 11: 0x026E [0x00] END_REQSTACK()
```
