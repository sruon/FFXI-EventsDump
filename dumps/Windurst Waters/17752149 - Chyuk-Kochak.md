...
- **9087**: Our editor is all fired up...more so than usual.
- **9088**: All our staff are tired and worn out from our last special report, "What Happened to the Star Reading Ceremony?". He's just wasting his breath and annoying us all.
- **9096**: There should be a reporter in each of the four districts of Windurst.
- **9116**: Heh. Looks like we scooped quite a story here.
- **9117**: We've all got to get cracking now! We've got to get this baby into production!
- **9120**: No matter how great and sensational the story is, it's totally useless if it turns out to be a bunch of lies.
- **9121**: A good reporter needs a good nose for uncovering the story, then good legs to go check [his/her] sources. Learn that and you'll go far.
- **9128**: The Cat Burglar must have smelt a rat... Nanaa Mihgo didn't want us to expose her hideaway, so she stole the story before it could go into circulation. Agh...someone really let the cat outta the bag!
- **9129**: Aa\`ah! This week's edition of the "Magic Paradise Weekly" is going to be canceled as well. It happens so often, though, that nobody's going to notice anyway...<Sigh>
- **9145**: The Cat Burglar must have caught wind of us... Nanaa Mihgo didn't want us to expose her hideaway, so she stole the story before it could go into circulation.
- **9146**: But you did real well to get the magic doll back off of her! It's practically impossible to retrieve anything stolen by the Cat Burglar!
...
```
  ...
  4: 0x007A [0x1D] PRINT_EVENT_MESSAGE(message_id=9087*)
    → "Our editor is all fired up...more so than usual."
  5: 0x007D [0x23] WAIT_FOR_DIALOG_INTERACTION
  6: 0x007E [0x1D] PRINT_EVENT_MESSAGE(message_id=9088*)
...
```
  ...
  4: 0x009C [0x1D] PRINT_EVENT_MESSAGE(message_id=9096*)
...
```
  ...
  4: 0x00B9 [0x1D] PRINT_EVENT_MESSAGE(message_id=9116*)
    → "Heh. Looks like we scooped quite a story here."
  5: 0x00BC [0x23] WAIT_FOR_DIALOG_INTERACTION
  6: 0x00BD [0x1D] PRINT_EVENT_MESSAGE(message_id=9117*)
...
```
  ...
  4: 0x00DA [0x1D] PRINT_EVENT_MESSAGE(message_id=9120*)
  ...
  6: 0x00DE [0x1D] PRINT_EVENT_MESSAGE(message_id=9121*)
...
```
  ...
  4: 0x00FB [0x1D] PRINT_EVENT_MESSAGE(message_id=9128*)
  ...
  8: 0x010D [0x1D] PRINT_EVENT_MESSAGE(message_id=9129*)
...
```
  ...
  4: 0x0131 [0x1D] PRINT_EVENT_MESSAGE(message_id=9145*)
  ...
  6: 0x0135 [0x1D] PRINT_EVENT_MESSAGE(message_id=9146*)
