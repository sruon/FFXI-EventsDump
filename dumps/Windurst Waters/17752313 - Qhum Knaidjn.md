...
- **7197**: Hello, [sir/ma'am]. I have been appointed by the Guildworkers' Union to manage the trading of manufactured crafts and the exchange of guild points.
- **7198**: However, to assure product quality, I cannot carry out any business with those who rank lower than novice.
- **7199**: I see that you currently have a trading contract with the [Fishermen's/Carpenters'/Blacksmiths'/Goldsmiths'/Weavers'/Tanners'/Boneworkers'/Alchemists'/Culinarians'] Guild. I'm sorry, but I cannot do any business with you.
- **7200**: You do have the option to terminate your contract with the [Fishermen's/Carpenters'/Blacksmiths'/Goldsmiths'/Weavers'/Tanners'/Boneworkers'/Alchemists'/Culinarians'] Guild and form one with us.
- **7201**: After signing a new contract, you will have to wait until the next delivery phase to trade Guildworkers' Union requested items.
- **7202**: Create a new contract with us? [Immediately./Not today.]
- **7203**: Are you certain? [I said, "Immediately."/On second thought...]
- **7205**: Our system works like this. We will ask you to provide us with a certain item depending on your craft skill rank. There is a set time limit (one day, Earth time) in which you may bring as many of the item as you can synthesize.
- **7206**: If you happen to create a high quality version of the requested item, we will be more than delighted to accept that as well.
- **7207**: Trading us these items will earn you "guild points" which can be later exchanged for other specialty goods--some of which are found exclusively at this guild.
- **7208**: The amount of guild points you will receive varies with the item requested, and there is a limit to how many points you can accumulate during the designated time frame. Trading items after you have reached the limit will result in no points gained.
- **7209**: When the time limit has expired, the item requested by the guild will change. It will also change in the event your craft skill rises.
- **7210**: I see that you have not formed a trading contract with any of the other guilds.
- **7213**: The current requested item is $2. You have $0 guild points accumulated.
- **7214**: Currently, no items are being requested by the guild. You have $0 guild points accumulated.
- **7215**: Is there something you need? [I'm just passing through./I would like to use my points./What are the current limits?/How does the trade system work?]
- **7216**: You have $0 more days (Vana'diel time) to provide us with the requested item.
- **7217**: Today (Vana'diel time) is the final day we will accept the requested item.
- **7218**: You can still receive up to $3 guild points by trading us the item before the time limit is up.
- **7219**: You are not eligible to receive guild points at this time.
- **7223**: Yes, this is what we are asking for. Good work, <Player>!
- **7226**: Trade $1 guild points for $0?
- **7227**: How many do you wish to trade for?
- **7228**: Trade $1 guild points for $2 $0 ?
- **7229**: Proceed with the transaction? (GP: $ ) [Yes./No.]
- **7231**: Exchange points for... [None of these./3($1)./$3($3)./$3($5)./$3($7)./$3($9)./$3($11)./$3($13).]
- **7232**: Trade $1 guild points for $3?
- **7282**: 3 is a special technique used by famous chefs in the Eastern Empire. I wouldn't lay a finger on a piece of sushi that hadn't been made by a culinary artist with this ability.
- **7283**: 3 is a special technique used by chefs to turn ordinary flour and water into all different types of pasta used in all sorts of delectable dishes!
- **7284**: 3 is a special technique used by chefs to create all sorts of delicious sweets! I'll be happy to work as your tast-tester...
- **7285**: 3 is a special technique used by chefs in the Eastern Empire to create amazing stewpots that everyone can enjoy!
...
```
  ...
  2: 0x000C [0x1D] PRINT_EVENT_MESSAGE(message_id=7223*)
...
```
  ...
  9: 0x0042 [0x1D] PRINT_EVENT_MESSAGE(message_id=7197*)
 ...
 16: 0x0065 [0x1D] PRINT_EVENT_MESSAGE(message_id=7198*)
...
SUBROUTINE_00FC:
 24: 0x00FC [0x24] CREATE_DIALOG(message_id=7203*, default_option=1*, option_flags=0*)
...
SUBROUTINE_011D:
 ...
 36: 0x012F [0x1D] PRINT_EVENT_MESSAGE(message_id=7214*)
 ...
 39: 0x0136 [0x1D] PRINT_EVENT_MESSAGE(message_id=7213*)
...
SUBROUTINE_014E:
 45: 0x014E [0x24] CREATE_DIALOG(message_id=7215*, default_option=ExtData[1]->WorkLocal[17], option_flags=0*)
...
185: 0x0D9B [0x1D] PRINT_EVENT_MESSAGE(message_id=7219*)
...
191: 0x0DB4 [0x1D] PRINT_EVENT_MESSAGE(message_id=7218*)
...
194: 0x0DBB [0x1D] PRINT_EVENT_MESSAGE(message_id=7219*)
    → "You are not eligible to receive guild points at this time."
...
# Dead code (unreachable instructions):
     ...
     0x0C2A [0x24] CREATE_DIALOG(message_id=7231*, default_option=ExtData[1]->WorkLocal[19], option_flags=ExtData[1]->WorkLocal[5])
    → "Exchange points for... [None of these./3($1)./$3($3)./$3($5)./$3($7)./$3($9)./$3($11)./$3($13).]"
     ...
     0x0D0B [0x1D] PRINT_EVENT_MESSAGE(message_id=7232*)
    → "Trade $1 guild points for $3?"
     0x0D0E [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x0D0F [0x24] CREATE_DIALOG(message_id=7229*, default_option=1*, option_flags=0*)
...
# Dead code (unreachable instructions):
     ...
     0x0E03 [0x1D] PRINT_EVENT_MESSAGE(message_id=7227*)
    → "How many do you wish to trade for?"
     ...
     0x0E38 [0x1D] PRINT_EVENT_MESSAGE(message_id=7226*)
    → "Trade $1 guild points for $0?"
     0x0E3B [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x0E3C [0x01] GOTO 0x0E43
     0x0E3F [0x1D] PRINT_EVENT_MESSAGE(message_id=7228*)
    → "Trade $1 guild points for $2 $0 ?"
     ...
     0x0E4B [0x24] CREATE_DIALOG(message_id=7229*, default_option=1*, option_flags=0*)
    → "Proceed with the transaction? (GP: $ ) [Yes./No.]"
     ...
     0x0E95 [0x1D] PRINT_EVENT_MESSAGE(message_id=7282*)
    → "3 is a special technique used by famous chefs in the Eastern Empire. I wouldn't lay a finger on a piece of sushi that hadn't been made by a culinary artist with this ability."
     ...
     0x0EA4 [0x1D] PRINT_EVENT_MESSAGE(message_id=7283*)
    → "3 is a special technique used by chefs to turn ordinary flour and water into all different types of pasta used in all sorts of delectable dishes!"
     ...
     0x0EB3 [0x1D] PRINT_EVENT_MESSAGE(message_id=7284*)
    → "3 is a special technique used by chefs to create all sorts of delicious sweets! I'll be happy to work as your tast-tester..."
     ...
     0x0EC2 [0x1D] PRINT_EVENT_MESSAGE(message_id=7285*)
