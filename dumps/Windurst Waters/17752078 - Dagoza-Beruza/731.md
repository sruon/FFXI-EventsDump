# Event 731

## Metadata

| Field           | Value    |
|-----------------|----------|
| Event Index     | 43/49    |
| Offset          | 0x03EE   |
| Data Size       | 33 bytes |
| Calculated Size | 33 bytes |
| Instructions    | 12       |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
03E0:                                            1E F0                ..
03F0: FF FF 7F 6F 70 29 08 0E  E0 0E 01 01 1D 2D 80 23  ...op).......-.#
0400: 1D 2E 80 23 29 08 0E E0  0E 01 02 20 00 21 00     ...#)...... .!. 
```

## Opcodes

```
  0: 0x03EE [0x1E] EventEntity looks at LocalPlayer and starts talking
  1: 0x03F3 [0x6F] WAIT_FRAME_DELAY: Yield until WaitTime reaches zero
  2: 0x03F4 [0x70] WAIT_ENTITY_RENDER_FLAG: Wait while EventEntity->Render.Flags3 bit 2 is set (cancel turn if not)
  3: 0x03F5 [0x29] REQ_SET_WAIT(priority=0x08, entity_id=Dagoza-Beruza (ID: 17752078/0x010EE00E), tag_num=0x01)
  4: 0x03FC [0x1D] PRINT_EVENT_MESSAGE(message_id=9713*)
    → "Most people think that the Rhinostery justaru researches plants, but actually, their research encompasses all living things.\u007F1\u0000\u0007"
  5: 0x03FF [0x23] WAIT_FOR_DIALOG_INTERACTION
  6: 0x0400 [0x1D] PRINT_EVENT_MESSAGE(message_id=9714*)
    → "Biology researchers wander all over Vana'diel.\u0007The mostaru capable adventurers of the Rhinostery always accompany these researchers on their treks into dangerous territory.\u007F1\u0000\u0007"
  7: 0x0403 [0x23] WAIT_FOR_DIALOG_INTERACTION
  8: 0x0404 [0x29] REQ_SET_WAIT(priority=0x08, entity_id=Dagoza-Beruza (ID: 17752078/0x010EE00E), tag_num=0x02)
  9: 0x040B [0x20] SET_CLI_EVENT_UC_FLAG: Unlock player control
 10: 0x040D [0x21] END_EVENT
 11: 0x040E [0x00] END_REQSTACK()
```
