# Event 65535

## Metadata

| Field           | Value    |
|-----------------|----------|
| Event Index     | 3/72     |
| Offset          | 0x0025   |
| Data Size       | 41 bytes |
| Calculated Size | 41 bytes |
| Instructions    | 6        |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0020:                5F 00 4B  2F E0 0E 01 01 80 1C 02       _.K/.......
0030: 80 5B 00 80 F8 FF FF 7F  F8 FF FF 7F 6D 65 69 30  .[..........mei0
0040: 53 F8 FF FF 7F F8 FF FF  7F 6D 65 69 30 00        S........mei0.  
```

## Opcodes

```
  0: 0x0025 [0x5F] MULTI_HANDLER_COMPLEX(mode=0x00 - Toggle Render.Flags0 bit 29)
  1: 0x0027 [0x4B] UPDATE_ENTITY_YAW(entity=Kerutoto (ID: 17752111/0x010EE02F), yaw=7.26°*)
  2: 0x002E [0x1C] WAIT(30* ticks)
  3: 0x0031 [0x5B] LOAD_EXT_SCHEDULER(work_offset=345*, entity1=EventEntity, entity2=EventEntity, action="mei0")
  4: 0x0040 [0x53] WAIT_SCHEDULER_TASK(entity1=EventEntity, entity2=EventEntity, action_id="mei0")
  5: 0x004D [0x00] END_REQSTACK()
```
