# Event 65535

## Metadata

| Field           | Value    |
|-----------------|----------|
| Event Index     | 7/13     |
| Offset          | 0x0085   |
| Data Size       | 36 bytes |
| Calculated Size | 36 bytes |
| Instructions    | 4        |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0080:                3A 19 E1  0E 01 00 00 BA 18 E1 0E       :..........
0090: 01 08 80 09 80 0A 80 00  00 5B 06 80 18 E1 0E 01  .........[......
00A0: 18 E1 0E 01 68 6F 6E 30  00                       ....hon0.       
```

## Opcodes

```
  0: 0x0085 [0x3A] CONVERT_YAW_TO_BYTE(entity=Khoto Rokkorah (ID: 17752345/0x010EE119), result_destination=ExtData[1]->WorkLocal[0])
  1: 0x008C [0xBA] SET_ENTITY_POSITION(entity_id=References[17719576], pos_x=10.954*, pos_z=20.168*, pos_y=-1.000*, direction=ExtData[1]->WorkLocal[0])
  2: 0x0099 [0x5B] LOAD_EXT_SCHEDULER(work_offset=1323*, entity1=Unnamed NPC (ID: 17752344/0x010EE118), entity2=Unnamed NPC (ID: 17752344/0x010EE118), action="hon0")
  3: 0x00A8 [0x00] END_REQSTACK()
```
