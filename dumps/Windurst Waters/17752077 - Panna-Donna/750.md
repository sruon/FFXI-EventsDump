# Event 750

## Metadata

| Field           | Value    |
|-----------------|----------|
| Event Index     | 50/55    |
| Offset          | 0x0500   |
| Data Size       | 33 bytes |
| Calculated Size | 33 bytes |
| Instructions    | 12       |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0500: 1E F0 FF FF 7F 6F 70 29  08 0D E0 0E 01 01 1D 43  .....op).......C
0510: 80 23 1D 44 80 23 29 08  0D E0 0E 01 02 20 00 21  .#.D.#)...... .!
0520: 00                                                .               
```

## Opcodes

```
  0: 0x0500 [0x1E] EventEntity looks at LocalPlayer and starts talking
  1: 0x0505 [0x6F] WAIT_FRAME_DELAY: Yield until WaitTime reaches zero
  2: 0x0506 [0x70] WAIT_ENTITY_RENDER_FLAG: Wait while EventEntity->Render.Flags3 bit 2 is set (cancel turn if not)
  3: 0x0507 [0x29] REQ_SET_WAIT(priority=0x08, entity_id=Panna-Donna (ID: 17752077/0x010EE00D), tag_num=0x01)
  4: 0x050E [0x1D] PRINT_EVENT_MESSAGE(message_id=9765*)
    → "Have you met Professor Koru-Moru?\u0007He's the very, um, unique professor in Windurst Walls.\u007F1\u0000\u0007"
  5: 0x0511 [0x23] WAIT_FOR_DIALOG_INTERACTION
  6: 0x0512 [0x1D] PRINT_EVENT_MESSAGE(message_id=9766*)
    → "He once retired as the principal of the School of Magic, but due to an incidentaru a couple of years ago, he was reinstated.\u007F1\u0000\u0007"
  7: 0x0515 [0x23] WAIT_FOR_DIALOG_INTERACTION
  8: 0x0516 [0x29] REQ_SET_WAIT(priority=0x08, entity_id=Panna-Donna (ID: 17752077/0x010EE00D), tag_num=0x02)
  9: 0x051D [0x20] SET_CLI_EVENT_UC_FLAG: Unlock player control
 10: 0x051F [0x21] END_EVENT
 11: 0x0520 [0x00] END_REQSTACK()
```
