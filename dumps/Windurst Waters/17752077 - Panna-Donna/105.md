# Event 105

## Metadata

| Field           | Value    |
|-----------------|----------|
| Event Index     | 16/55    |
| Offset          | 0x00E6   |
| Data Size       | 29 bytes |
| Calculated Size | 29 bytes |
| Instructions    | 10       |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
00E0:                   1E F0  FF FF 7F 6F 70 29 08 0D        .....op)..
00F0: E0 0E 01 01 1D 04 80 23  29 08 0D E0 0E 01 02 20  .......#)...... 
0100: 00 21 00                                          .!.             
```

## Opcodes

```
  0: 0x00E6 [0x1E] EventEntity looks at LocalPlayer and starts talking
  1: 0x00EB [0x6F] WAIT_FRAME_DELAY: Yield until WaitTime reaches zero
  2: 0x00EC [0x70] WAIT_ENTITY_RENDER_FLAG: Wait while EventEntity->Render.Flags3 bit 2 is set (cancel turn if not)
  3: 0x00ED [0x29] REQ_SET_WAIT(priority=0x08, entity_id=Panna-Donna (ID: 17752077/0x010EE00D), tag_num=0x01)
  4: 0x00F4 [0x1D] PRINT_EVENT_MESSAGE(message_id=7510*)
    → "Huh? You have some kind of business with us, the North Winds? Mokyokyo is our leader-weader, so speak to her first.\u007F1\u0000\u0007"
  5: 0x00F7 [0x23] WAIT_FOR_DIALOG_INTERACTION
  6: 0x00F8 [0x29] REQ_SET_WAIT(priority=0x08, entity_id=Panna-Donna (ID: 17752077/0x010EE00D), tag_num=0x02)
  7: 0x00FF [0x20] SET_CLI_EVENT_UC_FLAG: Unlock player control
  8: 0x0101 [0x21] END_EVENT
  9: 0x0102 [0x00] END_REQSTACK()
```
