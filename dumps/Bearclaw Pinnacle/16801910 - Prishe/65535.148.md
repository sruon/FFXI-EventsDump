# Event 65535

## Metadata

| Field           | Value    |
|-----------------|----------|
| Event Index     | 150/160  |
| Offset          | 0x08F4   |
| Data Size       | 15 bytes |
| Calculated Size | 15 bytes |
| Instructions    | 5        |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
08F0:             32 24 80 1F  00 28 80 29 80 2A 80 1F      2$...(.).*..
0900: 01 6F 00                                          .o.             
```

## Opcodes

```
  0: 0x08F4 [0x32] ExtData[1]->MainSpeed = 18* * 0.1
  1: 0x08F7 [0x1F] MOVE_ENTITY: EventEntity moves to X=-461.598*, Z=-242.092*, Y=0.749*
  2: 0x08FF [0x1F] MOVE_ENTITY: Update entity position (mode=1)
  3: 0x0901 [0x6F] WAIT_FRAME_DELAY: Yield until WaitTime reaches zero
  4: 0x0902 [0x00] END_REQSTACK()
```
