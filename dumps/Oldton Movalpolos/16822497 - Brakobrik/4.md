# Event 4

## Metadata

| Field           | Value     |
|-----------------|-----------|
| Event Index     | 4/16      |
| Offset          | 0x0057    |
| Data Size       | 222 bytes |
| Calculated Size | 222 bytes |
| Instructions    | 66        |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0050:                      42  1E F0 FF FF 7F 6F 70 1D         B.....op.
0060: 09 80 23 1D 0A 80 23 1D  0B 80 23 24 0C 80 03 80  ..#...#...#$....
0070: 03 80 25 02 00 10 03 80  00 9A 00 02 06 10 04 80  ..%.............
0080: 00 93 00 03 01 10 0D 80  1D 0E 80 23 20 00 21 00  ...........# .!.
0090: 01 97 00 1D 0F 80 23 01  F1 00 02 00 10 04 80 00  ......#.........
00A0: F1 00 02 05 10 04 80 00  D2 00 02 06 10 04 80 00  ................
00B0: B5 00 01 BE 00 03 01 10  04 10 43 00 43 01 03 01  ..........C.C...
00C0: 10 10 80 1D 11 80 23 1D  12 80 23 20 00 21 00 01  ......#...# .!..
00D0: EE 00 02 06 10 04 80 00  EA 00 03 01 10 0D 80 1D  ................
00E0: 0E 80 23 20 00 21 00 01  EE 00 1D 13 80 23 01 F1  ..# .!.......#..
00F0: 00 02 04 10 04 80 80 01  01 03 01 10 04 80 01 31  ...............1
0100: 01 02 04 10 05 80 80 11  01 03 01 10 05 80 01 31  ...............1
0110: 01 02 04 10 14 80 80 21  01 03 01 10 14 80 01 31  .......!.......1
0120: 01 02 04 10 15 80 80 31  01 03 01 10 15 80 01 31  .......1.......1
0130: 01 20 00 21 00                                    . .!.           
```

## Opcodes

```
  0: 0x0057 [0x42] SET_CLI_EVENT_CANCEL_DATA()
  1: 0x0058 [0x1E] EventEntity looks at LocalPlayer and starts talking
  2: 0x005D [0x6F] WAIT_FRAME_DELAY: Yield until WaitTime reaches zero
  3: 0x005E [0x70] WAIT_ENTITY_RENDER_FLAG: Wait while EventEntity->Render.Flags3 bit 2 is set (cancel turn if not)
  4: 0x005F [0x1D] PRINT_EVENT_MESSAGE(message_id=7731*)
    → "You bringing it!\u0007This being perfect! You not being so stupid after all.\u007F1\u0000\u0007"
  5: 0x0062 [0x23] WAIT_FOR_DIALOG_INTERACTION
  6: 0x0063 [0x1D] PRINT_EVENT_MESSAGE(message_id=7732*)
    → "Now what?\u0007You wanting something from me?\u0007I guess I should giving you a reward.\u007F1\u0000\u0007"
  7: 0x0066 [0x23] WAIT_FOR_DIALOG_INTERACTION
  8: 0x0067 [0x1D] PRINT_EVENT_MESSAGE(message_id=7733*)
    → "You can having this.\u0007Looking like garbage, but it being important to us Moblins.\u007F1\u0000\u0007"
  9: 0x006A [0x23] WAIT_FOR_DIALOG_INTERACTION
 10: 0x006B [0x24] CREATE_DIALOG(message_id=7734*, default_option=0*, option_flags=0*)
    → "Take it?\u0007\u000BI'll take it.\u0007No thanks.\u007F1\u0000\u0007"
 11: 0x0072 [0x25] WAIT_DIALOG_SELECT()
 12: 0x0073 [0x02] IF !(Work_Zone[0] == 0*) GOTO 0x009A
 13: 0x007B [0x02] IF !(Work_Zone[6] == 1*) GOTO 0x0093
 14: 0x0083 [0x03] Work_Zone[1] = 6*
 15: 0x0088 [0x1D] PRINT_EVENT_MESSAGE(message_id=7737*)
    → "Stupid people.\u0007You already having one.\u0007You don't needing two of the same thing.\u007F1\u0000\u0007"
 16: 0x008B [0x23] WAIT_FOR_DIALOG_INTERACTION
 17: 0x008C [0x20] SET_CLI_EVENT_UC_FLAG: Unlock player control
 18: 0x008E [0x21] END_EVENT
 19: 0x008F [0x00] END_REQSTACK()

SUBROUTINE_0093:
 20: 0x0093 [0x1D] PRINT_EVENT_MESSAGE(message_id=7735*)
    → "Good, good.\u0007You being smart. Very, very clever.\u007F1\u0000\u0007"
 21: 0x0096 [0x23] WAIT_FOR_DIALOG_INTERACTION
 22: 0x0097 [0x01] GOTO 0x00F1

SUBROUTINE_009A:
 23: 0x009A [0x02] IF !(Work_Zone[0] == 1*) GOTO 0x00F1
 24: 0x00A2 [0x02] IF !(Work_Zone[5] == 1*) GOTO 0x00D2
 25: 0x00AA [0x02] IF !(Work_Zone[6] == 1*) GOTO 0x00B5
 26: 0x00B2 [0x01] GOTO 0x00BE

SUBROUTINE_00B5:
 27: 0x00B5 [0x03] Work_Zone[1] = Work_Zone[4]
 28: 0x00BA [0x43] SEND_EVENT_UPDATE: Send pending tag to server (packet 0x005B)
 29: 0x00BC [0x43] SEND_EVENT_UPDATE: Check pending flag (skip if not pending)
 30: 0x00BE [0x03] Work_Zone[1] = 5*
 31: 0x00C3 [0x1D] PRINT_EVENT_MESSAGE(message_id=7739*)
    → "...\u0007You figuring it out?\u007F1\u0000\u0007"
 32: 0x00C6 [0x23] WAIT_FOR_DIALOG_INTERACTION
 33: 0x00C7 [0x1D] PRINT_EVENT_MESSAGE(message_id=7740*)
    → "Okay, okay. Don't being mad.\u0007You can having my treasure.\u0007It being very shiny. Very pretty.\u007F1\u0000\u0007"
 34: 0x00CA [0x23] WAIT_FOR_DIALOG_INTERACTION
 35: 0x00CB [0x20] SET_CLI_EVENT_UC_FLAG: Unlock player control
 36: 0x00CD [0x21] END_EVENT
 37: 0x00CE [0x00] END_REQSTACK()

SUBROUTINE_00D2:
 38: 0x00D2 [0x02] IF !(Work_Zone[6] == 1*) GOTO 0x00EA
 39: 0x00DA [0x03] Work_Zone[1] = 6*
 40: 0x00DF [0x1D] PRINT_EVENT_MESSAGE(message_id=7737*)
    → "Stupid people.\u0007You already having one.\u0007You don't needing two of the same thing.\u007F1\u0000\u0007"
 41: 0x00E2 [0x23] WAIT_FOR_DIALOG_INTERACTION
 42: 0x00E3 [0x20] SET_CLI_EVENT_UC_FLAG: Unlock player control
 43: 0x00E5 [0x21] END_EVENT
 44: 0x00E6 [0x00] END_REQSTACK()

SUBROUTINE_00EA:
 45: 0x00EA [0x1D] PRINT_EVENT_MESSAGE(message_id=7736*)
    → "Taking it, taking it.\u0007You being very stupid. You saying stupid things.\u007F1\u0000\u0007"
 46: 0x00ED [0x23] WAIT_FOR_DIALOG_INTERACTION
 47: 0x00EE [0x01] GOTO 0x00F1

SUBROUTINE_00F1:
 48: 0x00F1 [0x02] IF !(Work_Zone[4] == 1*) GOTO 0x0101
 49: 0x00F9 [0x03] Work_Zone[1] = 1*
 50: 0x00FE [0x01] GOTO 0x0131

SUBROUTINE_0101:
 51: 0x0101 [0x02] IF !(Work_Zone[4] == 2*) GOTO 0x0111
 52: 0x0109 [0x03] Work_Zone[1] = 2*
 53: 0x010E [0x01] GOTO 0x0131

SUBROUTINE_0111:
 54: 0x0111 [0x02] IF !(Work_Zone[4] == 3*) GOTO 0x0121
 55: 0x0119 [0x03] Work_Zone[1] = 3*
 56: 0x011E [0x01] GOTO 0x0131

SUBROUTINE_0121:
 57: 0x0121 [0x02] IF !(Work_Zone[4] == 4*) GOTO 0x0131
 58: 0x0129 [0x03] Work_Zone[1] = 4*
 59: 0x012E [0x01] GOTO 0x0131

SUBROUTINE_0131:
 60: 0x0131 [0x20] SET_CLI_EVENT_UC_FLAG: Unlock player control
 61: 0x0133 [0x21] END_EVENT
 62: 0x0134 [0x00] END_REQSTACK()
```

## Data or dead code:

```
# Dead code (unreachable instructions):
     0x0090 [0x01] GOTO 0x0097
# Dead code (unreachable instructions):
     0x00CF [0x01] GOTO 0x00EE
# Dead code (unreachable instructions):
     0x00E7 [0x01] GOTO 0x00EE
```