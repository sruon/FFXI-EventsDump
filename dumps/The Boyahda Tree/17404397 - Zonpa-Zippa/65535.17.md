# Event 65535

## Metadata

| Field           | Value    |
|-----------------|----------|
| Event Index     | 18/27    |
| Offset          | 0x0101   |
| Data Size       | 41 bytes |
| Calculated Size | 41 bytes |
| Instructions    | 5        |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0100:    7C 00 F8 FF FF 7F 5B  03 80 F8 FF FF 7F F8 FF   |.....[........
0110: FF 7F 6D 65 69 31 53 F8  FF FF 7F F8 FF FF 7F 6D  ..mei1S........m
0120: 65 69 31 7C 01 F8 FF FF  7F 00                    ei1|......      
```

## Opcodes

```
  0: 0x0101 [0x7C] EventEntity->Render.Flags2 |= 0x00
  1: 0x0107 [0x5B] LOAD_EXT_SCHEDULER(work_offset=345*, entity1=EventEntity, entity2=EventEntity, action="mei1")
  2: 0x0116 [0x53] WAIT_SCHEDULER_TASK(entity1=EventEntity, entity2=EventEntity, action_id="mei1")
  3: 0x0123 [0x7C] EventEntity->Render.Flags2 |= 0x01
  4: 0x0129 [0x00] END_REQSTACK()
```
