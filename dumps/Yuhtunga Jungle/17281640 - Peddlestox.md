...
- **7751**: What'sa matta with ya, [son/missy]? Ya thinkin' about walkin' arounda these here parts witha no map? Ya gotta be crazy!
- **7752**: When I was a young bambina likesa you, I's seen my share of loot. But over the years, I learneda one thing--the sweetest treasure lies outta sight. Thatsa what I'm collectin'.
- **7753**: Them crazy Sahagins livin' 'round here trades thesea strange dolls with the pirates for weapons. Now if I could gets my hands on one of them tasty pieces...
- **7756**: Unfortunately, to keeps their secret, thosea guys always breaks the goods up into four pieces.
- **7757**: If you can finds all four, you brings thema back here to yours truly. I'll make it worth your while.
- **7758**: What'sa that you gots there? Hmmm... It looks likea one of those fishy Sahagins' goods.
- **7761**: I needs to check this out to make sure you're not tryin' to pulla fast one.
- **7762**: You did some good work, kid. Now it'sa time for me to keep up my end of the bargain.
- **7763**: Here, lets me marka your map...
- **7764**: I swears on my grandmama's grave that theresa treasure buried somewhere around there.
- **7765**: What!? You goes where I tells ya, and you'll havea no problem findin' the stash. And don't ya forget your $0!
- **7766**: Whaddo I look like here, a charity?
...
```
  ...
  1: 0x0007 [0x1D] PRINT_EVENT_MESSAGE(message_id=7752*)
  ...
  3: 0x000B [0x1D] PRINT_EVENT_MESSAGE(message_id=7753*)
    → "Them crazy Sahagins livin' 'round here trades thesea strange dolls with the pirates for weapons. Now if I could gets my hands on one of them tasty pieces..."
  4: 0x000E [0x23] WAIT_FOR_DIALOG_INTERACTION
  5: 0x000F [0x1D] PRINT_EVENT_MESSAGE(message_id=7756*)
  ...
  7: 0x0013 [0x1D] PRINT_EVENT_MESSAGE(message_id=7757*)
...
```
  ...
  3: 0x0021 [0x1D] PRINT_EVENT_MESSAGE(message_id=7758*)
    → "What'sa that you gots there? Hmmm... It looks likea one of those fishy Sahagins' goods."
  4: 0x0024 [0x23] WAIT_FOR_DIALOG_INTERACTION
  5: 0x0025 [0x1D] PRINT_EVENT_MESSAGE(message_id=7761*)
 ...
 10: 0x004E [0x1D] PRINT_EVENT_MESSAGE(message_id=7762*)
 ...
 12: 0x0052 [0x1D] PRINT_EVENT_MESSAGE(message_id=7763*)
    → "Here, lets me marka your map..."
 13: 0x0055 [0x23] WAIT_FOR_DIALOG_INTERACTION
 14: 0x0056 [0x89] OPEN_MAP(map_id=0x0000800B)
 15: 0x0059 [0x8B] SET_EVENT_MARK: Add/update map marker on map 123* at (Work_Zone[4], Work_Zone[5]), index=0*, name=(no name)
...
# Dead code (unreachable instructions):
     ...
     0x0075 [0x1D] PRINT_EVENT_MESSAGE(message_id=7764*)
    → "I swears on my grandmama's grave that theresa treasure buried somewhere around there."
     ...
     0x007E [0x1D] PRINT_EVENT_MESSAGE(message_id=7765*)
...
```
  ...
  1: 0x008A [0x1D] PRINT_EVENT_MESSAGE(message_id=7751*)
...
```
  ...
  4: 0x009B [0x8B] SET_EVENT_MARK: Add/update map marker on map 123* at (Work_Zone[4], Work_Zone[5]), index=0*, name=(no name)
...
# Dead code (unreachable instructions):
     ...
     0x00B7 [0x1D] PRINT_EVENT_MESSAGE(message_id=7764*)
    → "I swears on my grandmama's grave that theresa treasure buried somewhere around there."
     0x00BA [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x00BB [0x03] Work_Zone[2] = 605*
     0x00C0 [0x1D] PRINT_EVENT_MESSAGE(message_id=7765*)
    → "What!? You goes where I tells ya, and you'll havea no problem findin' the stash. And don't ya forget your $0!"
...
```
  ...
  1: 0x00CC [0x1D] PRINT_EVENT_MESSAGE(message_id=7766*)
