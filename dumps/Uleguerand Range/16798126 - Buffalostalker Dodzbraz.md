...
- **7349**: Me hunter. Me gets food for other Orcs. Not here to fights with you.
- **7350**: Nasty snolls in mountains. You climbs mountains?
- **7351**: You climbs mountains? [Yep./Nope.]
- **7352**: Me hates snolls. Me tells you secret.
- **7353**: Brings me two cores from bomb clusters.
- **7354**: Me gives you present.
- **7355**: <Snuffle...grunt> Me needs these cores.
- **7356**: You takes this.
- **7357**: You uses on snolls. They starts to melts. Gahahaha!
...
```
  ...
  3: 0x0008 [0x1D] PRINT_EVENT_MESSAGE(message_id=7349*)
    → "Me hunter. Me gets food for other Orcs. Not here to fights with you."
  4: 0x000B [0x23] WAIT_FOR_DIALOG_INTERACTION
  5: 0x000C [0x1D] PRINT_EVENT_MESSAGE(message_id=7350*)
    → "Nasty snolls in mountains. You climbs mountains?"
  6: 0x000F [0x23] WAIT_FOR_DIALOG_INTERACTION
  7: 0x0010 [0x24] CREATE_DIALOG(message_id=7351*, default_option=0*, option_flags=0*)
...
```
  ...
  3: 0x004F [0x1D] PRINT_EVENT_MESSAGE(message_id=7352*)
    → "Me hates snolls. Me tells you secret."
  4: 0x0052 [0x23] WAIT_FOR_DIALOG_INTERACTION
  5: 0x0053 [0x1D] PRINT_EVENT_MESSAGE(message_id=7353*)
    → "Brings me two cores from bomb clusters."
  6: 0x0056 [0x23] WAIT_FOR_DIALOG_INTERACTION
  7: 0x0057 [0x1D] PRINT_EVENT_MESSAGE(message_id=7354*)
    → "Me gives you present."
...
```
  ...
  4: 0x0067 [0x1D] PRINT_EVENT_MESSAGE(message_id=7355*)
    → "<Snuffle...grunt> Me needs these cores."
  5: 0x006A [0x23] WAIT_FOR_DIALOG_INTERACTION
  6: 0x006B [0x1D] PRINT_EVENT_MESSAGE(message_id=7356*)
    → "You takes this."
  7: 0x006E [0x23] WAIT_FOR_DIALOG_INTERACTION
  8: 0x006F [0x1D] PRINT_EVENT_MESSAGE(message_id=7357*)
