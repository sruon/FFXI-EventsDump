# Event 35

## Metadata

| Field           | Value     |
|-----------------|-----------|
| Event Index     | 2/4       |
| Offset          | 0x0001    |
| Data Size       | 381 bytes |
| Calculated Size | 381 bytes |
| Instructions    | 51        |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0000:    1E F0 FF FF 7F 6F 70  66 00 80 F8 FF FF 7F F8   .....opf.......
0010: FF FF 7F 74 6C 6B 30 1D  01 80 23 5E 69 64 6C 30  ...tlk0...#^idl0
0020: 24 02 80 03 80 03 80 25  02 00 10 03 80 00 36 01  $......%......6.
0030: 42 66 00 80 F8 FF FF 7F  F8 FF FF 7F 74 6C 6B 30  Bf..........tlk0
0040: 1D 04 80 23 5E 69 64 6C  30 45 05 80 F0 FF FF 7F  ...#^idl0E......
0050: F0 FF FF 7F 66 64 6F 31  03 80 55 05 80 F0 FF FF  ....fdo1..U.....
0060: 7F F0 FF FF 7F 66 64 6F  31 46 01 45 06 80 F0 FF  .....fdo1F.E....
0070: FF 7F F0 FF FF 7F 73 65  30 36 03 80 45 05 80 F0  ......se06..E...
0080: FF FF 7F F0 FF FF 7F 66  64 69 31 03 80 55 05 80  .......fdi1..U..
0090: F0 FF FF 7F F0 FF FF 7F  66 64 69 31 39 07 80 6F  ........fdi19..o
00A0: 70 66 08 80 F8 FF FF 7F  F8 FF FF 7F 70 6F 69 31  pf..........poi1
00B0: 1D 09 80 23 55 06 80 F0  FF FF 7F F0 FF FF 7F 73  ...#U..........s
00C0: 65 30 36 53 F8 FF FF 7F  F8 FF FF 7F 70 6F 69 31  e06S........poi1
00D0: 45 05 80 F0 FF FF 7F F0  FF FF 7F 66 64 6F 31 03  E..........fdo1.
00E0: 80 55 05 80 F0 FF FF 7F  F0 FF FF 7F 66 64 6F 31  .U..........fdo1
00F0: 46 00 66 08 80 F8 FF FF  7F F8 FF FF 7F 70 6F 69  F.f..........poi
0100: 32 55 05 80 F0 FF FF 7F  F0 FF FF 7F 66 64 69 31  2U..........fdi1
0110: 45 05 80 F0 FF FF 7F F0  FF FF 7F 66 64 69 31 03  E..........fdi1.
0120: 80 53 F8 FF FF 7F F8 FF  FF 7F 74 6C 6B 30 03 01  .S........tlk0..
0130: 10 0A 80 01 7C 01 02 00  10 0B 80 00 7C 01 66 0C  ....|.......|.f.
0140: 80 F8 FF FF 7F F8 FF FF  7F 74 6C 6B 30 1D 0D 80  .........tlk0...
0150: 23 53 F8 FF FF 7F F8 FF  FF 7F 74 6C 6B 30 1C 0E  #S........tlk0..
0160: 80 66 00 80 F8 FF FF 7F  F8 FF FF 7F 74 6C 6B 30  .f..........tlk0
0170: 1D 0F 80 23 03 01 10 0A  80 01 7C 01 21 00        ...#......|.!.  
```

## Opcodes

```
  0: 0x0001 [0x1E] EventEntity looks at LocalPlayer and starts talking
  1: 0x0006 [0x6F] WAIT_FRAME_DELAY: Yield until WaitTime reaches zero
  2: 0x0007 [0x70] WAIT_ENTITY_RENDER_FLAG: Wait while EventEntity->Render.Flags3 bit 2 is set (cancel turn if not)
  3: 0x0008 [0x66] LOAD_EXT_SCHEDULER_MAIN(work_offset=50*, entity1=EventEntity, entity2=EventEntity, action="tlk0")
  4: 0x0017 [0x1D] PRINT_EVENT_MESSAGE(message_id=6847*)
    → "Hello.\u0007I haven't seen you around here beforrre. Are you new in town?\u007F1\u0000\u0007"
  5: 0x001A [0x23] WAIT_FOR_DIALOG_INTERACTION
  6: 0x001B [0x5E] EventEntity goes idle (kills current action) (animation: "idl0")
  7: 0x0020 [0x24] CREATE_DIALOG(message_id=6848*, default_option=0*, option_flags=0*)
    → "Are you new to Mhaura?\u0007\u000BYes, it's my first time here.\u0007No, I've come here plenty of times.\u007F1\u0000\u0007"
  8: 0x0027 [0x25] WAIT_DIALOG_SELECT()
  9: 0x0028 [0x02] IF !(Work_Zone[0] == 0*) GOTO 0x0136
 10: 0x0030 [0x42] SET_CLI_EVENT_CANCEL_DATA()
 11: 0x0031 [0x66] LOAD_EXT_SCHEDULER_MAIN(work_offset=50*, entity1=EventEntity, entity2=EventEntity, action="tlk0")
 12: 0x0040 [0x1D] PRINT_EVENT_MESSAGE(message_id=6849*)
    → "Ah, I thought so. Welcome to Mhaura.\u0007You should go to the governor's house. They'll tell you all you need to know about our little town.\u007F1\u0000\u0007"
 13: 0x0043 [0x23] WAIT_FOR_DIALOG_INTERACTION
 14: 0x0044 [0x5E] EventEntity goes idle (kills current action) (animation: "idl0")
 15: 0x0049 [0x45] LOAD_SCHEDULED_TASK(work_offset1=200*, entity1=LocalPlayer, entity2=LocalPlayer, scheduler_id="fdo1", work_offset2=0*)
 16: 0x005A [0x55] WAIT_LOAD_SCHEDULER(work_offset=200*, entity1=LocalPlayer, entity2=LocalPlayer, scheduler_id="fdo1")
 17: 0x0069 [0x46] CAMERA_CONTROL: Disable user control
 18: 0x006B [0x45] LOAD_SCHEDULED_TASK(work_offset1=149*, entity1=LocalPlayer, entity2=LocalPlayer, scheduler_id="se06", work_offset2=0*)
 19: 0x007C [0x45] LOAD_SCHEDULED_TASK(work_offset1=200*, entity1=LocalPlayer, entity2=LocalPlayer, scheduler_id="fdi1", work_offset2=0*)
 20: 0x008D [0x55] WAIT_LOAD_SCHEDULER(work_offset=200*, entity1=LocalPlayer, entity2=LocalPlayer, scheduler_id="fdi1")
 21: 0x009C [0x39] SET_ENTITY_DIRECTION(direction=8.1°*)
 22: 0x009F [0x6F] WAIT_FRAME_DELAY: Yield until WaitTime reaches zero
 23: 0x00A0 [0x70] WAIT_ENTITY_RENDER_FLAG: Wait while EventEntity->Render.Flags3 bit 2 is set (cancel turn if not)
 24: 0x00A1 [0x66] LOAD_EXT_SCHEDULER_MAIN(work_offset=52*, entity1=EventEntity, entity2=EventEntity, action="poi1")
 25: 0x00B0 [0x1D] PRINT_EVENT_MESSAGE(message_id=6850*)
    → "First, climb the stairs there, and walk along the ledge to another set of stairs. Go up, and you're therrre.\u007F1\u0000\u0007"
 26: 0x00B3 [0x23] WAIT_FOR_DIALOG_INTERACTION
 27: 0x00B4 [0x55] WAIT_LOAD_SCHEDULER(work_offset=149*, entity1=LocalPlayer, entity2=LocalPlayer, scheduler_id="se06")
 28: 0x00C3 [0x53] WAIT_SCHEDULER_TASK(entity1=EventEntity, entity2=EventEntity, action_id="poi1")
 29: 0x00D0 [0x45] LOAD_SCHEDULED_TASK(work_offset1=200*, entity1=LocalPlayer, entity2=LocalPlayer, scheduler_id="fdo1", work_offset2=0*)
 30: 0x00E1 [0x55] WAIT_LOAD_SCHEDULER(work_offset=200*, entity1=LocalPlayer, entity2=LocalPlayer, scheduler_id="fdo1")
 31: 0x00F0 [0x46] CAMERA_CONTROL: Restore default settings
 32: 0x00F2 [0x66] LOAD_EXT_SCHEDULER_MAIN(work_offset=52*, entity1=EventEntity, entity2=EventEntity, action="poi2")
 33: 0x0101 [0x55] WAIT_LOAD_SCHEDULER(work_offset=200*, entity1=LocalPlayer, entity2=LocalPlayer, scheduler_id="fdi1")
 34: 0x0110 [0x45] LOAD_SCHEDULED_TASK(work_offset1=200*, entity1=LocalPlayer, entity2=LocalPlayer, scheduler_id="fdi1", work_offset2=0*)
 35: 0x0121 [0x53] WAIT_SCHEDULER_TASK(entity1=EventEntity, entity2=EventEntity, action_id="tlk0")
 36: 0x012E [0x03] Work_Zone[1] = 2*
 37: 0x0133 [0x01] GOTO 0x017C

SUBROUTINE_0136:
 38: 0x0136 [0x02] IF !(Work_Zone[0] == 1*) GOTO 0x017C
 39: 0x013E [0x66] LOAD_EXT_SCHEDULER_MAIN(work_offset=51*, entity1=EventEntity, entity2=EventEntity, action="tlk0")
 40: 0x014D [0x1D] PRINT_EVENT_MESSAGE(message_id=6851*)
    → "You're not pulling my tail, are you? Hmm...I guess I must've just missed you. Still, you look lost.\u007F1\u0000\u0007"
 41: 0x0150 [0x23] WAIT_FOR_DIALOG_INTERACTION
 42: 0x0151 [0x53] WAIT_SCHEDULER_TASK(entity1=EventEntity, entity2=EventEntity, action_id="tlk0")
 43: 0x015E [0x1C] WAIT(60* ticks)
 44: 0x0161 [0x66] LOAD_EXT_SCHEDULER_MAIN(work_offset=50*, entity1=EventEntity, entity2=EventEntity, action="tlk0")
 45: 0x0170 [0x1D] PRINT_EVENT_MESSAGE(message_id=6852*)
    → "Have you been to the governor's house yet? If not, you should. They'll tell you all you need to know about our little town.\u007F1\u0000\u0007"
 46: 0x0173 [0x23] WAIT_FOR_DIALOG_INTERACTION
 47: 0x0174 [0x03] Work_Zone[1] = 2*
 48: 0x0179 [0x01] GOTO 0x017C

SUBROUTINE_017C:
 49: 0x017C [0x21] END_EVENT
 50: 0x017D [0x00] END_REQSTACK()
```
