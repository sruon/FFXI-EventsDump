...
- **8904**: That svine is up to no good. He's using a magic doll to manipulate-vate a constant stream of adventurers into entering the crystals.
- **8908**: Avatars are generally in a deep-veep slumber, but if they are approached with the tuning forks created by the Rhinostery, they come raging forth, looking to do battle-vattle.
- **8910**: I don't really know. Something is supposed to happen, but I never felt anything...
- **8912**: Yes. But I have no idea vhy. I vould like to ask him about that, but he alvays runs from me.
- **8916**: Vhat a blast from the past. That nagging-vagging old Fuepepe continued on to the Aurastery, didn't she?
- **8917**: And let's see... Furakku-Norakku is probably half-asleep at his desk in the Optistery.
- **8919**: Ah, he's Minister Shantotto's... Vait, it's Professor Shantotto now, isn't it? Anyvay, Sunny-Pabonny was a hugey-vuge fan of hers, as I recall.
- **8924**: Coming, Principal!
- **8941**: If you defeat the avatars that reside vithin the protocrystals, then you are granted a portion-vortion of that avatar's power.
- **8942**: It appears that Uran-Mafran offers a reward to adventurers who have besty-vested the avatars, and then steals the power that they have gained.
- **8947**: ...Principal Koru-Moru, ve need to talk to Uran-Mafran directly. I vill see if I can track him down.
- **8977**: Bring somevone back to life!?
- **8983**: But if all that is true... Who on Vana'diel is Uran-Mafran trying to resurrect...?
- **8987**: Great mages of the past... You mean...
- **8990**: Of course, Principal.
...
```
  0: 0x0230 [0x1D] PRINT_EVENT_MESSAGE(message_id=8904*)
...
```
  0: 0x0235 [0x1D] PRINT_EVENT_MESSAGE(message_id=8908*)
...
```
  ...
  2: 0x0246 [0x1D] PRINT_EVENT_MESSAGE(message_id=8910*)
...
```
  0: 0x024B [0x1D] PRINT_EVENT_MESSAGE(message_id=8912*)
...
```
  0: 0x0250 [0x1D] PRINT_EVENT_MESSAGE(message_id=8916*)
  ...
  2: 0x0254 [0x1D] PRINT_EVENT_MESSAGE(message_id=8917*)
...
```
  0: 0x0259 [0x1D] PRINT_EVENT_MESSAGE(message_id=8919*)
...
```
  ...
  2: 0x026A [0x1D] PRINT_EVENT_MESSAGE(message_id=8924*)
    → "Coming, Principal!"
...
```
  0: 0x0296 [0x1D] PRINT_EVENT_MESSAGE(message_id=8941*)
  ...
  2: 0x029A [0x1D] PRINT_EVENT_MESSAGE(message_id=8942*)
...
```
  0: 0x029F [0x1D] PRINT_EVENT_MESSAGE(message_id=8947*)
...
```
  0: 0x02BA [0x1D] PRINT_EVENT_MESSAGE(message_id=8977*)
...
```
  ...
  2: 0x02CB [0x1D] PRINT_EVENT_MESSAGE(message_id=8983*)
...
```
  0: 0x02D0 [0x1D] PRINT_EVENT_MESSAGE(message_id=8987*)
...
```
  ...
  2: 0x02E1 [0x1D] PRINT_EVENT_MESSAGE(message_id=8990*)
    → "Of course, Principal."
