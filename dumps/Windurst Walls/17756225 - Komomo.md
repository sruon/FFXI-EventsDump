...
- **7880**: These dolly-wollies with the starfruit heads are called Cardians.t
- **7881**: Cardians come from the Great Star Tree, so they speak the language of the stars!t
...
```
  ...
  5: 0x0035 [0x1D] PRINT_EVENT_MESSAGE(message_id=7880*)
    → "These dolly-wollies with the starfruit heads are called Cardians.t"
  6: 0x0038 [0x23] WAIT_FOR_DIALOG_INTERACTION
  7: 0x0039 [0x1D] PRINT_EVENT_MESSAGE(message_id=7881*)
    → "Cardians come from the Great Star Tree, so they speak the language of the stars!t"
