...
- **7721**: Oh my Gob! You're one of those adventurerurers, aren't ya? You got nothin' on me, [man./lady.] I was just mindin' my own businesesess, lookin' for junk an' stuff.
- **7722**: Hey, watch out. You don't wanna fight me. I'll turn you into meat jerky! Wait, how about you and me be friends instead? Sound good?
- **7723**: I travel all over the world and hear lots of differererent stories. I even hear your stories. I can tell you one, if you want.
- **7724**: Now, listen close...
- **7725**: Oh... Well, if that's the case, I'm outta here.
- **7731**: Do you want to hear a story? [No, thank you./Let's hear one./Let's hear some more!]
- **7732**: What story would you like to be told? [On second thought, maybe later./Pillar of Hope./Glimmer of Life./Time Slips Away(pt.1)./Time Slips Away(pt.2)./When Wills Collide(pt.1)./When Wills Collide(pt.2)./When Wills Collide(pt.3)./When Wills Collide(pt.4)./A Dreamy Interlude(pt.1)./A Dreamy Interlude(pt.2)./Cait in the Woods(pt.1)./Cait in the Woods(pt.2)./Maiden of the Dusk(pt.1)./Maiden of the Dusk(pt.2)./Maiden of the Dusk(pt.3)./Lest We Forget(pt.1)./Lest We Forget(pt.2).]
- **7733**: What story would you like to be told? [On second thought, maybe later./Message on the Wind(pt.1)./Message on the Wind(pt.2)./Message on the Wind(pt.3)./Message on the Wind(pt.4)./Fires of Discontent./The Unfinished Waltz(pt.1)./The Unfinished Waltz(pt.2)./Seeing Blood-red(pt.1)./Seeing Blood-red(pt.2)./Storm on the Horizon./Beast from the East(pt.1)./Beast from the East(pt.2)./The Forbidden Path(pt.1)./The Forbidden Path(pt.2)./The Forbidden Path(pt.3)./Her Memories: Of Malign Maladies(pt.1)./Her Memories: Of Malign Maladies(pt.2)./Somber Dreams(pt.1)./Somber Dreams(pt.2).]
- **7734**: What story would you like to be told? [On second thought, maybe later.]
- **7735**: What story would you like to be told? [On second thought, maybe later.]
...
```
 ...
 15: 0x005A [0x1D] PRINT_EVENT_MESSAGE(message_id=7721*)
 ...
 17: 0x005E [0x1D] PRINT_EVENT_MESSAGE(message_id=7722*)
 ...
 19: 0x0062 [0x1D] PRINT_EVENT_MESSAGE(message_id=7723*)
 ...
 29: 0x0092 [0x24] CREATE_DIALOG(message_id=7731*, default_option=0*, option_flags=ExtData[1]->WorkLocal[0])
...
SUBROUTINE_00A2:
 32: 0x00A2 [0x24] CREATE_DIALOG(message_id=7732*, default_option=0*, option_flags=Work_Zone[2])
 ...
 37: 0x00C8 [0x1D] PRINT_EVENT_MESSAGE(message_id=7725*)
...
SUBROUTINE_00E6:
 43: 0x00E6 [0x24] CREATE_DIALOG(message_id=7733*, default_option=0*, option_flags=Work_Zone[3])
 ...
 48: 0x010C [0x1D] PRINT_EVENT_MESSAGE(message_id=7725*)
    → "Oh... Well, if that's the case, I'm outta here."
...
SUBROUTINE_012A:
 54: 0x012A [0x24] CREATE_DIALOG(message_id=7734*, default_option=0*, option_flags=Work_Zone[4])
 ...
 59: 0x0150 [0x1D] PRINT_EVENT_MESSAGE(message_id=7725*)
    → "Oh... Well, if that's the case, I'm outta here."
...
SUBROUTINE_016E:
 65: 0x016E [0x24] CREATE_DIALOG(message_id=7735*, default_option=0*, option_flags=Work_Zone[5])
    → "What story would you like to be told? [On second thought, maybe later.]"
 ...
 70: 0x0194 [0x1D] PRINT_EVENT_MESSAGE(message_id=7725*)
    → "Oh... Well, if that's the case, I'm outta here."
...
SUBROUTINE_01A7:
 ...
 77: 0x01C0 [0x1D] PRINT_EVENT_MESSAGE(message_id=7725*)
    → "Oh... Well, if that's the case, I'm outta here."
...
SUBROUTINE_01C6:
 81: 0x01C6 [0x1D] PRINT_EVENT_MESSAGE(message_id=7724*)
    → "Now, listen close..."
