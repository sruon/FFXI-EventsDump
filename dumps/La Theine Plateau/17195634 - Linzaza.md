...
- **7524**: Who would've thought someone would get here before us?
- **7531**: Yes, you're rightaru. I need to be more of a villainy-willain, and start being suspicious of people.
- **7542**: It will be gone by the time we reach Tavnazia. Mightaru be a good idea to lose that tummy-wummy on the way, as well.
...
```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0170:          29 08 72 62 06  01 01 1D 08 80 23 29 08     ).rb......#).
0180: 72 62 06 01 02 00                                 rb....          
```

#### Opcodes

```
  0: 0x0173 [0x29] REQ_SET_WAIT(priority=0x08, entity_id=Linzaza (ID: 17195634/0x01066272), tag_num=0x01)
  1: 0x017A [0x1D] PRINT_EVENT_MESSAGE(message_id=7524*)
    → "Who would've thought someone would get here before us?"
  2: 0x017D [0x23] WAIT_FOR_DIALOG_INTERACTION
  3: 0x017E [0x29] REQ_SET_WAIT(priority=0x08, entity_id=Linzaza (ID: 17195634/0x01066272), tag_num=0x02)
...
```
  ...
  2: 0x0192 [0x1D] PRINT_EVENT_MESSAGE(message_id=7531*)
...
```
  0: 0x0197 [0x1D] PRINT_EVENT_MESSAGE(message_id=7542*)
