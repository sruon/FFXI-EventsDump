...
- **7421**: Hey!
- **7422**: Hey! Come take a look at this! Quickly!
- **7423**: Look, a cave! Maybe he went in there!
- **7745**: Sir Elmemague has taken the suspect to the chateau for questioning. It's back to regular training drills for us.
- **7746**: We have received a report from Sir Elmemague that the suspect we apprehended wasn't the criminal after all. We must be ready to send word to the chateau if any other suspicious characters are sighted.
- **7747**: Thanks to our effective training drills, we were able to capture the new suspect with little trouble. Although I do wonder where that other knight disappeared to...
...
```
  ...
  3: 0x0008 [0x1D] PRINT_EVENT_MESSAGE(message_id=7745*)
...
```
  ...
  3: 0x0015 [0x1D] PRINT_EVENT_MESSAGE(message_id=7746*)
...
```
  ...
  3: 0x0022 [0x1D] PRINT_EVENT_MESSAGE(message_id=7747*)
...
```
  ...
  4: 0x003E [0x1D] PRINT_EVENT_MESSAGE(message_id=7423*)
...
```
  0: 0x004D [0x1D] PRINT_EVENT_MESSAGE(message_id=7421*)
    → "Hey!"
...
```
  ...
  6: 0x0074 [0x1D] PRINT_EVENT_MESSAGE(message_id=7422*)
