...
- **7417**: Hey!
- **7418**: Hey! Come take a look at this! Quickly!
- **7419**: Look, a cave! Maybe he went in there!
- **7741**: Sir Elmemague has taken the suspect to the chateau for questioning. It's back to regular training drills for us.
- **7742**: We have received a report from Sir Elmemague that the suspect we apprehended wasn't the criminal after all. We must be ready to send word to the chateau if any other suspicious characters are sighted.
- **7743**: Thanks to our effective training drills, we were able to capture the new suspect with little trouble. Although I do wonder where that other knight disappeared to...
...
```
  ...
  3: 0x0008 [0x1D] PRINT_EVENT_MESSAGE(message_id=7741*)
...
```
  ...
  3: 0x0015 [0x1D] PRINT_EVENT_MESSAGE(message_id=7742*)
...
```
  ...
  3: 0x0022 [0x1D] PRINT_EVENT_MESSAGE(message_id=7743*)
...
```
  ...
  4: 0x003E [0x1D] PRINT_EVENT_MESSAGE(message_id=7419*)
...
```
  0: 0x004D [0x1D] PRINT_EVENT_MESSAGE(message_id=7417*)
    → "Hey!"
...
```
  ...
  6: 0x0074 [0x1D] PRINT_EVENT_MESSAGE(message_id=7418*)
