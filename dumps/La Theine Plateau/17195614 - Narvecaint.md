...
- **7416**: Hey!
- **7417**: Hey! Come take a look at this! Quickly!
- **7418**: Look, a cave! Maybe he went in there!
- **7740**: Sir Elmemague has taken the suspect to the chateau for questioning. It's back to regular training drills for us.
- **7741**: We have received a report from Sir Elmemague that the suspect we apprehended wasn't the criminal after all. We must be ready to send word to the chateau if any other suspicious characters are sighted.
- **7742**: Thanks to our effective training drills, we were able to capture the new suspect with little trouble. Although I do wonder where that other knight disappeared to...
...
```
  ...
  3: 0x0008 [0x1D] PRINT_EVENT_MESSAGE(message_id=7740*)
...
```
  ...
  3: 0x0015 [0x1D] PRINT_EVENT_MESSAGE(message_id=7741*)
...
```
  ...
  3: 0x0022 [0x1D] PRINT_EVENT_MESSAGE(message_id=7742*)
...
```
  ...
  4: 0x003E [0x1D] PRINT_EVENT_MESSAGE(message_id=7418*)
...
```
  0: 0x004D [0x1D] PRINT_EVENT_MESSAGE(message_id=7416*)
    → "Hey!"
...
```
  ...
  6: 0x0074 [0x1D] PRINT_EVENT_MESSAGE(message_id=7417*)
...
```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0090:          1E 7F 62 06 01  00                          ..b...       
```

#### Opcodes

```
  0: 0x0093 [0x1E] EventEntity looks at Elmemague (ID: 17195647/0x0106627F) and starts talking
...
```
  0: 0x0099 [0x1E] EventEntity looks at Vijartal (ID: 17195648/0x01066280) and starts talking
