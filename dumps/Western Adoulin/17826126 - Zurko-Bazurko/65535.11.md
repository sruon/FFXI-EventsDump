# Event 65535

## Metadata

| Field           | Value    |
|-----------------|----------|
| Event Index     | 13/20    |
| Offset          | 0x015A   |
| Data Size       | 33 bytes |
| Calculated Size | 33 bytes |
| Instructions    | 3        |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0150:                                5F 06 1A 80 F8 FF            _.....
0160: FF 7F F8 FF FF 7F 74 68  6B 32 1B 80 5F 07 F8 FF  ......thk2.._...
0170: FF 7F F8 FF FF 7F 74 68  6B 32 00                 ......thk2.     
```

## Opcodes

```
  0: 0x015A [0x5F] MULTI_HANDLER_COMPLEX(mode=0x06 - Load ext scheduler with flag (OpCode 0x5B mode 1, flag 1), ref=References[26], entity1=EventEntity, entity2=EventEntity, string="thk2", extra=References[27])
  1: 0x016C [0x5F] MULTI_HANDLER_COMPLEX(mode=0x07 - Wait scheduler task (OpCode 0x53))
  2: 0x017A [0x00] END_REQSTACK()
```
