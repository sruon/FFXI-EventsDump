# Event 306

## Metadata

| Field           | Value    |
|-----------------|----------|
| Event Index     | 2/8      |
| Offset          | 0x0001   |
| Data Size       | 42 bytes |
| Calculated Size | 42 bytes |
| Instructions    | 8        |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0000:    5B 00 80 F8 FF FF 7F  F8 FF FF 7F 67 6B 72 30   [..........gkr0
0010: 1D 01 80 23 53 F8 FF FF  7F F8 FF FF 7F 67 6B 72  ...#S........gkr
0020: 30 5E 69 64 6C 30 1C 02  80 21 00                 0^idl0...!.     
```

## Opcodes

```
  0: 0x0001 [0x5B] LOAD_EXT_SCHEDULER(work_offset=336*, entity1=EventEntity, entity2=EventEntity, action="gkr0")
  1: 0x0010 [0x1D] PRINT_EVENT_MESSAGE(message_id=7854*)
    → "My dear, beloved Boxuxu...\u0007They could postaru me here for an eternity, but with you by my side, every moment would be bliss!\u007F1\u0000\u0007"
  2: 0x0013 [0x23] WAIT_FOR_DIALOG_INTERACTION
  3: 0x0014 [0x53] WAIT_SCHEDULER_TASK(entity1=EventEntity, entity2=EventEntity, action_id="gkr0")
  4: 0x0021 [0x5E] EventEntity goes idle (kills current action) (animation: "idl0")
  5: 0x0026 [0x1C] WAIT(30* ticks)
  6: 0x0029 [0x21] END_EVENT
  7: 0x002A [0x00] END_REQSTACK()
```
