# Event 65535

## Metadata

| Field           | Value    |
|-----------------|----------|
| Event Index     | 3/5      |
| Offset          | 0x001F   |
| Data Size       | 15 bytes |
| Calculated Size | 15 bytes |
| Instructions    | 5        |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0010:                                               32                 2
0020: 02 80 1F 00 03 80 04 80  05 80 1F 01 6F 00        ............o.  
```

## Opcodes

```
  0: 0x001F [0x32] ExtData[1]->MainSpeed = 80* * 0.1
  1: 0x0022 [0x1F] MOVE_ENTITY: EventEntity moves to X=158.461*, Z=53.972*, Y=-41.759*
  2: 0x002A [0x1F] MOVE_ENTITY: Update entity position (mode=1)
  3: 0x002C [0x6F] WAIT_FRAME_DELAY: Yield until WaitTime reaches zero
  4: 0x002D [0x00] END_REQSTACK()
```
