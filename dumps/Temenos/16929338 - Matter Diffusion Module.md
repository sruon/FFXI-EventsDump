# 16929338 - Matter Diffusion Module

## Common Data

| Field            | Value            |
|------------------|------------------|
| Zone             | Temenos (ID: 37) |
| Block Size       | 604 bytes        |
| Total Events     | 2                |
| References Count | 33               |

## List of Events

| Event ID              | Entrypoint   |   Size |   Instructions |
|-----------------------|--------------|--------|----------------|
| [65535](#event-65535) | 0x0000       |      1 |              1 |
| [1003](#event-1003)   | 0x0001       |    447 |             83 |

## DAT References (imed_data)

|   Index | Hex Value   |   Dec Value |
|---------|-------------|-------------|
|       0 | 0x0000      |           0 |
|       1 | 0x000B      |          11 |
|       2 | 0x0004      |           4 |
|       3 | 0x0001      |           1 |
|       4 | 0x0005      |           5 |
|       5 | 0x0006      |           6 |
|       6 | 0x1C45      |        7237 |
|       7 | 0x000C      |          12 |
|       8 | 0x0002      |           2 |
|       9 | 0x000D      |          13 |
|      10 | 0x0003      |           3 |
|      11 | 0x000E      |          14 |
|      12 | 0x0007      |           7 |
|      13 | 0x0008      |           8 |
|      14 | 0x00FF      |         255 |
|      15 | 0x0064      |         100 |
|      16 | 0x8D9A0     |      580000 |
|      17 | 0x14FF0     |       86000 |
|      18 | 0x0400      |        1024 |
|      19 | 0xFFFDDD20  |  4294827296 |
|      20 | 0x5BCC0     |      376000 |
|      21 | 0xFFFEBE34  |  4294884916 |
|      22 | 0x0C00      |        3072 |
|      23 | 0xFFFC0860  |  4294707296 |
|      24 | 0x12F34     |       77620 |
|      25 | 0xFFF8FB20  |  4294507296 |
|      26 | 0xFFF72660  |  4294387296 |
|      27 | 0x004B      |          75 |
|      28 | 0x0028      |          40 |
|      29 | 0x00C8      |         200 |
|      30 | 0x003C      |          60 |
|      31 | 0x004C      |          76 |
|      32 | 0x00AA      |         170 |

## String References

- **7237**: Destination? (Currently: [/N/W/E/C] Tower, Floor $2). [[/Northern/Western/Eastern/Central] Tower - 1st Floor./[/Northern/Western/Eastern/Central] Tower - 2nd Floor./[/Northern/Western/Eastern/Central] Tower - 3rd Floor./[/Northern/Western/Eastern/Central] Tower - 4th Floor./[/Northern/Western/Eastern] Tower - 5th Floor./[/Northern/Western/Eastern] Tower - 6th Floor./[/Northern/Western/Eastern] Tower - 7th Floor./Entrance./Never mind.]

## Events

### Event 65535

#### Metadata

| Field        | Value   |
|--------------|---------|
| Entrypoint   | 0x0000  |
| Data Size    | 1 bytes |
| Instructions | 1       |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0000: 00                                                .               
```

#### Opcodes

```
  0: 0x0000 [0x00] END_REQSTACK()
```

### Event 1003

#### Metadata

| Field        | Value     |
|--------------|-----------|
| Entrypoint   | 0x0001    |
| Data Size    | 447 bytes |
| Instructions | 68        |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0000:    20 01 03 00 00 00 80  03 01 00 00 80 03 00 00    ..............
0010: 03 10 08 00 00 01 80 03  04 10 00 00 0B 04 10 3C  ...............<
0020: 01 00 02 80 03 80 3C 01  00 04 80 03 80 3C 01 00  ......<......<..
0030: 05 80 03 80 24 06 80 00  00 01 00 25 02 00 10 00  ....$......%....
0040: 80 00 4C 00 03 01 10 01  80 01 9C 00 02 00 10 03  ..L.............
0050: 80 00 5C 00 03 01 10 07  80 01 9C 00 02 00 10 08  ..\.............
0060: 80 00 6C 00 03 01 10 09  80 01 9C 00 02 00 10 0A  ..l.............
0070: 80 00 7C 00 03 01 10 0B  80 01 9C 00 02 00 10 0C  ..|.............
0080: 80 00 8C 00 03 01 10 03  80 01 9C 00 02 00 10 0D  ................
0090: 80 00 9C 00 03 01 10 00  80 01 9C 00 02 00 10 00  ................
00A0: 00 00 AC 00 03 01 10 0E  80 01 57 01 02 01 10 00  ..........W.....
00B0: 80 00 B7 00 01 57 01 02  01 10 03 10 00 C2 00 01  .....W..........
00C0: 57 01 42 03 02 00 01 10  03 01 10 0F 80 43 00 43  W.B..........C.C
00D0: 01 03 01 10 02 00 29 01  F0 FF FF 7F 02 02 01 10  ......).........
00E0: 03 80 80 F4 00 47 00 10  80 11 80 00 80 12 80 47  .....G.........G
00F0: 01 01 50 01 02 01 10 01  80 80 0B 01 47 00 13 80  ..P.........G...
0100: 14 80 15 80 16 80 47 01  01 50 01 02 01 10 07 80  ......G..P......
0110: 80 22 01 47 00 17 80 14  80 18 80 16 80 47 01 01  .".G.........G..
0120: 50 01 02 01 10 09 80 80  39 01 47 00 19 80 14 80  P.......9.G.....
0130: 15 80 16 80 47 01 01 50  01 02 01 10 0B 80 80 50  ....G..P.......P
0140: 01 47 00 1A 80 14 80 18  80 16 80 47 01 01 50 01  .G.........G..P.
0150: 29 01 F0 FF FF 7F 03 20  00 21 00 9F 1B 80 F0 FF  )...... .!......
0160: FF 7F F0 FF FF 7F 6D 61  69 6E 00 80 1C 0F 80 03  ......main......
0170: 02 00 01 10 03 01 10 0F  80 43 00 43 01 03 01 10  .........C.C....
0180: 02 00 1C 1C 80 45 1D 80  F0 FF FF 7F F0 FF FF 7F  .....E..........
0190: 66 64 6F 31 00 80 1C 1E  80 1B 45 1D 80 F0 FF FF  fdo1......E.....
01A0: 7F F0 FF FF 7F 66 64 69  31 00 80 9F 1F 80 F0 FF  .....fdi1.......
01B0: FF 7F F0 FF FF 7F 6D 61  69 6E 00 80 1C 20 80 1B  ......main... ..
```

#### Opcodes

```
  0: 0x0001 [0x20] SET_CLI_EVENT_UC_FLAG: Lock player control
  1: 0x0003 [0x03] ExtData[1]->WorkLocal[0] = 0*
  2: 0x0008 [0x03] ExtData[1]->WorkLocal[1] = 0*
  3: 0x000D [0x03] ExtData[1]->WorkLocal[0] = Work_Zone[3]
  4: 0x0012 [0x08] ExtData[1]->WorkLocal[0] -= 11*
  5: 0x0017 [0x03] Work_Zone[4] = ExtData[1]->WorkLocal[0]
  6: 0x001C [0x0B] Work_Zone[4]++
  7: 0x001F [0x3C] SET_BIT_FLAG_CONDITIONAL(target_work_offset=ExtData[1]->WorkLocal[1], bit_index_work_offset=4*, condition_work_offset=1*)
  8: 0x0026 [0x3C] SET_BIT_FLAG_CONDITIONAL(target_work_offset=ExtData[1]->WorkLocal[1], bit_index_work_offset=5*, condition_work_offset=1*)
  9: 0x002D [0x3C] SET_BIT_FLAG_CONDITIONAL(target_work_offset=ExtData[1]->WorkLocal[1], bit_index_work_offset=6*, condition_work_offset=1*)
 10: 0x0034 [0x24] CREATE_DIALOG(message_id=7237*, default_option=ExtData[1]->WorkLocal[0], option_flags=ExtData[1]->WorkLocal[1])
    → "Destination? (Currently: [/N/W/E/C] Tower, Floor $2). [[/Northern/Western/Eastern/Central] Tower - 1st Floor./[/Northern/Western/Eastern/Central] Tower - 2nd Floor./[/Northern/Western/Eastern/Central] Tower - 3rd Floor./[/Northern/Western/Eastern/Central] Tower - 4th Floor./[/Northern/Western/Eastern] Tower - 5th Floor./[/Northern/Western/Eastern] Tower - 6th Floor./[/Northern/Western/Eastern] Tower - 7th Floor./Entrance./Never mind.]"
 11: 0x003B [0x25] WAIT_DIALOG_SELECT()
 12: 0x003C [0x02] IF !(Work_Zone[0] == 0*) GOTO 0x004C
 13: 0x0044 [0x03] Work_Zone[1] = 11*
 14: 0x0049 [0x01] GOTO 0x009C
 15: 0x004C [0x02] IF !(Work_Zone[0] == 1*) GOTO 0x005C
 16: 0x0054 [0x03] Work_Zone[1] = 12*
 17: 0x0059 [0x01] GOTO 0x009C
 18: 0x005C [0x02] IF !(Work_Zone[0] == 2*) GOTO 0x006C
 19: 0x0064 [0x03] Work_Zone[1] = 13*
 20: 0x0069 [0x01] GOTO 0x009C
 21: 0x006C [0x02] IF !(Work_Zone[0] == 3*) GOTO 0x007C
 22: 0x0074 [0x03] Work_Zone[1] = 14*
 23: 0x0079 [0x01] GOTO 0x009C
 24: 0x007C [0x02] IF !(Work_Zone[0] == 7*) GOTO 0x008C
 25: 0x0084 [0x03] Work_Zone[1] = 1*
 26: 0x0089 [0x01] GOTO 0x009C
 27: 0x008C [0x02] IF !(Work_Zone[0] == 8*) GOTO 0x009C
 28: 0x0094 [0x03] Work_Zone[1] = 0*
 29: 0x0099 [0x01] GOTO 0x009C

SUBROUTINE_009C:
 30: 0x009C [0x02] IF !(Work_Zone[0] == ExtData[1]->WorkLocal[0]) GOTO 0x00AC
 31: 0x00A4 [0x03] Work_Zone[1] = 255*
 32: 0x00A9 [0x01] GOTO 0x0157
 33: 0x00AC [0x02] IF !(Work_Zone[1] == 0*) GOTO 0x00B7
 34: 0x00B4 [0x01] GOTO 0x0157
 35: 0x00B7 [0x02] IF !(Work_Zone[1] == Work_Zone[3]) GOTO 0x00C2
 36: 0x00BF [0x01] GOTO 0x0157
 37: 0x00C2 [0x42] SET_CLI_EVENT_CANCEL_DATA()
 38: 0x00C3 [0x03] ExtData[1]->WorkLocal[2] = Work_Zone[1]
 39: 0x00C8 [0x03] Work_Zone[1] = 100*
 40: 0x00CD [0x43] SEND_EVENT_UPDATE: Send pending tag to server (packet 0x005B)
 41: 0x00CF [0x43] SEND_EVENT_UPDATE: Check pending flag (skip if not pending)
 42: 0x00D1 [0x03] Work_Zone[1] = ExtData[1]->WorkLocal[2]
 43: 0x00D6 [0x29] REQ_SET_WAIT(priority=0x01, entity_id=LocalPlayer, tag_num=0x02)
 44: 0x00DD [0x02] IF !(Work_Zone[1] == 1*) GOTO 0x00F4
 45: 0x00E5 [0x47] UPDATE_PLAYER_POS(580.000*, 86.000*, 0.000*, yaw=90.0°*)
 46: 0x00EF [0x47] WAIT_PLAYER_POS_UPDATE
 47: 0x00F1 [0x01] GOTO 0x0150
 48: 0x00F4 [0x02] IF !(Work_Zone[1] == 11*) GOTO 0x010B
 49: 0x00FC [0x47] UPDATE_PLAYER_POS(-140.000*, 376.000*, -82.380*, yaw=270.0°*)
 50: 0x0106 [0x47] WAIT_PLAYER_POS_UPDATE
 51: 0x0108 [0x01] GOTO 0x0150
 52: 0x010B [0x02] IF !(Work_Zone[1] == 12*) GOTO 0x0122
 53: 0x0113 [0x47] UPDATE_PLAYER_POS(-260.000*, 376.000*, 77.620*, yaw=270.0°*)
 54: 0x011D [0x47] WAIT_PLAYER_POS_UPDATE
 55: 0x011F [0x01] GOTO 0x0150
 56: 0x0122 [0x02] IF !(Work_Zone[1] == 13*) GOTO 0x0139
 57: 0x012A [0x47] UPDATE_PLAYER_POS(-460.000*, 376.000*, -82.380*, yaw=270.0°*)
 58: 0x0134 [0x47] WAIT_PLAYER_POS_UPDATE
 59: 0x0136 [0x01] GOTO 0x0150
 60: 0x0139 [0x02] IF !(Work_Zone[1] == 14*) GOTO 0x0150
 61: 0x0141 [0x47] UPDATE_PLAYER_POS(-580.000*, 376.000*, 77.620*, yaw=270.0°*)
 62: 0x014B [0x47] WAIT_PLAYER_POS_UPDATE
 63: 0x014D [0x01] GOTO 0x0150

SUBROUTINE_0150:
 64: 0x0150 [0x29] REQ_SET_WAIT(priority=0x01, entity_id=LocalPlayer, tag_num=0x03)

SUBROUTINE_0157:
 65: 0x0157 [0x20] SET_CLI_EVENT_UC_FLAG: Unlock player control
 66: 0x0159 [0x21] END_EVENT
 67: 0x015A [0x00] END_REQSTACK()
```

#### Data or dead code:

```
# Dead code (unreachable instructions):
     0x015B [0x9F] LOAD_SCHEDULED_TASK_ALT: Load scheduler "main" with entities [LocalPlayer, LocalPlayer], work=[75*, 0*]
     0x016C [0x1C] WAIT(100* ticks)
     0x016F [0x03] ExtData[1]->WorkLocal[2] = Work_Zone[1]
     0x0174 [0x03] Work_Zone[1] = 100*
     0x0179 [0x43] SEND_EVENT_UPDATE: Send pending tag to server (packet 0x005B)
     0x017B [0x43] SEND_EVENT_UPDATE: Check pending flag (skip if not pending)
     0x017D [0x03] Work_Zone[1] = ExtData[1]->WorkLocal[2]
     0x0182 [0x1C] WAIT(40* ticks)
     0x0185 [0x45] LOAD_SCHEDULED_TASK: Load scheduler "fdo1" with entities [LocalPlayer, LocalPlayer], work=[200*, 0*]
     0x0196 [0x1C] WAIT(60* ticks)
     0x0199 [0x1B] RETURN
     0x019A [0x45] LOAD_SCHEDULED_TASK: Load scheduler "fdi1" with entities [LocalPlayer, LocalPlayer], work=[200*, 0*]
     0x01AB [0x9F] LOAD_SCHEDULED_TASK_ALT: Load scheduler "main" with entities [LocalPlayer, LocalPlayer], work=[76*, 0*]
     0x01BC [0x1C] WAIT(170* ticks)
     0x01BF [0x1B] RETURN
```
