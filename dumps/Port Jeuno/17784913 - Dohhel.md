# 17784913 - Dohhel

## Common Data

| Field            | Value                |
|------------------|----------------------|
| Zone             | Port Jeuno (ID: 246) |
| Block Size       | 532 bytes            |
| Total Events     | 2                    |
| References Count | 25                   |

## List of Events

| Event ID              | Entrypoint   |   Size |   Instructions |
|-----------------------|--------------|--------|----------------|
| [65535](#event-65535) | 0x0000       |      1 |              1 |
| [10028](#event-10028) | 0x0001       |    404 |             91 |

## DAT References (imed_data)

|   Index | Hex Value   |   Dec Value |
|---------|-------------|-------------|
|       0 | 0x1E79      |        7801 |
|       1 | 0x001E      |          30 |
|       2 | 0xFFFFFFFE  |  4294967294 |
|       3 | 0x0001      |           1 |
|       4 | 0x0002      |           2 |
|       5 | 0x0003      |           3 |
|       6 | 0x0004      |           4 |
|       7 | 0x1E78      |        7800 |
|       8 | 0x001C      |          28 |
|       9 | 0x001A      |          26 |
|      10 | 0x0016      |          22 |
|      11 | 0x000E      |          14 |
|      12 | 0x1E7B      |        7803 |
|      13 | 0x0000      |           0 |
|      14 | 0x1E7C      |        7804 |
|      15 | 0x40000000  |  1073741824 |
|      16 | 0x1E7D      |        7805 |
|      17 | 0x0020      |          32 |
|      18 | 0x1E7E      |        7806 |
|      19 | 0x0040      |          64 |
|      20 | 0x1E7F      |        7807 |
|      21 | 0x0060      |          96 |
|      22 | 0x18FF      |        6399 |
|      23 | 0x00C8      |         200 |
|      24 | 0x003C      |          60 |

## String References

- **6399**: You do not have enough gil.
- **7800**: The minstrel of melodies is my name, For a mere $6 gil I'll recall your past. Your stories of love, anger, triumph, and shame, The river of memories runs deep and vast.
- **7801**: The minstrel of melodies is my name, For a mere $6 gil I'd recall your past. But I find that you're lacking in cash or in fame, So until that time, my kind offer will last.
- **7803**: Which memory will you have recalled? [Nothing./Missions./Jeuno Quests./Add-on Scenarios.]
- **7804**: Which memory will you have recalled? [Nothing.]
- **7805**: Which memory will you have recalled? [Nothing./Borghertz's Hands(pt.1)./Borghertz's Hands(pt.2)./Past Reflections.]
- **7806**: Which memory will you have recalled? [Nothing./A Journey Begins./The Truth Beckons./Dawn of Death./First Contact./Heart of Madness./Tenuous Existence./Champions of Abyssea./A Sea Dog's Summons./Death and Rebirth./Emissaries of God./Meanwhile, Back on Abyssea./The Forbidden Frontier(pt.1)./The Forbidden Frontier(pt.2)./Scars of Abyssea(pt.1)./Scars of Abyssea(pt.2)./Heroes of Abyssea(pt.1)./Heroes of Abyssea(pt.2)./Numbering Days(pt.1)./Numbering Days(pt.2)./Numbering Days(pt.3)./Numbering Days(pt.4)./Numbering Days(pt.5)./Numbering Days(pt.6)./Delkfutt the Great.]
- **7807**: 

## Events

### Event 65535

#### Metadata

| Field        | Value   |
|--------------|---------|
| Entrypoint   | 0x0000  |
| Data Size    | 1 bytes |
| Instructions | 1       |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0000: 00                                                .               
```

#### Opcodes

```
  0: 0x0000 [0x00] END_REQSTACK()
```

### Event 10028

#### Metadata

| Field        | Value     |
|--------------|-----------|
| Entrypoint   | 0x0001    |
| Data Size    | 404 bytes |
| Instructions | 79        |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0000:    53 F8 FF FF 7F F8 FF  FF 7F 65 74 6F 6E 1E F0   S........eton..
0010: FF FF 7F 02 08 10 09 10  02 21 00 1D 00 80 23 21  .........!....#!
0020: 00 03 00 00 01 80 02 02  10 02 80 01 35 00 3D 00  ............5.=.
0030: 00 03 80 03 80 02 03 10  02 80 01 44 00 3D 00 00  ...........D.=..
0040: 04 80 03 80 02 04 10 02  80 01 53 00 3D 00 00 05  ..........S.=...
0050: 80 03 80 02 05 10 02 80  01 62 00 3D 00 00 06 80  .........b.=....
0060: 03 80 02 00 00 01 80 00  70 00 1D 00 80 23 21 00  ........p....#!.
0070: 1D 07 80 23 02 00 00 08  80 00 7F 00 01 B0 00 02  ...#............
0080: 00 00 09 80 00 8A 00 01  DF 00 02 00 00 0A 80 00  ................
0090: 95 00 01 0E 01 02 00 00  0B 80 00 A0 00 01 3D 01  ..............=.
00A0: 24 0C 80 0D 80 00 00 25  02 00 10 03 80 00 D7 00  $......%........
00B0: 24 0E 80 0D 80 02 10 25  02 00 10 0D 80 00 CA 00  $......%........
00C0: 03 01 10 0F 80 21 00 01  D4 00 03 01 10 0D 80 0E  .....!..........
00D0: 01 10 00 10 01 6B 01 02  00 10 04 80 00 06 01 24  .....k.........$
00E0: 10 80 0D 80 03 10 25 02  00 10 0D 80 00 F9 00 03  ......%.........
00F0: 01 10 0F 80 21 00 01 03  01 03 01 10 11 80 0E 01  ....!...........
0100: 10 00 10 01 6B 01 02 00  10 05 80 00 35 01 24 12  ....k.......5.$.
0110: 80 0D 80 04 10 25 02 00  10 0D 80 00 28 01 03 01  .....%......(...
0120: 10 0F 80 21 00 01 32 01  03 01 10 13 80 0E 01 10  ...!..2.........
0130: 00 10 01 6B 01 02 00 10  06 80 00 64 01 24 14 80  ...k.......d.$..
0140: 0D 80 05 10 25 02 00 10  0D 80 00 57 01 03 01 10  ....%......W....
0150: 0F 80 21 00 01 61 01 03  01 10 15 80 0E 01 10 00  ..!..a..........
0160: 10 01 6B 01 03 01 10 0F  80 21 00 42 43 00 43 01  ..k......!.BC.C.
0170: 02 02 10 0D 80 00 7E 01  48 16 80 01 93 01 45 17  ......~.H.....E.
0180: 80 F8 FF FF 7F F8 FF FF  7F 66 64 6F 31 0D 80 1C  .........fdo1...
0190: 18 80 30 21 00                                    ..0!.           
```

#### Opcodes

```
  0: 0x0001 [0x53] WAIT_SCHEDULER_TASK: Wait for scheduler "eton" with entities [EventEntity, EventEntity]
  1: 0x000E [0x1E] EventEntity looks at LocalPlayer and starts talking
  2: 0x0013 [0x02] IF !(Work_Zone[8] <= Work_Zone[9]) GOTO 0x0021
  3: 0x001B [0x1D] PRINT_EVENT_MESSAGE(message_id=7801*)
    → "The minstrel of melodies is my name, For a mere $6 gil I'd recall your past. But I find that you're lacking in cash or in fame, So until that time, my kind offer will last."
  4: 0x001E [0x23] WAIT_FOR_DIALOG_INTERACTION
  5: 0x001F [0x21] END_EVENT
  6: 0x0020 [0x00] END_REQSTACK()
  7: 0x0021 [0x03] ExtData[1]->WorkLocal[0] = 30*
  8: 0x0026 [0x02] IF !(Work_Zone[2] == 4294967294*) GOTO 0x0035
  9: 0x002E [0x3D] CLEAR_BIT_FLAG_CONDITIONAL(target_work_offset=ExtData[1]->WorkLocal[0], bit_index_work_offset=1*, condition_work_offset=1*)
 10: 0x0035 [0x02] IF !(Work_Zone[3] == 4294967294*) GOTO 0x0044
 11: 0x003D [0x3D] CLEAR_BIT_FLAG_CONDITIONAL(target_work_offset=ExtData[1]->WorkLocal[0], bit_index_work_offset=2*, condition_work_offset=1*)
 12: 0x0044 [0x02] IF !(Work_Zone[4] == 4294967294*) GOTO 0x0053
 13: 0x004C [0x3D] CLEAR_BIT_FLAG_CONDITIONAL(target_work_offset=ExtData[1]->WorkLocal[0], bit_index_work_offset=3*, condition_work_offset=1*)
 14: 0x0053 [0x02] IF !(Work_Zone[5] == 4294967294*) GOTO 0x0062
 15: 0x005B [0x3D] CLEAR_BIT_FLAG_CONDITIONAL(target_work_offset=ExtData[1]->WorkLocal[0], bit_index_work_offset=4*, condition_work_offset=1*)
 16: 0x0062 [0x02] IF !(ExtData[1]->WorkLocal[0] == 30*) GOTO 0x0070
 17: 0x006A [0x1D] PRINT_EVENT_MESSAGE(message_id=7801*)
    → "The minstrel of melodies is my name, For a mere $6 gil I'd recall your past. But I find that you're lacking in cash or in fame, So until that time, my kind offer will last."
 18: 0x006D [0x23] WAIT_FOR_DIALOG_INTERACTION
 19: 0x006E [0x21] END_EVENT
 20: 0x006F [0x00] END_REQSTACK()
 21: 0x0070 [0x1D] PRINT_EVENT_MESSAGE(message_id=7800*)
    → "The minstrel of melodies is my name, For a mere $6 gil I'll recall your past. Your stories of love, anger, triumph, and shame, The river of memories runs deep and vast."
 22: 0x0073 [0x23] WAIT_FOR_DIALOG_INTERACTION
 23: 0x0074 [0x02] IF !(ExtData[1]->WorkLocal[0] == 28*) GOTO 0x007F
 24: 0x007C [0x01] GOTO 0x00B0
 25: 0x007F [0x02] IF !(ExtData[1]->WorkLocal[0] == 26*) GOTO 0x008A
 26: 0x0087 [0x01] GOTO 0x00DF
 27: 0x008A [0x02] IF !(ExtData[1]->WorkLocal[0] == 22*) GOTO 0x0095
 28: 0x0092 [0x01] GOTO 0x010E
 29: 0x0095 [0x02] IF !(ExtData[1]->WorkLocal[0] == 14*) GOTO 0x00A0
 30: 0x009D [0x01] GOTO 0x013D
 31: 0x00A0 [0x24] CREATE_DIALOG(message_id=7803*, default_option=0*, option_flags=ExtData[1]->WorkLocal[0])
    → "Which memory will you have recalled? [Nothing./Missions./Jeuno Quests./Add-on Scenarios.]"
 32: 0x00A7 [0x25] WAIT_DIALOG_SELECT()
 33: 0x00A8 [0x02] IF !(Work_Zone[0] == 1*) GOTO 0x00D7

SUBROUTINE_00B0:
 34: 0x00B0 [0x24] CREATE_DIALOG(message_id=7804*, default_option=0*, option_flags=Work_Zone[2])
    → "Which memory will you have recalled? [Nothing.]"
 35: 0x00B7 [0x25] WAIT_DIALOG_SELECT()
 36: 0x00B8 [0x02] IF !(Work_Zone[0] == 0*) GOTO 0x00CA
 37: 0x00C0 [0x03] Work_Zone[1] = 1073741824*
 38: 0x00C5 [0x21] END_EVENT
 39: 0x00C6 [0x00] END_REQSTACK()

SUBROUTINE_00D4:
 40: 0x00D4 [0x01] GOTO 0x016B
 41: 0x00D7 [0x02] IF !(Work_Zone[0] == 2*) GOTO 0x0106

SUBROUTINE_00DF:
 42: 0x00DF [0x24] CREATE_DIALOG(message_id=7805*, default_option=0*, option_flags=Work_Zone[3])
    → "Which memory will you have recalled? [Nothing./Borghertz's Hands(pt.1)./Borghertz's Hands(pt.2)./Past Reflections.]"
 43: 0x00E6 [0x25] WAIT_DIALOG_SELECT()
 44: 0x00E7 [0x02] IF !(Work_Zone[0] == 0*) GOTO 0x00F9
 45: 0x00EF [0x03] Work_Zone[1] = 1073741824*
 46: 0x00F4 [0x21] END_EVENT
 47: 0x00F5 [0x00] END_REQSTACK()

SUBROUTINE_0103:
 48: 0x0103 [0x01] GOTO 0x016B
 49: 0x0106 [0x02] IF !(Work_Zone[0] == 3*) GOTO 0x0135

SUBROUTINE_010E:
 50: 0x010E [0x24] CREATE_DIALOG(message_id=7806*, default_option=0*, option_flags=Work_Zone[4])
    → "Which memory will you have recalled? [Nothing./A Journey Begins./The Truth Beckons./Dawn of Death./First Contact./Heart of Madness./Tenuous Existence./Champions of Abyssea./A Sea Dog's Summons./Death and Rebirth./Emissaries of God./Meanwhile, Back on Abyssea./The Forbidden Frontier(pt.1)./The Forbidden Frontier(pt.2)./Scars of Abyssea(pt.1)./Scars of Abyssea(pt.2)./Heroes of Abyssea(pt.1)./Heroes of Abyssea(pt.2)./Numbering Days(pt.1)./Numbering Days(pt.2)./Numbering Days(pt.3)./Numbering Days(pt.4)./Numbering Days(pt.5)./Numbering Days(pt.6)./Delkfutt the Great.]"
 51: 0x0115 [0x25] WAIT_DIALOG_SELECT()
 52: 0x0116 [0x02] IF !(Work_Zone[0] == 0*) GOTO 0x0128
 53: 0x011E [0x03] Work_Zone[1] = 1073741824*
 54: 0x0123 [0x21] END_EVENT
 55: 0x0124 [0x00] END_REQSTACK()

SUBROUTINE_0132:
 56: 0x0132 [0x01] GOTO 0x016B
 57: 0x0135 [0x02] IF !(Work_Zone[0] == 4*) GOTO 0x0164

SUBROUTINE_013D:
 58: 0x013D [0x24] CREATE_DIALOG(message_id=7807*, default_option=0*, option_flags=Work_Zone[5])
    → ""
 59: 0x0144 [0x25] WAIT_DIALOG_SELECT()
 60: 0x0145 [0x02] IF !(Work_Zone[0] == 0*) GOTO 0x0157
 61: 0x014D [0x03] Work_Zone[1] = 1073741824*
 62: 0x0152 [0x21] END_EVENT
 63: 0x0153 [0x00] END_REQSTACK()

SUBROUTINE_0161:
 64: 0x0161 [0x01] GOTO 0x016B
 65: 0x0164 [0x03] Work_Zone[1] = 1073741824*
 66: 0x0169 [0x21] END_EVENT
 67: 0x016A [0x00] END_REQSTACK()

SUBROUTINE_016B:
 68: 0x016B [0x42] SET_CLI_EVENT_CANCEL_DATA()
 69: 0x016C [0x43] SEND_EVENT_UPDATE: Send pending tag to server (packet 0x005B)
 70: 0x016E [0x43] SEND_EVENT_UPDATE: Check pending flag (skip if not pending)
 71: 0x0170 [0x02] IF !(Work_Zone[2] == 0*) GOTO 0x017E
 72: 0x0178 [0x48] [System] [6399*]:
    → "You do not have enough gil."
 73: 0x017B [0x01] GOTO 0x0193
 74: 0x017E [0x45] LOAD_SCHEDULED_TASK: Load scheduler "fdo1" with entities [EventEntity, EventEntity], work=[200*, 0*]
 75: 0x018F [0x1C] WAIT(60* ticks)
 76: 0x0192 [0x30] SET_UCOFF_CONTINUE_ZERO()

SUBROUTINE_0193:
 77: 0x0193 [0x21] END_EVENT
 78: 0x0194 [0x00] END_REQSTACK()
```

#### Data or dead code:

```
# Dead code (unreachable instructions):
     0x00C7 [0x01] GOTO 0x00D4
# Dead code (unreachable instructions):
     0x00F6 [0x01] GOTO 0x0103
# Dead code (unreachable instructions):
     0x0125 [0x01] GOTO 0x0132
# Dead code (unreachable instructions):
     0x0154 [0x01] GOTO 0x0161
```
