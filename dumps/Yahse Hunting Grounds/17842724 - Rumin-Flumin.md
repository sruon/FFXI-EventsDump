...
| [65535.15](#event-6553515) | 0x00EB       |      9 |              3 |
| [65535.16](#event-6553516) | 0x00F4       |      1 |              1 |
| [2550](#event-2550)        | 0x00F5       |     34 |             11 |
| [2551](#event-2551)        | 0x0117       |    135 |             38 |
| [2552](#event-2552)        | 0x019E       |     38 |             13 |
| [2553](#event-2553)        | 0x01C4       |     25 |              9 |
| [2555](#event-2555)        | 0x01DD       |     27 |              9 |
...
- **7542**: I can't entrustaru this task to you without $6. Only a bona fide pioneery-weer could help me.
- **7543**: Have you ever been to the Inventors' Coalition-wition? We make all sorts of goods and contraptions to make life easier. For example...this "$3"!
- **7544**: Placing it next to a swarm of bees will lure them inside and capture their yummy-wummy honey and nectar!
- **7545**: So! I've been collecting the nectar of numbing blossoms, a type of flower known for its paralytic effectarus, from bees located southwest of here.
- **7546**: But wait--there's more! The nectar from those flowers has special-wecial nutrients that we need.
- **7547**: "Why," you ask? These nutrientarus have antitoxin properties that help us resist the pungent fungi that dot the cave floors.
- **7548**: This is where you come in, [Mister/Miss] Pioneer. I want you to helpy-welp us collect some. You will help us, rightaru?
- **7549**: It's simple! Just go to the southwestaru until you come to a dead end and place our secret weapon near the swarm!
- **7550**: Then, kneel down and watch the magic-wagic begin. Wait until...oh, let's say ten bees fly inside, and collect the nectaru!
- **7551**: When that's done, head back to the Scouts' Coalition-wition and hand over your findings to Ndah Tolohjin.
- **7552**: Don't forgetaru--place the $3 near the bee swarm in the dead end toward the southwest.
- **7553**: And remember what I told you about kneeling down and staying perfectly still. That part is really-weally importantaru! When you're done, bring what you catch to Ndah Tolohjin at the Scouts' Coalition.
- **7554**: Did you collectaru what we needed? That's great! Now head over to Ndah Tolohjin. I'm pretty sure she's hanging around in the Scouts' Coalition.
- **7555**: Our research has taken on new forms! Now we're studying the eggs of local crawler-wallers. You think they might lead to new discoveries?
...
```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0060:          66 02 80 F8 FF  FF 7F F8 FF FF 7F 74 65     f..........te
0070: 68 30 00                                          h0.             
```
...
```
  0: 0x0063 [0x66] LOAD_EXT_SCHEDULER_MAIN: Load scheduler "teh0" with entities [EventEntity, EventEntity], work=42*
...
```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0070:          53 F8 FF FF 7F  F8 FF FF 7F 74 65 68 30     S........teh0
...
```
  0: 0x0073 [0x53] WAIT_SCHEDULER_TASK: Wait for scheduler "teh0" with entities [EventEntity, EventEntity]
...
0090: F8 FF FF 7F 70 61 73 30  00                       ....pas0.       
```

#### Opcodes

```
  0: 0x0089 [0x66] LOAD_EXT_SCHEDULER_MAIN: Load scheduler "pas0" with entities [EventEntity, EventEntity], work=40*
...
```
  0: 0x0099 [0x53] WAIT_SCHEDULER_TASK: Wait for scheduler "pas0" with entities [EventEntity, EventEntity]
...
00B0: FF 7F 77 61 69 30 00                              ..wai0.         
```

#### Opcodes

```
  0: 0x00A7 [0x66] LOAD_EXT_SCHEDULER_MAIN: Load scheduler "wai0" with entities [EventEntity, EventEntity], work=40*
...
00C0: 77 61 69 30 5E 69 64 6C  30 1C 01 80 00           wai0^idl0....   
```

#### Opcodes

```
  0: 0x00B7 [0x53] WAIT_SCHEDULER_TASK: Wait for scheduler "wai0" with entities [EventEntity, EventEntity]
...
```
      ...
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
00C0:                                         66 03 80               f..
00D0: F8 FF FF 7F F8 FF FF 7F  65 68 6E 31 00           ........ehn1.   
```

#### Opcodes

```
  0: 0x00CD [0x66] LOAD_EXT_SCHEDULER_MAIN: Load scheduler "ehn1" with entities [EventEntity, EventEntity], work=41*
...
```
      ...
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
00D0:                                         53 F8 FF               S..
00E0: FF 7F F8 FF FF 7F 65 68  6E 31 00                 ......ehn1.     
```

#### Opcodes

```
  0: 0x00DD [0x53] WAIT_SCHEDULER_TASK: Wait for scheduler "ehn1" with entities [EventEntity, EventEntity]
...
```
  0: 0x00EB [0x5E] EventEntity goes idle (kills current action) (animation: "idl0")
...
```
  ...
  5: 0x0108 [0x1D] PRINT_EVENT_MESSAGE(message_id=7542*)
...
```
  ...
  6: 0x012B [0x1D] PRINT_EVENT_MESSAGE(message_id=7543*)
  ...
  8: 0x012F [0x1D] PRINT_EVENT_MESSAGE(message_id=7544*)
 ...
 12: 0x0141 [0x1D] PRINT_EVENT_MESSAGE(message_id=7545*)
 ...
 16: 0x0153 [0x1D] PRINT_EVENT_MESSAGE(message_id=7546*)
 ...
 20: 0x0165 [0x1D] PRINT_EVENT_MESSAGE(message_id=7547*)
 ...
 22: 0x0169 [0x1D] PRINT_EVENT_MESSAGE(message_id=7548*)
 ...
 26: 0x017B [0x1D] PRINT_EVENT_MESSAGE(message_id=7549*)
 ...
 28: 0x017F [0x1D] PRINT_EVENT_MESSAGE(message_id=7550*)
 ...
 33: 0x0196 [0x1D] PRINT_EVENT_MESSAGE(message_id=7551*)
...
```
  ...
  5: 0x01B1 [0x1D] PRINT_EVENT_MESSAGE(message_id=7552*)
  ...
  7: 0x01B5 [0x1D] PRINT_EVENT_MESSAGE(message_id=7553*)
...
```
  ...
  5: 0x01D7 [0x1D] PRINT_EVENT_MESSAGE(message_id=7554*)
...
```
  ...
  4: 0x01EB [0x1D] PRINT_EVENT_MESSAGE(message_id=7555*)
