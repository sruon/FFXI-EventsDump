# 17269292 - Neriss

## Common Data

| Field            | Value                          |
|------------------|--------------------------------|
| Zone             | Sauromugue Champaign (ID: 120) |
| Block Size       | 2716 bytes                     |
| Total Events     | 9                              |
| References Count | 155                            |

## List of Events

| Event ID              | Entrypoint   |   Size |   Instructions |
|-----------------------|--------------|--------|----------------|
| [65535](#event-65535) | 0x0000       |      1 |              1 |
| [7001](#event-7001)   | 0x0001       |    540 |            127 |
| [7004](#event-7004)   | 0x021D       |    267 |             70 |
| [7005](#event-7005)   | 0x0328       |    135 |             34 |
| [7011](#event-7011)   | 0x03AF       |     54 |             14 |
| [7010](#event-7010)   | 0x03E5       |     24 |              8 |
| [7009](#event-7009)   | 0x03FD       |     32 |             10 |
| [7012](#event-7012)   | 0x041D       |     24 |              8 |
| [7014](#event-7014)   | 0x0435       |    967 |            202 |

## DAT References (imed_data)

|   Index | Hex Value   |   Dec Value |
|---------|-------------|-------------|
|       0 | 0x2AC0      |       10944 |
|       1 | 0x2AC1      |       10945 |
|       2 | 0x2AC2      |       10946 |
|       3 | 0x0000      |           0 |
|       4 | 0x0001      |           1 |
|       5 | 0x2AE9      |       10985 |
|       6 | 0x0003      |           3 |
|       7 | 0x2AD5      |       10965 |
|       8 | 0x0078      |         120 |
|       9 | 0x0009      |           9 |
|      10 | 0x2AD7      |       10967 |
|      11 | 0x2AD3      |       10963 |
|      12 | 0x2AD4      |       10964 |
|      13 | 0x0002      |           2 |
|      14 | 0x1914      |        6420 |
|      15 | 0x0004      |           4 |
|      16 | 0x2AC4      |       10948 |
|      17 | 0x2AC5      |       10949 |
|      18 | 0x2AC6      |       10950 |
|      19 | 0x2AC7      |       10951 |
|      20 | 0x2AC8      |       10952 |
|      21 | 0x2AC9      |       10953 |
|      22 | 0x2ACA      |       10954 |
|      23 | 0x2ACB      |       10955 |
|      24 | 0x2ACC      |       10956 |
|      25 | 0x2ACD      |       10957 |
|      26 | 0x2ACE      |       10958 |
|      27 | 0x2AE7      |       10983 |
|      28 | 0x2AE8      |       10984 |
|      29 | 0x2ACF      |       10959 |
|      30 | 0x000F      |          15 |
|      31 | 0x003C      |          60 |
|      32 | 0x0010      |          16 |
|      33 | 0x001F      |          31 |
|      34 | 0x2AD1      |       10961 |
|      35 | 0x2AD0      |       10960 |
|      36 | 0x2ADC      |       10972 |
|      37 | 0x2ADD      |       10973 |
|      38 | 0x2AD6      |       10966 |
|      39 | 0x2ADE      |       10974 |
|      40 | 0x0005      |           5 |
|      41 | 0x2ADF      |       10975 |
|      42 | 0x2AE0      |       10976 |
|      43 | 0x2AE1      |       10977 |
|      44 | 0x0007      |           7 |
|      45 | 0x2AE3      |       10979 |
|      46 | 0x2AE4      |       10980 |
|      47 | 0x2AE2      |       10978 |
|      48 | 0x2AF5      |       10997 |
|      49 | 0x2AE5      |       10981 |
|      50 | 0x2ADB      |       10971 |
|      51 | 0x2AD9      |       10969 |
|      52 | 0x2ADA      |       10970 |
|      53 | 0x2AE6      |       10982 |
|      54 | 0x2AC3      |       10947 |
|      55 | 0x000A      |          10 |
|      56 | 0xFFFAF6A4  |  4294637220 |
|      57 | 0xFFFD6178  |  4294795640 |
|      58 | 0xFFFD589D  |  4294793373 |
|      59 | 0xFFFF12EC  |  4294906604 |
|      60 | 0xA7F8      |       43000 |
|      61 | 0x1C7F2     |      116722 |
|      62 | 0x276C7     |      161479 |
|      63 | 0x40DB8     |      265656 |
|      64 | 0x6EF08     |      454408 |
|      65 | 0x52A17     |      338455 |
|      66 | 0x470D0     |      291024 |
|      67 | 0x4C423     |      312355 |
|      68 | 0x31D22     |      204066 |
|      69 | 0x1FF3C     |      130876 |
|      70 | 0x318F8     |      203000 |
|      71 | 0x44FFB     |      282619 |
|      72 | 0x26528     |      156968 |
|      73 | 0x2B3A9     |      177065 |
|      74 | 0x3FE9F     |      261791 |
|      75 | 0x4A9F5     |      305653 |
|      76 | 0x0021      |          33 |
|      77 | 0x4023      |       16419 |
|      78 | 0x3A5B      |       14939 |
|      79 | 0x4109      |       16649 |
|      80 | 0x5DBF      |       23999 |
|      81 | 0x7D00      |       32000 |
|      82 | 0x5DC0      |       24000 |
|      83 | 0x647A      |       25722 |
|      84 | 0x7B0B      |       31499 |
|      85 | 0x997F      |       39295 |
|      86 | 0xFFFFFBB5  |  4294966197 |
|      87 | 0x082B      |        2091 |
|      88 | 0xFFFFFE7B  |  4294966907 |
|      89 | 0xFFFFFB62  |  4294966114 |
|      90 | 0xFFFFF5FF  |  4294964735 |
|      91 | 0x0579      |        1401 |
|      92 | 0xFFFFFA4B  |  4294965835 |
|      93 | 0x0752      |        1874 |
|      94 | 0xFFFFF6C4  |  4294964932 |
|      95 | 0x094A      |        2378 |
|      96 | 0xD9C9      |       55753 |
|      97 | 0x74DD      |       29917 |
|      98 | 0x4318      |       17176 |
|      99 | 0x7431      |       29745 |
|     100 | 0xBAF9      |       47865 |
|     101 | 0xE699      |       59033 |
|     102 | 0x127F1     |       75761 |
|     103 | 0x8163      |       33123 |
|     104 | 0x5BDEB     |      376299 |
|     105 | 0x54E30     |      347696 |
|     106 | 0x4F65F     |      325215 |
|     107 | 0x4E265     |      320101 |
|     108 | 0x520BD     |      336061 |
|     109 | 0x57CD8     |      359640 |
|     110 | 0x58EDD     |      364253 |
|     111 | 0x34D72     |      216434 |
|     112 | 0x3546B     |      218219 |
|     113 | 0x31D83     |      204163 |
|     114 | 0x2EA94     |      191124 |
|     115 | 0x2E66D     |      190061 |
|     116 | 0x31F05     |      204549 |
|     117 | 0x2FAA3     |      195235 |
|     118 | 0x31EFC     |      204540 |
|     119 | 0x4988A     |      301194 |
|     120 | 0x41445     |      267333 |
|     121 | 0x46AF7     |      289527 |
|     122 | 0x4AE08     |      306696 |
|     123 | 0x4F2C6     |      324294 |
|     124 | 0x4F582     |      324994 |
|     125 | 0x4351A     |      275738 |
|     126 | 0x6435      |       25653 |
|     127 | 0x5BCC      |       23500 |
|     128 | 0x60E2      |       24802 |
|     129 | 0x5CC4      |       23748 |
|     130 | 0x5DA6      |       23974 |
|     131 | 0x5F50      |       24400 |
|     132 | 0x5E20      |       24096 |
|     133 | 0x9350      |       37712 |
|     134 | 0x7D9B      |       32155 |
|     135 | 0x9BA1      |       39841 |
|     136 | 0x9E48      |       40520 |
|     137 | 0x9F52      |       40786 |
|     138 | 0x9C71      |       40049 |
|     139 | 0x7CFC      |       31996 |
|     140 | 0x0032      |          50 |
|     141 | 0x0028      |          40 |
|     142 | 0x001E      |          30 |
|     143 | 0x0006      |           6 |
|     144 | 0x000C      |          12 |
|     145 | 0x0038      |          56 |
|     146 | 0x0069      |         105 |
|     147 | 0x0067      |         103 |
|     148 | 0x0068      |         104 |
|     149 | 0x008C      |         140 |
|     150 | 0x007A      |         122 |
|     151 | 0x00F3      |         243 |
|     152 | 0x00F4      |         244 |
|     153 | 0x023C      |         572 |
|     154 | 0x04D5      |        1237 |

## String References

- **6420**: You do not have enough gil.
- **10946**: What will you do? [Undertake training./Hear an explanation./What's the qualifying time?/What's the fastest time?/Nothing.]
- **10964**: Give to the children? [I live to give!/Charity begins at home.]
- **10973**: What will you do? [Hear an explanation./Where're the smile helpers again?/Cancel the training./Nothing.]
- **10978**: You obtain $1!
- **10997**: You obtain $1 $0 !

## Events

### Event 65535

#### Metadata

| Field        | Value   |
|--------------|---------|
| Entrypoint   | 0x0000  |
| Data Size    | 1 bytes |
| Instructions | 1       |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0000: 00                                                .               
```

#### Opcodes

```
  0: 0x0000 [0x00] END_REQSTACK()
```

### Event 7001

#### Metadata

| Field        | Value     |
|--------------|-----------|
| Entrypoint   | 0x0001    |
| Data Size    | 540 bytes |
| Instructions | 48        |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0000:    1E F0 FF FF 7F 2B 2C  82 07 01 00 80 23 2B 2C   .....+,.....#+,
0010: 82 07 01 01 80 23 24 02  80 03 80 03 80 25 02 00  .....#$......%..
0020: 10 03 80 00 FB 00 02 03  10 04 80 00 3F 00 2B 2C  ............?.+,
0030: 82 07 01 05 80 23 03 01  10 06 80 21 01 F8 00 02  .....#.....!....
0040: 04 10 04 80 00 81 00 42  2B 2C 82 07 01 07 80 23  .......B+,.....#
0050: 1A 55 04 89 08 80 8B 08  80 09 80 01 00 02 00 53  .U.............S
0060: 6D 69 6C 65 5F 48 65 6C  70 65 72 00 00 00 00 2B  mile_Helper....+
0070: 2C 82 07 01 0A 80 23 8A  03 01 10 04 80 21 01 F8  ,.....#......!..
0080: 00 2B 2C 82 07 01 0B 80  23 24 0C 80 03 80 03 80  .+,.....#$......
0090: 25 02 00 10 03 80 00 E7  00 02 05 10 04 80 00 DB  %...............
00A0: 00 42 2B 2C 82 07 01 07  80 23 1A 55 04 89 08 80  .B+,.....#.U....
00B0: 8B 08 80 09 80 01 00 02  00 53 6D 69 6C 65 5F 48  .........Smile_H
00C0: 65 6C 70 65 72 00 00 00  00 2B 2C 82 07 01 0A 80  elper....+,.....
00D0: 23 8A 03 01 10 0D 80 21  01 E4 00 48 0E 80 03 01  #......!...H....
00E0: 10 06 80 21 01 F8 00 02  00 10 04 80 00 F8 00 03  ...!............
00F0: 01 10 0F 80 21 01 F8 00  01 1B 02 02 00 10 04 80  ....!...........
0100: 00 84 01 2B 2C 82 07 01  10 80 23 2B 2C 82 07 01  ...+,.....#+,...
0110: 11 80 23 2B 2C 82 07 01  12 80 23 2B 2C 82 07 01  ..#+,.....#+,...
0120: 13 80 23 2B 2C 82 07 01  14 80 23 2B 2C 82 07 01  ..#+,.....#+,...
0130: 15 80 23 2B 2C 82 07 01  16 80 23 2B 2C 82 07 01  ..#+,.....#+,...
0140: 17 80 23 2B 2C 82 07 01  18 80 23 2B 2C 82 07 01  ..#+,.....#+,...
0150: 19 80 23 02 04 10 03 80  00 73 01 2B 2C 82 07 01  ..#......s.+,...
0160: 1A 80 23 2B 2C 82 07 01  1B 80 23 2B 2C 82 07 01  ..#+,.....#+,...
0170: 1C 80 23 2B 2C 82 07 01  1D 80 23 03 01 10 03 80  ..#+,.....#.....
0180: 21 01 1B 02 02 00 10 0D  80 00 DB 01 41 03 80 1E  !...........A...
0190: 80 07 10 00 00 03 03 10  00 00 15 03 10 1F 80 03  ................
01A0: 04 10 00 00 3F 04 10 04  10 1F 80 41 20 80 21 80  ....?......A .!.
01B0: 07 10 00 00 03 05 10 00  00 15 05 10 1F 80 03 06  ................
01C0: 10 00 00 3F 06 10 06 10  1F 80 2B 2C 82 07 01 22  ...?......+,..."
01D0: 80 23 03 01 10 03 80 21  01 1B 02 02 00 10 06 80  .#.....!........
01E0: 00 0A 02 03 07 10 06 10  15 07 10 1F 80 03 08 10  ................
01F0: 06 10 3F 08 10 08 10 1F  80 2B 2C 82 07 01 23 80  ..?......+,...#.
0200: 23 03 01 10 03 80 21 01  1B 02 02 00 10 0F 80 00  #.....!.........
0210: 1B 02 03 01 10 0F 80 21  01 1B 02 21 00           .......!...!.   
```

#### Opcodes

```
  0: 0x0001 [0x1E] EventEntity looks at LocalPlayer and starts talking
  1: 0x0006 [0x2B] Neriss (ID: 17269292/0x0107822C) [10944*]:
    → "Hi there! I'm your friendly neighborhood smile sergeant! I'm in charge of running the [beginner/intermediate/advanced] level Smilebringer Boot Camp in this area."
  2: 0x000D [0x23] WAIT_FOR_DIALOG_INTERACTION
  3: 0x000E [0x2B] Neriss (ID: 17269292/0x0107822C) [10945*]:
    → "I'd be thrilled to answer any questions you might have...and of course, sign you up, too! You'd like that, wouldn't you? Hmmm?"
  4: 0x0015 [0x23] WAIT_FOR_DIALOG_INTERACTION
  5: 0x0016 [0x24] CREATE_DIALOG(message_id=10946*, default_option=0*, option_flags=0*)
    → "What will you do? [Undertake training./Hear an explanation./What's the qualifying time?/What's the fastest time?/Nothing.]"
  6: 0x001D [0x25] WAIT_DIALOG_SELECT()
  7: 0x001E [0x02] IF !(Work_Zone[0] == 0*) GOTO 0x00FB
  8: 0x0026 [0x02] IF !(Work_Zone[3] == 1*) GOTO 0x003F
  9: 0x002E [0x2B] Neriss (ID: 17269292/0x0107822C) [10985*]:
    → "Awww, seeing the way you're simply bursting with enthusiasm makes me want to cry tears of joy. But you have to be cruel to be kind. No more training until tomorrow, young [man/lady]! No siree!"
 10: 0x0035 [0x23] WAIT_FOR_DIALOG_INTERACTION
 11: 0x0036 [0x03] Work_Zone[1] = 3*
 12: 0x003B [0x21] END_EVENT

SUBROUTINE_00F8:
 13: 0x00F8 [0x01] GOTO 0x021B
 14: 0x00FB [0x02] IF !(Work_Zone[0] == 1*) GOTO 0x0184
 15: 0x0103 [0x2B] Neriss (ID: 17269292/0x0107822C) [10948*]:
    → "It'd be my pleasure! Here goes nothing! Ahem! The Smilebringer Boot Camp was established for two purposes, the first being the ongoing training and conditioning of smilebringers. The second--and this is where you come in, yay!--is for us to scout hopefuls from the adventurer community to join our jovial ranks!"
 16: 0x010A [0x23] WAIT_FOR_DIALOG_INTERACTION
 17: 0x010B [0x2B] Neriss (ID: 17269292/0x0107822C) [10949*]:
    → "The population of vicious monsters has grown in recent years, you see. And because of this, more smilebringers were being--<gasp!>--injured in the line of duty than ever before."
 18: 0x0112 [0x23] WAIT_FOR_DIALOG_INTERACTION
 19: 0x0113 [0x2B] Neriss (ID: 17269292/0x0107822C) [10950*]:
    → "But we weren't going to take that sitting down, no siree. The day we smilebringers stop doing our job--that is, bringing smiles--is the day the world ends!"
 20: 0x011A [0x23] WAIT_FOR_DIALOG_INTERACTION
 21: 0x011B [0x2B] Neriss (ID: 17269292/0x0107822C) [10951*]:
    → "Whatever the predicament, and no matter the size of the ruszor-inflicted gash wound, we will--we must--pull through...for the sake of the children!"
 22: 0x0122 [0x23] WAIT_FOR_DIALOG_INTERACTION
 23: 0x0123 [0x2B] Neriss (ID: 17269292/0x0107822C) [10952*]:
    → "With these heart-warming ideals in mind, the Smilebringer Boot Camp was established. Ta-da! Smilebringers graduating from this camp are better equipped to meet the ever-changing needs of these unstable times we live in."
 24: 0x012A [0x23] WAIT_FOR_DIALOG_INTERACTION
 25: 0x012B [0x2B] Neriss (ID: 17269292/0x0107822C) [10953*]:
    → "The rules are as simple as sunshine. Smilebringer's honor! When I give you the word, run to each of the checkpoints we've set up in the area. "Smile helpers" are posted at each, so you can't miss them!"
 26: 0x0132 [0x23] WAIT_FOR_DIALOG_INTERACTION
 27: 0x0133 [0x2B] Neriss (ID: 17269292/0x0107822C) [10954*]:
    → "Talk to the smile helpers and they'll confirm your passage. The route you take is completely up to you--just be sure you don't miss any checkpoints. You got all that? Hmmm?"
 28: 0x013A [0x23] WAIT_FOR_DIALOG_INTERACTION
 29: 0x013B [0x2B] Neriss (ID: 17269292/0x0107822C) [10955*]:
    → "I'll mark each smile helper's location on your map beforehand, so you don't need to worry about getting lost."
 30: 0x0142 [0x23] WAIT_FOR_DIALOG_INTERACTION
 31: 0x0143 [0x2B] Neriss (ID: 17269292/0x0107822C) [10956*]:
    → "Once you've finished visiting all the checkpoints, you have to--you guessed it!--come back and report to me. It's okay, I won't bite!"
 32: 0x014A [0x23] WAIT_FOR_DIALOG_INTERACTION
 33: 0x014B [0x2B] Neriss (ID: 17269292/0x0107822C) [10957*]:
    → "What I will do, though, is tell you how quickly you completed the course. After all, to a smilebringer, speed is everything. Some folks run faster with a fell beast hot on their tail, but we smilebringers need no more than the thought of a child's laughter to spur us on! Zoom zoom!"
 34: 0x0152 [0x23] WAIT_FOR_DIALOG_INTERACTION
 35: 0x0153 [0x02] IF !(Work_Zone[4] == 0*) GOTO 0x0173
 36: 0x015B [0x2B] Neriss (ID: 17269292/0x0107822C) [10958*]:
    → "Oh, and I should mention that at this time of year, we're collecting donations in the form of a small fee to participate in the training. All proceeds go towards presents for the children during the Starlight Celebration. Thank you for your cooperation!"
 37: 0x0162 [0x23] WAIT_FOR_DIALOG_INTERACTION
 38: 0x0163 [0x2B] Neriss (ID: 17269292/0x0107822C) [10983*]:
    → "It might surprise you to learn this, but even the toughest--grrrrrr!--and roughest of us smilebringers need rest and relaxation. Oh, wipe that grin off your face! I knew you wouldn't believe me!"
 39: 0x016A [0x23] WAIT_FOR_DIALOG_INTERACTION
 40: 0x016B [0x2B] Neriss (ID: 17269292/0x0107822C) [10984*]:
    → "Aaanyway! What I meant to say is, "work hard, play hard" is our motto, and it applies to boot camp participants, too. After you complete a run with a qualifying time--yay!--you're required to rest up until the next day (Vana'diel time)."
 41: 0x0172 [0x23] WAIT_FOR_DIALOG_INTERACTION
 42: 0x0173 [0x2B] Neriss (ID: 17269292/0x0107822C) [10959*]:
    → "One more thing you should know. If you venture out of the area, the training will automatically be canceled. But I can trust you not to walk out on your favorite smile sergeant, right? Hmmm?"
 43: 0x017A [0x23] WAIT_FOR_DIALOG_INTERACTION
 44: 0x017B [0x03] Work_Zone[1] = 0*
 45: 0x0180 [0x21] END_EVENT

SUBROUTINE_021B:
 46: 0x021B [0x21] END_EVENT
 47: 0x021C [0x00] END_REQSTACK()
```

#### Data or dead code:

```
# Dead code (unreachable instructions):
     0x003C [0x01] GOTO 0x00F8
     0x006D [0x00] END_REQSTACK()
     0x006E [0x00] END_REQSTACK()
     0x006F [0x2B] Neriss (ID: 17269292/0x0107822C) [10967*]:
    → "There! I've marked each smile helper's location for you. If you ever lose your bearings in the middle of a run, just open up your map and everything will be aaall right. Trust me! <Wink>"
     0x0076 [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x0077 [0x8A] CLOSE_MAP()
     0x0078 [0x03] Work_Zone[1] = 1*
     0x007D [0x21] END_EVENT
     0x007E [0x01] GOTO 0x00F8
     0x00C7 [0x00] END_REQSTACK()
     0x00C8 [0x00] END_REQSTACK()
     0x00C9 [0x2B] Neriss (ID: 17269292/0x0107822C) [10967*]:
    → "There! I've marked each smile helper's location for you. If you ever lose your bearings in the middle of a run, just open up your map and everything will be aaall right. Trust me! <Wink>"
     0x00D0 [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x00D1 [0x8A] CLOSE_MAP()
     0x00D2 [0x03] Work_Zone[1] = 2*
     0x00D7 [0x21] END_EVENT
     0x00D8 [0x01] GOTO 0x00E4
     0x00E4 [0x01] GOTO 0x00F8
     0x00F5 [0x01] GOTO 0x00F8
# Dead code (unreachable instructions):
     0x0181 [0x01] GOTO 0x021B
     0x01D8 [0x01] GOTO 0x021B
     0x0207 [0x01] GOTO 0x021B
     0x0218 [0x01] GOTO 0x021B
```

### Event 7004

#### Metadata

| Field        | Value     |
|--------------|-----------|
| Entrypoint   | 0x021D    |
| Data Size    | 267 bytes |
| Instructions | 38        |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0210:                                         1E F0 FF               ...
0220: FF 7F 2B 2C 82 07 01 24  80 23 24 25 80 03 80 03  ..+,...$.#$%....
0230: 80 25 02 00 10 03 80 00  B4 02 2B 2C 82 07 01 10  .%........+,....
0240: 80 23 2B 2C 82 07 01 11  80 23 2B 2C 82 07 01 12  .#+,.....#+,....
0250: 80 23 2B 2C 82 07 01 13  80 23 2B 2C 82 07 01 14  .#+,.....#+,....
0260: 80 23 2B 2C 82 07 01 15  80 23 2B 2C 82 07 01 16  .#+,.....#+,....
0270: 80 23 2B 2C 82 07 01 17  80 23 2B 2C 82 07 01 18  .#+,.....#+,....
0280: 80 23 2B 2C 82 07 01 19  80 23 02 08 10 03 80 00  .#+,.....#......
0290: A2 02 2B 2C 82 07 01 1B  80 23 2B 2C 82 07 01 1C  ..+,.....#+,....
02A0: 80 23 2B 2C 82 07 01 1D  80 23 03 01 10 03 80 21  .#+,.....#.....!
02B0: 00 01 26 03 02 00 10 04  80 00 F6 02 2B 2C 82 07  ..&.........+,..
02C0: 01 07 80 23 1A 55 04 89  08 80 8B 08 80 09 80 01  ...#.U..........
02D0: 00 02 00 53 6D 69 6C 65  5F 48 65 6C 70 65 72 00  ...Smile_Helper.
02E0: 00 00 00 2B 2C 82 07 01  26 80 23 8A 03 01 10 03  ...+,...&.#.....
02F0: 80 21 00 01 26 03 02 00  10 0D 80 00 14 03 42 1A  .!..&.........B.
0300: 98 04 2B 2C 82 07 01 27  80 23 03 01 10 28 80 21  ..+,...'.#...(.!
0310: 00 01 26 03 02 00 10 06  80 00 26 03 03 01 10 06  ..&.......&.....
0320: 80 21 00 01 26 03 21 00                           .!..&.!.        
```

#### Opcodes

```
  0: 0x021D [0x1E] EventEntity looks at LocalPlayer and starts talking
  1: 0x0222 [0x2B] Neriss (ID: 17269292/0x0107822C) [10972*]:
    → "Oh, what're you doing here? Aren't you in the middle of a run right now? Couldn't stand to be parted from your favorite smile sergeant, maybe? Teehee!"
  2: 0x0229 [0x23] WAIT_FOR_DIALOG_INTERACTION
  3: 0x022A [0x24] CREATE_DIALOG(message_id=10973*, default_option=0*, option_flags=0*)
    → "What will you do? [Hear an explanation./Where're the smile helpers again?/Cancel the training./Nothing.]"
  4: 0x0231 [0x25] WAIT_DIALOG_SELECT()
  5: 0x0232 [0x02] IF !(Work_Zone[0] == 0*) GOTO 0x02B4
  6: 0x023A [0x2B] Neriss (ID: 17269292/0x0107822C) [10948*]:
    → "It'd be my pleasure! Here goes nothing! Ahem! The Smilebringer Boot Camp was established for two purposes, the first being the ongoing training and conditioning of smilebringers. The second--and this is where you come in, yay!--is for us to scout hopefuls from the adventurer community to join our jovial ranks!"
  7: 0x0241 [0x23] WAIT_FOR_DIALOG_INTERACTION
  8: 0x0242 [0x2B] Neriss (ID: 17269292/0x0107822C) [10949*]:
    → "The population of vicious monsters has grown in recent years, you see. And because of this, more smilebringers were being--<gasp!>--injured in the line of duty than ever before."
  9: 0x0249 [0x23] WAIT_FOR_DIALOG_INTERACTION
 10: 0x024A [0x2B] Neriss (ID: 17269292/0x0107822C) [10950*]:
    → "But we weren't going to take that sitting down, no siree. The day we smilebringers stop doing our job--that is, bringing smiles--is the day the world ends!"
 11: 0x0251 [0x23] WAIT_FOR_DIALOG_INTERACTION
 12: 0x0252 [0x2B] Neriss (ID: 17269292/0x0107822C) [10951*]:
    → "Whatever the predicament, and no matter the size of the ruszor-inflicted gash wound, we will--we must--pull through...for the sake of the children!"
 13: 0x0259 [0x23] WAIT_FOR_DIALOG_INTERACTION
 14: 0x025A [0x2B] Neriss (ID: 17269292/0x0107822C) [10952*]:
    → "With these heart-warming ideals in mind, the Smilebringer Boot Camp was established. Ta-da! Smilebringers graduating from this camp are better equipped to meet the ever-changing needs of these unstable times we live in."
 15: 0x0261 [0x23] WAIT_FOR_DIALOG_INTERACTION
 16: 0x0262 [0x2B] Neriss (ID: 17269292/0x0107822C) [10953*]:
    → "The rules are as simple as sunshine. Smilebringer's honor! When I give you the word, run to each of the checkpoints we've set up in the area. "Smile helpers" are posted at each, so you can't miss them!"
 17: 0x0269 [0x23] WAIT_FOR_DIALOG_INTERACTION
 18: 0x026A [0x2B] Neriss (ID: 17269292/0x0107822C) [10954*]:
    → "Talk to the smile helpers and they'll confirm your passage. The route you take is completely up to you--just be sure you don't miss any checkpoints. You got all that? Hmmm?"
 19: 0x0271 [0x23] WAIT_FOR_DIALOG_INTERACTION
 20: 0x0272 [0x2B] Neriss (ID: 17269292/0x0107822C) [10955*]:
    → "I'll mark each smile helper's location on your map beforehand, so you don't need to worry about getting lost."
 21: 0x0279 [0x23] WAIT_FOR_DIALOG_INTERACTION
 22: 0x027A [0x2B] Neriss (ID: 17269292/0x0107822C) [10956*]:
    → "Once you've finished visiting all the checkpoints, you have to--you guessed it!--come back and report to me. It's okay, I won't bite!"
 23: 0x0281 [0x23] WAIT_FOR_DIALOG_INTERACTION
 24: 0x0282 [0x2B] Neriss (ID: 17269292/0x0107822C) [10957*]:
    → "What I will do, though, is tell you how quickly you completed the course. After all, to a smilebringer, speed is everything. Some folks run faster with a fell beast hot on their tail, but we smilebringers need no more than the thought of a child's laughter to spur us on! Zoom zoom!"
 25: 0x0289 [0x23] WAIT_FOR_DIALOG_INTERACTION
 26: 0x028A [0x02] IF !(Work_Zone[8] == 0*) GOTO 0x02A2
 27: 0x0292 [0x2B] Neriss (ID: 17269292/0x0107822C) [10983*]:
    → "It might surprise you to learn this, but even the toughest--grrrrrr!--and roughest of us smilebringers need rest and relaxation. Oh, wipe that grin off your face! I knew you wouldn't believe me!"
 28: 0x0299 [0x23] WAIT_FOR_DIALOG_INTERACTION
 29: 0x029A [0x2B] Neriss (ID: 17269292/0x0107822C) [10984*]:
    → "Aaanyway! What I meant to say is, "work hard, play hard" is our motto, and it applies to boot camp participants, too. After you complete a run with a qualifying time--yay!--you're required to rest up until the next day (Vana'diel time)."
 30: 0x02A1 [0x23] WAIT_FOR_DIALOG_INTERACTION
 31: 0x02A2 [0x2B] Neriss (ID: 17269292/0x0107822C) [10959*]:
    → "One more thing you should know. If you venture out of the area, the training will automatically be canceled. But I can trust you not to walk out on your favorite smile sergeant, right? Hmmm?"
 32: 0x02A9 [0x23] WAIT_FOR_DIALOG_INTERACTION
 33: 0x02AA [0x03] Work_Zone[1] = 0*
 34: 0x02AF [0x21] END_EVENT
 35: 0x02B0 [0x00] END_REQSTACK()

SUBROUTINE_0326:
 36: 0x0326 [0x21] END_EVENT
 37: 0x0327 [0x00] END_REQSTACK()
```

#### Data or dead code:

```
# Dead code (unreachable instructions):
     0x02B1 [0x01] GOTO 0x0326
     0x02E1 [0x00] END_REQSTACK()
     0x02E2 [0x00] END_REQSTACK()
     0x02E3 [0x2B] Neriss (ID: 17269292/0x0107822C) [10966*]:
    → "You'll find each of the ten smile helpers by the points I've indicated. Don't forget to talk with every one of them or it won't count, okay?"
     0x02EA [0x23] WAIT_FOR_DIALOG_INTERACTION
     0x02EB [0x8A] CLOSE_MAP()
     0x02EC [0x03] Work_Zone[1] = 0*
     0x02F1 [0x21] END_EVENT
     0x02F2 [0x00] END_REQSTACK()
     0x02F3 [0x01] GOTO 0x0326
     0x0311 [0x01] GOTO 0x0326
     0x0323 [0x01] GOTO 0x0326
```

### Event 7005

#### Metadata

| Field        | Value     |
|--------------|-----------|
| Entrypoint   | 0x0328    |
| Data Size    | 135 bytes |
| Instructions | 34        |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0320:                          42 1A 98 04 1E F0 FF FF          B.......
0330: 7F 03 06 10 02 10 15 06  10 1F 80 03 07 10 02 10  ................
0340: 3F 07 10 07 10 1F 80 2B  2C 82 07 01 29 80 23 3E  ?......+,...).#>
0350: 09 10 0D 80 5E 03 2B 2C  82 07 01 2A 80 23 2B 2C  ....^.+,...*.#+,
0360: 82 07 01 2B 80 23 3E 09  10 2C 80 75 03 2B 2C 82  ...+.#>..,.u.+,.
0370: 07 01 2D 80 23 2B 2C 82  07 01 2E 80 23 3E 09 10  ..-.#+,.....#>..
0380: 03 80 8B 03 48 2F 80 23  01 A8 03 02 08 10 04 80  ....H/.#........
0390: 00 9A 03 48 2F 80 23 01  A8 03 03 02 10 03 10 03  ...H/.#.........
03A0: 03 10 08 10 48 30 80 23  03 01 10 28 80 21 00     ....H0.#...(.!. 
```

#### Opcodes

```
  0: 0x0328 [0x42] SET_CLI_EVENT_CANCEL_DATA()
  1: 0x0329 [0x1A] CALL_SUBROUTINE(address=0x0498)
  2: 0x032C [0x1E] EventEntity looks at LocalPlayer and starts talking
  3: 0x0331 [0x03] Work_Zone[6] = Work_Zone[2]
  4: 0x0336 [0x15] Work_Zone[6] /= 60*
  5: 0x033B [0x03] Work_Zone[7] = Work_Zone[2]
  6: 0x0340 [0x3F] Work_Zone[7] = Work_Zone[7] % 60*
  7: 0x0347 [0x2B] Neriss (ID: 17269292/0x0107822C) [10975*]:
    → "Welcome back! Let's check your time, shall we? You cleared the course in $4 minute[/s] $5 second[/s]."
  8: 0x034E [0x23] WAIT_FOR_DIALOG_INTERACTION
  9: 0x034F [0x3E] IF !(Work_Zone[9] bit 2*) GOTO 0x035E
 10: 0x0356 [0x2B] Neriss (ID: 17269292/0x0107822C) [10976*]:
    → "Oh my Goddess! You've set a new record for this area! Hooray! Somebody, break out the fireworks!"
 11: 0x035D [0x23] WAIT_FOR_DIALOG_INTERACTION
 12: 0x035E [0x2B] Neriss (ID: 17269292/0x0107822C) [10977*]:
    → "You've successfully qualified! Yay! Here's a little something to commemorate this marvelous achievement."
 13: 0x0365 [0x23] WAIT_FOR_DIALOG_INTERACTION
 14: 0x0366 [0x3E] IF !(Work_Zone[9] bit 7*) GOTO 0x0375
 15: 0x036D [0x2B] Neriss (ID: 17269292/0x0107822C) [10979*]:
    → "Ah, just the knowledge that there're capable adventurers like you around helps me sleep better at night. The children...they're in good hands... <Sob>"
 16: 0x0374 [0x23] WAIT_FOR_DIALOG_INTERACTION
 17: 0x0375 [0x2B] Neriss (ID: 17269292/0x0107822C) [10980*]:
    → "You did just fine, but don't be resting on those laurels, you hear? As we smilebringers say, there's always a bigger, brighter smile to be had!"
 18: 0x037C [0x23] WAIT_FOR_DIALOG_INTERACTION
 19: 0x037D [0x3E] IF !(Work_Zone[9] bit 0*) GOTO 0x038B
 20: 0x0384 [0x48] [System] [10978*]:
    → "You obtain $1!"
 21: 0x0387 [0x23] WAIT_FOR_DIALOG_INTERACTION
 22: 0x0388 [0x01] GOTO 0x03A8
 23: 0x038B [0x02] IF !(Work_Zone[8] == 1*) GOTO 0x039A
 24: 0x0393 [0x48] [System] [10978*]:
    → "You obtain $1!"
 25: 0x0396 [0x23] WAIT_FOR_DIALOG_INTERACTION
 26: 0x0397 [0x01] GOTO 0x03A8
 27: 0x039A [0x03] Work_Zone[2] = Work_Zone[3]
 28: 0x039F [0x03] Work_Zone[3] = Work_Zone[8]
 29: 0x03A4 [0x48] [System] [10997*]:
    → "You obtain $1 $0 !"
 30: 0x03A7 [0x23] WAIT_FOR_DIALOG_INTERACTION

SUBROUTINE_03A8:
 31: 0x03A8 [0x03] Work_Zone[1] = 5*
 32: 0x03AD [0x21] END_EVENT
 33: 0x03AE [0x00] END_REQSTACK()
```

### Event 7011

#### Metadata

| Field        | Value    |
|--------------|----------|
| Entrypoint   | 0x03AF   |
| Data Size    | 54 bytes |
| Instructions | 14       |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
03A0:                                               42                 B
03B0: 1A 98 04 1E F0 FF FF 7F  03 06 10 02 10 15 06 10  ................
03C0: 1F 80 03 07 10 02 10 3F  07 10 07 10 1F 80 2B 2C  .......?......+,
03D0: 82 07 01 29 80 23 2B 2C  82 07 01 31 80 23 03 01  ...).#+,...1.#..
03E0: 10 28 80 21 00                                    .(.!.           
```

#### Opcodes

```
  0: 0x03AF [0x42] SET_CLI_EVENT_CANCEL_DATA()
  1: 0x03B0 [0x1A] CALL_SUBROUTINE(address=0x0498)
  2: 0x03B3 [0x1E] EventEntity looks at LocalPlayer and starts talking
  3: 0x03B8 [0x03] Work_Zone[6] = Work_Zone[2]
  4: 0x03BD [0x15] Work_Zone[6] /= 60*
  5: 0x03C2 [0x03] Work_Zone[7] = Work_Zone[2]
  6: 0x03C7 [0x3F] Work_Zone[7] = Work_Zone[7] % 60*
  7: 0x03CE [0x2B] Neriss (ID: 17269292/0x0107822C) [10975*]:
    → "Welcome back! Let's check your time, shall we? You cleared the course in $4 minute[/s] $5 second[/s]."
  8: 0x03D5 [0x23] WAIT_FOR_DIALOG_INTERACTION
  9: 0x03D6 [0x2B] Neriss (ID: 17269292/0x0107822C) [10981*]:
    → "Oh no! I'm afraid your time falls short of qualification! As much as I want to give you a passing grade, headquarters wouldn't let me hear the end of it."
 10: 0x03DD [0x23] WAIT_FOR_DIALOG_INTERACTION
 11: 0x03DE [0x03] Work_Zone[1] = 5*
 12: 0x03E3 [0x21] END_EVENT
 13: 0x03E4 [0x00] END_REQSTACK()
```

### Event 7010

#### Metadata

| Field        | Value    |
|--------------|----------|
| Entrypoint   | 0x03E5   |
| Data Size    | 24 bytes |
| Instructions | 8        |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
03E0:                42 1E F0  FF FF 7F 1A 98 04 2B 2C       B........+,
03F0: 82 07 01 32 80 23 03 01  10 28 80 21 00           ...2.#...(.!.   
```

#### Opcodes

```
  0: 0x03E5 [0x42] SET_CLI_EVENT_CANCEL_DATA()
  1: 0x03E6 [0x1E] EventEntity looks at LocalPlayer and starts talking
  2: 0x03EB [0x1A] CALL_SUBROUTINE(address=0x0498)
  3: 0x03EE [0x2B] Neriss (ID: 17269292/0x0107822C) [10971*]:
    → "Oh, you're back! I was afraid we'd lost you to some foul creature prowling the wilderness. Let's start from the top, shall we?"
  4: 0x03F5 [0x23] WAIT_FOR_DIALOG_INTERACTION
  5: 0x03F6 [0x03] Work_Zone[1] = 5*
  6: 0x03FB [0x21] END_EVENT
  7: 0x03FC [0x00] END_REQSTACK()
```

### Event 7009

#### Metadata

| Field        | Value    |
|--------------|----------|
| Entrypoint   | 0x03FD   |
| Data Size    | 32 bytes |
| Instructions | 10       |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
03F0:                                         42 1E F0               B..
0400: FF FF 7F 1A 98 04 2B 2C  82 07 01 33 80 23 2B 2C  ......+,...3.#+,
0410: 82 07 01 34 80 23 03 01  10 28 80 21 00           ...4.#...(.!.   
```

#### Opcodes

```
  0: 0x03FD [0x42] SET_CLI_EVENT_CANCEL_DATA()
  1: 0x03FE [0x1E] EventEntity looks at LocalPlayer and starts talking
  2: 0x0403 [0x1A] CALL_SUBROUTINE(address=0x0498)
  3: 0x0406 [0x2B] Neriss (ID: 17269292/0x0107822C) [10969*]:
    → "Since the Starlight Celebration has ended, we've had to make a few changes."
  4: 0x040D [0x23] WAIT_FOR_DIALOG_INTERACTION
  5: 0x040E [0x2B] Neriss (ID: 17269292/0x0107822C) [10970*]:
    → "But the Smilebringer Boot Camp will be available for off-season conditioning the whole year round, so come back whenever you're in the mood. I'll be waiting for you!"
  6: 0x0415 [0x23] WAIT_FOR_DIALOG_INTERACTION
  7: 0x0416 [0x03] Work_Zone[1] = 5*
  8: 0x041B [0x21] END_EVENT
  9: 0x041C [0x00] END_REQSTACK()
```

### Event 7012

#### Metadata

| Field        | Value    |
|--------------|----------|
| Entrypoint   | 0x041D   |
| Data Size    | 24 bytes |
| Instructions | 8        |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0410:                                         42 1E F0               B..
0420: FF FF 7F 1A 98 04 2B 2C  82 07 01 35 80 23 03 01  ......+,...5.#..
0430: 10 28 80 21 00                                    .(.!.           
```

#### Opcodes

```
  0: 0x041D [0x42] SET_CLI_EVENT_CANCEL_DATA()
  1: 0x041E [0x1E] EventEntity looks at LocalPlayer and starts talking
  2: 0x0423 [0x1A] CALL_SUBROUTINE(address=0x0498)
  3: 0x0426 [0x2B] Neriss (ID: 17269292/0x0107822C) [10982*]:
    → "Oh no! Please don't be mad, but...I forgot to record your time. I hope you'll find it in your heart to forgive this scatterbrain of a smile sergeant..."
  4: 0x042D [0x23] WAIT_FOR_DIALOG_INTERACTION
  5: 0x042E [0x03] Work_Zone[1] = 5*
  6: 0x0433 [0x21] END_EVENT
  7: 0x0434 [0x00] END_REQSTACK()
```

### Event 7014

#### Metadata

| Field        | Value     |
|--------------|-----------|
| Entrypoint   | 0x0435    |
| Data Size    | 967 bytes |
| Instructions | 16        |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0430:                42 1E F0  FF FF 7F 1A 98 04 2B 2C       B........+,
0440: 82 07 01 00 80 23 2B 2C  82 07 01 36 80 23 03 01  .....#+,...6.#..
0450: 10 03 80 21 00 03 00 00  03 80 02 00 00 37 80 03  ...!.........7..
0460: 97 04 0B 00 00 9D 00 2C  05 01 00 00 00 9D 00 4C  .......,.......L
0470: 05 02 00 00 00 0C 00 00  8B 08 80 00 00 01 00 02  ................
0480: 00 53 6D 69 6C 65 5F 48  65 6C 70 65 72 00 00 00  .Smile_Helper...
0490: 00 0B 00 00 01 5A 04 1B  03 00 00 03 80 02 00 00  .....Z..........
04A0: 37 80 03 B1 04 A8 00 08  80 00 00 0B 00 00 01 9D  7...............
04B0: 04 1B 4E 00 2D 82 07 01  4E 00 2E 82 07 01 4E 00  ..N.-...N.....N.
04C0: 2F 82 07 01 4E 00 30 82  07 01 4E 00 31 82 07 01  /...N.0...N.1...
04D0: 4E 00 32 82 07 01 4E 00  33 82 07 01 4E 00 34 82  N.2...N.3...N.4.
04E0: 07 01 4E 00 35 82 07 01  4E 00 36 82 07 01 1B 4E  ..N.5...N.6....N
04F0: 01 2D 82 07 01 4E 01 2E  82 07 01 4E 01 2F 82 07  .-...N.....N./..
0500: 01 4E 01 30 82 07 01 4E  01 31 82 07 01 4E 01 32  .N.0...N.1...N.2
0510: 82 07 01 4E 01 33 82 07  01 4E 01 34 82 07 01 4E  ...N.3...N.4...N
0520: 01 35 82 07 01 4E 01 36  82 07 01 1B 03 80 38 80  .5...N.6......8.
0530: 39 80 3A 80 3B 80 3C 80  3D 80 3E 80 3F 80 40 80  9.:.;.<.=.>.?.@.
0540: 41 80 03 80 03 80 03 80  03 80 03 80 03 80 42 80  A.............B.
0550: 43 80 44 80 45 80 46 80  47 80 48 80 49 80 4A 80  C.D.E.F.G.H.I.J.
0560: 4B 80 03 80 03 80 03 80  03 80 03 80 03 80 4C 80  K.............L.
0570: 4D 80 4E 80 4F 80 50 80  51 80 52 80 53 80 54 80  M.N.O.P.Q.R.S.T.
0580: 55 80 03 80 03 80 03 80  03 80 03 80 03 80 56 80  U.............V.
0590: 57 80 58 80 59 80 5A 80  5B 80 5C 80 5D 80 5E 80  W.X.Y.Z.[.\.].^.
05A0: 5F 80 03 80 03 80 03 80  03 80 03 80 60 80 61 80  _...........`.a.
05B0: 62 80 63 80 64 80 65 80  66 80 67 80 68 80 69 80  b.c.d.e.f.g.h.i.
05C0: 6A 80 6B 80 41 80 6C 80  6D 80 6E 80 6F 80 70 80  j.k.A.l.m.n.o.p.
05D0: 71 80 72 80 73 80 74 80  75 80 76 80 77 80 78 80  q.r.s.t.u.v.w.x.
05E0: 79 80 7A 80 4B 80 7B 80  7C 80 7D 80 7E 80 7F 80  y.z.K.{.|.}.~...
05F0: 80 80 81 80 82 80 83 80  52 80 84 80 85 80 86 80  ........R.......
0600: 87 80 88 80 55 80 89 80  8A 80 8B 80 03 80 03 80  ....U...........
0610: 03 80 03 80 03 80 04 80  03 80 03 80 03 80 03 80  ................
0620: 0D 80 03 80 03 80 03 80  03 80 03 80 1F 80 8C 80  ................
0630: 8D 80 8E 80 8E 80 03 80  03 80 03 80 04 80 28 80  ..............(.
0640: 8F 80 28 80 8F 80 28 80  8F 80 28 80 8F 80 28 80  ..(...(...(...(.
0650: 8F 80 03 80 03 80 03 80  03 80 03 80 04 80 03 80  ................
0660: 1E 80 06 80 04 80 28 80  0D 80 2C 80 0F 80 90 80  ......(...,.....
0670: 2C 80 03 80 03 80 03 80  03 80 03 80 91 80 91 80  ,...............
0680: 91 80 91 80 91 80 91 80  91 80 91 80 91 80 03 80  ................
0690: 03 80 03 80 03 80 03 80  03 80 03 80 92 80 92 80  ................
06A0: 92 80 92 80 92 80 92 80  92 80 92 80 92 80 03 80  ................
06B0: 03 80 03 80 03 80 03 80  03 80 03 80 03 80 03 80  ................
06C0: 03 80 03 80 03 80 03 80  03 80 03 80 03 80 03 80  ................
06D0: 03 80 03 80 03 80 03 80  03 80 03 80 93 80 93 80  ................
06E0: 93 80 93 80 93 80 93 80  93 80 93 80 93 80 03 80  ................
06F0: 03 80 03 80 03 80 03 80  03 80 03 80 94 80 94 80  ................
0700: 94 80 94 80 94 80 94 80  94 80 94 80 94 80 03 80  ................
0710: 03 80 03 80 03 80 03 80  03 80 03 80 95 80 95 80  ................
0720: 95 80 95 80 95 80 95 80  95 80 95 80 95 80 03 80  ................
0730: 03 80 03 80 03 80 03 80  03 80 03 80 95 80 95 80  ................
0740: 95 80 95 80 95 80 95 80  95 80 95 80 95 80 03 80  ................
0750: 03 80 03 80 03 80 03 80  03 80 03 80 96 80 96 80  ................
0760: 96 80 96 80 96 80 96 80  96 80 96 80 96 80 03 80  ................
0770: 03 80 03 80 03 80 03 80  03 80 03 80 97 80 97 80  ................
0780: 97 80 97 80 97 80 97 80  98 80 98 80 98 80 03 80  ................
0790: 03 80 03 80 03 80 03 80  03 80 03 80 95 80 95 80  ................
07A0: 95 80 95 80 95 80 95 80  95 80 95 80 95 80 03 80  ................
07B0: 03 80 03 80 03 80 03 80  03 80 03 80 99 80 99 80  ................
07C0: 99 80 99 80 99 80 99 80  99 80 99 80 04 80 04 80  ................
07D0: 04 80 04 80 04 80 04 80  04 80 04 80 9A 80 9A 80  ................
07E0: 9A 80 9A 80 9A 80 9A 80  9A 80 9A 80 04 80 04 80  ................
07F0: 04 80 04 80 04 80 04 80  04 80 04 80              ............    
```

#### Opcodes

```
  0: 0x0435 [0x42] SET_CLI_EVENT_CANCEL_DATA()
  1: 0x0436 [0x1E] EventEntity looks at LocalPlayer and starts talking
  2: 0x043B [0x1A] CALL_SUBROUTINE(address=0x0498)
  3: 0x043E [0x2B] Neriss (ID: 17269292/0x0107822C) [10944*]:
    → "Hi there! I'm your friendly neighborhood smile sergeant! I'm in charge of running the [beginner/intermediate/advanced] level Smilebringer Boot Camp in this area."
  4: 0x0445 [0x23] WAIT_FOR_DIALOG_INTERACTION
  5: 0x0446 [0x2B] Neriss (ID: 17269292/0x0107822C) [10947*]:
    → "Fantastic! But, oh...it seems you don't have $6. Well, that's just too bad!"
  6: 0x044D [0x23] WAIT_FOR_DIALOG_INTERACTION
  7: 0x044E [0x03] Work_Zone[1] = 0*
  8: 0x0453 [0x21] END_EVENT
  9: 0x0454 [0x00] END_REQSTACK()

SUBROUTINE_0498:
 10: 0x0498 [0x03] ExtData[1]->WorkLocal[0] = 0*

SUBROUTINE_049D:
 11: 0x049D [0x02] IF !(ExtData[1]->WorkLocal[0] >= 10*) GOTO 0x04B1
 12: 0x04A5 [0xA8] MAP_MARKER_CONTROL: Reset/unlock markers (no map display), zone=120*, marker=ExtData[1]->WorkLocal[0]
 13: 0x04AB [0x0B] ExtData[1]->WorkLocal[0]++
 14: 0x04AE [0x01] GOTO 0x049D
 15: 0x04B1 [0x1B] RETURN
```

#### Data or dead code:

```
# Dead code (unreachable instructions):
     0x0455 [0x03] ExtData[1]->WorkLocal[0] = 0*
     0x045A [0x02] IF !(ExtData[1]->WorkLocal[0] >= 10*) GOTO 0x0497
     0x0462 [0x0B] ExtData[1]->WorkLocal[0]++
     0x0465 [0x9D] ExtData[1]->WorkLocal[1] = 0x052C[ExtData[1]->WorkLocal[0]] // Read WORD
     0x046D [0x9D] ExtData[1]->WorkLocal[2] = 0x054C[ExtData[1]->WorkLocal[0]] // Read WORD
     0x0475 [0x0C] ExtData[1]->WorkLocal[0]--
     0x0478 [0x8B] SET_EVENT_MARK: Add/update map marker on map 120* at (ExtData[1]->WorkLocal[1], ExtData[1]->WorkLocal[2]), index=ExtData[1]->WorkLocal[0], name=(no name)
     0x0481 [0x53] WAIT_SCHEDULER_TASK: Wait for scheduler "per" with entities [Unknown NPC (ID: 1701603693/0x656C696D), Unknown NPC (ID: 1818576991/0x6C65485F)]
     0x048E [0x00] END_REQSTACK()
     0x048F [0x00] END_REQSTACK()
     0x0490 [0x00] END_REQSTACK()
     0x0491 [0x0B] ExtData[1]->WorkLocal[0]++
     0x0494 [0x01] GOTO 0x045A
     0x0497 [0x1B] RETURN
# Dead code (unreachable instructions):
     0x04B2 [0x4E] SET_ENTITY_HIDE_FLAG: Show Smile Helper (ID: 17269293/0x0107822D)
     0x04B8 [0x4E] SET_ENTITY_HIDE_FLAG: Show Smile Helper (ID: 17269294/0x0107822E)
     0x04BE [0x4E] SET_ENTITY_HIDE_FLAG: Show Smile Helper (ID: 17269295/0x0107822F)
     0x04C4 [0x4E] SET_ENTITY_HIDE_FLAG: Show Smile Helper (ID: 17269296/0x01078230)
     0x04CA [0x4E] SET_ENTITY_HIDE_FLAG: Show Smile Helper (ID: 17269297/0x01078231)
     0x04D0 [0x4E] SET_ENTITY_HIDE_FLAG: Show Smile Helper (ID: 17269298/0x01078232)
     0x04D6 [0x4E] SET_ENTITY_HIDE_FLAG: Show Smile Helper (ID: 17269299/0x01078233)
     0x04DC [0x4E] SET_ENTITY_HIDE_FLAG: Show Smile Helper (ID: 17269300/0x01078234)
     0x04E2 [0x4E] SET_ENTITY_HIDE_FLAG: Show Smile Helper (ID: 17269301/0x01078235)
     0x04E8 [0x4E] SET_ENTITY_HIDE_FLAG: Show Smile Helper (ID: 17269302/0x01078236)
     0x04EE [0x1B] RETURN
     0x04EF [0x4E] SET_ENTITY_HIDE_FLAG: Hide Smile Helper (ID: 17269293/0x0107822D)
     0x04F5 [0x4E] SET_ENTITY_HIDE_FLAG: Hide Smile Helper (ID: 17269294/0x0107822E)
     0x04FB [0x4E] SET_ENTITY_HIDE_FLAG: Hide Smile Helper (ID: 17269295/0x0107822F)
     0x0501 [0x4E] SET_ENTITY_HIDE_FLAG: Hide Smile Helper (ID: 17269296/0x01078230)
     0x0507 [0x4E] SET_ENTITY_HIDE_FLAG: Hide Smile Helper (ID: 17269297/0x01078231)
     0x050D [0x4E] SET_ENTITY_HIDE_FLAG: Hide Smile Helper (ID: 17269298/0x01078232)
     0x0513 [0x4E] SET_ENTITY_HIDE_FLAG: Hide Smile Helper (ID: 17269299/0x01078233)
     0x0519 [0x4E] SET_ENTITY_HIDE_FLAG: Hide Smile Helper (ID: 17269300/0x01078234)
     0x051F [0x4E] SET_ENTITY_HIDE_FLAG: Hide Smile Helper (ID: 17269301/0x01078235)
     0x0525 [0x4E] SET_ENTITY_HIDE_FLAG: Hide Smile Helper (ID: 17269302/0x01078236)
     0x052B [0x1B] RETURN
     0x052C [0x03] 0x3880 = 0x3980
     0x0531 [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2151383098/0x803B803A))
     0x0536 [0x3C] SET_BIT_FLAG_CONDITIONAL(target_work_offset=0x3D80, bit_index_work_offset=0x3E80, condition_work_offset=0x3F80)
     0x053D [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2151776320/0x80418040))
     0x0542 [0x03] 0x0380 = 0x0380
     0x0547 [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2147713027/0x80038003))
     0x054C [0x03] 0x4280 = 0x4380
     0x0551 [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2152038468/0x80458044))
     0x0556 [0x46] CAMERA_CONTROL: Unknown mode 0x80
     0x0558 [0x47] WAIT_PLAYER_POS_UPDATE
     0x055A [0x48] [System] [0x4980]:
     0x055D [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2152431690/0x804B804A))
     0x0562 [0x03] 0x0380 = 0x0380
     0x0567 [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2147713027/0x80038003))
     0x056C [0x03] 0x4C80 = 0x4D80
     0x0571 [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2152693838/0x804F804E))
     0x0576 [0x50] END_SCHEDULER_TASK: End scheduler 0x00005580 with entities [0x5180, 0x5380], work=[21120, 21632]
     0x0583 [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2147713027/0x80038003))
     0x0588 [0x03] 0x0380 = 0x0380
     0x058D [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2153218134/0x80578056))
     0x0592 [0x58] YIELD_EVENT_VM()
     0x0593 [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2153414745/0x805A8059))
     0x0598 [0x5B] LOAD_EXT_SCHEDULER: Load scheduler 0x03800380 with entities [Unknown NPC (ID: 1585470848/0x5E805D80), Unknown NPC (ID: 58744704/0x03805F80)], work=0x5C80
     0x05A7 [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2147713027/0x80038003))
     0x05AC [0x60] ADJUST_RENDER_FLAGS1_MULTI: Deprecated mode 0x80 (no operation)
     0x05AE [0x61] EventEntity->Render.Flags2 &= ~0x00000001
     0x05B0 [0x62] LOAD_EVENT_SCHEDULER: Load scheduler 0x69806880 with entities [Unknown NPC (ID: 1702913152/0x65806480), Unknown NPC (ID: 1736468096/0x67806680)], work=[0x6380, 0x6A80]
     0x05C1 [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2151776363/0x8041806B))
     0x05C6 [0x6C] FADE_ENTITY_COLOR(entity_id=Unknown NPC (ID: 1853910400/0x6E806D80), end_alpha=0x6F80, fade_time=0x7080)
     0x05CF [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2154987633/0x80728071))
     0x05D4 [0x73] Unknown NPC (ID: 1988130176/0x76807580) casts magic 0x7480 on Unknown NPC (ID: 2021685120/0x78807780)
     0x05DF [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2155511929/0x807A8079))
     0x05E4 [0x4B] UPDATE_ENTITY_YAW(entity=Unknown NPC (ID: 2088795008/0x7C807B80), yaw=0x7D80)
     0x05EB [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2155839614/0x807F807E))
     0x05F0 [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2189459840/0x82808180))
     0x05F5 [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2152890499/0x80528083))
     0x05FA [0x84] ADJUST_RENDER_FLAGS3_BIT0()
     0x05FB [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2156298373/0x80868085))
     0x0600 [0x87] WORLD_PASS_HANDLER_A: Send world pass packet 0x1B (Para=0)
     0x0601 [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2153087112/0x80558088))
     0x0606 [0x89] OPEN_MAP(map_id=0x00008A80)
     0x0609 [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2147713163/0x8003808B))
     0x060E [0x03] 0x0380 = 0x0380
     0x0613 [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2147778563/0x80048003))
     0x0618 [0x03] 0x0380 = 0x0380
     0x061D [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2148368387/0x800D8003))
     0x0622 [0x03] 0x0380 = 0x0380
     0x0627 [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2147713027/0x80038003))
     0x062C [0x1F] MOVE_ENTITY: Update entity position (mode=128)
     0x062E [0x8C] CRAFTING_HANDLER(mode=0x80) // Unknown mode 128
     0x0630 [0x8D] OPEN_MAP_WITH_PROPERTIES(map_id=References[3712], properties=References[3712])
     0x0635 [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2147713027/0x80038003))
     0x063A [0x03] 0x0480 = 0x2880
     0x063F [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2150137999/0x8028808F))
     0x0644 [0x8F] SET_ENTITY_STATUS_EVENT_46()
     0x0645 [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2156888104/0x808F8028))
     0x064A [0x28] REQ_SET_WITH_CONDITIONS(priority=0x80, target_entity=Unknown NPC (ID: 2150137999/0x8028808F), tag_num=0x8F)
     0x0651 [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2147713027/0x80038003))
     0x0656 [0x03] 0x0380 = 0x0380
     0x065B [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2147713028/0x80038004))
     0x0660 [0x1E] EventEntity looks at Unknown NPC (ID: 75499136/0x04800680) and starts talking
     0x0665 [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2148368424/0x800D8028))
     0x066A [0x2C] CREATE_SCHEDULER_TASK: Create scheduler 0x03800380 with entities [Unknown NPC (ID: 2424311680/0x90800F80), Unknown NPC (ID: 58731648/0x03802C80)]
     0x0677 [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2147713027/0x80038003))
     0x067C [0x91] ExtData[1]->MainSpeedBase = 0x9180 * 10.0
     0x067F [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2157019281/0x80918091))
     0x0684 [0x91] ExtData[1]->MainSpeedBase = 0x9180 * 10.0
     0x0687 [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2157019281/0x80918091))
     0x068C [0x91] ExtData[1]->MainSpeedBase = 0x0380 * 10.0
     0x068F [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2147713027/0x80038003))
     0x0694 [0x03] 0x0380 = 0x0380
     0x0699 [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2157084675/0x80928003))
     0x069E [0x92] Unknown NPC (ID: 2157084818/0x80928092)->Render.Flags3 ^= 0x80
     0x06A4 [0x92] Unknown NPC (ID: 2157084818/0x80928092)->Render.Flags3 ^= 0x80
     0x06AA [0x92] Unknown NPC (ID: 2147713170/0x80038092)->Render.Flags3 ^= 0x80
     0x06B0 [0x03] 0x0380 = 0x0380
     0x06B5 [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2147713027/0x80038003))
     0x06BA [0x03] 0x0380 = 0x0380
     0x06BF [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2147713027/0x80038003))
     0x06C4 [0x03] 0x0380 = 0x0380
     0x06C9 [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2147713027/0x80038003))
     0x06CE [0x03] 0x0380 = 0x0380
     0x06D3 [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2147713027/0x80038003))
     0x06D8 [0x03] 0x0380 = 0x9380
     0x06DD [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2157150355/0x80938093))
     0x06E2 [0x93] DISPLAY_ITEM_INFO(item_id=0x9380)
     0x06E5 [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2157150355/0x80938093))
     0x06EA [0x93] DISPLAY_ITEM_INFO(item_id=0x9380)
     0x06ED [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2147713027/0x80038003))
     0x06F2 [0x03] 0x0380 = 0x0380
     0x06F7 [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2147713027/0x80038003))
     0x06FC [0x94] Unknown NPC (ID: 2157215892/0x80948094)->Render.Flags3 ^= 0x80
     0x0702 [0x94] Unknown NPC (ID: 2157215892/0x80948094)->Render.Flags3 ^= 0x80
     0x0708 [0x94] Unknown NPC (ID: 2157215892/0x80948094)->Render.Flags3 ^= 0x80
     0x070E [0x03] 0x0380 = 0x0380
     0x0713 [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2147713027/0x80038003))
     0x0718 [0x03] 0x0380 = 0x9580
     0x071D [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2157281429/0x80958095))
     0x0722 [0x95] SETUP_EVENT_NPC: Prepare NPC for event (Render.Flags3 = 0x9580)
     0x0725 [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2157281429/0x80958095))
     0x072A [0x95] SETUP_EVENT_NPC: Prepare NPC for event (Render.Flags3 = 0x9580)
     0x072D [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2147713027/0x80038003))
     0x0732 [0x03] 0x0380 = 0x0380
     0x0737 [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2147713027/0x80038003))
     0x073C [0x95] SETUP_EVENT_NPC: Prepare NPC for event (Render.Flags3 = 0x9580)
     0x073F [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2157281429/0x80958095))
     0x0744 [0x95] SETUP_EVENT_NPC: Prepare NPC for event (Render.Flags3 = 0x9580)
     0x0747 [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2157281429/0x80958095))
     0x074C [0x95] SETUP_EVENT_NPC: Prepare NPC for event (Render.Flags3 = 0x0380)
     0x074F [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2147713027/0x80038003))
     0x0754 [0x03] 0x0380 = 0x0380
     0x0759 [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2157346819/0x80968003))
     0x075E [0x96] UNSET_EVENT_NPC: Restore NPC after event (removes event NPC status)
     0x075F [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2157346966/0x80968096))
     0x0764 [0x96] UNSET_EVENT_NPC: Restore NPC after event (removes event NPC status)
     0x0765 [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2157346966/0x80968096))
     0x076A [0x96] UNSET_EVENT_NPC: Restore NPC after event (removes event NPC status)
     0x076B [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2147713174/0x80038096))
     0x0770 [0x03] 0x0380 = 0x0380
     0x0775 [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2147713027/0x80038003))
     0x077A [0x03] 0x9780 = 0x9780
     0x077F [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2157412503/0x80978097))
     0x0784 [0x97] SAVE_SET_WIND_VALUES(wind_base=0x9780, wind_width=0x9880)
     0x0789 [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2157478040/0x80988098))
     0x078E [0x03] 0x0380 = 0x0380
     0x0793 [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2147713027/0x80038003))
     0x0798 [0x03] 0x0380 = 0x9580
     0x079D [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2157281429/0x80958095))
     0x07A2 [0x95] SETUP_EVENT_NPC: Prepare NPC for event (Render.Flags3 = 0x9580)
     0x07A5 [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2157281429/0x80958095))
     0x07AA [0x95] SETUP_EVENT_NPC: Prepare NPC for event (Render.Flags3 = 0x9580)
     0x07AD [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2147713027/0x80038003))
     0x07B2 [0x03] 0x0380 = 0x0380
     0x07B7 [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2147713027/0x80038003))
     0x07BC [0x99] Wait for Unknown NPC (ID: 2575341952/0x99809980) animation to complete
     0x07C1 [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2157543577/0x80998099))
     0x07C6 [0x99] Wait for Unknown NPC (ID: 2575341952/0x99809980) animation to complete
     0x07CB [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2147778564/0x80048004))
     0x07D0 [0x04] DEPRECATED_NOP(unused=0x0480)
     0x07D3 [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2147778564/0x80048004))
     0x07D8 [0x04] DEPRECATED_NOP(unused=0x0480)
     0x07DB [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2157609114/0x809A809A))
     0x07E0 [0x9A] WAIT_MUSIC_SERVER()
     0x07E1 [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2157609114/0x809A809A))
     0x07E6 [0x9A] WAIT_MUSIC_SERVER()
     0x07E7 [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2157609114/0x809A809A))
     0x07EC [0x04] DEPRECATED_NOP(unused=0x0480)
     0x07EF [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2147778564/0x80048004))
     0x07F4 [0x04] DEPRECATED_NOP(unused=0x0480)
     0x07F7 [0x80] LOAD_WAIT(entity=Unknown NPC (ID: 2147778564/0x80048004))
```
