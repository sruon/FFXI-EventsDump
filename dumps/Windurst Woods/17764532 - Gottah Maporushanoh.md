...
- **8518**: How's it goin', mate? Enjoyin' a spot of adventurrrin', I see...
- **8519**: Life's long, and you can put off till tommora what you don't wanta do today. So don't you be wearin' yourself out with all that adventurrrin', you hear?
- **8701**: Of course I've lost me passion for adventurrre! I've come all the way to Windurst lookin' for the old pirate king's treasurrre, only to have 'em tell me that all that talk o' pirate treasurrre is just a fairy tale.
- **8702**: Speakin' of which...I bet that legend o' the great monk Scorpio who fought a giant scorrrpion with his bare hands was also just made up to excite the kiddies, too.
- **8707**: Ho-hum...
- **8712**: Ah!?
- **8714**: His eye carved out by a briny blusterrr His heart pierced by a scorpion's sting Cloaked in his treasure's golden lusterrr Beneath the earth sleeps that pirate king.
- **8716**: Hold your horrrses, Soni-Muni! I've hearrrd that maze is no walk in the park. We don't stand a chance at our current level, so we'd better get some trainin' in firrrst!
- **8718**: But then we wouldn't have heard the lowdown on the Maze of Shakhrami, would we? Rrright?
- **11876**: Ask if this person is the chick's owner? [Yes./No.]
- **11910**: You've got me chocobo!?
- **11911**: Me feathered mate is livin' in safety, then? I've neverrr heard such happy tidin's! I'll be sure to watch the little fella close from now on!
- **11912**: Let me show me grrratitude by tellin' you a story I often tell me little mate...
- **11929**: You say you've got me chocobo?
- **11930**: I'm afraid you've got the wrong person. Adventurers are bound to make mistakes from time to time, so don't let it get you down, mate.
...
```
  ...
  4: 0x019F [0x1D] PRINT_EVENT_MESSAGE(message_id=8518*)
    → "How's it goin', mate? Enjoyin' a spot of adventurrrin', I see..."
  5: 0x01A2 [0x23] WAIT_FOR_DIALOG_INTERACTION
  6: 0x01A3 [0x1D] PRINT_EVENT_MESSAGE(message_id=8519*)
...
```
  ...
  4: 0x01C3 [0x1D] PRINT_EVENT_MESSAGE(message_id=8701*)
  ...
  7: 0x01CE [0x1D] PRINT_EVENT_MESSAGE(message_id=8702*)
...
```
  0: 0x01DA [0x1D] PRINT_EVENT_MESSAGE(message_id=8707*)
    → "Ho-hum..."
...
```
  0: 0x01DF [0x1D] PRINT_EVENT_MESSAGE(message_id=8712*)
    → "Ah!?"
...
```
  0: 0x01E4 [0x1D] PRINT_EVENT_MESSAGE(message_id=8714*)
...
```
  0: 0x01E9 [0x1D] PRINT_EVENT_MESSAGE(message_id=8716*)
...
```
  ...
  4: 0x01FC [0x1D] PRINT_EVENT_MESSAGE(message_id=8718*)
...
# Dead code (unreachable instructions):
     ...
     0x022B [0x24] CREATE_DIALOG(message_id=11876*, default_option=1*, option_flags=0*)
    → "Ask if this person is the chick's owner? [Yes./No.]"
     ...
     0x0254 [0x1D] PRINT_EVENT_MESSAGE(message_id=11910*)
    → "You've got me chocobo!?"
     ...
     0x0267 [0x1D] PRINT_EVENT_MESSAGE(message_id=11911*)
    → "Me feathered mate is livin' in safety, then? I've neverrr heard such happy tidin's! I'll be sure to watch the little fella close from now on!"
     ...
     0x027A [0x1D] PRINT_EVENT_MESSAGE(message_id=11912*)
    → "Let me show me grrratitude by tellin' you a story I often tell me little mate..."
     ...
     0x0281 [0x1D] PRINT_EVENT_MESSAGE(message_id=11929*)
    → "You say you've got me chocobo?"
     ...
     0x0294 [0x1D] PRINT_EVENT_MESSAGE(message_id=11930*)
