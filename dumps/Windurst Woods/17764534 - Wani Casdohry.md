...
- **7612**: You can get your paws on $1 by defeating those Yagudo beastmen. But you won't please the Tarutaru very much if you go arrround fighting Yagudo!
- **7613**: Windurst is currently trying to maintain amicable relations with the Yagudo. It's a very different worrrld from what it was twenty years ago...
- **8529**: Our chieftainness is verrry old... It seems that she wants to pass the chieftainness's bow onto the Sibyl Guard Semih Lafihna.
- **8530**: But I doubt if Semih Lafihna would accept that honorable responsibility. Quite a prrredicament we're in, eh?
- **8720**: Our chieftainness recently invited Semih Lafihna over to open dialogue, but Semih Lafihna did not take her up on the offerrr.
- **8722**: I wonderrr if the chieftainness was going to tell her the truth. But I guess it doesn't matter whetherrr the truth is told or not, as the bad blood will still run thick between them.
- **8725**: I guess you are right... Oh, and what good timing, for there is an adventurerrr passing by now! Why don't we ask [him/her] whetherrr [he/she] can help us out with that task?
- **8730**: The $1 that can be found in the Meriphataud Mountains belong to the Mithran warriors who were felled there twenty years ago. If you find any, please hand them overrr to Gioh Ajihri, thank you.
- **8735**: Twinstones are, as their name suggests, a perrrfect pair of gemstones that are originally found joined as one.
- **8736**: No otherrr stones can come close to matching them once they are separated. Since they were once a single stone, only their corresponding halves fit perrrfectly back as one.
- **8737**: Utilizing this very properrrty, twinstones are used as proof of bonds between Mithra back in our homeland.
- **9408**: We found many earrings but had a terrible time trying to find the missing halves...I thought furrr was going to fly.
- **9409**: I wonder if the Mithran Tracker really believes that the daughter is dead... We can't let our guarrrd down yet.
...
```
  ...
  4: 0x0052 [0x1D] PRINT_EVENT_MESSAGE(message_id=7612*)
  ...
  6: 0x0056 [0x1D] PRINT_EVENT_MESSAGE(message_id=7613*)
...
```
  0: 0x0066 [0x1D] PRINT_EVENT_MESSAGE(message_id=8720*)
...
```
  0: 0x006B [0x1D] PRINT_EVENT_MESSAGE(message_id=8722*)
...
```
  0: 0x0070 [0x1D] PRINT_EVENT_MESSAGE(message_id=8725*)
...
```
  ...
  4: 0x0083 [0x1D] PRINT_EVENT_MESSAGE(message_id=8730*)
...
```
  ...
  4: 0x00A0 [0x1D] PRINT_EVENT_MESSAGE(message_id=8735*)
  ...
  6: 0x00A4 [0x1D] PRINT_EVENT_MESSAGE(message_id=8736*)
  ...
  8: 0x00A8 [0x1D] PRINT_EVENT_MESSAGE(message_id=8737*)
...
```
  ...
  4: 0x00C5 [0x1D] PRINT_EVENT_MESSAGE(message_id=9408*)
...
```
  ...
  4: 0x00E2 [0x1D] PRINT_EVENT_MESSAGE(message_id=9408*)
    → "We found many earrings but had a terrible time trying to find the missing halves...I thought furrr was going to fly."
  5: 0x00E5 [0x23] WAIT_FOR_DIALOG_INTERACTION
  6: 0x00E6 [0x1D] PRINT_EVENT_MESSAGE(message_id=9409*)
