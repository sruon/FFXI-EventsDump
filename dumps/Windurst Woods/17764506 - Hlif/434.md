...
```
      ...
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0020:          1E F0 FF FF 7F  6F 70 29 08 9A 10 0F 01     .....op).....
0030: 01 1D 02 80 23 1D 03 80  23 29 08 9A 10 0F 01 02  ....#...#)......
0040: 20 00 21 00                                        .!.            
```
...
```
  ...
  3: 0x002A [0x29] REQ_SET_WAIT(priority=0x08, entity_id=Hlif (ID: 17764506/0x010F109A), tag_num=0x01)
  4: 0x0031 [0x1D] PRINT_EVENT_MESSAGE(message_id=8543*)
    → "Ha-ha...\u0007Now I get it!\u007F1\u0000\u0007"
  5: 0x0034 [0x23] WAIT_FOR_DIALOG_INTERACTION
  6: 0x0035 [0x1D] PRINT_EVENT_MESSAGE(message_id=8544*)
    → "It's against the rules to ride a chocobo into the town. So that's why those strange automaton Cardian guard things chased me around and around like that!\u007F1\u0000\u0007"
  7: 0x0038 [0x23] WAIT_FOR_DIALOG_INTERACTION
  8: 0x0039 [0x29] REQ_SET_WAIT(priority=0x08, entity_id=Hlif (ID: 17764506/0x010F109A), tag_num=0x02)
