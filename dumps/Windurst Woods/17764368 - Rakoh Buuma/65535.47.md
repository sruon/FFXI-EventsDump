# Event 65535

## Metadata

| Field           | Value    |
|-----------------|----------|
| Event Index     | 71/105   |
| Offset          | 0x0F75   |
| Data Size       | 19 bytes |
| Calculated Size | 19 bytes |
| Instructions    | 5        |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0F70:                29 08 10  10 0F 01 01 1D 60 80 23       ).......`.#
0F80: 29 08 10 10 0F 01 02 00                           ).......        
```

## Opcodes

```
  0: 0x0F75 [0x29] REQ_SET_WAIT(priority=0x08, entity_id=Rakoh Buuma (ID: 17764368/0x010F1010), tag_num=0x01)
  1: 0x0F7C [0x1D] PRINT_EVENT_MESSAGE(message_id=7850*)
    → "You came at just the rrright time.\u0007We just rrreceived a rrrequest for help in a large-scale mission from Heavens Tower, but we're short of adventurers to dispatch just now.\u007F1\u0000\u0007"
  2: 0x0F7F [0x23] WAIT_FOR_DIALOG_INTERACTION
  3: 0x0F80 [0x29] REQ_SET_WAIT(priority=0x08, entity_id=Rakoh Buuma (ID: 17764368/0x010F1010), tag_num=0x02)
  4: 0x0F87 [0x00] END_REQSTACK()
```
