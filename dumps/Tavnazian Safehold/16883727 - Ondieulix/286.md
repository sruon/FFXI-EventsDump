# Event 286

## Metadata

| Field           | Value    |
|-----------------|----------|
| Event Index     | 6/13     |
| Offset          | 0x006B   |
| Data Size       | 30 bytes |
| Calculated Size | 30 bytes |
| Instructions    | 8        |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0060:                                   1E F0 FF FF 7F             .....
0070: 1D 0C 80 23 66 09 80 F8  FF FF 7F F8 FF FF 7F 74  ...#f..........t
0080: 6C 6B 30 1D 0D 80 23 21  00                       lk0...#!.       
```

## Opcodes

```
  0: 0x006B [0x1E] EventEntity looks at LocalPlayer and starts talking
  1: 0x0070 [0x1D] PRINT_EVENT_MESSAGE(message_id=11002*)
    → "The sudden influx of visitors from Jeuno has left many of the safehold residents scared and confused. However, I welcome their presence as a blessing.\u007F1\u0000\u0007"
  2: 0x0073 [0x23] WAIT_FOR_DIALOG_INTERACTION
  3: 0x0074 [0x66] LOAD_EXT_SCHEDULER_MAIN(work_offset=20*, entity1=EventEntity, entity2=EventEntity, action="tlk0")
  4: 0x0083 [0x1D] PRINT_EVENT_MESSAGE(message_id=11003*)
    → "I do not wish to be tied to this cursed place for the rest of my life...\u007F1\u0000\u0007"
  5: 0x0086 [0x23] WAIT_FOR_DIALOG_INTERACTION
  6: 0x0087 [0x21] END_EVENT
  7: 0x0088 [0x00] END_REQSTACK()
```
