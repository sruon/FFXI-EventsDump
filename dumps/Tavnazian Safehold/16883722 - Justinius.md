# 16883722 - Justinius

## Common Data

| Field            | Value                       |
|------------------|-----------------------------|
| Zone             | Tavnazian Safehold (ID: 26) |
| Block Size       | 1976 bytes                  |
| Total Events     | 53                          |
| References Count | 127                         |

## List of Events

| Event ID                   | Entrypoint   |   Size |   Instructions |
|----------------------------|--------------|--------|----------------|
| [65535](#event-65535)      | 0x0000       |      1 |              1 |
| [65535.1](#event-655351)   | 0x0001       |     16 |              2 |
| [65535.2](#event-655352)   | 0x0011       |     16 |              2 |
| [65535.3](#event-655353)   | 0x0021       |     16 |              2 |
| [65535.4](#event-655354)   | 0x0031       |     16 |              2 |
| [65535.5](#event-655355)   | 0x0041       |     16 |              2 |
| [65535.6](#event-655356)   | 0x0051       |     16 |              2 |
| [65535.7](#event-655357)   | 0x0061       |     16 |              2 |
| [570](#event-570)          | 0x0071       |      1 |              1 |
| [571](#event-571)          | 0x0072       |     19 |              9 |
| [575](#event-575)          | 0x0085       |     23 |             11 |
| [574](#event-574)          | 0x009C       |     19 |              9 |
| [572](#event-572)          | 0x00AF       |     74 |             16 |
| [573](#event-573)          | 0x00F9       |     90 |             22 |
| [101](#event-101)          | 0x0153       |     18 |              4 |
| [65535.8](#event-655358)   | 0x0165       |     17 |              6 |
| [65535.9](#event-655359)   | 0x0176       |     14 |              4 |
| [65535.10](#event-6553510) | 0x0184       |     14 |              4 |
| [65535.11](#event-6553511) | 0x0192       |     14 |              4 |
| [103](#event-103)          | 0x01A0       |      7 |              2 |
| [65535.12](#event-6553512) | 0x01A7       |     25 |              6 |
| [65535.13](#event-6553513) | 0x01C0       |     14 |              4 |
| [105](#event-105)          | 0x01CE       |      7 |              2 |
| [65535.14](#event-6553514) | 0x01D5       |     25 |              6 |
| [107](#event-107)          | 0x01EE       |      7 |              2 |
| [65535.15](#event-6553515) | 0x01F5       |     33 |              6 |
| [109](#event-109)          | 0x0216       |      1 |              1 |
| [110](#event-110)          | 0x0217       |      1 |              1 |
| [65535.16](#event-6553516) | 0x0218       |     35 |              9 |
| [123](#event-123)          | 0x023B       |     11 |              5 |
| [124](#event-124)          | 0x0246       |     30 |              8 |
| [125](#event-125)          | 0x0264       |     49 |             11 |
| [127](#event-127)          | 0x0295       |     11 |              5 |
| [128](#event-128)          | 0x02A0       |     49 |             11 |
| [129](#event-129)          | 0x02D1       |     49 |             11 |
| [130](#event-130)          | 0x0302       |     49 |             11 |
| [131](#event-131)          | 0x0333       |     53 |             13 |
| [135](#event-135)          | 0x0368       |     30 |              8 |
| [265](#event-265)          | 0x0386       |     30 |              8 |
| [266](#event-266)          | 0x03A4       |     35 |              9 |
| [267](#event-267)          | 0x03C7       |     30 |              8 |
| [350](#event-350)          | 0x03E5       |     11 |              5 |
| [360](#event-360)          | 0x03F0       |     30 |              8 |
| [512](#event-512)          | 0x040E       |      1 |              1 |
| [65535.17](#event-6553517) | 0x040F       |     15 |              3 |
| [513](#event-513)          | 0x041E       |      1 |              1 |
| [548](#event-548)          | 0x041F       |      1 |              1 |
| [616](#event-616)          | 0x0420       |    109 |             29 |
| [629](#event-629)          | 0x048D       |      1 |              1 |
| [65535.18](#event-6553518) | 0x048E       |     14 |              4 |
| [65535.19](#event-6553519) | 0x049C       |     14 |              4 |
| [65535.20](#event-6553520) | 0x04AA       |     22 |              6 |
| [65535.21](#event-6553521) | 0x04C0       |     24 |              6 |

## DAT References (imed_data)

|   Index | Hex Value   |   Dec Value |
|---------|-------------|-------------|
|       0 | 0x0009      |           9 |
|       1 | 0x0000      |           0 |
|       2 | 0x02D0      |         720 |
|       3 | 0x2E22      |       11810 |
|       4 | 0x2E23      |       11811 |
|       5 | 0x2E24      |       11812 |
|       6 | 0x2E25      |       11813 |
|       7 | 0x2E26      |       11814 |
|       8 | 0x2E27      |       11815 |
|       9 | 0x2E28      |       11816 |
|      10 | 0x00C9      |         201 |
|      11 | 0x2E29      |       11817 |
|      12 | 0x2E1F      |       11807 |
|      13 | 0x0001      |           1 |
|      14 | 0x2E20      |       11808 |
|      15 | 0x2E21      |       11809 |
|      16 | 0xFFFFC6C1  |  4294952641 |
|      17 | 0x1DE40     |      122432 |
|      18 | 0xFFFF97FE  |  4294940670 |
|      19 | 0x0BD9      |        3033 |
|      20 | 0x000D      |          13 |
|      21 | 0xFFFFE4C5  |  4294960325 |
|      22 | 0x1FEE0     |      130784 |
|      23 | 0xFFFF9A90  |  4294941328 |
|      24 | 0xFFFFED1C  |  4294962460 |
|      25 | 0x1FF1E     |      130846 |
|      26 | 0xFFFF9AE2  |  4294941410 |
|      27 | 0xFFFFF011  |  4294963217 |
|      28 | 0x20141     |      131393 |
|      29 | 0xFFFF9B02  |  4294941442 |
|      30 | 0x0813      |        2067 |
|      31 | 0x1FB19     |      129817 |
|      32 | 0xFFFF99AC  |  4294941100 |
|      33 | 0x6FFC      |       28668 |
|      34 | 0x92A8      |       37544 |
|      35 | 0xFFFFD1BD  |  4294955453 |
|      36 | 0x0C63      |        3171 |
|      37 | 0x6FD2      |       28626 |
|      38 | 0x9C2B      |       39979 |
|      39 | 0xFFFFD162  |  4294955362 |
|      40 | 0x6D80      |       28032 |
|      41 | 0xA1CF      |       41423 |
|      42 | 0xFFFFD138  |  4294955320 |
|      43 | 0x0012      |          18 |
|      44 | 0x12A8      |        4776 |
|      45 | 0x572B      |       22315 |
|      46 | 0xFFFFA744  |  4294944580 |
|      47 | 0x070B      |        1803 |
|      48 | 0x0A6E      |        2670 |
|      49 | 0x552B      |       21803 |
|      50 | 0xFFFFA721  |  4294944545 |
|      51 | 0xFFFFEF86  |  4294963078 |
|      52 | 0x007B      |         123 |
|      53 | 0xFFFFDCE0  |  4294958304 |
|      54 | 0x0150      |         336 |
|      55 | 0xFFFFF981  |  4294965633 |
|      56 | 0xFFFFFB35  |  4294966069 |
|      57 | 0x1410B     |       82187 |
|      58 | 0x10D77     |       68983 |
|      59 | 0xFFFF7B45  |  4294933317 |
|      60 | 0x14348     |       82760 |
|      61 | 0x10B04     |       68356 |
|      62 | 0xFFFF7B5B  |  4294933339 |
|      63 | 0x1444F     |       83023 |
|      64 | 0x107D8     |       67544 |
|      65 | 0xFFFF7B5D  |  4294933341 |
|      66 | 0x26BE      |        9918 |
|      67 | 0x26C0      |        9920 |
|      68 | 0x26C1      |        9921 |
|      69 | 0x26E5      |        9957 |
|      70 | 0x26E6      |        9958 |
|      71 | 0x26E7      |        9959 |
|      72 | 0x26FC      |        9980 |
|      73 | 0x270D      |        9997 |
|      74 | 0x270E      |        9998 |
|      75 | 0x270F      |        9999 |
|      76 | 0x2718      |       10008 |
|      77 | 0x2719      |       10009 |
|      78 | 0x271A      |       10010 |
|      79 | 0x2738      |       10040 |
|      80 | 0x2739      |       10041 |
|      81 | 0x273A      |       10042 |
|      82 | 0x2778      |       10104 |
|      83 | 0x2779      |       10105 |
|      84 | 0x277A      |       10106 |
|      85 | 0x277B      |       10107 |
|      86 | 0x28B9      |       10425 |
|      87 | 0x28BA      |       10426 |
|      88 | 0x2ACF      |       10959 |
|      89 | 0x2AD0      |       10960 |
|      90 | 0x2AD1      |       10961 |
|      91 | 0x2AD2      |       10962 |
|      92 | 0x2AD3      |       10963 |
|      93 | 0x2AD4      |       10964 |
|      94 | 0x2737      |       10039 |
|      95 | 0x269B      |        9883 |
|      96 | 0x269C      |        9884 |
|      97 | 0x122B0     |       74416 |
|      98 | 0x10081     |       65665 |
|      99 | 0xFFFF7B30  |  4294933296 |
|     100 | 0x0610      |        1552 |
|     101 | 0x2F0B      |       12043 |
|     102 | 0x0016      |          22 |
|     103 | 0x2F0C      |       12044 |
|     104 | 0x2F0D      |       12045 |
|     105 | 0x0019      |          25 |
|     106 | 0x2F0E      |       12046 |
|     107 | 0x001A      |          26 |
|     108 | 0x2F0F      |       12047 |
|     109 | 0x0022      |          34 |
|     110 | 0x2F10      |       12048 |
|     111 | 0xFFFF9AF4  |  4294941428 |
|     112 | 0x1E2EB     |      123627 |
|     113 | 0xFFFF96B4  |  4294940340 |
|     114 | 0xFFFFC6BB  |  4294952635 |
|     115 | 0x1AA6A     |      109162 |
|     116 | 0xFFFF9310  |  4294939408 |
|     117 | 0xFFFFCEF7  |  4294954743 |
|     118 | 0x1E431     |      123953 |
|     119 | 0xFFFF996B  |  4294941035 |
|     120 | 0x001E      |          30 |
|     121 | 0xFFFFDBF6  |  4294958070 |
|     122 | 0x1DBD7     |      121815 |
|     123 | 0xFFFF97DE  |  4294940638 |
|     124 | 0xFFFFE4E5  |  4294960357 |
|     125 | 0x1C8D4     |      116948 |
|     126 | 0xFFFF9298  |  4294939288 |

## String References

- **9883**: You say that Nag'molada fellow is not an acquaintance of yours?
- **9884**: Well, that is of no matter. If you happen to see him again, I strongly suggest you warn him not to go anywhere near the door leading to the aqueducts.
- **9918**: Beyond that gate lie the aqueducts that connect to the deeper parts of the Sacrarium. The entire area has had a powerful magic spell cast upon it by the cardinal Mildaurion C Giloumet.
- **9920**: You have defeated the minotaur? But your companion has yet to return.
- **9921**: It's not that I doubt your word, but I must hear the story from both of you. Now, back into the aqueducts to search for your friend.
- **9957**: Were you surprised when you met Prishe? She might look like a little girl, but she's the strongest member of the Tavnazian Patrol.
- **9958**: It is strange that you should carry the same amulet. It seems that fate has taken a hand here.
- **9959**: But Prishe despises words like "fate." You should head to her quarters before she starts to get impatient.
- **9980**: Ulmia? I just saw her leaving the safehold. Any time she wants to be alone, Ulmia usually goes to Snowmint Point, located on the Misareaux Coast...
- **9997**: The Chebukki brothers were headed to Cape Riverne, northwest of the Misareaux Coast.
- **9998**: Those three were carved out of pure curiosity. Once they'd heard about the dragon sightings, they probably couldn't wait to go see it for themselves.
- **9999**: But why did your companion go with them? When did he become such good friends with those troublemakers?
- **10008**: Greetings, <Player>. Thanks to your actions on the mainland, a messenger from Jeuno has arrived here in Tavnazia.
- **10009**: It looks like arrangements will be made for aiding the safehold. Their government will also be considering applications from those who wish to emigrate to Jeuno.
- **10010**: The only problem is... Have you talked to Prishe yet?
- **10039**: You are looking for Ulmia? She probably went down to the coast. I must say, it is quite uncommon for her to argue with her grandfather like she did earlier...
- **10040**: <Player>, you weren't able to dissuade Ulmia?
- **10041**: She's determined to march up Cape Riverne and speak to the Wyrmking, alone if need be. She won't listen to me, no matter how much I try to explain the dangers involved.
- **10042**: I understand how much she wants to prove that Prishe is not the abhorrent one, but...
- **10104**: Lord Bahamut. So, he has appeared to wreak devastation on the people of Vana'diel...
- **10105**: And his minions have been called to his side in preparation for this genocide.
- **10106**: But we cannot fight against so many dragons. All we can do is sit back and watch helplessly as events unfold.
- **10107**: If the will of Promathia is mixed up in this, we may not have any choice in the matter in any case. Will Prishe be able to find some way to avert this catastrophe?
- **10425**: For the first time in my life, I prayed to Altana--I prayed for your safe return. And look... All of you managed to escape that terror in the skies.
- **10426**: Perhaps Altana truly does exist... I wonder if it is not too late to start believing?
- **10959**: Do you find our enclosed city strange? That is probably because it is so different from what you have seen on the mainland.
- **10960**: However, for us--the forgotten--this city is all we know and all we have.
- **10961**: Safehold elder Despachiaire has welcomed that envoy from Jeuno with open arms. However...
- **10962**: I have a terrible feeling something grave is upon us. Even those three menacing Tarutaru are acting strange...
- **10963**: A great challenge awaits you in the skies, <Player>.
- **10964**: But I cannot help but think it ironic that a forgotten city such as ours would be the site of a battle that could alter the history of all Vana'diel.
- **11807**: Take up this duty? [On my way./Not today.]
- **11808**: Excellent. Take this and make your way to Monarch Linn. You may have fought them before, but be careful. Who knows what tricks they have hiding up their sleeves?
- **11809**: Lost your will for battle, hm? There might be another way to deal with this, but let me know if you change your mind. It would make my job a lot easier.
- **11810**: Take the $3 and head for Monarch Linn. You may have fought them before, but be careful. Who knows what tricks they have hiding up their sleeves?
- **11811**: You failed to oust the intruders from Monarch Linn!? And you lost the $3 into the bargain...!?
- **11812**: It will take some time to have another permit approved. You may as well return to your adventures for a while. (This is what I deserve for trusting an adventurer...)
- **11813**: A new $3 has been approved. Perhaps this time you'll be properly prepared for the challenge that awaits!
- **11814**: So, you have routed those black-robed menaces from Monarch Linn? Excellent news.
- **11815**: Your reward--for a job well done. Go on, take it. You deserve something for putting your neck on the line.
- **11816**: We cannot know if these intruders are gone for good, however. I will continue my surveillance of the area, but would appreciate it if you checked in with me again in the near future.
- **11817**: Do you remember those black-robed intruders you chased away from Monarch Linn? We've had another sighting. Do you think you could go and "persuade" them to leave again?
- **12043**: Who...? Ah, it's you, <Player>. I never know when one of you adventurers will choose to darken my doorstep.
- **12044**: It may not be my place to say it, but Prishe seemed upset with you.
- **12045**: You see, I found this unusual ring on one of my patrols. When I showed it to Prishe and Ulmia, they were certain it was the same ring they gave to you.
- **12046**: Poor Ulmia seemed on the verge of tears. Is this how you treat a person's heartfelt gift!?
- **12047**: It might be nothing more than a bauble to you, but for those girls it was a symbol of your friendship--your unbreakable bond!
- **12048**: Here, take it. Maybe this time you'll think twice about how much it's really worth...

## Events

### Event 65535

#### Metadata

| Field        | Value   |
|--------------|---------|
| Entrypoint   | 0x0000  |
| Data Size    | 1 bytes |
| Instructions | 1       |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0000: 00                                                .               
```

#### Opcodes

```
  0: 0x0000 [0x00] END_REQSTACK()
```

### Event 65535.1

#### Metadata

| Field        | Value    |
|--------------|----------|
| Entrypoint   | 0x0001   |
| Data Size    | 16 bytes |
| Instructions | 2        |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0000:    66 00 80 F8 FF FF 7F  F8 FF FF 7F 74 77 61 30   f..........twa0
0010: 00                                                .               
```

#### Opcodes

```
  0: 0x0001 [0x66] LOAD_EXT_SCHEDULER_MAIN: Load scheduler "twa0" with entities [EventEntity, EventEntity], work=9*
  1: 0x0010 [0x00] END_REQSTACK()
```

### Event 65535.2

#### Metadata

| Field        | Value    |
|--------------|----------|
| Entrypoint   | 0x0011   |
| Data Size    | 16 bytes |
| Instructions | 2        |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0010:    66 00 80 F8 FF FF 7F  F8 FF FF 7F 74 77 61 31   f..........twa1
0020: 00                                                .               
```

#### Opcodes

```
  0: 0x0011 [0x66] LOAD_EXT_SCHEDULER_MAIN: Load scheduler "twa1" with entities [EventEntity, EventEntity], work=9*
  1: 0x0020 [0x00] END_REQSTACK()
```

### Event 65535.3

#### Metadata

| Field        | Value    |
|--------------|----------|
| Entrypoint   | 0x0021   |
| Data Size    | 16 bytes |
| Instructions | 2        |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0020:    66 00 80 F8 FF FF 7F  F8 FF FF 7F 74 77 62 30   f..........twb0
0030: 00                                                .               
```

#### Opcodes

```
  0: 0x0021 [0x66] LOAD_EXT_SCHEDULER_MAIN: Load scheduler "twb0" with entities [EventEntity, EventEntity], work=9*
  1: 0x0030 [0x00] END_REQSTACK()
```

### Event 65535.4

#### Metadata

| Field        | Value    |
|--------------|----------|
| Entrypoint   | 0x0031   |
| Data Size    | 16 bytes |
| Instructions | 2        |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0030:    66 00 80 F8 FF FF 7F  F8 FF FF 7F 74 77 62 31   f..........twb1
0040: 00                                                .               
```

#### Opcodes

```
  0: 0x0031 [0x66] LOAD_EXT_SCHEDULER_MAIN: Load scheduler "twb1" with entities [EventEntity, EventEntity], work=9*
  1: 0x0040 [0x00] END_REQSTACK()
```

### Event 65535.5

#### Metadata

| Field        | Value    |
|--------------|----------|
| Entrypoint   | 0x0041   |
| Data Size    | 16 bytes |
| Instructions | 2        |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0040:    66 01 80 F8 FF FF 7F  F8 FF FF 7F 70 61 73 30   f..........pas0
0050: 00                                                .               
```

#### Opcodes

```
  0: 0x0041 [0x66] LOAD_EXT_SCHEDULER_MAIN: Load scheduler "pas0" with entities [EventEntity, EventEntity], work=0*
  1: 0x0050 [0x00] END_REQSTACK()
```

### Event 65535.6

#### Metadata

| Field        | Value    |
|--------------|----------|
| Entrypoint   | 0x0051   |
| Data Size    | 16 bytes |
| Instructions | 2        |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0050:    66 00 80 F8 FF FF 7F  F8 FF FF 7F 74 68 6B 31   f..........thk1
0060: 00                                                .               
```

#### Opcodes

```
  0: 0x0051 [0x66] LOAD_EXT_SCHEDULER_MAIN: Load scheduler "thk1" with entities [EventEntity, EventEntity], work=9*
  1: 0x0060 [0x00] END_REQSTACK()
```

### Event 65535.7

#### Metadata

| Field        | Value    |
|--------------|----------|
| Entrypoint   | 0x0061   |
| Data Size    | 16 bytes |
| Instructions | 2        |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0060:    66 00 80 F8 FF FF 7F  F8 FF FF 7F 74 68 6B 32   f..........thk2
0070: 00                                                .               
```

#### Opcodes

```
  0: 0x0061 [0x66] LOAD_EXT_SCHEDULER_MAIN: Load scheduler "thk2" with entities [EventEntity, EventEntity], work=9*
  1: 0x0070 [0x00] END_REQSTACK()
```

### Event 570

#### Metadata

| Field        | Value   |
|--------------|---------|
| Entrypoint   | 0x0071  |
| Data Size    | 1 bytes |
| Instructions | 1       |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0070:    00                                              .              
```

#### Opcodes

```
  0: 0x0071 [0x00] END_REQSTACK()
```

### Event 571

#### Metadata

| Field        | Value    |
|--------------|----------|
| Entrypoint   | 0x0072   |
| Data Size    | 19 bytes |
| Instructions | 9        |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0070:       42 03 02 10 02 80  1E F0 FF FF 7F 6F 70 1D    B..........op.
0080: 03 80 23 21 00                                    ..#!.           
```

#### Opcodes

```
  0: 0x0072 [0x42] SET_CLI_EVENT_CANCEL_DATA()
  1: 0x0073 [0x03] Work_Zone[2] = 720*
  2: 0x0078 [0x1E] EventEntity looks at LocalPlayer and starts talking
  3: 0x007D [0x6F] WAIT_FRAME_DELAY: Yield until WaitTime reaches zero
  4: 0x007E [0x70] WAIT_ENTITY_RENDER_FLAG: Wait while EventEntity->Render.Flags3 bit 2 is set (cancel turn if not)
  5: 0x007F [0x1D] PRINT_EVENT_MESSAGE(message_id=11810*)
    → "Take the $3 and head for Monarch Linn. You may have fought them before, but be careful. Who knows what tricks they have hiding up their sleeves?"
  6: 0x0082 [0x23] WAIT_FOR_DIALOG_INTERACTION
  7: 0x0083 [0x21] END_EVENT
  8: 0x0084 [0x00] END_REQSTACK()
```

### Event 575

#### Metadata

| Field        | Value    |
|--------------|----------|
| Entrypoint   | 0x0085   |
| Data Size    | 23 bytes |
| Instructions | 11       |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0080:                42 03 02  10 02 80 1E F0 FF FF 7F       B..........
0090: 6F 70 1D 04 80 23 1D 05  80 23 21 00              op...#...#!.    
```

#### Opcodes

```
  0: 0x0085 [0x42] SET_CLI_EVENT_CANCEL_DATA()
  1: 0x0086 [0x03] Work_Zone[2] = 720*
  2: 0x008B [0x1E] EventEntity looks at LocalPlayer and starts talking
  3: 0x0090 [0x6F] WAIT_FRAME_DELAY: Yield until WaitTime reaches zero
  4: 0x0091 [0x70] WAIT_ENTITY_RENDER_FLAG: Wait while EventEntity->Render.Flags3 bit 2 is set (cancel turn if not)
  5: 0x0092 [0x1D] PRINT_EVENT_MESSAGE(message_id=11811*)
    → "You failed to oust the intruders from Monarch Linn!? And you lost the $3 into the bargain...!?"
  6: 0x0095 [0x23] WAIT_FOR_DIALOG_INTERACTION
  7: 0x0096 [0x1D] PRINT_EVENT_MESSAGE(message_id=11812*)
    → "It will take some time to have another permit approved. You may as well return to your adventures for a while. (This is what I deserve for trusting an adventurer...)"
  8: 0x0099 [0x23] WAIT_FOR_DIALOG_INTERACTION
  9: 0x009A [0x21] END_EVENT
 10: 0x009B [0x00] END_REQSTACK()
```

### Event 574

#### Metadata

| Field        | Value    |
|--------------|----------|
| Entrypoint   | 0x009C   |
| Data Size    | 19 bytes |
| Instructions | 9        |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0090:                                      42 03 02 10              B...
00A0: 02 80 1E F0 FF FF 7F 6F  70 1D 06 80 23 21 00     .......op...#!. 
```

#### Opcodes

```
  0: 0x009C [0x42] SET_CLI_EVENT_CANCEL_DATA()
  1: 0x009D [0x03] Work_Zone[2] = 720*
  2: 0x00A2 [0x1E] EventEntity looks at LocalPlayer and starts talking
  3: 0x00A7 [0x6F] WAIT_FRAME_DELAY: Yield until WaitTime reaches zero
  4: 0x00A8 [0x70] WAIT_ENTITY_RENDER_FLAG: Wait while EventEntity->Render.Flags3 bit 2 is set (cancel turn if not)
  5: 0x00A9 [0x1D] PRINT_EVENT_MESSAGE(message_id=11813*)
    → "A new $3 has been approved. Perhaps this time you'll be properly prepared for the challenge that awaits!"
  6: 0x00AC [0x23] WAIT_FOR_DIALOG_INTERACTION
  7: 0x00AD [0x21] END_EVENT
  8: 0x00AE [0x00] END_REQSTACK()
```

### Event 572

#### Metadata

| Field        | Value    |
|--------------|----------|
| Entrypoint   | 0x00AF   |
| Data Size    | 74 bytes |
| Instructions | 16       |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
00A0:                                               42                 B
00B0: 03 02 10 02 80 1E F0 FF  FF 7F 6F 70 66 00 80 F8  ..........opf...
00C0: FF FF 7F F8 FF FF 7F 74  77 61 30 1D 07 80 23 66  .......twa0...#f
00D0: 00 80 F8 FF FF 7F F8 FF  FF 7F 74 77 61 31 1D 08  ..........twa1..
00E0: 80 23 1D 09 80 23 45 0A  80 F0 FF FF 7F F0 FF FF  .#...#E.........
00F0: 7F 71 73 74 63 01 80 21  00                       .qstc..!.       
```

#### Opcodes

```
  0: 0x00AF [0x42] SET_CLI_EVENT_CANCEL_DATA()
  1: 0x00B0 [0x03] Work_Zone[2] = 720*
  2: 0x00B5 [0x1E] EventEntity looks at LocalPlayer and starts talking
  3: 0x00BA [0x6F] WAIT_FRAME_DELAY: Yield until WaitTime reaches zero
  4: 0x00BB [0x70] WAIT_ENTITY_RENDER_FLAG: Wait while EventEntity->Render.Flags3 bit 2 is set (cancel turn if not)
  5: 0x00BC [0x66] LOAD_EXT_SCHEDULER_MAIN: Load scheduler "twa0" with entities [EventEntity, EventEntity], work=9*
  6: 0x00CB [0x1D] PRINT_EVENT_MESSAGE(message_id=11814*)
    → "So, you have routed those black-robed menaces from Monarch Linn? Excellent news."
  7: 0x00CE [0x23] WAIT_FOR_DIALOG_INTERACTION
  8: 0x00CF [0x66] LOAD_EXT_SCHEDULER_MAIN: Load scheduler "twa1" with entities [EventEntity, EventEntity], work=9*
  9: 0x00DE [0x1D] PRINT_EVENT_MESSAGE(message_id=11815*)
    → "Your reward--for a job well done. Go on, take it. You deserve something for putting your neck on the line."
 10: 0x00E1 [0x23] WAIT_FOR_DIALOG_INTERACTION
 11: 0x00E2 [0x1D] PRINT_EVENT_MESSAGE(message_id=11816*)
    → "We cannot know if these intruders are gone for good, however. I will continue my surveillance of the area, but would appreciate it if you checked in with me again in the near future."
 12: 0x00E5 [0x23] WAIT_FOR_DIALOG_INTERACTION
 13: 0x00E6 [0x45] LOAD_SCHEDULED_TASK: Load scheduler "qstc" with entities [LocalPlayer, LocalPlayer], work=[201*, 0*]
 14: 0x00F7 [0x21] END_EVENT
 15: 0x00F8 [0x00] END_REQSTACK()
```

### Event 573

#### Metadata

| Field        | Value    |
|--------------|----------|
| Entrypoint   | 0x00F9   |
| Data Size    | 90 bytes |
| Instructions | 22       |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
00F0:                             42 1E F0 FF FF 7F 6F           B.....o
0100: 70 66 00 80 F8 FF FF 7F  F8 FF FF 7F 74 77 62 30  pf..........twb0
0110: 1D 0B 80 23 06 01 10 24  0C 80 0D 80 01 80 25 66  ...#...$......%f
0120: 00 80 F8 FF FF 7F F8 FF  FF 7F 74 77 62 31 02 00  ..........twb1..
0130: 10 01 80 00 42 01 1D 0E  80 23 03 01 10 0D 80 01  ....B....#......
0140: 51 01 02 00 10 0D 80 00  51 01 1D 0F 80 23 01 51  Q.......Q....#.Q
0150: 01 21 00                                          .!.             
```

#### Opcodes

```
  0: 0x00F9 [0x42] SET_CLI_EVENT_CANCEL_DATA()
  1: 0x00FA [0x1E] EventEntity looks at LocalPlayer and starts talking
  2: 0x00FF [0x6F] WAIT_FRAME_DELAY: Yield until WaitTime reaches zero
  3: 0x0100 [0x70] WAIT_ENTITY_RENDER_FLAG: Wait while EventEntity->Render.Flags3 bit 2 is set (cancel turn if not)
  4: 0x0101 [0x66] LOAD_EXT_SCHEDULER_MAIN: Load scheduler "twb0" with entities [EventEntity, EventEntity], work=9*
  5: 0x0110 [0x1D] PRINT_EVENT_MESSAGE(message_id=11817*)
    → "Do you remember those black-robed intruders you chased away from Monarch Linn? We've had another sighting. Do you think you could go and "persuade" them to leave again?"
  6: 0x0113 [0x23] WAIT_FOR_DIALOG_INTERACTION
  7: 0x0114 [0x06] Work_Zone[1] = 0
  8: 0x0117 [0x24] CREATE_DIALOG(message_id=11807*, default_option=1*, option_flags=0*)
    → "Take up this duty? [On my way./Not today.]"
  9: 0x011E [0x25] WAIT_DIALOG_SELECT()
 10: 0x011F [0x66] LOAD_EXT_SCHEDULER_MAIN: Load scheduler "twb1" with entities [EventEntity, EventEntity], work=9*
 11: 0x012E [0x02] IF !(Work_Zone[0] == 0*) GOTO 0x0142
 12: 0x0136 [0x1D] PRINT_EVENT_MESSAGE(message_id=11808*)
    → "Excellent. Take this and make your way to Monarch Linn. You may have fought them before, but be careful. Who knows what tricks they have hiding up their sleeves?"
 13: 0x0139 [0x23] WAIT_FOR_DIALOG_INTERACTION
 14: 0x013A [0x03] Work_Zone[1] = 1*
 15: 0x013F [0x01] GOTO 0x0151
 16: 0x0142 [0x02] IF !(Work_Zone[0] == 1*) GOTO 0x0151
 17: 0x014A [0x1D] PRINT_EVENT_MESSAGE(message_id=11809*)
    → "Lost your will for battle, hm? There might be another way to deal with this, but let me know if you change your mind. It would make my job a lot easier."
 18: 0x014D [0x23] WAIT_FOR_DIALOG_INTERACTION
 19: 0x014E [0x01] GOTO 0x0151

SUBROUTINE_0151:
 20: 0x0151 [0x21] END_EVENT
 21: 0x0152 [0x00] END_REQSTACK()
```

### Event 101

#### Metadata

| Field        | Value    |
|--------------|----------|
| Entrypoint   | 0x0153   |
| Data Size    | 18 bytes |
| Instructions | 4        |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0150:          92 01 F8 FF FF  7F 22 01 37 10 80 11 80     ......".7....
0160: 12 80 13 80 00                                    .....           
```

#### Opcodes

```
  0: 0x0153 [0x92] EventEntity->Render.Flags3 ^= 0x01
  1: 0x0159 [0x22] ENTITY_HIDE_FLAG(enabled=0x01)
  2: 0x015B [0x37] UPDATE_EVENT_POSITION_AND_DIR: x=-14.655*, z=122.432*, y=-26.626*, direction=266.6°*
  3: 0x0164 [0x00] END_REQSTACK()
```

### Event 65535.8

#### Metadata

| Field        | Value    |
|--------------|----------|
| Entrypoint   | 0x0165   |
| Data Size    | 17 bytes |
| Instructions | 6        |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0160:                22 00 32  14 80 1F 00 15 80 16 80       ".2........
0170: 17 80 1F 01 6F 00                                 ....o.          
```

#### Opcodes

```
  0: 0x0165 [0x22] ENTITY_HIDE_FLAG(enabled=0x00)
  1: 0x0167 [0x32] ExtData[1]->MainSpeed = 13* * 0.1
  2: 0x016A [0x1F] MOVE_ENTITY: EventEntity moves to X=-6.971*, Z=130.784*, Y=-25.968*
  3: 0x0172 [0x1F] MOVE_ENTITY: Update entity position (mode=1)
  4: 0x0174 [0x6F] WAIT_FRAME_DELAY: Yield until WaitTime reaches zero
  5: 0x0175 [0x00] END_REQSTACK()
```

### Event 65535.9

#### Metadata

| Field        | Value    |
|--------------|----------|
| Entrypoint   | 0x0176   |
| Data Size    | 14 bytes |
| Instructions | 4        |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0170:                   32 14  80 1F 00 18 80 19 80 1A        2.........
0180: 80 1F 01 00                                       ....            
```

#### Opcodes

```
  0: 0x0176 [0x32] ExtData[1]->MainSpeed = 13* * 0.1
  1: 0x0179 [0x1F] MOVE_ENTITY: EventEntity moves to X=-4.836*, Z=130.846*, Y=-25.886*
  2: 0x0181 [0x1F] MOVE_ENTITY: Update entity position (mode=1)
  3: 0x0183 [0x00] END_REQSTACK()
```

### Event 65535.10

#### Metadata

| Field        | Value    |
|--------------|----------|
| Entrypoint   | 0x0184   |
| Data Size    | 14 bytes |
| Instructions | 4        |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0180:             32 14 80 1F  00 1B 80 1C 80 1D 80 1F      2...........
0190: 01 00                                             ..              
```

#### Opcodes

```
  0: 0x0184 [0x32] ExtData[1]->MainSpeed = 13* * 0.1
  1: 0x0187 [0x1F] MOVE_ENTITY: EventEntity moves to X=-4.079*, Z=131.393*, Y=-25.854*
  2: 0x018F [0x1F] MOVE_ENTITY: Update entity position (mode=1)
  3: 0x0191 [0x00] END_REQSTACK()
```

### Event 65535.11

#### Metadata

| Field        | Value    |
|--------------|----------|
| Entrypoint   | 0x0192   |
| Data Size    | 14 bytes |
| Instructions | 4        |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0190:       32 14 80 1F 00 1E  80 1F 80 20 80 1F 01 00    2........ ....
```

#### Opcodes

```
  0: 0x0192 [0x32] ExtData[1]->MainSpeed = 13* * 0.1
  1: 0x0195 [0x1F] MOVE_ENTITY: EventEntity moves to X=2.067*, Z=129.817*, Y=-26.196*
  2: 0x019D [0x1F] MOVE_ENTITY: Update entity position (mode=1)
  3: 0x019F [0x00] END_REQSTACK()
```

### Event 103

#### Metadata

| Field        | Value   |
|--------------|---------|
| Entrypoint   | 0x01A0  |
| Data Size    | 7 bytes |
| Instructions | 2       |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
01A0: 92 01 F8 FF FF 7F 00                              .......         
```

#### Opcodes

```
  0: 0x01A0 [0x92] EventEntity->Render.Flags3 ^= 0x01
  1: 0x01A6 [0x00] END_REQSTACK()
```

### Event 65535.12

#### Metadata

| Field        | Value    |
|--------------|----------|
| Entrypoint   | 0x01A7   |
| Data Size    | 25 bytes |
| Instructions | 6        |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
01A0:                      32  14 80 37 21 80 22 80 23         2..7!.".#
01B0: 80 24 80 22 00 1F 00 25  80 26 80 27 80 1F 01 00  .$."...%.&.'....
```

#### Opcodes

```
  0: 0x01A7 [0x32] ExtData[1]->MainSpeed = 13* * 0.1
  1: 0x01AA [0x37] UPDATE_EVENT_POSITION_AND_DIR: x=28.668*, z=37.544*, y=-11.843*, direction=278.7°*
  2: 0x01B3 [0x22] ENTITY_HIDE_FLAG(enabled=0x00)
  3: 0x01B5 [0x1F] MOVE_ENTITY: EventEntity moves to X=28.626*, Z=39.979*, Y=-11.934*
  4: 0x01BD [0x1F] MOVE_ENTITY: Update entity position (mode=1)
  5: 0x01BF [0x00] END_REQSTACK()
```

### Event 65535.13

#### Metadata

| Field        | Value    |
|--------------|----------|
| Entrypoint   | 0x01C0   |
| Data Size    | 14 bytes |
| Instructions | 4        |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
01C0: 32 14 80 1F 00 28 80 29  80 2A 80 1F 01 00        2....(.).*....  
```

#### Opcodes

```
  0: 0x01C0 [0x32] ExtData[1]->MainSpeed = 13* * 0.1
  1: 0x01C3 [0x1F] MOVE_ENTITY: EventEntity moves to X=28.032*, Z=41.423*, Y=-11.976*
  2: 0x01CB [0x1F] MOVE_ENTITY: Update entity position (mode=1)
  3: 0x01CD [0x00] END_REQSTACK()
```

### Event 105

#### Metadata

| Field        | Value   |
|--------------|---------|
| Entrypoint   | 0x01CE  |
| Data Size    | 7 bytes |
| Instructions | 2       |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
01C0:                                            92 01                ..
01D0: F8 FF FF 7F 00                                    .....           
```

#### Opcodes

```
  0: 0x01CE [0x92] EventEntity->Render.Flags3 ^= 0x01
  1: 0x01D4 [0x00] END_REQSTACK()
```

### Event 65535.14

#### Metadata

| Field        | Value    |
|--------------|----------|
| Entrypoint   | 0x01D5   |
| Data Size    | 25 bytes |
| Instructions | 6        |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
01D0:                22 00 32  2B 80 37 2C 80 2D 80 2E       ".2+.7,.-..
01E0: 80 2F 80 1F 00 30 80 31  80 32 80 1F 01 00        ./...0.1.2....  
```

#### Opcodes

```
  0: 0x01D5 [0x22] ENTITY_HIDE_FLAG(enabled=0x00)
  1: 0x01D7 [0x32] ExtData[1]->MainSpeed = 18* * 0.1
  2: 0x01DA [0x37] UPDATE_EVENT_POSITION_AND_DIR: x=4.776*, z=22.315*, y=-22.716*, direction=158.5°*
  3: 0x01E3 [0x1F] MOVE_ENTITY: EventEntity moves to X=2.670*, Z=21.803*, Y=-22.751*
  4: 0x01EB [0x1F] MOVE_ENTITY: Update entity position (mode=1)
  5: 0x01ED [0x00] END_REQSTACK()
```

### Event 107

#### Metadata

| Field        | Value   |
|--------------|---------|
| Entrypoint   | 0x01EE  |
| Data Size    | 7 bytes |
| Instructions | 2       |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
01E0:                                            92 01                ..
01F0: F8 FF FF 7F 00                                    .....           
```

#### Opcodes

```
  0: 0x01EE [0x92] EventEntity->Render.Flags3 ^= 0x01
  1: 0x01F4 [0x00] END_REQSTACK()
```

### Event 65535.15

#### Metadata

| Field        | Value    |
|--------------|----------|
| Entrypoint   | 0x01F5   |
| Data Size    | 33 bytes |
| Instructions | 6        |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
01F0:                32 14 80  79 00 F8 FF FF 7F 04 A0       2..y.......
0200: 01 01 37 33 80 34 80 35  80 36 80 1F 00 37 80 38  ..73.4.5.6...7.8
0210: 80 35 80 1F 01 00                                 .5....          
```

#### Opcodes

```
  0: 0x01F5 [0x32] ExtData[1]->MainSpeed = 13* * 0.1
  1: 0x01F8 [0x79] EventEntity looks at Prishe (ID: 16883716/0x0101A004) (Basic look)
  2: 0x0202 [0x37] UPDATE_EVENT_POSITION_AND_DIR: x=-4.218*, z=0.123*, y=-8.992*, direction=29.5°*
  3: 0x020B [0x1F] MOVE_ENTITY: EventEntity moves to X=-1.663*, Z=-1.227*, Y=-8.992*
  4: 0x0213 [0x1F] MOVE_ENTITY: Update entity position (mode=1)
  5: 0x0215 [0x00] END_REQSTACK()
```

### Event 109

#### Metadata

| Field        | Value   |
|--------------|---------|
| Entrypoint   | 0x0216  |
| Data Size    | 1 bytes |
| Instructions | 1       |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0210:                   00                                    .         
```

#### Opcodes

```
  0: 0x0216 [0x00] END_REQSTACK()
```

### Event 110

#### Metadata

| Field        | Value   |
|--------------|---------|
| Entrypoint   | 0x0217  |
| Data Size    | 1 bytes |
| Instructions | 1       |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0210:                      00                                  .        
```

#### Opcodes

```
  0: 0x0217 [0x00] END_REQSTACK()
```

### Event 65535.16

#### Metadata

| Field        | Value    |
|--------------|----------|
| Entrypoint   | 0x0218   |
| Data Size    | 35 bytes |
| Instructions | 9        |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0210:                          32 14 80 1F 00 39 80 3A          2....9.:
0220: 80 3B 80 1F 01 1F 00 3C  80 3D 80 3E 80 1F 01 1F  .;.....<.=.>....
0230: 00 3F 80 40 80 41 80 1F  01 6F 00                 .?.@.A...o.     
```

#### Opcodes

```
  0: 0x0218 [0x32] ExtData[1]->MainSpeed = 13* * 0.1
  1: 0x021B [0x1F] MOVE_ENTITY: EventEntity moves to X=82.187*, Z=68.983*, Y=-33.979*
  2: 0x0223 [0x1F] MOVE_ENTITY: Update entity position (mode=1)
  3: 0x0225 [0x1F] MOVE_ENTITY: EventEntity moves to X=82.760*, Z=68.356*, Y=-33.957*
  4: 0x022D [0x1F] MOVE_ENTITY: Update entity position (mode=1)
  5: 0x022F [0x1F] MOVE_ENTITY: EventEntity moves to X=83.023*, Z=67.544*, Y=-33.955*
  6: 0x0237 [0x1F] MOVE_ENTITY: Update entity position (mode=1)
  7: 0x0239 [0x6F] WAIT_FRAME_DELAY: Yield until WaitTime reaches zero
  8: 0x023A [0x00] END_REQSTACK()
```

### Event 123

#### Metadata

| Field        | Value    |
|--------------|----------|
| Entrypoint   | 0x023B   |
| Data Size    | 11 bytes |
| Instructions | 5        |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0230:                                   1E F0 FF FF 7F             .....
0240: 1D 42 80 23 21 00                                 .B.#!.          
```

#### Opcodes

```
  0: 0x023B [0x1E] EventEntity looks at LocalPlayer and starts talking
  1: 0x0240 [0x1D] PRINT_EVENT_MESSAGE(message_id=9918*)
    → "Beyond that gate lie the aqueducts that connect to the deeper parts of the Sacrarium. The entire area has had a powerful magic spell cast upon it by the cardinal Mildaurion C Giloumet."
  2: 0x0243 [0x23] WAIT_FOR_DIALOG_INTERACTION
  3: 0x0244 [0x21] END_EVENT
  4: 0x0245 [0x00] END_REQSTACK()
```

### Event 124

#### Metadata

| Field        | Value    |
|--------------|----------|
| Entrypoint   | 0x0246   |
| Data Size    | 30 bytes |
| Instructions | 8        |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0240:                   1E F0  FF FF 7F 1D 43 80 23 66        ......C.#f
0250: 00 80 0A A0 01 01 0A A0  01 01 74 77 62 30 1D 44  ..........twb0.D
0260: 80 23 21 00                                       .#!.            
```

#### Opcodes

```
  0: 0x0246 [0x1E] EventEntity looks at LocalPlayer and starts talking
  1: 0x024B [0x1D] PRINT_EVENT_MESSAGE(message_id=9920*)
    → "You have defeated the minotaur? But your companion has yet to return."
  2: 0x024E [0x23] WAIT_FOR_DIALOG_INTERACTION
  3: 0x024F [0x66] LOAD_EXT_SCHEDULER_MAIN: Load scheduler "twb0" with entities [Justinius (ID: 16883722/0x0101A00A), Justinius (ID: 16883722/0x0101A00A)], work=9*
  4: 0x025E [0x1D] PRINT_EVENT_MESSAGE(message_id=9921*)
    → "It's not that I doubt your word, but I must hear the story from both of you. Now, back into the aqueducts to search for your friend."
  5: 0x0261 [0x23] WAIT_FOR_DIALOG_INTERACTION
  6: 0x0262 [0x21] END_EVENT
  7: 0x0263 [0x00] END_REQSTACK()
```

### Event 125

#### Metadata

| Field        | Value    |
|--------------|----------|
| Entrypoint   | 0x0264   |
| Data Size    | 49 bytes |
| Instructions | 11       |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0260:             1E F0 FF FF  7F 1D 45 80 23 66 00 80      ......E.#f..
0270: 0A A0 01 01 0A A0 01 01  74 77 61 30 1D 46 80 23  ........twa0.F.#
0280: 66 00 80 0A A0 01 01 0A  A0 01 01 74 77 61 31 1D  f..........twa1.
0290: 47 80 23 21 00                                    G.#!.           
```

#### Opcodes

```
  0: 0x0264 [0x1E] EventEntity looks at LocalPlayer and starts talking
  1: 0x0269 [0x1D] PRINT_EVENT_MESSAGE(message_id=9957*)
    → "Were you surprised when you met Prishe? She might look like a little girl, but she's the strongest member of the Tavnazian Patrol."
  2: 0x026C [0x23] WAIT_FOR_DIALOG_INTERACTION
  3: 0x026D [0x66] LOAD_EXT_SCHEDULER_MAIN: Load scheduler "twa0" with entities [Justinius (ID: 16883722/0x0101A00A), Justinius (ID: 16883722/0x0101A00A)], work=9*
  4: 0x027C [0x1D] PRINT_EVENT_MESSAGE(message_id=9958*)
    → "It is strange that you should carry the same amulet. It seems that fate has taken a hand here."
  5: 0x027F [0x23] WAIT_FOR_DIALOG_INTERACTION
  6: 0x0280 [0x66] LOAD_EXT_SCHEDULER_MAIN: Load scheduler "twa1" with entities [Justinius (ID: 16883722/0x0101A00A), Justinius (ID: 16883722/0x0101A00A)], work=9*
  7: 0x028F [0x1D] PRINT_EVENT_MESSAGE(message_id=9959*)
    → "But Prishe despises words like "fate." You should head to her quarters before she starts to get impatient."
  8: 0x0292 [0x23] WAIT_FOR_DIALOG_INTERACTION
  9: 0x0293 [0x21] END_EVENT
 10: 0x0294 [0x00] END_REQSTACK()
```

### Event 127

#### Metadata

| Field        | Value    |
|--------------|----------|
| Entrypoint   | 0x0295   |
| Data Size    | 11 bytes |
| Instructions | 5        |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0290:                1E F0 FF  FF 7F 1D 48 80 23 21 00       ......H.#!.
```

#### Opcodes

```
  0: 0x0295 [0x1E] EventEntity looks at LocalPlayer and starts talking
  1: 0x029A [0x1D] PRINT_EVENT_MESSAGE(message_id=9980*)
    → "Ulmia? I just saw her leaving the safehold. Any time she wants to be alone, Ulmia usually goes to Snowmint Point, located on the Misareaux Coast..."
  2: 0x029D [0x23] WAIT_FOR_DIALOG_INTERACTION
  3: 0x029E [0x21] END_EVENT
  4: 0x029F [0x00] END_REQSTACK()
```

### Event 128

#### Metadata

| Field        | Value    |
|--------------|----------|
| Entrypoint   | 0x02A0   |
| Data Size    | 49 bytes |
| Instructions | 11       |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
02A0: 1E F0 FF FF 7F 1D 49 80  23 66 00 80 0A A0 01 01  ......I.#f......
02B0: 0A A0 01 01 74 77 62 30  1D 4A 80 23 66 00 80 0A  ....twb0.J.#f...
02C0: A0 01 01 0A A0 01 01 74  77 62 31 1D 4B 80 23 21  .......twb1.K.#!
02D0: 00                                                .               
```

#### Opcodes

```
  0: 0x02A0 [0x1E] EventEntity looks at LocalPlayer and starts talking
  1: 0x02A5 [0x1D] PRINT_EVENT_MESSAGE(message_id=9997*)
    → "The Chebukki brothers were headed to Cape Riverne, northwest of the Misareaux Coast."
  2: 0x02A8 [0x23] WAIT_FOR_DIALOG_INTERACTION
  3: 0x02A9 [0x66] LOAD_EXT_SCHEDULER_MAIN: Load scheduler "twb0" with entities [Justinius (ID: 16883722/0x0101A00A), Justinius (ID: 16883722/0x0101A00A)], work=9*
  4: 0x02B8 [0x1D] PRINT_EVENT_MESSAGE(message_id=9998*)
    → "Those three were carved out of pure curiosity. Once they'd heard about the dragon sightings, they probably couldn't wait to go see it for themselves."
  5: 0x02BB [0x23] WAIT_FOR_DIALOG_INTERACTION
  6: 0x02BC [0x66] LOAD_EXT_SCHEDULER_MAIN: Load scheduler "twb1" with entities [Justinius (ID: 16883722/0x0101A00A), Justinius (ID: 16883722/0x0101A00A)], work=9*
  7: 0x02CB [0x1D] PRINT_EVENT_MESSAGE(message_id=9999*)
    → "But why did your companion go with them? When did he become such good friends with those troublemakers?"
  8: 0x02CE [0x23] WAIT_FOR_DIALOG_INTERACTION
  9: 0x02CF [0x21] END_EVENT
 10: 0x02D0 [0x00] END_REQSTACK()
```

### Event 129

#### Metadata

| Field        | Value    |
|--------------|----------|
| Entrypoint   | 0x02D1   |
| Data Size    | 49 bytes |
| Instructions | 11       |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
02D0:    1E F0 FF FF 7F 1D 4C  80 23 66 00 80 0A A0 01   ......L.#f.....
02E0: 01 0A A0 01 01 74 77 61  30 1D 4D 80 23 66 00 80  .....twa0.M.#f..
02F0: 0A A0 01 01 0A A0 01 01  74 77 61 31 1D 4E 80 23  ........twa1.N.#
0300: 21 00                                             !.              
```

#### Opcodes

```
  0: 0x02D1 [0x1E] EventEntity looks at LocalPlayer and starts talking
  1: 0x02D6 [0x1D] PRINT_EVENT_MESSAGE(message_id=10008*)
    → "Greetings, <Player>. Thanks to your actions on the mainland, a messenger from Jeuno has arrived here in Tavnazia."
  2: 0x02D9 [0x23] WAIT_FOR_DIALOG_INTERACTION
  3: 0x02DA [0x66] LOAD_EXT_SCHEDULER_MAIN: Load scheduler "twa0" with entities [Justinius (ID: 16883722/0x0101A00A), Justinius (ID: 16883722/0x0101A00A)], work=9*
  4: 0x02E9 [0x1D] PRINT_EVENT_MESSAGE(message_id=10009*)
    → "It looks like arrangements will be made for aiding the safehold. Their government will also be considering applications from those who wish to emigrate to Jeuno."
  5: 0x02EC [0x23] WAIT_FOR_DIALOG_INTERACTION
  6: 0x02ED [0x66] LOAD_EXT_SCHEDULER_MAIN: Load scheduler "twa1" with entities [Justinius (ID: 16883722/0x0101A00A), Justinius (ID: 16883722/0x0101A00A)], work=9*
  7: 0x02FC [0x1D] PRINT_EVENT_MESSAGE(message_id=10010*)
    → "The only problem is... Have you talked to Prishe yet?"
  8: 0x02FF [0x23] WAIT_FOR_DIALOG_INTERACTION
  9: 0x0300 [0x21] END_EVENT
 10: 0x0301 [0x00] END_REQSTACK()
```

### Event 130

#### Metadata

| Field        | Value    |
|--------------|----------|
| Entrypoint   | 0x0302   |
| Data Size    | 49 bytes |
| Instructions | 11       |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0300:       1E F0 FF FF 7F 1D  4F 80 23 66 00 80 0A A0    ......O.#f....
0310: 01 01 0A A0 01 01 74 77  62 30 1D 50 80 23 66 00  ......twb0.P.#f.
0320: 80 0A A0 01 01 0A A0 01  01 74 77 62 31 1D 51 80  .........twb1.Q.
0330: 23 21 00                                          #!.             
```

#### Opcodes

```
  0: 0x0302 [0x1E] EventEntity looks at LocalPlayer and starts talking
  1: 0x0307 [0x1D] PRINT_EVENT_MESSAGE(message_id=10040*)
    → "<Player>, you weren't able to dissuade Ulmia?"
  2: 0x030A [0x23] WAIT_FOR_DIALOG_INTERACTION
  3: 0x030B [0x66] LOAD_EXT_SCHEDULER_MAIN: Load scheduler "twb0" with entities [Justinius (ID: 16883722/0x0101A00A), Justinius (ID: 16883722/0x0101A00A)], work=9*
  4: 0x031A [0x1D] PRINT_EVENT_MESSAGE(message_id=10041*)
    → "She's determined to march up Cape Riverne and speak to the Wyrmking, alone if need be. She won't listen to me, no matter how much I try to explain the dangers involved."
  5: 0x031D [0x23] WAIT_FOR_DIALOG_INTERACTION
  6: 0x031E [0x66] LOAD_EXT_SCHEDULER_MAIN: Load scheduler "twb1" with entities [Justinius (ID: 16883722/0x0101A00A), Justinius (ID: 16883722/0x0101A00A)], work=9*
  7: 0x032D [0x1D] PRINT_EVENT_MESSAGE(message_id=10042*)
    → "I understand how much she wants to prove that Prishe is not the abhorrent one, but..."
  8: 0x0330 [0x23] WAIT_FOR_DIALOG_INTERACTION
  9: 0x0331 [0x21] END_EVENT
 10: 0x0332 [0x00] END_REQSTACK()
```

### Event 131

#### Metadata

| Field        | Value    |
|--------------|----------|
| Entrypoint   | 0x0333   |
| Data Size    | 53 bytes |
| Instructions | 13       |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0330:          1E F0 FF FF 7F  1D 52 80 23 66 00 80 0A     ......R.#f...
0340: A0 01 01 0A A0 01 01 74  77 61 30 1D 53 80 23 1D  .......twa0.S.#.
0350: 54 80 23 66 00 80 0A A0  01 01 0A A0 01 01 74 77  T.#f..........tw
0360: 61 31 1D 55 80 23 21 00                           a1.U.#!.        
```

#### Opcodes

```
  0: 0x0333 [0x1E] EventEntity looks at LocalPlayer and starts talking
  1: 0x0338 [0x1D] PRINT_EVENT_MESSAGE(message_id=10104*)
    → "Lord Bahamut. So, he has appeared to wreak devastation on the people of Vana'diel..."
  2: 0x033B [0x23] WAIT_FOR_DIALOG_INTERACTION
  3: 0x033C [0x66] LOAD_EXT_SCHEDULER_MAIN: Load scheduler "twa0" with entities [Justinius (ID: 16883722/0x0101A00A), Justinius (ID: 16883722/0x0101A00A)], work=9*
  4: 0x034B [0x1D] PRINT_EVENT_MESSAGE(message_id=10105*)
    → "And his minions have been called to his side in preparation for this genocide."
  5: 0x034E [0x23] WAIT_FOR_DIALOG_INTERACTION
  6: 0x034F [0x1D] PRINT_EVENT_MESSAGE(message_id=10106*)
    → "But we cannot fight against so many dragons. All we can do is sit back and watch helplessly as events unfold."
  7: 0x0352 [0x23] WAIT_FOR_DIALOG_INTERACTION
  8: 0x0353 [0x66] LOAD_EXT_SCHEDULER_MAIN: Load scheduler "twa1" with entities [Justinius (ID: 16883722/0x0101A00A), Justinius (ID: 16883722/0x0101A00A)], work=9*
  9: 0x0362 [0x1D] PRINT_EVENT_MESSAGE(message_id=10107*)
    → "If the will of Promathia is mixed up in this, we may not have any choice in the matter in any case. Will Prishe be able to find some way to avert this catastrophe?"
 10: 0x0365 [0x23] WAIT_FOR_DIALOG_INTERACTION
 11: 0x0366 [0x21] END_EVENT
 12: 0x0367 [0x00] END_REQSTACK()
```

### Event 135

#### Metadata

| Field        | Value    |
|--------------|----------|
| Entrypoint   | 0x0368   |
| Data Size    | 30 bytes |
| Instructions | 8        |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0360:                          1E F0 FF FF 7F 1D 56 80          ......V.
0370: 23 66 00 80 0A A0 01 01  0A A0 01 01 74 77 62 30  #f..........twb0
0380: 1D 57 80 23 21 00                                 .W.#!.          
```

#### Opcodes

```
  0: 0x0368 [0x1E] EventEntity looks at LocalPlayer and starts talking
  1: 0x036D [0x1D] PRINT_EVENT_MESSAGE(message_id=10425*)
    → "For the first time in my life, I prayed to Altana--I prayed for your safe return. And look... All of you managed to escape that terror in the skies."
  2: 0x0370 [0x23] WAIT_FOR_DIALOG_INTERACTION
  3: 0x0371 [0x66] LOAD_EXT_SCHEDULER_MAIN: Load scheduler "twb0" with entities [Justinius (ID: 16883722/0x0101A00A), Justinius (ID: 16883722/0x0101A00A)], work=9*
  4: 0x0380 [0x1D] PRINT_EVENT_MESSAGE(message_id=10426*)
    → "Perhaps Altana truly does exist... I wonder if it is not too late to start believing?"
  5: 0x0383 [0x23] WAIT_FOR_DIALOG_INTERACTION
  6: 0x0384 [0x21] END_EVENT
  7: 0x0385 [0x00] END_REQSTACK()
```

### Event 265

#### Metadata

| Field        | Value    |
|--------------|----------|
| Entrypoint   | 0x0386   |
| Data Size    | 30 bytes |
| Instructions | 8        |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0380:                   1E F0  FF FF 7F 1D 58 80 23 66        ......X.#f
0390: 00 80 0A A0 01 01 0A A0  01 01 74 77 62 30 1D 59  ..........twb0.Y
03A0: 80 23 21 00                                       .#!.            
```

#### Opcodes

```
  0: 0x0386 [0x1E] EventEntity looks at LocalPlayer and starts talking
  1: 0x038B [0x1D] PRINT_EVENT_MESSAGE(message_id=10959*)
    → "Do you find our enclosed city strange? That is probably because it is so different from what you have seen on the mainland."
  2: 0x038E [0x23] WAIT_FOR_DIALOG_INTERACTION
  3: 0x038F [0x66] LOAD_EXT_SCHEDULER_MAIN: Load scheduler "twb0" with entities [Justinius (ID: 16883722/0x0101A00A), Justinius (ID: 16883722/0x0101A00A)], work=9*
  4: 0x039E [0x1D] PRINT_EVENT_MESSAGE(message_id=10960*)
    → "However, for us--the forgotten--this city is all we know and all we have."
  5: 0x03A1 [0x23] WAIT_FOR_DIALOG_INTERACTION
  6: 0x03A2 [0x21] END_EVENT
  7: 0x03A3 [0x00] END_REQSTACK()
```

### Event 266

#### Metadata

| Field        | Value    |
|--------------|----------|
| Entrypoint   | 0x03A4   |
| Data Size    | 35 bytes |
| Instructions | 9        |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
03A0:             1E F0 FF FF  7F 1D 5A 80 23 7B 0A A0      ......Z.#{..
03B0: 01 01 66 00 80 0A A0 01  01 0A A0 01 01 74 77 61  ..f..........twa
03C0: 30 1D 5B 80 23 21 00                              0.[.#!.         
```

#### Opcodes

```
  0: 0x03A4 [0x1E] EventEntity looks at LocalPlayer and starts talking
  1: 0x03A9 [0x1D] PRINT_EVENT_MESSAGE(message_id=10961*)
    → "Safehold elder Despachiaire has welcomed that envoy from Jeuno with open arms. However..."
  2: 0x03AC [0x23] WAIT_FOR_DIALOG_INTERACTION
  3: 0x03AD [0x7B] Justinius (ID: 16883722/0x0101A00A) stops talking
  4: 0x03B2 [0x66] LOAD_EXT_SCHEDULER_MAIN: Load scheduler "twa0" with entities [Justinius (ID: 16883722/0x0101A00A), Justinius (ID: 16883722/0x0101A00A)], work=9*
  5: 0x03C1 [0x1D] PRINT_EVENT_MESSAGE(message_id=10962*)
    → "I have a terrible feeling something grave is upon us. Even those three menacing Tarutaru are acting strange..."
  6: 0x03C4 [0x23] WAIT_FOR_DIALOG_INTERACTION
  7: 0x03C5 [0x21] END_EVENT
  8: 0x03C6 [0x00] END_REQSTACK()
```

### Event 267

#### Metadata

| Field        | Value    |
|--------------|----------|
| Entrypoint   | 0x03C7   |
| Data Size    | 30 bytes |
| Instructions | 8        |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
03C0:                      1E  F0 FF FF 7F 1D 5C 80 23         ......\.#
03D0: 66 00 80 0A A0 01 01 0A  A0 01 01 74 77 62 30 1D  f..........twb0.
03E0: 5D 80 23 21 00                                    ].#!.           
```

#### Opcodes

```
  0: 0x03C7 [0x1E] EventEntity looks at LocalPlayer and starts talking
  1: 0x03CC [0x1D] PRINT_EVENT_MESSAGE(message_id=10963*)
    → "A great challenge awaits you in the skies, <Player>."
  2: 0x03CF [0x23] WAIT_FOR_DIALOG_INTERACTION
  3: 0x03D0 [0x66] LOAD_EXT_SCHEDULER_MAIN: Load scheduler "twb0" with entities [Justinius (ID: 16883722/0x0101A00A), Justinius (ID: 16883722/0x0101A00A)], work=9*
  4: 0x03DF [0x1D] PRINT_EVENT_MESSAGE(message_id=10964*)
    → "But I cannot help but think it ironic that a forgotten city such as ours would be the site of a battle that could alter the history of all Vana'diel."
  5: 0x03E2 [0x23] WAIT_FOR_DIALOG_INTERACTION
  6: 0x03E3 [0x21] END_EVENT
  7: 0x03E4 [0x00] END_REQSTACK()
```

### Event 350

#### Metadata

| Field        | Value    |
|--------------|----------|
| Entrypoint   | 0x03E5   |
| Data Size    | 11 bytes |
| Instructions | 5        |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
03E0:                1E F0 FF  FF 7F 1D 5E 80 23 21 00       ......^.#!.
```

#### Opcodes

```
  0: 0x03E5 [0x1E] EventEntity looks at LocalPlayer and starts talking
  1: 0x03EA [0x1D] PRINT_EVENT_MESSAGE(message_id=10039*)
    → "You are looking for Ulmia? She probably went down to the coast. I must say, it is quite uncommon for her to argue with her grandfather like she did earlier..."
  2: 0x03ED [0x23] WAIT_FOR_DIALOG_INTERACTION
  3: 0x03EE [0x21] END_EVENT
  4: 0x03EF [0x00] END_REQSTACK()
```

### Event 360

#### Metadata

| Field        | Value    |
|--------------|----------|
| Entrypoint   | 0x03F0   |
| Data Size    | 30 bytes |
| Instructions | 8        |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
03F0: 1E F0 FF FF 7F 1D 5F 80  23 66 00 80 0A A0 01 01  ......_.#f......
0400: 0A A0 01 01 74 77 61 30  1D 60 80 23 21 00        ....twa0.`.#!.  
```

#### Opcodes

```
  0: 0x03F0 [0x1E] EventEntity looks at LocalPlayer and starts talking
  1: 0x03F5 [0x1D] PRINT_EVENT_MESSAGE(message_id=9883*)
    → "You say that Nag'molada fellow is not an acquaintance of yours?"
  2: 0x03F8 [0x23] WAIT_FOR_DIALOG_INTERACTION
  3: 0x03F9 [0x66] LOAD_EXT_SCHEDULER_MAIN: Load scheduler "twa0" with entities [Justinius (ID: 16883722/0x0101A00A), Justinius (ID: 16883722/0x0101A00A)], work=9*
  4: 0x0408 [0x1D] PRINT_EVENT_MESSAGE(message_id=9884*)
    → "Well, that is of no matter. If you happen to see him again, I strongly suggest you warn him not to go anywhere near the door leading to the aqueducts."
  5: 0x040B [0x23] WAIT_FOR_DIALOG_INTERACTION
  6: 0x040C [0x21] END_EVENT
  7: 0x040D [0x00] END_REQSTACK()
```

### Event 512

#### Metadata

| Field        | Value   |
|--------------|---------|
| Entrypoint   | 0x040E  |
| Data Size    | 1 bytes |
| Instructions | 1       |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0400:                                            00                   . 
```

#### Opcodes

```
  0: 0x040E [0x00] END_REQSTACK()
```

### Event 65535.17

#### Metadata

| Field        | Value    |
|--------------|----------|
| Entrypoint   | 0x040F   |
| Data Size    | 15 bytes |
| Instructions | 3        |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0400:                                               37                 7
0410: 61 80 62 80 63 80 64 80  1E 0D A0 01 01 00        a.b.c.d.......  
```

#### Opcodes

```
  0: 0x040F [0x37] UPDATE_EVENT_POSITION_AND_DIR: x=74.416*, z=65.665*, y=-34.000*, direction=136.4°*
  1: 0x0418 [0x1E] EventEntity looks at Mengrenaux (ID: 16883725/0x0101A00D) and starts talking
  2: 0x041D [0x00] END_REQSTACK()
```

### Event 513

#### Metadata

| Field        | Value   |
|--------------|---------|
| Entrypoint   | 0x041E  |
| Data Size    | 1 bytes |
| Instructions | 1       |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0410:                                            00                   . 
```

#### Opcodes

```
  0: 0x041E [0x00] END_REQSTACK()
```

### Event 548

#### Metadata

| Field        | Value   |
|--------------|---------|
| Entrypoint   | 0x041F  |
| Data Size    | 1 bytes |
| Instructions | 1       |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0410:                                               00                 .
```

#### Opcodes

```
  0: 0x041F [0x00] END_REQSTACK()
```

### Event 616

#### Metadata

| Field        | Value     |
|--------------|-----------|
| Entrypoint   | 0x0420    |
| Data Size    | 109 bytes |
| Instructions | 29        |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0420: 42 1E F0 FF FF 7F 4A F0  FF FF 7F F8 FF FF 7F 1D  B.....J.........
0430: 65 80 23 6E F8 FF FF 7F  66 80 99 F8 FF FF 7F 99  e.#n....f.......
0440: F8 FF FF 7F 1D 67 80 23  1D 68 80 23 6E F8 FF FF  .....g.#.h.#n...
0450: 7F 69 80 99 F8 FF FF 7F  99 F8 FF FF 7F 1D 6A 80  .i............j.
0460: 23 6E F8 FF FF 7F 6B 80  99 F8 FF FF 7F 99 F8 FF  #n....k.........
0470: FF 7F 1D 6C 80 23 6E F8  FF FF 7F 6D 80 99 F8 FF  ...l.#n....m....
0480: FF 7F 99 F8 FF FF 7F 1D  6E 80 23 21 00           ........n.#!.   
```

#### Opcodes

```
  0: 0x0420 [0x42] SET_CLI_EVENT_CANCEL_DATA()
  1: 0x0421 [0x1E] EventEntity looks at LocalPlayer and starts talking
  2: 0x0426 [0x4A] LocalPlayer looks at EventEntity
  3: 0x042F [0x1D] PRINT_EVENT_MESSAGE(message_id=12043*)
    → "Who...? Ah, it's you, <Player>. I never know when one of you adventurers will choose to darken my doorstep."
  4: 0x0432 [0x23] WAIT_FOR_DIALOG_INTERACTION
  5: 0x0433 [0x6E] EventEntity uses emote 22*
  6: 0x043A [0x99] Wait for EventEntity animation to complete
  7: 0x043F [0x99] Wait for EventEntity animation to complete
  8: 0x0444 [0x1D] PRINT_EVENT_MESSAGE(message_id=12044*)
    → "It may not be my place to say it, but Prishe seemed upset with you."
  9: 0x0447 [0x23] WAIT_FOR_DIALOG_INTERACTION
 10: 0x0448 [0x1D] PRINT_EVENT_MESSAGE(message_id=12045*)
    → "You see, I found this unusual ring on one of my patrols. When I showed it to Prishe and Ulmia, they were certain it was the same ring they gave to you."
 11: 0x044B [0x23] WAIT_FOR_DIALOG_INTERACTION
 12: 0x044C [0x6E] EventEntity uses emote 25*
 13: 0x0453 [0x99] Wait for EventEntity animation to complete
 14: 0x0458 [0x99] Wait for EventEntity animation to complete
 15: 0x045D [0x1D] PRINT_EVENT_MESSAGE(message_id=12046*)
    → "Poor Ulmia seemed on the verge of tears. Is this how you treat a person's heartfelt gift!?"
 16: 0x0460 [0x23] WAIT_FOR_DIALOG_INTERACTION
 17: 0x0461 [0x6E] EventEntity uses emote 26*
 18: 0x0468 [0x99] Wait for EventEntity animation to complete
 19: 0x046D [0x99] Wait for EventEntity animation to complete
 20: 0x0472 [0x1D] PRINT_EVENT_MESSAGE(message_id=12047*)
    → "It might be nothing more than a bauble to you, but for those girls it was a symbol of your friendship--your unbreakable bond!"
 21: 0x0475 [0x23] WAIT_FOR_DIALOG_INTERACTION
 22: 0x0476 [0x6E] EventEntity uses emote 34*
 23: 0x047D [0x99] Wait for EventEntity animation to complete
 24: 0x0482 [0x99] Wait for EventEntity animation to complete
 25: 0x0487 [0x1D] PRINT_EVENT_MESSAGE(message_id=12048*)
    → "Here, take it. Maybe this time you'll think twice about how much it's really worth..."
 26: 0x048A [0x23] WAIT_FOR_DIALOG_INTERACTION
 27: 0x048B [0x21] END_EVENT
 28: 0x048C [0x00] END_REQSTACK()
```

### Event 629

#### Metadata

| Field        | Value   |
|--------------|---------|
| Entrypoint   | 0x048D  |
| Data Size    | 1 bytes |
| Instructions | 1       |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0480:                                         00                     .  
```

#### Opcodes

```
  0: 0x048D [0x00] END_REQSTACK()
```

### Event 65535.18

#### Metadata

| Field        | Value    |
|--------------|----------|
| Entrypoint   | 0x048E   |
| Data Size    | 14 bytes |
| Instructions | 4        |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0480:                                            32 14                2.
0490: 80 1F 00 6F 80 70 80 71  80 1F 01 00              ...o.p.q....    
```

#### Opcodes

```
  0: 0x048E [0x32] ExtData[1]->MainSpeed = 13* * 0.1
  1: 0x0491 [0x1F] MOVE_ENTITY: EventEntity moves to X=-25.868*, Z=123.627*, Y=-26.956*
  2: 0x0499 [0x1F] MOVE_ENTITY: Update entity position (mode=1)
  3: 0x049B [0x00] END_REQSTACK()
```

### Event 65535.19

#### Metadata

| Field        | Value    |
|--------------|----------|
| Entrypoint   | 0x049C   |
| Data Size    | 14 bytes |
| Instructions | 4        |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0490:                                      32 14 80 1F              2...
04A0: 00 72 80 73 80 74 80 1F  01 00                    .r.s.t....      
```

#### Opcodes

```
  0: 0x049C [0x32] ExtData[1]->MainSpeed = 13* * 0.1
  1: 0x049F [0x1F] MOVE_ENTITY: EventEntity moves to X=-14.661*, Z=109.162*, Y=-27.888*
  2: 0x04A7 [0x1F] MOVE_ENTITY: Update entity position (mode=1)
  3: 0x04A9 [0x00] END_REQSTACK()
```

### Event 65535.20

#### Metadata

| Field        | Value    |
|--------------|----------|
| Entrypoint   | 0x04AA   |
| Data Size    | 22 bytes |
| Instructions | 6        |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
04A0:                                32 14 80 1F 00 75            2....u
04B0: 80 76 80 77 80 1F 01 1E  F0 FF FF 7F 1C 78 80 00  .v.w.........x..
```

#### Opcodes

```
  0: 0x04AA [0x32] ExtData[1]->MainSpeed = 13* * 0.1
  1: 0x04AD [0x1F] MOVE_ENTITY: EventEntity moves to X=-12.553*, Z=123.953*, Y=-26.261*
  2: 0x04B5 [0x1F] MOVE_ENTITY: Update entity position (mode=1)
  3: 0x04B7 [0x1E] EventEntity looks at LocalPlayer and starts talking
  4: 0x04BC [0x1C] WAIT(30* ticks)
  5: 0x04BF [0x00] END_REQSTACK()
```

### Event 65535.21

#### Metadata

| Field        | Value    |
|--------------|----------|
| Entrypoint   | 0x04C0   |
| Data Size    | 24 bytes |
| Instructions | 6        |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
04C0: 32 14 80 1F 00 79 80 7A  80 7B 80 1F 01 1F 00 7C  2....y.z.{.....|
04D0: 80 7D 80 7E 80 1F 01 00                           .}.~....        
```

#### Opcodes

```
  0: 0x04C0 [0x32] ExtData[1]->MainSpeed = 13* * 0.1
  1: 0x04C3 [0x1F] MOVE_ENTITY: EventEntity moves to X=-9.226*, Z=121.815*, Y=-26.658*
  2: 0x04CB [0x1F] MOVE_ENTITY: Update entity position (mode=1)
  3: 0x04CD [0x1F] MOVE_ENTITY: EventEntity moves to X=-6.939*, Z=116.948*, Y=-28.008*
  4: 0x04D5 [0x1F] MOVE_ENTITY: Update entity position (mode=1)
  5: 0x04D7 [0x00] END_REQSTACK()
```
