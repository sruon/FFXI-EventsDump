...
| [65535.20](#event-6553520) | 0x0121       |     16 |              4 |
| [65535.21](#event-6553521) | 0x0131       |     16 |              4 |
| [9](#event-9)              | 0x0141       |     45 |             10 |
| [10](#event-10)            | 0x016E       |     45 |             10 |
| [53](#event-53)            | 0x019B       |     54 |             13 |
| [49](#event-49)            | 0x01D1       |    128 |             35 |
| [50](#event-50)            | 0x0251       |     49 |             12 |
| [52](#event-52)            | 0x0282       |     40 |             12 |
| [51](#event-51)            | 0x02AA       |      1 |              1 |
| [65535.22](#event-6553522) | 0x02AB       |     10 |              2 |
...
- **7779**: Isn't this just wonderrrful? First the Velkk get in my way, and now thorn-covered ivy. The thought of those two together makes my fur stand on end.
- **7780**: Speaking of those beasts, keep your claws sharrrp. They've been known to wander up to the station from time to time.
- **7781**: Those cursed Velkk, always showing up to rrruin a good time.
- **7782**: If only you had $6, you might be able to lend a girrrl a hand...
- **7783**: You there, pioneer! You may have your paws full with some colonization prrroject or other, but I've got a small task for you.
- **7784**: No doubt you're familiar with the brrrutishness of the Velkk--but the ones here are a bit smarter than the rest.
- **7785**: Not only do they assault us with weapons and spells, but these annoyances even plant spinescent protuberances to impede our prrrogress!
- **7786**: One of the things damn near tore my tail to shrrreds! We've got to do something about it!
- **7787**: I rrresearched as much as I could, but there's a limit to how much can be done by myself.
- **7788**: My team suggested that since pioneers make a living going against the forces of nature, maybe we could hire one to deal with this prrrickly issue.
- **7789**: I know this idea must sound like claws scrrratching against metal, but I want you to suffer an amnesic attack from one of those things.
- **7790**: Did I say "an" amnesic attack? Because I meant five. That should do the trrrick.
- **7791**: Pioneers are grrreat for doing my dirty...I mean helping out those in need. In this case, what I need is for you to learn more about spinescent protuberances.
- **7792**: Suffer five amnesic attacks from those things and then come back here. Come on--what's the worrrst that could happen?
...
```
  0: 0x00F1 [0x6E] EventEntity uses emote 21*
...
```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0110:    6E F8 FF FF 7F 06 80  99 F8 FF FF 7F 1C 04 80   n..............
0120: 00                                                .               
```
...
```
  0: 0x0111 [0x6E] EventEntity uses emote 7*
...
```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0130:    6E F8 FF FF 7F 08 80  99 F8 FF FF 7F 1C 09 80   n..............
0140: 00                                                .               
```
...
```
  0: 0x0131 [0x6E] EventEntity uses emote 11*
  1: 0x0138 [0x99] Wait for EventEntity animation to complete
  2: 0x013D [0x1C] WAIT(120* ticks)
...
```
  ...
  4: 0x0157 [0x1D] PRINT_EVENT_MESSAGE(message_id=7779*)
...
```
  ...
  4: 0x0184 [0x1D] PRINT_EVENT_MESSAGE(message_id=7780*)
...
```
  ...
  4: 0x01B1 [0x1D] PRINT_EVENT_MESSAGE(message_id=7781*)
  ...
  7: 0x01BA [0x1D] PRINT_EVENT_MESSAGE(message_id=7782*)
...
```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
01D0:    42 1E F0 FF FF 7F 6F  70 29 08 AF A1 10 01 01   B.....op)......
01E0: 1D 0F 80 23 1D 10 80 23  29 08 AF A1 10 01 02 29  ...#...#)......)
01F0: 08 AF A1 10 01 03 1D 11  80 23 29 08 AF A1 10 01  .........#).....
0200: 04 29 08 AF A1 10 01 0D  1D 12 80 23 29 08 AF A1  .).........#)...
0210: 10 01 0E 29 08 AF A1 10  01 0F 1D 13 80 23 29 08  ...).........#).
0220: AF A1 10 01 10 29 08 AF  A1 10 01 09 1D 14 80 23  .....).........#
0230: 1D 15 80 23 29 08 AF A1  10 01 0A 29 08 AF A1 10  ...#)......)....
0240: 01 0B 1D 16 80 23 29 08  AF A1 10 01 0C 20 00 21  .....#)...... .!
0250: 00                                                .               
```
...
```
  ...
  4: 0x01D9 [0x29] REQ_SET_WAIT(priority=0x08, entity_id=Udip Ferawoh (ID: 17867183/0x0110A1AF), tag_num=0x01)
  5: 0x01E0 [0x1D] PRINT_EVENT_MESSAGE(message_id=7783*)
  ...
  7: 0x01E4 [0x1D] PRINT_EVENT_MESSAGE(message_id=7784*)
  ...
  9: 0x01E8 [0x29] REQ_SET_WAIT(priority=0x08, entity_id=Udip Ferawoh (ID: 17867183/0x0110A1AF), tag_num=0x02)
 10: 0x01EF [0x29] REQ_SET_WAIT(priority=0x08, entity_id=Udip Ferawoh (ID: 17867183/0x0110A1AF), tag_num=0x03)
 11: 0x01F6 [0x1D] PRINT_EVENT_MESSAGE(message_id=7785*)
 ...
 13: 0x01FA [0x29] REQ_SET_WAIT(priority=0x08, entity_id=Udip Ferawoh (ID: 17867183/0x0110A1AF), tag_num=0x04)
 14: 0x0201 [0x29] REQ_SET_WAIT(priority=0x08, entity_id=Udip Ferawoh (ID: 17867183/0x0110A1AF), tag_num=0x0D)
 15: 0x0208 [0x1D] PRINT_EVENT_MESSAGE(message_id=7786*)
 ...
 17: 0x020C [0x29] REQ_SET_WAIT(priority=0x08, entity_id=Udip Ferawoh (ID: 17867183/0x0110A1AF), tag_num=0x0E)
 18: 0x0213 [0x29] REQ_SET_WAIT(priority=0x08, entity_id=Udip Ferawoh (ID: 17867183/0x0110A1AF), tag_num=0x0F)
 19: 0x021A [0x1D] PRINT_EVENT_MESSAGE(message_id=7787*)
 ...
 21: 0x021E [0x29] REQ_SET_WAIT(priority=0x08, entity_id=Udip Ferawoh (ID: 17867183/0x0110A1AF), tag_num=0x10)
 22: 0x0225 [0x29] REQ_SET_WAIT(priority=0x08, entity_id=Udip Ferawoh (ID: 17867183/0x0110A1AF), tag_num=0x09)
 23: 0x022C [0x1D] PRINT_EVENT_MESSAGE(message_id=7788*)
 ...
 25: 0x0230 [0x1D] PRINT_EVENT_MESSAGE(message_id=7789*)
 ...
 27: 0x0234 [0x29] REQ_SET_WAIT(priority=0x08, entity_id=Udip Ferawoh (ID: 17867183/0x0110A1AF), tag_num=0x0A)
 28: 0x023B [0x29] REQ_SET_WAIT(priority=0x08, entity_id=Udip Ferawoh (ID: 17867183/0x0110A1AF), tag_num=0x0B)
 29: 0x0242 [0x1D] PRINT_EVENT_MESSAGE(message_id=7790*)
    → "Did I say "an" amnesic attack? Because I meant five. That should do the trrrick."
 30: 0x0245 [0x23] WAIT_FOR_DIALOG_INTERACTION
 31: 0x0246 [0x29] REQ_SET_WAIT(priority=0x08, entity_id=Udip Ferawoh (ID: 17867183/0x0110A1AF), tag_num=0x0C)
...
```
  ...
  4: 0x0267 [0x1D] PRINT_EVENT_MESSAGE(message_id=7791*)
  ...
  6: 0x026B [0x1D] PRINT_EVENT_MESSAGE(message_id=7792*)
...
```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
0280:       1E F0 FF FF 7F 6F  70 27 08 AF A1 10 01 15    .....op'......
0290: 2B AF A1 10 01 19 80 23  2B AF A1 10 01 1A 80 23  +......#+......#
02A0: 2A 08 AF A1 10 01 20 00  21 00                    *..... .!.      
```
...
```
  ...
  3: 0x0289 [0x27] REQ_SET(priority=0x08, entity_id=Udip Ferawoh (ID: 17867183/0x0110A1AF), tag_num=0x15)
  4: 0x0290 [0x2B] Udip Ferawoh (ID: 17867183/0x0110A1AF) [7817*]:
  ...
  6: 0x0298 [0x2B] Udip Ferawoh (ID: 17867183/0x0110A1AF) [7818*]:
    → "It'll be easy trrrekking from here on out!"
  7: 0x029F [0x23] WAIT_FOR_DIALOG_INTERACTION
  8: 0x02A0 [0x2A] GET_REQ_LEVEL(level=8, entity_id=Udip Ferawoh (ID: 17867183/0x0110A1AF))
