# Event 65535

## Metadata

| Field           | Value    |
|-----------------|----------|
| Event Index     | 9/11     |
| Offset          | 0x00AB   |
| Data Size       | 40 bytes |
| Calculated Size | 40 bytes |
| Instructions    | 5        |

```
      00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F
      -- -- -- -- -- -- -- --  -- -- -- -- -- -- -- --
00A0:                                   2C F8 FF FF 7F             ,....
00B0: F8 FF FF 7F 73 73 77 68  62 12 80 F8 FF FF 7F F8  ....sswhb.......
00C0: FF FF 7F 6D 61 69 6E 09  80 1C 13 80 92 01 F8 FF  ...main.........
00D0: FF 7F 00                                          ...             
```

## Opcodes

```
  0: 0x00AB [0x2C] CREATE_SCHEDULER_TASK: Create scheduler "sswh" for entities [EventEntity, EventEntity]
  1: 0x00B8 [0x62] LOAD_EVENT_SCHEDULER (arg=5012): Load scheduler "main" with entities [EventEntity, EventEntity], work=[1*, 0*]
  2: 0x00C9 [0x1C] WAIT(60* ticks)
  3: 0x00CC [0x92] EventEntity->Render.Flags3 ^= 0x01
  4: 0x00D2 [0x00] END_REQSTACK()
```
